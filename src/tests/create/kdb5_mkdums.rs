...
#[c2rust::header_src = "/home/nmavis/dev/gssapi-rs/code/src/include/kdb.h:33"]
pub mod kdb_h {
    /* # of array elements */
 ...
 */
    ...
    use super::krb5_h::{krb5_int16, krb5_data, krb5_magic, krb5_ui_2,
                        krb5_ui_4, krb5_flags, krb5_deltat, krb5_timestamp,
                        krb5_kvno, krb5_octet, krb5_principal, krb5_context,
...
pub use self::krb5_h::{krb5_octet, krb5_int16, krb5_ui_2, krb5_int32,
                       ...
                       krb5_c_valid_enctype, krb5_init_context,
                       krb5_free_context, krb5_parse_name,
                       krb5_copy_principal, krb5_principal2salt,
                       krb5_free_principal, krb5_free_data_contents,
                       krb5_timeofday, krb5_get_default_realm,
...
pub use self::kdb_h::{krb5_keysalt, _krb5_keysalt, krb5_db_entry,
                      _krb5_db_entry_new, krb5_key_data, _krb5_key_data,
                      krb5_tl_data, _krb5_tl_data, krb5_db_open, krb5_db_fini,
                      krb5_db_get_principal, krb5_db_free_principal,
                      krb5_db_put_principal, krb5_db_fetch_mkey,
                      krb5_db_fetch_mkey_list, krb5_db_setup_mkey_name,
                      krb5_dbe_encrypt_key_data, krb5_dbe_create_key_data,
                      krb5_dbe_update_mod_princ_data};
use self::stdlib_h::{calloc, free, exit, atoi};
...
pub static mut mblock: mblock =
    {
        let mut init =
            mblock{max_life:
                   ...
                   mkvno: 1 as libc::c_int as krb5_kvno,};
        init
    };
#[c2rust::src_loc = "58:1"]
unsafe extern "C" fn usage(mut who: *mut libc::c_char,
    ...
    fprintf(stderr,
            b"usage: %s -p prefix -n num_to_create [-d dbpathname] [-r realmname]\n\x00"
...
#[no_mangle]
#[c2rust::src_loc = "71:5"]
pub static mut master_princ_set: libc::c_int = 0 as libc::c_int;
#[no_mangle]
#[c2rust::src_loc = "72:15"]
...
unsafe fn main_0(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char)
 -> libc::c_int {
    ...
    num_to_create = 0 as libc::c_int;
    depth = 1 as libc::c_int;
    loop  {
        optchar =
            getopt(argc, argv as *const *mut libc::c_char,
                   b"D:P:p:n:d:r:k:M:e:m\x00" as *const u8 as
                       *const libc::c_char);
    ...
    if !(num_to_create != 0 && !suffix.is_null()) {
    ...
    retval = set_dbname_help(progname, dbname);
    if retval != 0 { exit(retval); }
    n = 1 as libc::c_int;
    while n <= num_to_create {
        ...
        tmp[(::std::mem::size_of::<[libc::c_char; 4096]>() as
                 libc::c_ulong).wrapping_sub(1 as libc::c_int as
                                                 libc::c_ulong) as usize] =
            '\u{0}' as i32 as libc::c_char;
        str_newprinc = tmp.as_mut_ptr();
...
pub unsafe extern "C" fn add_princ(mut context: krb5_context,
                                   mut str_newprinc: *mut libc::c_char) {
    let mut retval: krb5_error_code = 0;
    let mut newprinc: krb5_principal = 0 as *mut krb5_principal_data;
    let mut newentry: *mut krb5_db_entry = 0 as *mut krb5_db_entry;
    let mut princ_name: [libc::c_char; 4096] = [0; 4096];
    newentry =
    ...
    if newentry.is_null() {
        com_err(progname, 12 as libc::c_int as errcode_t,
                b"while allocating DB entry\x00" as *const u8 as
                    *const libc::c_char);
        return
    }
    snprintf(princ_name.as_mut_ptr(),
             ::std::mem::size_of::<[libc::c_char; 4096]>() as libc::c_ulong,
             b"%s@%s\x00" as *const u8 as *const libc::c_char, str_newprinc,
             cur_realm);
    retval = krb5_parse_name(context, princ_name.as_mut_ptr(), &mut newprinc);
    if retval != 0 {
        com_err(progname, retval as errcode_t,
    ...
    /* Add basic data */
    (*newentry).len = 38 as libc::c_int as krb5_ui_2;
    ...
    retval =
        krb5_copy_principal(context, newprinc as krb5_const_principal,
                            &mut (*newentry).princ);
    if retval != 0 {
        com_err(progname, retval as errcode_t,
                b"while encoding princ to db entry for \'%s\'\x00" as
        ...
        if retval != 0 {
            com_err(progname, retval as errcode_t,
                    b"while fetching date\x00" as *const u8 as
                        *const libc::c_char);
            krb5_free_principal(context, newprinc);
        } else {
            retval =
                krb5_dbe_update_mod_princ_data(context, newentry, now,
                                               master_princ as
                                                   krb5_const_principal);
            if retval != 0 {
                com_err(progname, retval as errcode_t,
                        b"while encoding mod_princ data\x00" as *const u8 as
                            *const libc::c_char);
                krb5_free_principal(context, newprinc);
            } else {
                ...
                let mut pwd: krb5_data =
                    krb5_data{magic: 0,
                              ...
                              data:
                                  0 as
                                      *mut libc::c_char,}; /* must be able to regenerate */
                ...
                retval =
                    krb5_principal2salt(context,
                                        newprinc as krb5_const_principal,
                                        &mut salt);
                if retval != 0 {
                    com_err(progname, retval as errcode_t,
                            b"while converting princ to salt for \'%s\'\x00"
                    ...
                    krb5_free_principal(context, newprinc);
                    pwd.length =
                        strlen(princ_name.as_mut_ptr()) as libc::c_uint;
                    pwd.data = princ_name.as_mut_ptr();
                    retval =
                        krb5_c_string_to_key(context, master_keyblock.enctype,
                                             &mut pwd, &mut salt, &mut key);
                    if retval != 0 {
                        com_err(progname, retval as errcode_t,
                                b"while converting password to key for \'%s\'\x00"
                                    as *const u8 as *const libc::c_char,
                                princ_name.as_mut_ptr());
                        krb5_free_data_contents(context, &mut salt);
                    } else {
                        krb5_free_data_contents(context, &mut salt);
                        retval = krb5_dbe_create_key_data(context, newentry);
                        if retval != 0 {
                            com_err(progname, retval as errcode_t,
                                    b"while creating key_data for \'%s\'\x00"
                                        as *const u8 as *const libc::c_char,
                                    princ_name.as_mut_ptr());
                            ...
                            if retval != 0 {
                                com_err(progname, retval as errcode_t,
                                        b"while encrypting key for \'%s\'\x00"
                                            as *const u8 as
                                            *const libc::c_char,
                                        princ_name.as_mut_ptr());
                                free(key.contents as *mut libc::c_void);
                                ...
                                free(key.contents as *mut libc::c_void);
                                retval =
                                    krb5_db_put_principal(context, newentry);
                                if retval != 0 {
                                    com_err(progname, retval as errcode_t,
                                            b"while storing principal date\x00"
                                                as *const u8 as
                                                *const libc::c_char);
                                }
                            }
    ...
    /* Do cleanup of newentry regardless of error */
    krb5_db_free_principal(context, newentry);
}
#[no_mangle]
#[c2rust::src_loc = "319:1"]
pub unsafe extern "C" fn set_dbname_help(mut pname: *mut libc::c_char,
 ...
 -> libc::c_int {
    ...
    retval =
        krb5_db_setup_mkey_name(test_context, mkey_name, cur_realm,
    ...
    if !mkey_password.is_null() {
        ...
        retval =
            krb5_c_string_to_key(test_context, master_keyblock.enctype,
                                 &mut pwd, &mut scratch,
                                 &mut master_keyblock);
        if retval != 0 {
            com_err(pname, retval as errcode_t,
        ...
        retval =
            krb5_db_fetch_mkey(test_context, master_princ,
                               master_keyblock.enctype, manual_mkey,
                               0 as libc::c_int as krb5_boolean,
                               0 as *mut libc::c_char, 0 as *mut krb5_kvno,
    ...
    retval = krb5_set_default_realm(test_context, cur_realm);
    if retval != 0 {
        com_err(pname, retval as errcode_t,
    ...
    retval = krb5_db_open(test_context, args.as_mut_ptr(), 1 as libc::c_int);
    if retval != 0 {
        com_err(pname, retval as errcode_t,
    ...
    retval =
        krb5_db_fetch_mkey_list(test_context, master_princ,
                                &mut master_keyblock);
    if retval != 0 {
        com_err(pname, retval as errcode_t,
                b"while verifying master key\x00" as *const u8 as
                    *const libc::c_char);
        krb5_db_fini(test_context);
        return 1 as libc::c_int
    }
    retval =
        krb5_db_get_principal(test_context,
                              master_princ as krb5_const_principal,
