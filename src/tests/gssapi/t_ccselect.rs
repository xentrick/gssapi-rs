...
unsafe fn main_0(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char)
 -> libc::c_int {
    ...
    let mut initiator_cred: gss_cred_id_t = 0 as gss_cred_id_t;
    let mut target_name: gss_name_t = 0 as *mut gss_name_struct;
    let mut initiator_name: gss_name_t = 0 as gss_name_t;
    let mut real_initiator_name: gss_name_t = 0 as *mut gss_name_struct;
    ...
    if argc < 2 as libc::c_int || argc > 3 as libc::c_int {
        fprintf(stderr,
                b"Usage: %s targetname [initiatorname|-]\n\x00" as *const u8
    ...
    if argc >= 3 as libc::c_int {
        /* Get initiator cred. */
        if strcmp(*argv.offset(2 as libc::c_int as isize),
                  b"-\x00" as *const u8 as *const libc::c_char) !=
               0 as libc::c_int {
            initiator_name =
                import_name(*argv.offset(2 as libc::c_int as isize))
        }
        major =
            gss_acquire_cred(&mut minor, initiator_name,
                             0xffffffff as libc::c_ulong as OM_uint32,
                             0 as gss_OID_set, 1 as libc::c_int,
                             &mut initiator_cred, 0 as *mut gss_OID_set,
                             0 as *mut OM_uint32);
    ...
    establish_contexts(&mut mech_krb5, initiator_cred, 0 as gss_cred_id_t,
                       target_name, flags, &mut initiator_context,
                       &mut acceptor_context, &mut real_initiator_name,
                       0 as *mut gss_OID, 0 as *mut gss_cred_id_t);
    namebuf.value = 0 as *mut libc::c_void;
    namebuf.length = 0 as libc::c_int as size_t;
    major =
        gss_display_name(&mut minor, real_initiator_name, &mut namebuf,
                         0 as *mut gss_OID);
    check_gsserr(b"gss_display_name(initiator)\x00" as *const u8 as
                     *const libc::c_char, major, minor);
    ...
    gss_release_name(&mut minor, &mut real_initiator_name);
