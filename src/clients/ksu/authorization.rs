...
pub mod ksu_h {
    ...
    extern "C" {
        ...
        #[no_mangle]
        #[c2rust::src_loc = "243:30"]
        pub fn xasprintf(format: *const libc::c_char, _: ...)
         -> *mut libc::c_char;
        #[no_mangle]
...
pub use self::krb5_h::{krb5_int32, krb5_boolean, krb5_enctype, krb5_flags,
                       ...
                       krb5_unparse_name, krb5_aname_to_localname};
...
use self::stdio_h::{stderr, fclose, fopen, fprintf, snprintf, asprintf, fgets,
                    perror, fileno};
use self::libintl_h::dgettext;
use self::string_h::{strlen, strrchr, strchr, strcmp};
use self::sys_stat_h::{stat, fstat};
use self::ksu_h::{prog_name, auth_debug, k5login_path, k5users_path, xcalloc,
                  xasprintf, xstrdup};
#[no_mangle]
#[c2rust::src_loc = "33:1"]
pub unsafe extern "C" fn fowner(mut fp: *mut FILE, mut uid: uid_t)
 -> krb5_boolean {
    let mut sbuf: stat =
        stat{st_dev: 0,
             st_ino: 0,
...
#[no_mangle]
#[c2rust::src_loc = "62:1"]
pub unsafe extern "C" fn krb5_authorization(mut context: krb5_context,
                                            mut principal: krb5_principal,
                                            mut luser: *const libc::c_char,
                                            mut cmd: *mut libc::c_char,
                                            mut ok: *mut krb5_boolean,
                                            mut out_fcmd:
                                                *mut *mut libc::c_char)
 -> krb5_error_code {
    ...
    let mut retbool: krb5_boolean = 0 as libc::c_int as krb5_boolean;
    ...
    let mut st_temp: stat =
        stat{st_dev: 0,
             ...
             st_ctim: timespec{tv_sec: 0, tv_nsec: 0,},
             __glibc_reserved: [0; 3],};
    *ok = 0 as libc::c_int as krb5_boolean;
    /* no account => no access */
    pwd = getpwnam(luser);
    if pwd.is_null() { return 0 as libc::c_int }
    retval =
        krb5_unparse_name(context, principal as krb5_const_principal,
                          &mut princname);
    if retval != 0 { return retval }
    k5login_flag = stat(k5login_path.as_mut_ptr(), &mut st_temp);
    k5users_flag = stat(k5users_path.as_mut_ptr(), &mut st_temp);
    ...
    if auth_debug != 0 {
        fprintf(stderr,
                b"In krb5_authorization: if auth files exist -> can access\n\x00"
    ...
    if k5login_flag == 0 {
        if auth_debug != 0 {
            fprintf(stderr,
                    b"In krb5_authorization: principal to be authorized %s\n\x00"
                        as *const u8 as *const libc::c_char, princname);
        }
        retval = k5login_lookup(login_fp, princname, &mut retbool);
        if retval != 0 {
            auth_cleanup(users_fp, login_fp, princname);
            return retval
        }
        if retbool != 0 { if !cmd.is_null() { *out_fcmd = xstrdup(cmd) } }
    }
    if k5users_flag == 0 && retbool == 0 as libc::c_int as libc::c_uint {
        retval =
            k5users_lookup(users_fp, princname, cmd, &mut retbool, out_fcmd);
        if retval != 0 {
            auth_cleanup(users_fp, login_fp, princname);
            return retval
        }
    }
    if k5login_flag != 0 && k5users_flag != 0 {
        let mut kuser: *mut libc::c_char =
            xcalloc(strlen(princname),
                    ::std::mem::size_of::<libc::c_char>() as libc::c_ulong) as
                *mut libc::c_char;
        if krb5_aname_to_localname(context, principal as krb5_const_principal,
                                   strlen(princname) as libc::c_int, kuser) ==
               0 && strcmp(kuser, luser) == 0 as libc::c_int {
            retbool = 1 as libc::c_int as krb5_boolean
        }
        free(kuser as *mut libc::c_void);
    }
    *ok = retbool;
    auth_cleanup(users_fp, login_fp, princname);
    return 0 as libc::c_int;
...
pub unsafe extern "C" fn k5login_lookup(mut fp: *mut FILE,
                                        mut princname: *mut libc::c_char,
 ...
 -> krb5_error_code {
    ...
    let mut loc_found: krb5_boolean = 0 as libc::c_int as krb5_boolean;
    retval = get_line(fp, &mut line);
    if retval != 0 { return retval }
    while !line.is_null() {
        fprinc = get_first_token(line, &mut lp);
        if !fprinc.is_null() && strcmp(princname, fprinc) == 0 {
            if !get_next_token(&mut lp).is_null() {
                free(line as *mut libc::c_void);
                break ;
                /* nothing should follow princname*/
            } else {
                loc_found = 1 as libc::c_int as krb5_boolean;
                free(line as *mut libc::c_void);
                break ;
            }
        } else {
...
pub unsafe extern "C" fn k5users_lookup(mut fp: *mut FILE,
                                        mut princname: *mut libc::c_char,
                                        mut cmd: *mut libc::c_char,
                                        mut found: *mut krb5_boolean,
                                        mut out_fcmd: *mut *mut libc::c_char)
 -> krb5_error_code {
    let mut retval: krb5_error_code = 0;
    let mut line: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut fprinc: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut fcmd: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut lp: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut loc_fcmd: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut loc_found: krb5_boolean = 0 as libc::c_int as krb5_boolean;
    retval = get_line(fp, &mut line);
    if retval != 0 { return retval }
    while !line.is_null() {
        fprinc = get_first_token(line, &mut lp);
        if !fprinc.is_null() && strcmp(princname, fprinc) == 0 {
            fcmd = get_next_token(&mut lp);
            if !fcmd.is_null() &&
                   strcmp(fcmd, b"*\x00" as *const u8 as *const libc::c_char)
                       == 0 {
                if get_next_token(&mut lp).is_null() {
                    loc_fcmd =
                        if !cmd.is_null() {
                ...
                if !fcmd.is_null() {
                    let mut temp_rfcmd: *mut libc::c_char =
                        0 as *mut libc::c_char;
                    let mut err: *mut libc::c_char = 0 as *mut libc::c_char;
                    let mut match_0: krb5_boolean = 0;
                    loop  {
                        if match_commands(fcmd, cmd, &mut match_0,
                                          &mut temp_rfcmd, &mut err) != 0 {
                            if auth_debug != 0 {
                                fprintf(stderr,
                            ...
                            loc_fcmd = err;
                            break ;
                        } else if match_0 == 1 as libc::c_int as libc::c_uint
                         {
                            loc_fcmd = temp_rfcmd;
                            loc_found = 1 as libc::c_int as krb5_boolean;
                            break ;
...
pub unsafe extern "C" fn fcmd_resolve(mut fcmd: *mut libc::c_char,
                                      mut out_fcmd:
                                          *mut *mut *mut libc::c_char,
                                      mut out_err: *mut *mut libc::c_char)
 -> krb5_boolean {
    let mut err: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut tmp_fcmd: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    let mut path_ptr: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut path: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut lp: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut tc: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut i: libc::c_int = 0 as libc::c_int;
    tmp_fcmd =
        xcalloc(2048 as libc::c_int as size_t,
                ::std::mem::size_of::<*mut libc::c_char>() as libc::c_ulong)
            as *mut *mut libc::c_char;
    if *fcmd as libc::c_int == '/' as i32 {
        /* must be full path */
        let ref mut fresh0 = *tmp_fcmd.offset(0 as libc::c_int as isize);
        *fresh0 = xstrdup(fcmd);
        let ref mut fresh1 = *tmp_fcmd.offset(1 as libc::c_int as isize);
        *fresh1 = 0 as *mut libc::c_char;
        *out_fcmd = tmp_fcmd;
        return 1 as libc::c_int as krb5_boolean
    } else {
        /* must be either full path or just the cmd name */
        if !strchr(fcmd, '/' as i32).is_null() {
            asprintf(&mut err as *mut *mut libc::c_char,
                     dgettext(b"mit-krb5\x00" as *const u8 as
                                  *const libc::c_char,
                              b"Error: bad entry - %s in %s file, must be either full path or just the cmd name\n\x00"
                                  as *const u8 as *const libc::c_char), fcmd,
                     b".k5users\x00" as *const u8 as *const libc::c_char);
            *out_err = err;
            return 0 as libc::c_int as krb5_boolean
        }
        path =
            xstrdup(b"/usr/local/sbin /usr/local/bin /sbin /bin /usr/sbin /usr/bin\x00"
                        as *const u8 as *const libc::c_char);
        path_ptr = path;
        while *path_ptr as libc::c_int == ' ' as i32 ||
                  *path_ptr as libc::c_int == '\t' as i32 {
            path_ptr = path_ptr.offset(1)
        }
        tc = get_first_token(path_ptr, &mut lp);
        if tc.is_null() {
            asprintf(&mut err as *mut *mut libc::c_char,
                     dgettext(b"mit-krb5\x00" as *const u8 as
                                  *const libc::c_char,
                              b"Error: bad entry - %s in %s file, CMD_PATH contains no paths \n\x00"
                                  as *const u8 as *const libc::c_char), fcmd,
                     b".k5users\x00" as *const u8 as *const libc::c_char);
            *out_err = err;
            return 0 as libc::c_int as krb5_boolean
        ...
        i = 0 as libc::c_int;
        loop  {
            if *tc as libc::c_int != '/' as i32 {
                /* CMD_PATH */
                /* must be full path */
                asprintf(&mut err as *mut *mut libc::c_char,
                         dgettext(b"mit-krb5\x00" as *const u8 as
                                      *const libc::c_char,
                                  b"Error: bad path %s in CMD_PATH for %s must start with \'/\' \n\x00"
                                      as *const u8 as *const libc::c_char),
                         tc,
                         b".k5users\x00" as *const u8 as *const libc::c_char);
                *out_err = err;
            ...
            let ref mut fresh2 = *tmp_fcmd.offset(i as isize);
            *fresh2 =
                xasprintf(b"%s/%s\x00" as *const u8 as *const libc::c_char,
                          tc, fcmd);
            i += 1;
            tc = get_next_token(&mut lp);
            if tc.is_null() { break ; }
        }
        let ref mut fresh3 = *tmp_fcmd.offset(i as isize);
...
pub unsafe extern "C" fn cmd_single(mut cmd: *mut libc::c_char)
 -> krb5_boolean {
    if strrchr(cmd, '/' as i32).is_null() {
        return 1 as libc::c_int as krb5_boolean
...
pub unsafe extern "C" fn cmd_arr_cmp_postfix(mut fcmd_arr:
                                                 *mut *mut libc::c_char,
                                             mut cmd: *mut libc::c_char)
 -> libc::c_int {
    let mut temp_fcmd: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut ptr: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut result: libc::c_int = 1 as libc::c_int;
    let mut i: libc::c_int = 0 as libc::c_int;
    while !(*fcmd_arr.offset(i as isize)).is_null() {
        ptr = strrchr(*fcmd_arr.offset(i as isize), '/' as i32);
        if ptr.is_null() {
            temp_fcmd = *fcmd_arr.offset(i as isize)
        } else { temp_fcmd = ptr.offset(1 as libc::c_int as isize) }
...
pub unsafe extern "C" fn cmd_arr_cmp(mut fcmd_arr: *mut *mut libc::c_char,
                                     mut cmd: *mut libc::c_char)
 -> libc::c_int {
    let mut result: libc::c_int = 1 as libc::c_int;
    let mut i: libc::c_int = 0 as libc::c_int;
    while !(*fcmd_arr.offset(i as isize)).is_null() {
        result = strcmp(*fcmd_arr.offset(i as isize), cmd);
        if result == 0 as libc::c_int { break ; }
        i += 1
    }
    return result;
...
pub unsafe extern "C" fn find_first_cmd_that_exists(mut fcmd_arr:
                                                        *mut *mut libc::c_char,
                                                    mut cmd_out:
                                                        *mut *mut libc::c_char,
                                                    mut err_out:
 ...
 -> krb5_boolean {
    ...
    let mut buf: k5buf =
        k5buf{buftype: K5BUF_ERROR,
              ...
              len: 0,};
    while !(*fcmd_arr.offset(i as isize)).is_null() {
        if stat(*fcmd_arr.offset(i as isize), &mut st_temp) == 0 {
            *cmd_out = xstrdup(*fcmd_arr.offset(i as isize));
            retbool = 1 as libc::c_int as krb5_boolean;
            break ;
        } else { i += 1 }
    }
    if retbool == 0 as libc::c_int as libc::c_uint {
        k5_buf_init_dynamic(&mut buf);
        k5_buf_add(&mut buf,
                   dgettext(b"mit-krb5\x00" as *const u8 as
                                *const libc::c_char,
                            b"Error: not found -> \x00" as *const u8 as
                                *const libc::c_char));
        j = 0 as libc::c_int;
        while j < i {
            k5_buf_add_fmt(&mut buf as *mut k5buf,
                           b" %s \x00" as *const u8 as *const libc::c_char,
                           *fcmd_arr.offset(j as isize));
            j += 1
        }
        k5_buf_add(&mut buf, b"\n\x00" as *const u8 as *const libc::c_char);
        if k5_buf_status(&mut buf) != 0 as libc::c_int {
            perror(prog_name);
            exit(1 as libc::c_int);
        }
        *err_out = buf.data as *mut libc::c_char
    }
    return retbool;
}
/* **************************************************************
...
pub unsafe extern "C" fn match_commands(mut fcmd: *mut libc::c_char,
                                        mut cmd: *mut libc::c_char,
                                        mut match_0: *mut krb5_boolean,
                                        mut cmd_out: *mut *mut libc::c_char,
                                        mut err_out: *mut *mut libc::c_char)
 -> libc::c_int {
    let mut fcmd_arr: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    let mut err: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut cmd_temp: *mut libc::c_char = 0 as *mut libc::c_char;
    if fcmd_resolve(fcmd, &mut fcmd_arr, &mut err) ==
           0 as libc::c_int as libc::c_uint {
        *err_out = err;
        return 1 as libc::c_int
    }
    if cmd_single(cmd) == 1 as libc::c_int as libc::c_uint {
        if cmd_arr_cmp_postfix(fcmd_arr, cmd) == 0 {
            /* found */
            if find_first_cmd_that_exists(fcmd_arr, &mut cmd_temp, &mut err)
                   == 1 as libc::c_int as libc::c_uint {
                *match_0 = 1 as libc::c_int as krb5_boolean;
                *cmd_out = cmd_temp;
                return 0 as libc::c_int
            } else { *err_out = err; return 1 as libc::c_int }
        } else {
            *match_0 = 0 as libc::c_int as krb5_boolean;
            return 0 as libc::c_int
        }
    } else if cmd_arr_cmp(fcmd_arr, cmd) == 0 {
        /* found */
        *match_0 = 1 as libc::c_int as krb5_boolean;
        *cmd_out = xstrdup(cmd);
        return 0 as libc::c_int
...
pub unsafe extern "C" fn get_line(mut fp: *mut FILE,
                                  mut out_line: *mut *mut libc::c_char)
 -> krb5_error_code {
    let mut line: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    line =
        xcalloc(8192 as libc::c_int as size_t,
                ::std::mem::size_of::<libc::c_char>() as libc::c_ulong) as
            *mut libc::c_char;
    line_ptr = line;
    *line.offset(0 as libc::c_int as isize) = '\u{0}' as i32 as libc::c_char;
    loop  {
        r = fgets(line_ptr, 8192 as libc::c_int, fp);
        if r.is_null() { break ; }
        newline = strchr(line_ptr, '\n' as i32);
        if !newline.is_null() {
            *newline = '\u{0}' as i32 as libc::c_char;
            break ;
        } else {
            chunk_count += 1;
            line =
                realloc(line as *mut libc::c_void,
                        (chunk_count as
                             libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                             as
                                                             libc::c_ulong).wrapping_mul(8192
                                                                                             as
                                                                                             libc::c_int
                                                                                             as
                                                                                             libc::c_ulong))
                    as *mut libc::c_char;
            if line.is_null() { return 12 as libc::c_int }
            line_ptr =
                line.offset(((8192 as libc::c_int - 1 as libc::c_int) *
                                 (chunk_count - 1 as libc::c_int)) as isize)
        }
    }
    if r.is_null() && strlen(line) == 0 as libc::c_int as libc::c_ulong {
        *out_line = 0 as *mut libc::c_char
    } else { *out_line = line }
    return 0 as libc::c_int;
}
/* ******************************************************
...
pub unsafe extern "C" fn get_first_token(mut line: *mut libc::c_char,
                                         mut lnext: *mut *mut libc::c_char)
 -> *mut libc::c_char {
    let mut lptr: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut out_ptr: *mut libc::c_char = 0 as *mut libc::c_char;
    out_ptr = line;
    lptr = line;
    while *lptr as libc::c_int == ' ' as i32 ||
              *lptr as libc::c_int == '\t' as i32 {
        lptr = lptr.offset(1)
    }
    if strlen(lptr) == 0 as libc::c_int as libc::c_ulong {
        return 0 as *mut libc::c_char
    }
    while *lptr as libc::c_int != ' ' as i32 &&
              *lptr as libc::c_int != '\t' as i32 &&
              *lptr as libc::c_int != '\u{0}' as i32 {
        lptr = lptr.offset(1)
    }
    if *lptr as libc::c_int == '\u{0}' as i32 {
        *lnext = lptr
    } else {
        *lptr = '\u{0}' as i32 as libc::c_char;
        *lnext = lptr.offset(1 as libc::c_int as isize)
    }
    return out_ptr;
}
/* *********************************************************
...
pub unsafe extern "C" fn get_next_token(mut lnext: *mut *mut libc::c_char)
 -> *mut libc::c_char {
    let mut lptr: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut out_ptr: *mut libc::c_char = 0 as *mut libc::c_char;
    lptr = *lnext;
...
unsafe extern "C" fn auth_cleanup(mut users_fp: *mut FILE,
                                  mut login_fp: *mut FILE,
                                  mut princname: *mut libc::c_char) {
    free(princname as *mut libc::c_void);
    if !users_fp.is_null() { fclose(users_fp); }
    if !login_fp.is_null() { fclose(login_fp); };
}
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
/*
 * Copyright (c) 1994 by the University of Southern California
...
pub unsafe extern "C" fn init_auth_names(mut pw_dir: *mut libc::c_char) {
    let mut sep: *const libc::c_char = 0 as *const libc::c_char;
    let mut r1: libc::c_int = 0;
    let mut r2: libc::c_int = 0;
    sep =
        if strlen(pw_dir) == 1 as libc::c_int as libc::c_ulong &&
               *pw_dir as libc::c_int == '/' as i32 {
            b"\x00" as *const u8 as *const libc::c_char
        } else { b"/\x00" as *const u8 as *const libc::c_char };
    r1 =
        snprintf(k5login_path.as_mut_ptr(),
                 ::std::mem::size_of::<[libc::c_char; 4096]>() as
                     libc::c_ulong,
                 b"%s%s%s\x00" as *const u8 as *const libc::c_char, pw_dir,
                 sep, b".k5login\x00" as *const u8 as *const libc::c_char);
    r2 =
        snprintf(k5users_path.as_mut_ptr(),
                 ::std::mem::size_of::<[libc::c_char; 4096]>() as
                     libc::c_ulong,
                 b"%s%s%s\x00" as *const u8 as *const libc::c_char, pw_dir,
                 sep, b".k5users\x00" as *const u8 as *const libc::c_char);
    if r1 as libc::c_uint as libc::c_ulong >=
           ::std::mem::size_of::<[libc::c_char; 4096]>() as libc::c_ulong ||
           r2 as libc::c_uint as libc::c_ulong >=
               ::std::mem::size_of::<[libc::c_char; 4096]>() as libc::c_ulong
       {
        fprintf(stderr,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"home directory name `%s\' too long, can\'t search for .k5login\n\x00"
                             as *const u8 as *const libc::c_char), pw_dir);
        exit(1 as libc::c_int);
    };
}
