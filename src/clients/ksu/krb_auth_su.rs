...
pub mod ksu_h {
    ...
    use super::krb5_h::{krb5_context, krb5_ticket_times, krb5_timestamp,
                        krb5_error_code, krb5_data, krb5_principal};
    extern "C" {
        #[no_mangle]
...
pub use self::types_h::{__uint8_t, __int32_t, __uid_t, __off_t, __off64_t};
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_verify_init_creds_opt, _profile_t, _krb5_ccache,
                       krb5_cc_retrieve_cred, krb5_parse_name,
                       krb5_unparse_name, krb5_copy_principal,
                       krb5_free_principal, krb5_free_cred_contents,
                       krb5_timeofday, krb5_read_password,
                       krb5_prompter_posix, krb5_get_init_creds_password,
...
use self::string_h::{explicit_bzero, memcmp, memset};
use self::ksu_h::{prog_name, auth_debug, krb5_check_exp, ksu_tgtname};
#[no_mangle]
#[c2rust::src_loc = "34:1"]
pub unsafe extern "C" fn krb5_auth_check(mut context: krb5_context,
                                         mut client_pname: krb5_principal,
                                         mut hostname: *mut libc::c_char,
                                         mut options:
                                             *mut krb5_get_init_creds_opt,
                                         mut target_user: *mut libc::c_char,
                                         mut cc: krb5_ccache,
                                         mut path_passwd: *mut libc::c_int,
                                         mut target_uid: uid_t)
 -> krb5_boolean {
    let mut client: krb5_principal = 0 as *mut krb5_principal_data;
    let mut vfy_opts: krb5_verify_init_creds_opt =
        krb5_verify_init_creds_opt{flags: 0, ap_req_nofail: 0,};
    ...
    let mut got_it: libc::c_int = 0 as libc::c_int;
    let mut zero_password: krb5_boolean = 0;
    *path_passwd = 0 as libc::c_int;
    ...
    memset(&mut tgt as *mut krb5_creds as *mut libc::c_void, 0 as libc::c_int,
           ::std::mem::size_of::<krb5_creds>() as libc::c_ulong);
    retval =
        krb5_copy_principal(context, client_pname as krb5_const_principal,
                            &mut client);
    if retval != 0 {
        com_err(prog_name, retval as errcode_t,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"while copying client principal\x00" as *const u8 as
                             *const libc::c_char));
    ...
    retval =
        ksu_tgtname(context, &mut (*client).realm, &mut (*client).realm,
                    &mut tgtq.server);
    if retval != 0 {
        com_err(prog_name, retval as errcode_t,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"while creating tgt for local realm\x00" as
                             *const u8 as *const libc::c_char));
        krb5_free_principal(context, client);
        return 0 as libc::c_int as krb5_boolean
    }
    if auth_debug != 0 {
        dump_principal(context,
                       b"local tgt principal name\x00" as *const u8 as
        ...
        if retval as libc::c_long != -(1765328243 as libc::c_long) &&
            ...
            com_err(prog_name, retval as errcode_t,
                    dgettext(b"mit-krb5\x00" as *const u8 as
                                 *const libc::c_char,
                             b"while retrieving creds from cache\x00" as
                                 *const u8 as *const libc::c_char));
            return 0 as libc::c_int as krb5_boolean
        }
    } else { got_it = 1 as libc::c_int }
    if got_it == 0 {
        if seteuid(0 as libc::c_int as uid_t) != 0 || seteuid(target_uid) != 0
           {
            com_err(b"ksu\x00" as *const u8 as *const libc::c_char,
                    *__errno_location() as errcode_t,
                    dgettext(b"mit-krb5\x00" as *const u8 as
                                 *const libc::c_char,
                             b"while switching to target uid\x00" as *const u8
        ...
        fprintf(stderr,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"         in remotely using an unsecure (non-encrypted) channel. \n\x00"
                             as *const u8 as *const libc::c_char));
        /* GET_TGT_VIA_PASSWD */
        if ksu_get_tgt_via_passwd(context, client, options,
                                  &mut zero_password, &mut tgt) ==
               0 as libc::c_int as libc::c_uint {
            seteuid(0 as libc::c_int as uid_t);
            return 0 as libc::c_int as krb5_boolean
        }
        *path_passwd = 1 as libc::c_int;
    ...
    krb5_verify_init_creds_opt_init(&mut vfy_opts);
    krb5_verify_init_creds_opt_set_ap_req_nofail(&mut vfy_opts,
                                                 1 as libc::c_int);
    retval =
        krb5_verify_init_creds(context, &mut tgt, 0 as krb5_principal,
                               0 as krb5_keytab, 0 as *mut krb5_ccache,
                               &mut vfy_opts);
    if retval != 0 {
        com_err(prog_name, retval as errcode_t,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"while verifying ticket for server\x00" as *const u8
                             as *const libc::c_char));
        return 0 as libc::c_int as krb5_boolean
    }
...
pub unsafe extern "C" fn ksu_get_tgt_via_passwd(mut context: krb5_context,
 ...
 -> krb5_boolean {
    ...
    let mut pwsize: libc::c_uint = 0;
    let mut password: [libc::c_char; 255] = [0; 255];
    let mut client_name: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut prompt: [libc::c_char; 255] = [0; 255];
    let mut result: libc::c_int = 0;
    *zero_password = 0 as libc::c_int as krb5_boolean;
    if !creds_out.is_null() {
        memset(creds_out as *mut libc::c_void, 0 as libc::c_int,
               ::std::mem::size_of::<krb5_creds>() as libc::c_ulong);
    }
    code =
        krb5_unparse_name(context, client as krb5_const_principal,
                          &mut client_name);
    if code != 0 {
        com_err(prog_name, code as errcode_t,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
    ...
    result =
        snprintf(prompt.as_mut_ptr(),
                 ::std::mem::size_of::<[libc::c_char; 255]>() as
                     libc::c_ulong,
                 dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                          b"Kerberos password for %s: \x00" as *const u8 as
                              *const libc::c_char), client_name);
    if result as libc::c_uint as libc::c_ulong >=
           ::std::mem::size_of::<[libc::c_char; 255]>() as libc::c_ulong {
        fprintf(stderr,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"principal name %s too long for internal buffer space\n\x00"
                             as *const u8 as *const libc::c_char),
                client_name);
        return 0 as libc::c_int as krb5_boolean
    }
    pwsize =
        ::std::mem::size_of::<[libc::c_char; 255]>() as libc::c_ulong as
            libc::c_uint;
    code =
        krb5_read_password(context, prompt.as_mut_ptr(),
                           0 as *const libc::c_char, password.as_mut_ptr(),
                           &mut pwsize);
    if code != 0 {
        com_err(prog_name, code as errcode_t,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"while reading password for \'%s\'\n\x00" as
                             *const u8 as *const libc::c_char), client_name);
        return 0 as libc::c_int as krb5_boolean
    }
    if pwsize == 0 as libc::c_int as libc::c_uint {
        fprintf(stderr,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"No password given\n\x00" as *const u8 as
                             *const libc::c_char));
        *zero_password = 1 as libc::c_int as krb5_boolean;
        return 0 as libc::c_int as krb5_boolean
    }
    code =
        krb5_get_init_creds_password(context, &mut creds, client,
                                     password.as_mut_ptr(),
    ...
    if code != 0 {
        if code as libc::c_long == -(1765328353 as libc::c_long) {
            fprintf(stderr,
                    dgettext(b"mit-krb5\x00" as *const u8 as
                                 *const libc::c_char,
                             b"%s: Password incorrect\n\x00" as *const u8 as
                                 *const libc::c_char), prog_name);
        } else {
            com_err(prog_name, code as errcode_t,
                    dgettext(b"mit-krb5\x00" as *const u8 as
                                 *const libc::c_char,
                             b"while getting initial credentials\x00" as
                                 *const u8 as *const libc::c_char));
        }
        return 0 as libc::c_int as krb5_boolean
    }
    if !creds_out.is_null() {
        *creds_out = creds
    } else { krb5_free_cred_contents(context, &mut creds); }
    return 1 as libc::c_int as krb5_boolean;
}
#[no_mangle]
#[c2rust::src_loc = "216:1"]
pub unsafe extern "C" fn dump_principal(mut context: krb5_context,
                                        mut str: *mut libc::c_char,
                                        mut p: krb5_principal) {
    let mut stname: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut retval: krb5_error_code = 0;
    retval =
        krb5_unparse_name(context, p as krb5_const_principal, &mut stname);
    if retval != 0 {
        fprintf(stderr,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b" %s while unparsing name\n\x00" as *const u8 as
                             *const libc::c_char),
...
pub unsafe extern "C" fn plain_dump_principal(mut context: krb5_context,
                                              mut p: krb5_principal) {
    let mut stname: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut retval: krb5_error_code = 0;
    retval =
    ...
    fprintf(stderr, b"%s \x00" as *const u8 as *const libc::c_char, stname);
}
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
/*
 * Copyright (c) 1994 by the University of Southern California
...
pub unsafe extern "C" fn get_best_principal(mut context: krb5_context,
                                            mut plist: *mut *mut libc::c_char,
                                            mut client: *mut krb5_principal)
 -> krb5_error_code {
    let mut retval: krb5_error_code = 0 as libc::c_int;
    ...
    while !(*plist.offset(i as isize)).is_null() {
        ...
        if data_eq((**client).realm, (*temp_client).realm) != 0 {
            if nelem != 0 && (**client).length > 0 as libc::c_int &&
                   (*temp_client).length > 0 as libc::c_int {
                let mut p1: *mut krb5_data =
                    if (0 as libc::c_int) < (**client).length {
                        (**client).data.offset(0 as libc::c_int as isize)
                    } else { 0 as *mut krb5_data };
                let mut p2: *mut krb5_data =
                    if (0 as libc::c_int) < (*temp_client).length {
                        (*temp_client).data.offset(0 as libc::c_int as isize)
                    } else { 0 as *mut krb5_data };
                if data_eq(*p1, *p2) != 0 {
                    if auth_debug != 0 {
                        fprintf(stderr,
                                b"get_best_principal: compare with %s\n\x00"
                                    as *const u8 as *const libc::c_char,
                                *plist.offset(i as isize));
                    }
    ...
    if !best_client.is_null() { *client = best_client }
