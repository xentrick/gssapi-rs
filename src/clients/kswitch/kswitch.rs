...
pub use self::krb5_h::{krb5_int32, krb5_boolean, krb5_enctype, krb5_flags,
                       ...
                       _krb5_ccache, krb5_cc_close, krb5_init_context,
                       krb5_free_context, krb5_parse_name, krb5_cc_resolve,
                       krb5_cc_switch, krb5_cc_cache_match,
                       krb5_free_principal};
...
unsafe extern "C" fn usage() {
    fprintf(stderr,
            dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                     b"Usage: %s {-c cache_name | -p principal}\n\x00" as
...
unsafe fn main_0(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char)
 -> libc::c_int {
    ...
    let mut errflag: krb5_boolean = 0 as libc::c_int as krb5_boolean;
    ...
    loop  {
        c =
            getopt(argc, argv,
                   b"c:p:\x00" as *const u8 as *const libc::c_char);
        if !(c != -(1 as libc::c_int)) { break ; }
        match c {
            99 | 112 => {
                if !cache_name.is_null() || !princ_name.is_null() {
                    fprintf(stderr,
                            dgettext(b"mit-krb5\x00" as *const u8 as
                                         *const libc::c_char,
                                     b"Only one -c or -p option allowed\n\x00"
                                         as *const u8 as
                                         *const libc::c_char));
                    errflag = 1 as libc::c_int as krb5_boolean
                } else if c == 'c' as i32 {
                    cache_name = optarg
                } else { princ_name = optarg }
            }
            63 | _ => { errflag = 1 as libc::c_int as krb5_boolean }
        }
    }
    if optind != argc { errflag = 1 as libc::c_int as krb5_boolean }
    if cache_name.is_null() && princ_name.is_null() {
        fprintf(stderr,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"One of -c or -p must be specified\n\x00" as
                             *const u8 as *const libc::c_char));
        errflag = 1 as libc::c_int as krb5_boolean
    }
    if errflag != 0 { usage(); }
    ...
    if !cache_name.is_null() {
        ret = krb5_cc_resolve(context, cache_name, &mut cache);
        if ret != 0 as libc::c_int {
            com_err(progname, ret as errcode_t,
                    dgettext(b"mit-krb5\x00" as *const u8 as
                                 *const libc::c_char,
                             b"while resolving %s\x00" as *const u8 as
                                 *const libc::c_char), cache_name);
            exit(1 as libc::c_int);
        }
    } else {
        ret = krb5_parse_name(context, princ_name, &mut princ);
        if ret != 0 {
            com_err(progname, ret as errcode_t,
                    dgettext(b"mit-krb5\x00" as *const u8 as
                                 *const libc::c_char,
        ...
        ret = krb5_cc_cache_match(context, princ, &mut cache);
        if ret != 0 {
            com_err(progname, ret as errcode_t,
                    dgettext(b"mit-krb5\x00" as *const u8 as
                                 *const libc::c_char,
                             b"while searching for ccache for %s\x00" as
                                 *const u8 as *const libc::c_char),
                    princ_name);
            exit(1 as libc::c_int);
        }
    ...
    if ret != 0 as libc::c_int {
        com_err(progname, ret as errcode_t,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"while switching to credential cache\x00" as
                             *const u8 as *const libc::c_char));
        exit(1 as libc::c_int);
    }
