...
#[c2rust::header_src = "/home/nmavis/dev/gssapi-rs/code/src/include/gssapi/gssapi_alloc.h:28"]
pub mod gssapi_alloc_h {
    ...
    pub unsafe extern "C" fn gssalloc_strdup(mut str: *const i8) -> *mut i8 {
        let mut size = strlen(str).wrapping_add(1usize);
        let mut copy = gssalloc_malloc(size) as *mut i8;
        if !copy.is_null() {
            memcpy(copy as *mut libc::c_void, str as *const libc::c_void, size);
...
pub use self::gssapi_alloc_h::gssalloc_strdup;
