...
pub mod thread_shared_types_h {
    ...
    #[repr(C)]
    #[c2rust::src_loc = "82:16"]#[derive(Copy, Clone)]
    ...
    #[repr(C)]
    #[c2rust::src_loc = "118:8"]#[derive(Copy, Clone)]
...
pub mod pthreadtypes_h {
    ...
    #[repr(C)]
    #[c2rust::src_loc = "67:9"]#[derive(Copy, Clone)]
...
pub mod struct_FILE_h {
    ...
    #[repr(C)]
    #[c2rust::src_loc = "49:8"]#[derive(Copy, Clone)]
    pub struct _IO_FILE {
        pub _flags: i32,
        ...
        if r != 0 as i32 {
            fprintf(stderr,
                    b"k5_mutex_lock: Received error %d (%s)\n\x00" as
                        *const u8 as *const i8, r, strerror(r));
        }
        if r == 0 as i32 {
        } else {
            __assert_fail(b"r == 0\x00" as *const u8 as *const i8,
                          b"../../../include/k5-thread.h\x00" as *const u8 as
                              *const i8,
                          376 as i32 as u32,
                          (*::std::mem::transmute::<&[u8; 33],
                                                    &[i8; 33]>(b"void k5_mutex_lock(k5_mutex_t *)\x00")).as_ptr());
        };
    }
    #[inline]
        ...
        if r == 0 as i32 {
            ...
            __assert_fail(b"r == 0\x00" as *const u8 as *const i8,
                          ...
                          (*::std::mem::transmute::<&[u8; 35],
                                                    &[i8; 35]>(b"void k5_mutex_unlock(k5_mutex_t *)\x00")).as_ptr());
        };
    }
 ...
 */
    ...
    #[repr(C)]
    #[c2rust::src_loc = "29:8"]#[derive(Copy, Clone)]
    ...
    #[repr(C)]
    #[c2rust::src_loc = "151:9"]#[derive(Copy, Clone)]
 ...
 */
    ...
    #[repr(C)]
    #[c2rust::src_loc = "58:8"]#[derive(Copy, Clone)]
...
/* start of t_bimap body template */
   ...
   "const" before a type that turns out to be "char *"  */
    ...
    pub unsafe extern "C" fn mecherrmap__pairarray_init(mut arr:
     ...
     -> i32 {
        (*arr).elts =
            calloc(10 as i32 as usize,
                   ::std::mem::size_of::<mecherrmap__pair>() as usize)
                as *mut mecherrmap__pair;
        if (*arr).elts.is_null() { return 12 as i32 }
        (*arr).allocated = 10 as i32 as size_t;
        return 0 as i32;
    }
    #[inline]
    #[c2rust::src_loc = "74:1"]
     ...
     -> usize {
        ...
        upper_bound =
            (18446744073709551615 as
                 usize).wrapping_div(::std::mem::size_of::<mecherrmap__pair>()
                                                 as usize);
        if upper_bound >
     ...
     -> i32 {
        let mut oldsize =
            (::std::mem::size_of::<mecherrmap__pair>() as
                 usize).wrapping_mul((*arr).allocated);
        let mut newsize: size_t = 0;
        let mut ptr = 0 as *mut libc::c_void;
        if newcount > 9223372036854775807 as isize as usize {
            return -(1 as i32)
        }
        if newcount < (*arr).allocated { return 0 as i32 }
        if newcount > mecherrmap__pairarray_max_size(arr) {
            return -(1 as i32)
        }
        newsize =
            (::std::mem::size_of::<mecherrmap__pair>() as
                 usize).wrapping_mul(newcount);
        ptr = realloc((*arr).elts as *mut libc::c_void, newsize);
        if ptr.is_null() { return -(1 as i32) }
        memset((ptr as *mut i8).offset(oldsize as isize) as
                   *mut libc::c_void, 0 as i32,
    ...
    pub unsafe extern "C" fn mecherrmap__pairarray_getaddr(mut arr:
     ...
     -> *mut mecherrmap__pair {
        if idx < 0 as i32 as isize ||
               idx as usize >= (*arr).allocated {
            abort();
        }
        ...
        while i < sz {
            ...
            if Some(Some(cmp_OM_uint32 as
                             unsafe extern "C" fn(_: OM_uint32, _: OM_uint32)
                                 ->
                                     i32).expect("non-null function pointer")).expect("non-null function pointer")(l,
                                                                                                                           (*pair).l)
                   != 0 as i32 {
            } else {
                __assert_fail(b"(*cmp_OM_uint32)(l, pair->l) != 0\x00" as
                              ...
                              (*::std::mem::transmute::<&[u8; 62],
                                                        &[i8; 62]>(b"int mecherrmap_add(mecherrmap *, OM_uint32, struct mecherror)\x00")).as_ptr());
            }
            if Some(Some(cmp_OM_uint32 as
                             unsafe extern "C" fn(_: OM_uint32, _: OM_uint32)
                                 ->
            ...
            if Some(Some(mecherror_cmp as
                             unsafe extern "C" fn(_: mecherror, _: mecherror)
                                 ->
                                     i32).expect("non-null function pointer")).expect("non-null function pointer")(r,
                                                                                                                           (*pair).r)
                   != 0 as i32 {
            } else {
...
pub mod stdio_h {
    ...
    extern "C" {
        ...
        pub fn fprintf(_: *mut FILE, _: *const i8, _: ...)
         -> i32;
    }
}
...
#[repr(C)]
#[c2rust::src_loc = "40:8"]#[derive(Copy, Clone)]
...
unsafe extern "C" fn mecherror_cmp(mut m1: mecherror, mut m2: mecherror)
 -> i32 {
    if m1.code < m2.code { return -(1 as i32) }
    if m1.code > m2.code { return 1 as i32 }
    if m1.mech.length < m2.mech.length { return -(1 as i32) }
    if m1.mech.length > m2.mech.length { return 1 as i32 }
    if m1.mech.length == 0 as i32 as u32 {
        return 0 as i32
    }
    return memcmp(m1.mech.elements, m2.mech.elements,
                  m1.mech.length as usize);
}
#[inline]
#[c2rust::src_loc = "78:1"]
unsafe extern "C" fn mecherror_copy(mut dest: *mut mecherror,
                                    mut src: mecherror) -> i32 {
    *dest = src;
    if src.mech.length > 0 as i32 as u32 {
        (*dest).mech.elements = malloc(src.mech.length as usize);
        if (*dest).mech.elements.is_null() { return 12 as i32 }
...
static mut mutex: k5_mutex_t =
    pthread_mutex_t{__data:
                        {
                            let mut init =
                                __pthread_mutex_s{__lock: 0 as i32,
                                                  __count:
                                                      0 as i32 as
                                                          u32,
                                                  __owner: 0 as i32,
...
static mut next_fake: OM_uint32 = 100000 as i32 as OM_uint32;
#[no_mangle]
...
static mut no_oid: gss_OID_desc =
    {
        let mut init =
            gss_OID_desc_struct{length: 0 as i32 as OM_uint32,
                                elements:
...
pub unsafe extern "C" fn gssint_mecherrmap_get(mut minor: OM_uint32,
 ...
 -> i32 {
    ...
    if minor == 0 as i32 as u32 { return 22 as i32 }
    k5_mutex_lock(&mut mutex);
    p = mecherrmap_findleft(&mut m, minor);
    k5_mutex_unlock(&mut mutex);
    if p.is_null() { return 22 as i32 }
    *mech_oid = (*p).mech;
    *mech_minor = (*p).code;
    return 0 as i32;
}
