...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_ui_4, krb5_boolean,
                       ...
                       krb5_replay_data, krb5_ccache, krb5_kt_cursor,
                       ...
                       krb5_get_credentials, krb5_rd_safe, krb5_unparse_name,
                       krb5_build_principal_ext, krb5_cc_default,
                       krb5_free_ticket, krb5_free_creds,
                       krb5_free_cred_contents, krb5_free_data_contents,
                       krb5_rd_req, krb5_auth_con_init, krb5_auth_con_free,
                       krb5_auth_con_setflags, krb5_auth_con_setuseruserkey,
                       krb5_get_host_realm, krb5_free_host_realm,
...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _kdb5_dal_handle, krb5_read_message,
                         krb5_write_message};
...
pub use self::sys_socket_h::{__CONST_SOCKADDR_ARG, sockaddr_x25, sockaddr_un,
                             ...
                             sockaddr_ax25, sockaddr_at, socket, bind,
                             connect};
...
use self::stdio_h::{stderr, fprintf, printf, fputs};
...
unsafe fn main_0(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char)
 -> libc::c_int {
    let mut s: libc::c_int = 0; /* full name of server */
    let mut retval: libc::c_int = 0; /* realm(s) of server */
    let mut i: libc::c_int = 0; /* principal in credentials cache */
    let mut hname: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut srealms: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    ...
    let mut serv_net_addr: sockaddr_in =
        sockaddr_in{sin_family: 0,
                    sin_port: 0,
                    sin_addr: in_addr{s_addr: 0,},
                    sin_zero: [0; 8],};
    let mut cli_net_addr: sockaddr_in =
        sockaddr_in{sin_family: 0,
                    sin_port: 0,
                    sin_addr: in_addr{s_addr: 0,},
                    sin_zero: [0; 8],};
    ...
    if argc < 2 as libc::c_int || argc > 4 as libc::c_int {
        fputs(b"usage: uu-client <hostname> [message [port]]\n\x00" as
    ...
    if argc == 4 as libc::c_int {
        ...
        if serv.is_null() {
            fputs(b"uu-client: unknown service \"uu-sample/tcp\"\n\x00" as
                      *const u8 as *const libc::c_char, stderr);
            exit(2 as libc::c_int);
        } else { port = (*serv).s_port as libc::c_ushort }
    }
    host = gethostbyname(*argv.offset(1 as libc::c_int as isize));
    if host.is_null() {
        fprintf(stderr,
                b"uu-client: can\'t get address of host \"%s\".\n\x00" as
                    *const u8 as *const libc::c_char,
                *argv.offset(1 as libc::c_int as isize));
        exit(3 as libc::c_int);
    }
    if (*host).h_addrtype != 2 as libc::c_int {
        fprintf(stderr,
                b"uu-client: bad address type %d for \"%s\".\n\x00" as
                    *const u8 as *const libc::c_char, (*host).h_addrtype,
                *argv.offset(1 as libc::c_int as isize));
        exit(3 as libc::c_int);
    }
    ...
    if s < 0 as libc::c_int {
        ...
        cli_net_addr.sin_family = 2 as libc::c_int as sa_family_t;
        cli_net_addr.sin_port = 0 as libc::c_int as in_port_t;
        cli_net_addr.sin_addr.s_addr = 0 as libc::c_int as in_addr_t;
        if bind(s,
                __CONST_SOCKADDR_ARG{__sockaddr__:
                                         &mut cli_net_addr as *mut sockaddr_in
                                             as *mut sockaddr,},
                ::std::mem::size_of::<sockaddr_in>() as libc::c_ulong as
                    socklen_t) < 0 as libc::c_int {
            com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                    *__errno_location() as errcode_t,
                    b"binding socket\x00" as *const u8 as
                        *const libc::c_char);
            exit(4 as libc::c_int);
        }
    }
    serv_net_addr.sin_family = 2 as libc::c_int as sa_family_t;
    serv_net_addr.sin_port = port;
    i = 0 as libc::c_int;
    loop  {
        if (*(*host).h_addr_list.offset(i as isize)).is_null() {
            fprintf(stderr,
                    b"uu-client: unable to connect to \"%s\"\n\x00" as
                        *const u8 as *const libc::c_char, hname);
            exit(5 as libc::c_int);
        }
        let fresh0 = i;
        i = i + 1;
        memcpy(&mut serv_net_addr.sin_addr as *mut in_addr as
                   *mut libc::c_void,
               *(*host).h_addr_list.offset(fresh0 as isize) as
                   *const libc::c_void,
               ::std::mem::size_of::<in_addr>() as libc::c_ulong);
        if connect(s,
                   __CONST_SOCKADDR_ARG{__sockaddr__:
                                            &mut serv_net_addr as
                                                *mut sockaddr_in as
                                                *mut sockaddr,},
                   ::std::mem::size_of::<sockaddr_in>() as libc::c_ulong as
                       socklen_t) == 0 as libc::c_int {
        ...
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                *__errno_location() as errcode_t,
                b"connecting to \"%s\" (%s).\x00" as *const u8 as
                    *const libc::c_char, hname,
                inet_ntoa(serv_net_addr.sin_addr));
    }
    ...
    retval =
        krb5_unparse_name(context, creds.client as krb5_const_principal,
                          &mut princ);
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"printing principal name\x00" as *const u8 as
                    *const libc::c_char);
        exit(7 as libc::c_int);
    } else {
        fprintf(stderr,
                b"uu-client: client principal is \"%s\".\n\x00" as *const u8
                    as *const libc::c_char, princ);
    }
    retval = krb5_get_host_realm(context, hname, &mut srealms);
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"getting realms for \"%s\"\x00" as *const u8 as
                    *const libc::c_char, hname);
        exit(7 as libc::c_int);
    }
    retval =
        krb5_build_principal_ext(context,
                                 ...
                                 (*creds.client).realm.data, 6 as libc::c_int,
    ...
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"setting up tgt server name\x00" as *const u8 as
                    *const libc::c_char);
        exit(7 as libc::c_int);
    }
    /* Get TGT from credentials cache */
    retval =
        krb5_get_credentials(context, 2 as libc::c_int, cc, &mut creds,
                             &mut new_creds); /* include null terminator for
                                             server's convenience */
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"getting TGT\x00" as *const u8 as *const libc::c_char);
        exit(6 as libc::c_int);
    }
    i =
        strlen(princ).wrapping_add(1 as libc::c_int as libc::c_ulong) as
            libc::c_int;
    fprintf(stderr,
            b"uu-client: sending %d bytes\n\x00" as *const u8 as
                *const libc::c_char,
            (*new_creds).ticket.length.wrapping_add(i as libc::c_uint));
    princ_data.data = princ;
    princ_data.length = i as libc::c_uint;
    retval =
        krb5_write_message(context,
                           &mut s as *mut libc::c_int as krb5_pointer,
                           &mut princ_data);
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"sending principal name to server\x00" as *const u8 as
                    *const libc::c_char);
    ...
    retval =
        krb5_write_message(context,
                           &mut s as *mut libc::c_int as krb5_pointer,
                           &mut (*new_creds).ticket);
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"sending ticket to server\x00" as *const u8 as
                    *const libc::c_char);
        exit(8 as libc::c_int);
    }
    retval =
        krb5_read_message(context, &mut s as *mut libc::c_int as krb5_pointer,
                          &mut reply);
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"reading reply from server\x00" as *const u8 as
                    *const libc::c_char);
    ...
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"initializing the auth_context\x00" as *const u8 as
                    *const libc::c_char);
        exit(9 as libc::c_int);
    }
    retval =
    ...
    retval =
        krb5_auth_con_setuseruserkey(context, auth_context,
                                     &mut (*new_creds).keyblock);
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"setting useruserkey for authcontext\x00" as *const u8 as
                    *const libc::c_char);
        exit(9 as libc::c_int);
    }
    /* read the ap_req to get the session key */
    retval =
        krb5_rd_req(context, &mut auth_context, &mut reply,
                    creds.client as krb5_const_principal, 0 as krb5_keytab,
                    0 as *mut krb5_flags, &mut ticket);
    krb5_free_data_contents(context, &mut reply);
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"reading AP_REQ from server\x00" as *const u8 as
                    *const libc::c_char);
        exit(9 as libc::c_int);
    }
    retval =
        krb5_unparse_name(context,
                          (*(*ticket).enc_part2).client as
                              krb5_const_principal, &mut princ);
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
    ...
    retval =
        krb5_rd_safe(context, auth_context, &mut reply, &mut msg,
                     0 as *mut krb5_replay_data);
    if retval != 0 {
        com_err(b"uu-client\x00" as *const u8 as *const libc::c_char,
                retval as errcode_t,
                b"decoding reply from server\x00" as *const u8 as
                    *const libc::c_char);
        exit(10 as libc::c_int);
    }
    printf(b"uu-client: server says \"%s\".\n\x00" as *const u8 as
               *const libc::c_char, msg.data);
    close(s);
