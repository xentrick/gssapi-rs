...
#[c2rust::header_src = "/usr/include/bits/getopt_core.h:41"]
...
#[c2rust::header_src = "/usr/include/sys/syslog.h:51"]
pub mod syslog_h {
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "181:1"]
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_ticket, krb5_kt_cursor, krb5_keytab_entry_st,
                       krb5_keytab_entry, krb5_keytab, _profile_t,
                       _krb5_auth_context, krb5_kt_close, krb5_init_context,
                       krb5_free_context, krb5_unparse_name, krb5_kt_resolve,
                       krb5_free_principal, krb5_free_ticket,
                       krb5_sname_to_principal, krb5_recvauth,
                       krb5_auth_con_free};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _kdb5_dal_handle, krb5_net_write};
...
pub use self::sys_socket_h::{__SOCKADDR_ARG, __CONST_SOCKADDR_ARG,
                             ...
                             sockaddr_at, socket, bind, getpeername,
...
use self::string_h::{strlen, strncat, strncpy};
use self::syslog_h::{openlog, syslog};
#[c2rust::src_loc = "67:1"]
unsafe extern "C" fn usage(mut name: *mut libc::c_char) {
    fprintf(stderr,
            b"usage: %s [-p port] [-s service] [-S keytab]\n\x00" as *const u8
                as *const libc::c_char, name); /* incoming connection fd */
}
#[c2rust::src_loc = "74:1"]
unsafe fn main_0(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char)
 -> libc::c_int {
    let mut context: krb5_context =
        0 as *mut _krb5_context; /* If user specifies port */
    let mut auth_context: krb5_auth_context =
        0 as krb5_auth_context; /* Allow specification on command line */
    let mut ticket: *mut krb5_ticket = 0 as *mut krb5_ticket;
    let mut peername: sockaddr_in =
    ...
    let mut repbuf: [libc::c_char; 8192] = [0; 8192];
    ...
    openlog(b"sserver\x00" as *const u8 as *const libc::c_char,
            0 as libc::c_int, (3 as libc::c_int) << 3 as libc::c_int);
    /*
    ...
    if argc > 1 as libc::c_int {
        port = atoi(*argv.offset(1 as libc::c_int as isize)) as libc::c_short
    }
    retval =
        krb5_sname_to_principal(context, 0 as *const libc::c_char, service,
                                3 as libc::c_int, &mut server);
    if retval != 0 {
        syslog(3 as libc::c_int,
               b"while generating service name (%s): %s\x00" as *const u8 as
    ...
    if port != 0 {
        ...
        if sock < 0 as libc::c_int {
            syslog(3 as libc::c_int,
                   b"socket: %m\x00" as *const u8 as *const libc::c_char);
        ...
        setsockopt(sock, 1 as libc::c_int, 2 as libc::c_int,
                   ...
                   ::std::mem::size_of::<libc::c_int>() as libc::c_ulong as
                       socklen_t);
        sockin.sin_family = 2 as libc::c_int as sa_family_t;
        sockin.sin_addr.s_addr = 0 as libc::c_int as in_addr_t;
        sockin.sin_port = htons(port as uint16_t);
        if bind(sock,
                __CONST_SOCKADDR_ARG{__sockaddr__:
                                         &mut sockin as *mut sockaddr_in as
                                             *mut sockaddr,},
                ::std::mem::size_of::<sockaddr_in>() as libc::c_ulong as
                    socklen_t) != 0 {
            syslog(3 as libc::c_int,
                   b"bind: %m\x00" as *const u8 as *const libc::c_char);
            exit(3 as libc::c_int);
        }
        if listen(sock, 1 as libc::c_int) == -(1 as libc::c_int) {
            syslog(3 as libc::c_int,
                   b"listen: %m\x00" as *const u8 as *const libc::c_char);
            exit(3 as libc::c_int);
        }
        acc =
            accept(sock,
                   __SOCKADDR_ARG{__sockaddr__:
                                      &mut peername as *mut sockaddr_in as
                                          *mut sockaddr,}, &mut namelen);
        if acc == -(1 as libc::c_int) {
            syslog(3 as libc::c_int,
                   b"accept: %m\x00" as *const u8 as *const libc::c_char);
            exit(3 as libc::c_int);
        }
         ...
         * client.
         */
        if getpeername(0 as libc::c_int,
                       __SOCKADDR_ARG{__sockaddr__:
                                          &mut peername as *mut sockaddr_in as
                                              *mut sockaddr,}, &mut namelen) <
               0 as libc::c_int {
            syslog(3 as libc::c_int,
                   b"getpeername: %m\x00" as *const u8 as
                       *const libc::c_char);
            exit(1 as libc::c_int);
        }
        sock = 0 as libc::c_int
    ...
    retval =
        krb5_recvauth(context, &mut auth_context,
                      ...
                      b"KRB5_sample_protocol_v1.0\x00" as *const u8 as
                          *const libc::c_char as *mut libc::c_char, server,
                      0 as libc::c_int, keytab, &mut ticket);
    if retval != 0 {
        syslog(3 as libc::c_int,
               b"recvauth failed--%s\x00" as *const u8 as *const libc::c_char,
               error_message(retval as errcode_t));
        exit(1 as libc::c_int);
    }
    /* Get client name */
    repbuf[(::std::mem::size_of::<[libc::c_char; 8192]>() as
    ...
    retval =
        krb5_unparse_name(context,
                          (*(*ticket).enc_part2).client as
                              krb5_const_principal, &mut cname);
    if retval != 0 {
        syslog(3 as libc::c_int,
               b"unparse failed: %s\x00" as *const u8 as *const libc::c_char,
               error_message(retval as errcode_t));
        strncpy(repbuf.as_mut_ptr(),
                b"You are <unparse error>\n\x00" as *const u8 as
                    *const libc::c_char,
        ...
        strncpy(repbuf.as_mut_ptr(),
                b"You are \x00" as *const u8 as *const libc::c_char,
                (::std::mem::size_of::<[libc::c_char; 8192]>() as
                     libc::c_ulong).wrapping_sub(1 as libc::c_int as
                                                     libc::c_ulong));
        strncat(repbuf.as_mut_ptr(), cname,
                (::std::mem::size_of::<[libc::c_char; 8192]>() as
                     libc::c_ulong).wrapping_sub(1 as libc::c_int as
                                                     libc::c_ulong).wrapping_sub(strlen(repbuf.as_mut_ptr())));
        strncat(repbuf.as_mut_ptr(),
                b"\n\x00" as *const u8 as *const libc::c_char,
                (::std::mem::size_of::<[libc::c_char; 8192]>() as
                     libc::c_ulong).wrapping_sub(1 as libc::c_int as
                                                     libc::c_ulong).wrapping_sub(strlen(repbuf.as_mut_ptr())));
        free(cname as *mut libc::c_void);
    }
    xmitlen = htons(strlen(repbuf.as_mut_ptr()) as uint16_t) as libc::c_short;
    recv_data.length = strlen(repbuf.as_mut_ptr()) as libc::c_uint;
    recv_data.data = repbuf.as_mut_ptr();
    retval =
        krb5_net_write(context, 0 as libc::c_int,
                       &mut xmitlen as *mut libc::c_short as
                           *mut libc::c_char,
                       ::std::mem::size_of::<libc::c_short>() as libc::c_ulong
                           as libc::c_int);
    if retval < 0 as libc::c_int {
        syslog(3 as libc::c_int,
               b"%m: while writing len to client\x00" as *const u8 as
                   *const libc::c_char);
        exit(1 as libc::c_int);
    }
    retval =
        krb5_net_write(context, 0 as libc::c_int, recv_data.data,
                       recv_data.length as libc::c_int);
    if retval < 0 as libc::c_int {
        syslog(3 as libc::c_int,
               b"%m: while writing data to client\x00" as *const u8 as
                   *const libc::c_char);
        exit(1 as libc::c_int);
    }
