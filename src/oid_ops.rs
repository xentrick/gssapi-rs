...
pub mod k5_buf_h {
    ...
    #[repr(C)]
    #[c2rust::src_loc = "50:8"]#[derive(Copy, Clone)]
    pub struct k5buf {
    ...
    extern "C" {
        ...
        #[no_mangle]
        #[c2rust::src_loc = "77:1"]
        pub fn k5_buf_add_fmt(buf: *mut k5buf, fmt: *const i8,
                              _: ...);
        #[no_mangle]
        #[c2rust::src_loc = "74:1"]
...
pub mod gssapiP_generic_h {
    ...
    pub unsafe extern "C" fn k5buf_to_gss(mut minor: *mut OM_uint32,
     ...
     -> OM_uint32 {
        let mut status = 0 as i32 as OM_uint32;
        if k5_buf_status(input_k5buf) != 0 as i32 {
            *minor = 12 as i32 as OM_uint32;
            return (13 as usize as OM_uint32) << 16 as i32
        }
        (*output_buffer).length = (*input_k5buf).len;
        (*output_buffer).value = (*input_k5buf).data;
        memset(input_k5buf as *mut libc::c_void, 0 as i32,
               ::std::mem::size_of::<k5buf>() as usize);
        return status;
    }
...
pub mod string_h {
    extern "C" {
        ...
        pub fn memset(_: *mut libc::c_void, _: i32, _: usize)
         -> *mut libc::c_void;
        #[no_mangle]
...
pub unsafe extern "C" fn generic_gss_copy_oid(mut minor_status:
 ...
 -> OM_uint32 {
    ...
    *minor_status = 0 as i32 as OM_uint32;
    p =
        malloc(::std::mem::size_of::<gss_OID_desc>() as usize) as
            gss_OID;
    if p.is_null() {
 ...
 -> OM_uint32 {
    ...
    *oid_set =
        gssalloc_malloc(::std::mem::size_of::<gss_OID_set_desc>() as
                            usize) as gss_OID_set;
    if !(*oid_set).is_null() {
        memset(*oid_set as *mut libc::c_void, 0 as i32,
               ::std::mem::size_of::<gss_OID_set_desc>() as usize);
        return 0 as i32 as OM_uint32
    } else {
        *minor_status = 12 as i32 as OM_uint32;
        return (13 as usize as OM_uint32) << 16 as i32
 ...
 -> OM_uint32 {
    ...
    *minor_status = 0 as i32 as OM_uint32;
    if member_oid.is_null() ||
           (*member_oid).length == 0 as i32 as u32 ||
           (*member_oid).elements.is_null() {
        return (1 as usize as OM_uint32) << 24 as i32
    }
    if oid_set.is_null() {
        return (2 as usize as OM_uint32) << 24 as i32
    }
    ...
    (**oid_set).elements =
        gssalloc_malloc((**oid_set).count.wrapping_add(1 as i32 as
                                                           usize).wrapping_mul(::std::mem::size_of::<gss_OID_desc>()
                                                                                           as
                                                                                           usize))
            as gss_OID;
...
pub unsafe extern "C" fn generic_gss_oid_to_str(mut minor_status:
 ...
 -> OM_uint32 {
    ...
    if !oid_str.is_null() {
        (*oid_str).length = 0 as i32 as size_t;
        (*oid_str).value = 0 as *mut libc::c_void
    }
    if oid.is_null() || (*oid).length == 0 as i32 as u32 ||
    ...
    while i < (*oid).length {
        number =
            number << 7 as i32 |
                (*cp.offset(i as isize) as i32 & 0x7f as i32)
                    as usize;
        if *cp.offset(i as isize) as i32 & 0x80 as i32 ==
               0 as i32 {
            if first != 0 {
                n =
                    if number < 40 as i32 as usize {
                        0 as i32
                    } else if number < 80 as i32 as usize {
                        1 as i32
                    } else { 2 as i32 } as usize;
    ...
    k5_buf_add_len(&mut buf,
                   b"}\x00\x00" as *const u8 as *const i8 as
                       *const libc::c_void, 2 as i32 as size_t);
...
unsafe extern "C" fn arc_encoded_length(mut arc: usize) -> size_t {
    let mut len = 1 as i32 as size_t;
    arc >>= 7 as i32;
    while arc != 0 { len = len.wrapping_add(1); arc >>= 7 as i32 }
    return len;
}
/* Encode a subidentifier into *bufp and advance it to the encoding's end. */
#[c2rust::src_loc = "300:1"]
unsafe extern "C" fn arc_encode(mut arc: usize,
                                mut bufp: *mut *mut u8) {
    ...
    *bufp = (*bufp).offset(arc_encoded_length(arc) as isize);
    p = *bufp;
    p = p.offset(-1);
    *p = (arc & 0x7f as i32 as usize) as u8;
    arc >>= 7 as i32;
    while arc != 0 {
        p = p.offset(-1);
        *p =
            (arc & 0x7f as i32 as usize |
                 0x80 as i32 as usize) as u8;
        arc >>= 7 as i32
    };
}
...
unsafe extern "C" fn get_arc(mut bufp: *mut *const u8,
                             mut end: *const u8,
                             mut arc_out: *mut usize) -> i32 {
    let mut p = *bufp;
    let mut arc = 0 as i32 as usize;
    let mut newval: usize = 0;
    if p == end ||
           *(*__ctype_b_loc()).offset(*p as i32 as isize) as
               i32 &
               _ISdigit as i32 as u16 as i32 == 0 {
        return 0 as i32
    }
    while p < end &&
              *(*__ctype_b_loc()).offset(*p as i32 as isize) as
                  ...
                  0 {
        newval =
            arc.wrapping_mul(10 as i32 as
                                 usize).wrapping_add((*p as
                                                                  i32
                                                                  -
                                                                  '0' as i32)
                                                                 as
                                                                 usize);
        if newval < arc { return 0 as i32 }
        arc = newval;
        p = p.offset(1)
    }
    while p < end &&
...
pub unsafe extern "C" fn generic_gss_str_to_oid(mut minor_status:
 ...
 -> OM_uint32 {
    ...
    if oid_str.is_null() || (*oid_str).value.is_null() ||
           (*oid_str).length == 0 as i32 as usize {
        return (1 as usize as OM_uint32) << 24 as i32
    }
    if oid_out.is_null() {
        return (2 as usize as OM_uint32) << 24 as i32
    ...
    if arc1 > 2 as i32 as usize ||
           arc1 < 2 as i32 as usize &&
               arc2 > 39 as i32 as usize ||
           arc2 >
               (9223372036854775807 as isize as
                    usize).wrapping_mul(2 as
                                                    usize).wrapping_add(1
                                                                                    as
                                                                                    usize).wrapping_sub(80
                                                                                                                    as
                                                                                                                    i32
                                                                                                                    as
                                                                                                                    usize)
    ...
    nbytes =
        arc_encoded_length(arc1.wrapping_mul(40 as i32 as
                                                 usize).wrapping_add(arc2));
    while get_arc(&mut p, end, &mut arc) != 0 {
        nbytes =
            (nbytes as usize).wrapping_add(arc_encoded_length(arc)) as
                size_t as size_t
    }
    ...
    oid =
        malloc(::std::mem::size_of::<gss_OID_desc_struct>() as usize)
            as gss_OID;
    if oid.is_null() {
        return (13 as usize as OM_uint32) << 16 as i32
    }
    ...
    arc_encode(arc1.wrapping_mul(40 as i32 as
                                     usize).wrapping_add(arc2),
               &mut out);
    p = arc3_start;
    ...
    if out.offset(-(nbytes as isize)) == (*oid).elements as *mut u8
       {
        ...
        __assert_fail(b"out - nbytes == oid->elements\x00" as *const u8 as
                      ...
                      (*::std::mem::transmute::<&[u8; 71],
                                                &[i8; 71]>(b"OM_uint32 generic_gss_str_to_oid(OM_uint32 *, gss_buffer_t, gss_OID *)\x00")).as_ptr());
    }
    *oid_out = oid;
    return 0 as i32 as OM_uint32;
}
...
pub unsafe extern "C" fn generic_gss_oid_compose(mut minor_status:
 ...
 -> OM_uint32 {
    ...
    if oid.is_null() {
        *minor_status = 22 as i32 as OM_uint32;
        return (13 as usize as OM_uint32) << 16 as i32
    }
    if ((*oid).length as usize) < prefix_len {
        *minor_status = 34 as i32 as OM_uint32;
        return (13 as usize as OM_uint32) << 16 as i32
    }
    memcpy((*oid).elements, prefix as *const libc::c_void, prefix_len);
    nbytes = 0 as i32 as size_t;
    osuffix = suffix;
    while suffix != 0 {
    ...
    op =
        ((*oid).elements as
             *mut u8).offset(prefix_len as
                                            isize).offset(nbytes as isize);
    i = -(1 as i32);
    while suffix != 0 {
        *op.offset(i as isize) =
            (suffix as u8 as i32 & 0x7f as i32) as
                u8;
        if i != -(1 as i32) {
            let ref mut fresh0 = *op.offset(i as isize);
            *fresh0 =
                (*fresh0 as i32 | 0x80 as i32) as
                    u8
        }
        i -= 1;
        suffix >>= 7 as i32
 ...
 -> OM_uint32 {
    ...
    if ((*oid).length as usize) < prefix_len ||
           memcmp((*oid).elements, prefix as *const libc::c_void, prefix_len)
               != 0 as i32 {
        return (1 as usize as OM_uint32) << 16 as i32
    }
    ...
    *suffix = 0 as i32;
    ...
    while i < slen {
        *suffix =
            *suffix << 7 as i32 |
                *op.offset(i as isize) as i32 & 0x7f as i32;
        if i.wrapping_add(1 as i32 as usize) != slen &&
               *op.offset(i as isize) as i32 & 0x80 as i32 ==
                   0 as i32 {
            *minor_status = 22 as i32 as OM_uint32;
            return (13 as usize as OM_uint32) << 16 as i32
        }
...
pub unsafe extern "C" fn generic_gss_copy_oid_set(mut minor_status:
 ...
 -> OM_uint32 {
    ...
    let mut minor = 0 as i32 as OM_uint32;
    let mut major = 0 as i32 as OM_uint32;
    let mut i: OM_uint32 = 0;
    if !minor_status.is_null() {
        *minor_status = 0 as i32 as OM_uint32
    }
    ...
    copy =
        gssalloc_calloc(1 as i32 as size_t,
                        ::std::mem::size_of::<gss_OID_set_desc>() as
                            usize) as *mut gss_OID_set_desc;
    if copy.is_null() {
        major = (13 as usize as OM_uint32) << 16 as i32
    } else {
        (*copy).elements =
            gssalloc_calloc((*oidset).count,
                            ::std::mem::size_of::<gss_OID_desc_struct>() as
                                usize) as *mut gss_OID_desc;
