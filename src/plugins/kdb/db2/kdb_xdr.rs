...
pub use self::kdb_h::{krb5_tl_data, _krb5_tl_data, krb5_key_data,
                      _krb5_key_data, krb5_db_entry, _krb5_db_entry_new,
                      krb5_db_free_principal};
use self::stdlib_h::{malloc, calloc, free, abort};
use self::string_h::{strlen, memchr, memset, memcpy};
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
...
#[no_mangle]
#[c2rust::src_loc = "33:1"]
pub unsafe extern "C" fn krb5_encode_princ_dbkey(mut context: krb5_context,
                                                 mut key: *mut krb5_data,
                                                 mut principal:
                                                     krb5_const_principal)
 -> krb5_error_code {
    let mut princ_name: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut retval: krb5_error_code = 0;
    retval = krb5_unparse_name(context, principal, &mut princ_name);
    if retval == 0 {
        /* need to store the NULL for decoding */
        (*key).length =
            strlen(princ_name).wrapping_add(1 as libc::c_int as libc::c_ulong)
                as libc::c_uint;
        (*key).data = princ_name
    }
    return retval;
...
#[no_mangle]
#[c2rust::src_loc = "48:1"]
pub unsafe extern "C" fn krb5_encode_princ_entry(mut context: krb5_context,
                                                 mut content: *mut krb5_data,
                                                 mut entry:
                                                     *mut krb5_db_entry)
 -> krb5_error_code {
    let mut i: libc::c_int = 0;
    let mut j: libc::c_int = 0;
    let mut unparse_princ_size: libc::c_uint = 0;
    let mut unparse_princ: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut nextloc: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut tl_data: *mut krb5_tl_data = 0 as *mut krb5_tl_data;
    let mut retval: krb5_error_code = 0;
    let mut psize16: krb5_int16 = 0;
    /*
     ...
     */
    (*content).length =
        ((*entry).len as libc::c_int + (*entry).e_length as libc::c_int) as
            libc::c_uint;
    retval =
    ...
    unparse_princ_size =
        strlen(unparse_princ).wrapping_add(1 as libc::c_int as libc::c_ulong)
            as libc::c_uint;
    (*content).length = (*content).length.wrapping_add(unparse_princ_size);
    (*content).length =
        (*content).length.wrapping_add(2 as libc::c_int as libc::c_uint);
    i = 0 as libc::c_int;
    /* tl_data is a linked list */
    tl_data = (*entry).tl_data; /* type, length */
    while !tl_data.is_null() {
        (*content).length =
            (*content).length.wrapping_add((*tl_data).tl_data_length as
                                               libc::c_uint);
        (*content).length =
            (*content).length.wrapping_add(4 as libc::c_int as libc::c_uint);
        i += 1;
        tl_data = (*tl_data).tl_data_next
    }
    if i != (*entry).n_tl_data as libc::c_int {
        retval = -(1780008439 as libc::c_long) as krb5_error_code
    } else {
        /* key_data is an array */
        i = 0 as libc::c_int; /* Version, KVNO */
        while i < (*entry).n_key_data as libc::c_int {
            (*content).length =
                (*content).length.wrapping_add(4 as libc::c_int as
                                                   libc::c_uint);
            j = 0 as libc::c_int;
            while j <
                      (*(*entry).key_data.offset(i as isize)).key_data_ver as
                          libc::c_int {
                (*content).length =
                    (*content).length.wrapping_add((*(*entry).key_data.offset(i
                                                                                  as
                                                                                  isize)).key_data_length[j
                                                                                                              as
                                                                                                              usize]
                                                       as libc::c_uint);
                (*content).length =
        ...
        (*content).data =
            malloc((*content).length as libc::c_ulong) as *mut libc::c_char;
     ...
     */
            nextloc = (*content).data as *mut libc::c_uchar;
            /* Base Length */
            store_16_le((*entry).len as libc::c_uint,
                        nextloc as *mut libc::c_void);
            nextloc = nextloc.offset(2 as libc::c_int as isize);
            /* Attributes */
            store_32_le((*entry).attributes as libc::c_uint,
                        nextloc as *mut libc::c_void);
            nextloc = nextloc.offset(4 as libc::c_int as isize);
            /* Max Life */
            store_32_le((*entry).max_life as libc::c_uint,
                        nextloc as *mut libc::c_void);
            nextloc = nextloc.offset(4 as libc::c_int as isize);
            /* Max Renewable Life */
            store_32_le((*entry).max_renewable_life as libc::c_uint,
                        nextloc as *mut libc::c_void);
            nextloc = nextloc.offset(4 as libc::c_int as isize);
            /* When the client expires */
            store_32_le((*entry).expiration as libc::c_uint,
                        nextloc as *mut libc::c_void);
            nextloc = nextloc.offset(4 as libc::c_int as isize);
            /* When its passwd expires */
            store_32_le((*entry).pw_expiration as libc::c_uint,
                        nextloc as *mut libc::c_void);
            nextloc = nextloc.offset(4 as libc::c_int as isize);
            /* Last successful passwd */
            ...
            nextloc = nextloc.offset(4 as libc::c_int as isize);
            /* # tl_data structures */
            store_16_le((*entry).n_tl_data as libc::c_uint,
                        nextloc as *mut libc::c_void);
            nextloc = nextloc.offset(2 as libc::c_int as isize);
            /* # key_data structures */
            store_16_le((*entry).n_key_data as libc::c_uint,
                        nextloc as *mut libc::c_void);
            nextloc = nextloc.offset(2 as libc::c_int as isize);
            /* Put extended fields here */
            if (*entry).len as libc::c_int != 38 as libc::c_int { abort(); }
            /* Any extra data that this version doesn't understand. */
            if (*entry).e_length != 0 {
                memcpy(nextloc as *mut libc::c_void,
                       (*entry).e_data as *const libc::c_void,
                       (*entry).e_length as libc::c_ulong);
                nextloc =
                    nextloc.offset((*entry).e_length as libc::c_int as isize)
            }
            /*
     ...
     * after the base type.
     */
            psize16 = unparse_princ_size as krb5_int16;
            store_16_le(psize16 as libc::c_uint,
                        nextloc as *mut libc::c_void);
            nextloc = nextloc.offset(2 as libc::c_int as isize);
            memcpy(nextloc as *mut libc::c_void,
                   unparse_princ as *const libc::c_void,
                   unparse_princ_size as libc::c_ulong);
            nextloc = nextloc.offset(unparse_princ_size as isize);
            /* tl_data is a linked list, of type, legth, contents */
            tl_data = (*entry).tl_data;
            while !tl_data.is_null() {
                store_16_le((*tl_data).tl_data_type as libc::c_uint,
                            nextloc as *mut libc::c_void);
                nextloc = nextloc.offset(2 as libc::c_int as isize);
                store_16_le((*tl_data).tl_data_length as libc::c_uint,
                            nextloc as *mut libc::c_void);
                nextloc = nextloc.offset(2 as libc::c_int as isize);
                memcpy(nextloc as *mut libc::c_void,
                ...
                nextloc =
                    nextloc.offset((*tl_data).tl_data_length as libc::c_int as
                                       isize);
                tl_data = (*tl_data).tl_data_next
            }
            /* key_data is an array */
            i = 0 as libc::c_int;
            while i < (*entry).n_key_data as libc::c_int {
                store_16_le((*(*entry).key_data.offset(i as
                                                           isize)).key_data_ver
                                as libc::c_uint,
                            nextloc as *mut libc::c_void);
                nextloc = nextloc.offset(2 as libc::c_int as isize);
                ...
                while j <
                          (*(*entry).key_data.offset(i as isize)).key_data_ver
                              as libc::c_int {
                    let mut type_0: krb5_int16 =
                        (*(*entry).key_data.offset(i as
                                                       isize)).key_data_type[j
                                                                                 as
                                                                                 usize];
                    let mut length: krb5_ui_2 =
                        (*(*entry).key_data.offset(i as
                                                       isize)).key_data_length[j
                                                                                   as
                                                                                   usize];
                    store_16_le(type_0 as libc::c_uint,
                                nextloc as *mut libc::c_void);
                    nextloc = nextloc.offset(2 as libc::c_int as isize);
                    store_16_le(length as libc::c_uint,
                                nextloc as *mut libc::c_void);
                    nextloc = nextloc.offset(2 as libc::c_int as isize);
                    if length != 0 {
                        memcpy(nextloc as *mut libc::c_void,
                               (*(*entry).key_data.offset(i as
                                                              isize)).key_data_contents[j
                                                                                            as
                                                                                            usize]
                                   as *const libc::c_void,
                               length as libc::c_ulong);
                        nextloc =
                            nextloc.offset(length as libc::c_int as isize)
                    }
                    j += 1
                }
                i += 1
    ...
    free(unparse_princ as *mut libc::c_void);
    return retval;
}
#[no_mangle]
#[c2rust::src_loc = "224:1"]
pub unsafe extern "C" fn krb5_decode_princ_entry(mut context: krb5_context,
                                                 mut content: *mut krb5_data,
                                                 mut entry_ptr:
                                                     *mut *mut krb5_db_entry)
 -> krb5_error_code {
    let mut current_block: u64;
    let mut sizeleft: libc::c_int = 0;
    let mut i: libc::c_int = 0;
    let mut nextloc: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut tl_data: *mut *mut krb5_tl_data = 0 as *mut *mut krb5_tl_data;
    let mut i16: krb5_int16 = 0;
    let mut entry: *mut krb5_db_entry = 0 as *mut krb5_db_entry;
    let mut retval: krb5_error_code = 0;
    *entry_ptr = 0 as *mut krb5_db_entry;
    entry =
        k5alloc(::std::mem::size_of::<krb5_db_entry>() as libc::c_ulong,
                &mut retval) as *mut krb5_db_entry;
    if entry.is_null() { return retval }
    /*
    ...
    nextloc = (*content).data as *mut libc::c_uchar;
    sizeleft = (*content).length as libc::c_int;
    if sizeleft < 38 as libc::c_int {
        retval = -(1780008439 as libc::c_long) as krb5_error_code
    } else {
        sizeleft -= 38 as libc::c_int;
        /* Base Length */
        *(&mut (*entry).len as *mut krb5_ui_2 as *mut krb5_int16) =
            load_16_le(nextloc as *const libc::c_void) as krb5_int16;
        nextloc = nextloc.offset(2 as libc::c_int as isize);
        /* Attributes */
        *(&mut (*entry).attributes as *mut krb5_flags as *mut krb5_int32) =
            load_32_le(nextloc as *const libc::c_void) as krb5_int32;
        nextloc = nextloc.offset(4 as libc::c_int as isize);
        /* Max Life */
        *(&mut (*entry).max_life as *mut krb5_deltat as *mut krb5_int32) =
            load_32_le(nextloc as *const libc::c_void) as krb5_int32;
        nextloc = nextloc.offset(4 as libc::c_int as isize);
        /* Max Renewable Life */
        *(&mut (*entry).max_renewable_life as *mut krb5_deltat as
              *mut krb5_int32) =
            load_32_le(nextloc as *const libc::c_void) as krb5_int32;
        nextloc = nextloc.offset(4 as libc::c_int as isize);
        /* When the client expires */
        *(&mut (*entry).expiration as *mut krb5_timestamp as *mut krb5_int32)
            = load_32_le(nextloc as *const libc::c_void) as krb5_int32;
        nextloc = nextloc.offset(4 as libc::c_int as isize);
        /* When its passwd expires */
        *(&mut (*entry).pw_expiration as *mut krb5_timestamp as
              *mut krb5_int32) =
            load_32_le(nextloc as *const libc::c_void) as krb5_int32;
        nextloc = nextloc.offset(4 as libc::c_int as isize);
        /* Last successful passwd */
        *(&mut (*entry).last_success as *mut krb5_timestamp as
              *mut krb5_int32) =
            load_32_le(nextloc as *const libc::c_void) as krb5_int32;
        nextloc = nextloc.offset(4 as libc::c_int as isize);
        /* Last failed passwd attempt */
        *(&mut (*entry).last_failed as *mut krb5_timestamp as *mut krb5_int32)
            = load_32_le(nextloc as *const libc::c_void) as krb5_int32;
        nextloc = nextloc.offset(4 as libc::c_int as isize);
        /* # of failed passwd attempt */
        *(&mut (*entry).fail_auth_count as *mut krb5_kvno as *mut krb5_int32)
            = load_32_le(nextloc as *const libc::c_void) as krb5_int32;
        nextloc = nextloc.offset(4 as libc::c_int as isize);
        /* # tl_data structures */
        *(&mut (*entry).n_tl_data as *mut krb5_int16) =
            load_16_le(nextloc as *const libc::c_void) as krb5_int16;
        nextloc = nextloc.offset(2 as libc::c_int as isize);
        if ((*entry).n_tl_data as libc::c_int) < 0 as libc::c_int {
            ...
            /* # key_data structures */
            *(&mut (*entry).n_key_data as *mut krb5_int16) =
                load_16_le(nextloc as *const libc::c_void) as krb5_int16;
            nextloc = nextloc.offset(2 as libc::c_int as isize);
            if ((*entry).n_key_data as libc::c_int) < 0 as libc::c_int {
                ...
                if (*entry).len as libc::c_int > 38 as libc::c_int {
                    (*entry).e_length =
                        ((*entry).len as libc::c_int - 38 as libc::c_int) as
                            krb5_ui_2;
                    (*entry).e_data =
                        k5memdup(nextloc as *const libc::c_void,
                                 (*entry).e_length as size_t, &mut retval) as
                            *mut krb5_octet;
                    if (*entry).e_data.is_null() {
                        current_block = 7674102261283392686;
                    } else {
                        nextloc =
                            nextloc.offset((*entry).e_length as libc::c_int as
                                               isize);
     ...
     */
                    {
                        if sizeleft < 2 as libc::c_int {
                            retval =
                                -(1780008439 as libc::c_long) as
                                    krb5_error_code
                        } else {
                            sizeleft -= 2 as libc::c_int;
                            i = 0 as libc::c_int;
                            *(&mut i16 as *mut krb5_int16) =
                                load_16_le(nextloc as *const libc::c_void) as
                                    krb5_int16;
                            i = i16 as libc::c_int;
                            nextloc =
                                nextloc.offset(2 as libc::c_int as isize);
                            if i <= 0 as libc::c_int || i > sizeleft ||
                                   *nextloc.offset((i - 1 as libc::c_int) as
                                                       isize) as libc::c_int
                                       != '\u{0}' as i32 ||
                                   !memchr(nextloc as *mut libc::c_char as
                                               *const libc::c_void,
                                           '\u{0}' as i32,
                                ...
                                if !(retval != 0) {
                                    sizeleft -= i;
                                    nextloc = nextloc.offset(i as isize);
                                    /* tl_data is a linked list */
                                    tl_data = &mut (*entry).tl_data;
                                    i = 0 as libc::c_int;
                                    loop  {
                                        if !(i <
                                                 (*entry).n_tl_data as
                                        ...
                                        if sizeleft < 4 as libc::c_int {
                                            retval =
                                                -(1780008439 as libc::c_long)
                                                    as krb5_error_code;
                                            current_block =
                                                7674102261283392686;
                                            break ;
                                        } else {
                                            sizeleft -= 4 as libc::c_int;
                                            *tl_data =
                                                malloc(::std::mem::size_of::<krb5_tl_data>()
                                                           as libc::c_ulong)
                                                    as *mut krb5_tl_data;
                                            if (*tl_data).is_null() {
                                                retval = 12 as libc::c_int;
                                                ...
                                                (**tl_data).tl_data_next =
                                                    0 as *mut _krb5_tl_data;
                                                (**tl_data).tl_data_contents =
                                                    0 as *mut krb5_octet;
                                                *(&mut (**tl_data).tl_data_type
                                                      as *mut krb5_int16) =
                                                    load_16_le(nextloc as
                                                                   *const libc::c_void)
                                                        as krb5_int16;
                                                nextloc =
                                                    nextloc.offset(2 as
                                                ...
                                                *(&mut (**tl_data).tl_data_length
                                                ...
                                                if (**tl_data).tl_data_length
                                                    ...
                                                    nextloc =
                                                        nextloc.offset((**tl_data).tl_data_length
                                                                           as
                                                                           libc::c_int
                                                                           as
                                                                           isize);
                                                    tl_data =
                                                        &mut (**tl_data).tl_data_next;
                                                    i += 1
                                                }
                                            }
                                        }
                                                   ...
                                                   libc::c_int != 0 &&
                                                   {
                                                       (*entry).key_data =
                                                           malloc((::std::mem::size_of::<krb5_key_data>()
                                                                       as
                                                                       libc::c_ulong).wrapping_mul((*entry).n_key_data
                                                                                                       as
                                                                                                       libc::c_ulong))
                                                               as
                                                               *mut krb5_key_data;
                                                                               ...
                                                                               libc::c_int
                                                                       {
                                                                        ...
                                                                        sizeleft
                                                                            ...
                                                                            4
                                                                                as
                                                                                libc::c_int;
                                                                        *(&mut *(*key_data).key_data_type.as_mut_ptr().offset(j
                                                                                                                                  as
                                                                                                                                  isize)
                                                                              as
                                                                              *mut krb5_int16)
                                                                        ...
                                                                        nextloc
                                                                            ...
                                                                            nextloc.offset(2
                                                                                               ...
                                                                                               as
                                                                                               isize);
                                                                        *(&mut *(*key_data).key_data_length.as_mut_ptr().offset(j
                                                                                                                                    as
                                                                                                                                    isize)
                                                                              as
                                                                              *mut krb5_ui_2
                                                                                   ...
                                                                                   0
                                                                               {
                                                                                (*key_data).key_data_contents[j
                                                                                    ...
                                                                                    k5memdup(nextloc
                                                                                        ...
                                                                                        as
                                                                                        *mut krb5_octet;
                                                                                if (*key_data).key_data_contents[j
                                                                                                                     as
                                                                                                                     usize].is_null()
                                                                                   {
                                                                                    current_block
                                                                                ...
                                                                                nextloc
                                                                                    =
                                                                                    nextloc.offset((*key_data).key_data_length[j
                                                                                                                                   as
                                                                                                                                   usize]
                                                                                                       as
                                                                                                       libc::c_int
