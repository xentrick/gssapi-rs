...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/include/kadm5/admin.h:36"]
pub mod admin_h {
    #[c2rust::src_loc = "71:1"]
...
#[c2rust::header_src = "/usr/include/libintl.h:35"]
pub mod libintl_h {
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "44:1"]
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/include/adm_proto.h:37"]
pub mod adm_proto_h {
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/kadmin/cli/kadmin.h:38"]
pub mod kadmin_h {
    use super::krb5_h::{krb5_principal_data, krb5_principal, krb5_boolean,
                        krb5_keyblock, krb5_error_code, krb5_context};
    use super::kdb_h::krb5_key_salt_tuple;
    use super::k5_int_h::_krb5_context;
    extern "C" {
        #[no_mangle]
...
use self::kadmin_h::{randkey_princ, context, whoami, handle};
...
static mut quiet: libc::c_int = 0;
#[c2rust::src_loc = "50:12"]
static mut norandkey: libc::c_int = 0;
#[c2rust::src_loc = "52:1"]
unsafe extern "C" fn add_usage() {
    fprintf(stderr,
...
#[c2rust::src_loc = "66:1"]
unsafe extern "C" fn process_keytab(mut my_context: krb5_context,
                                    mut keytab_str: *mut *mut libc::c_char,
    ...
    if name.is_null() {
        ...
        if name.is_null() {
            com_err(whoami, 12 as libc::c_int as errcode_t,
                    dgettext(b"mit-krb5\x00" as *const u8 as
                                 *const libc::c_char,
                             b"while creating keytab name\x00" as *const u8 as
                                 *const libc::c_char));
        ...
        if code != 0 as libc::c_int {
            com_err(whoami, code as errcode_t,
                    dgettext(b"mit-krb5\x00" as *const u8 as
                                 *const libc::c_char,
                             b"while opening default keytab\x00" as *const u8
...
pub unsafe extern "C" fn kadmin_keytab_add(mut argc: libc::c_int,
    ...
    if process_keytab(context, &mut keytab_str, &mut keytab) != 0 { return }
    while !(*argv).is_null() {
        if strcmp(*argv, b"-glob\x00" as *const u8 as *const libc::c_char) ==
            ...
            if (*argv).is_null() {
                ...
                code =
                    kadm5_get_principals(handle, *argv, &mut princs, &mut num)
                        as libc::c_int;
                if code != 0 {
                    com_err(whoami, code as errcode_t,
                            dgettext(b"mit-krb5\x00" as *const u8 as
...
pub unsafe extern "C" fn kadmin_keytab_remove(mut argc: libc::c_int,
    ...
    while argc != 0 {
        if strncmp(*argv, b"-k\x00" as *const u8 as *const libc::c_char,
            ...
            if !(strcmp(*argv, b"-q\x00" as *const u8 as *const libc::c_char)
                     == 0 as libc::c_int) {
                break ;
            }
            quiet += 1
...
unsafe extern "C" fn fetch_new_keys(mut lhandle: *mut libc::c_void,
 ...
 -> krb5_error_code {
    ...
    *nkeys_out = 0 as libc::c_int;
            ...
            key_data =
                k5calloc(nkeys as size_t,
                         ::std::mem::size_of::<kadm5_key_data>() as
                             libc::c_ulong, &mut code) as *mut kadm5_key_data;
            if !key_data.is_null() {
    ...
    i = 0 as libc::c_int;
    while i < nkeys {
        krb5_free_keyblock_contents(context, &mut *keys.offset(i as isize));
        i += 1
    }
    free(keys as *mut libc::c_void);
...
unsafe extern "C" fn add_principal(mut lhandle: *mut libc::c_void,
        ...
        com_err(whoami, code as errcode_t,
                dgettext(b"mit-krb5\x00" as *const u8 as *const libc::c_char,
                         b"while parsing -add principal name %s\x00" as
                             *const u8 as *const libc::c_char), princ_str);
    } else {
        if norandkey != 0 {
            code =
            ...
            code =
                fetch_new_keys(handle, princ, keepold, n_ks_tuple, ks_tuple,
                               &mut key_data, &mut nkeys)
        }
        if code != 0 as libc::c_int {
            if code as libc::c_long == 43787532 as libc::c_long {
                ...
                com_err(whoami, code as errcode_t,
                        dgettext(b"mit-krb5\x00" as *const u8 as
                                     *const libc::c_char,
                                 b"while changing %s\'s key\x00" as *const u8
                                     as *const libc::c_char), princ_str);
            }
        } else {
            i = 0 as libc::c_int;
            while i < nkeys {
...
unsafe extern "C" fn remove_principal(mut keytab_str: *mut libc::c_char,
                                      mut keytab: krb5_keytab,
                                      mut princ_str: *mut libc::c_char,
                                      mut kvno_str: *mut libc::c_char) {
    ...
    if code != 0 as libc::c_int {
        ...
        code =
            krb5_kt_get_entry(context, keytab, princ as krb5_const_principal,
                              kvno, 0 as libc::c_int, &mut entry);
        if code != 0 as libc::c_int {
                ...
                fprintf(stderr,
                        dgettext(b"mit-krb5\x00" as *const u8 as
                                 ...
                                 b"%s: Keytab %s does not exist.\n\x00" as
                                     *const u8 as *const libc::c_char),
                        whoami, keytab_str);
            } else if code as libc::c_long == -(1765328203 as libc::c_long) {
                    ...
                    _ => {
                        if code != 0 &&
                            ...
                            com_err(whoami, code as errcode_t,
                                    dgettext(b"mit-krb5\x00" as *const u8 as
                                                 *const libc::c_char,
                                             b"while scanning keytab\x00" as
                                                 *const u8 as
                                                 *const libc::c_char));
                        } else {
                            code =
