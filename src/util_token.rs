 ...
 */
#[c2rust::src_loc = "61:1"]
unsafe extern "C" fn der_length_size(mut length: i32)
 -> u32 {
    if length < (1 as i32) << 7 as i32 {
        return 1 as i32 as u32
    } else if length < (1 as i32) << 8 as i32 {
        return 2 as i32 as u32
    } else if length < (1 as i32) << 16 as i32 {
        return 3 as i32 as u32
    } else if length < (1 as i32) << 24 as i32 {
        return 4 as i32 as u32
    } else { return 5 as i32 as u32 };
}
#[c2rust::src_loc = "81:1"]
unsafe extern "C" fn der_write_length(mut buf: *mut *mut u8,
                                      mut length: i32) {
    if length < (1 as i32) << 7 as i32 {
        let fresh0 = *buf;
        *buf = (*buf).offset(1);
        ...
        *buf = (*buf).offset(1);
        *fresh1 =
            der_length_size(length).wrapping_add(127 as i32 as
                                                     u32) as
                u8;
        if length >= (1 as i32) << 24 as i32 {
            let fresh2 = *buf;
            *buf = (*buf).offset(1);
            *fresh2 = (length >> 24 as i32) as u8
        }
        if length >= (1 as i32) << 16 as i32 {
            let fresh3 = *buf;
            *buf = (*buf).offset(1);
            *fresh3 =
                (length >> 16 as i32 & 0xff as i32) as
                    u8
        }
        if length >= (1 as i32) << 8 as i32 {
            let fresh4 = *buf;
            *buf = (*buf).offset(1);
            *fresh4 =
                (length >> 8 as i32 & 0xff as i32) as
                    u8
        }
        let fresh5 = *buf;
        *buf = (*buf).offset(1);
        *fresh5 = (length & 0xff as i32) as u8
    };
}
/* returns decoded length, or < 0 on failure.  Advances buf and
   decrements bufsize */
#[c2rust::src_loc = "103:1"]
unsafe extern "C" fn der_read_length(mut buf: *mut *mut u8,
                                     mut bufsize: *mut i32)
 -> i32 {
    let mut sf: u8 = 0;
    let mut ret: i32 = 0;
    if *bufsize < 1 as i32 { return -(1 as i32) }
    let fresh6 = *buf;
    *buf = (*buf).offset(1);
    sf = *fresh6;
    *bufsize -= 1;
    if sf as i32 & 0x80 as i32 != 0 {
        sf = (sf as i32 & 0x7f as i32) as u8;
        if sf as i32 > *bufsize - 1 as i32 {
            return -(1 as i32)
        }
        if sf as usize >
        ...
        while sf != 0 {
            let fresh7 = *buf;
            *buf = (*buf).offset(1);
            ret = (ret << 8 as i32) + *fresh7 as i32;
            *bufsize -= 1;
...
pub unsafe extern "C" fn gssint_g_token_size(mut mech: *const gss_OID_desc,
 ...
 -> u32 {
    ...
    body_size =
        body_size.wrapping_add((4 as i32 as
                                    u32).wrapping_add((*mech).length)); /* NEED overflow check */
    return (1 as i32 as
                u32).wrapping_add(der_length_size(body_size as
                                                               i32)).wrapping_add(body_size);
}
...
pub unsafe extern "C" fn gssint_g_make_token_header(mut mech:
    ...
    let fresh8 = *buf;
    *buf = (*buf).offset(1);
    *fresh8 = 0x60 as i32 as u8;
    der_write_length(buf,
                     ((if tok_type == -(1 as i32) {
                           2 as i32
                       } else { 4 as i32 }) as
                          u32).wrapping_add((*mech).length).wrapping_add(body_size)
                         as i32);
    let fresh9 = *buf;
    *buf = (*buf).offset(1);
    *fresh9 = 0x6 as i32 as u8;
    let fresh10 = *buf;
    *buf = (*buf).offset(1);
    ...
    *buf = (*buf).offset((*mech).length as isize);
    if tok_type != -(1 as i32) {
        ...
        *buf = (*buf).offset(1);
        *fresh11 =
            (tok_type >> 8 as i32 & 0xff as i32) as
                u8;
        let fresh12 = *buf;
        *buf = (*buf).offset(1);
        *fresh12 = (tok_type & 0xff as i32) as u8
    };
}
...
pub unsafe extern "C" fn gssint_g_verify_token_header(mut mech:
 ...
 -> gss_int32 {
    ...
    let mut toid = gss_OID_desc{length: 0, elements: 0 as *mut libc::c_void,};
    let mut toksize = toksize_in as i32;
    toksize -= 1 as i32;
    if toksize < 0 as i32 {
        return -(2045022964 as isize) as gss_int32
    }
    let fresh13 = buf;
    buf = buf.offset(1);
    if *fresh13 as i32 != 0x60 as i32 {
        ...
        toksize =
            (toksize as u32).wrapping_sub(toid.length) as i32
                as i32;
        if toksize < 0 as i32 {
            return -(2045022964 as isize) as gss_int32
        }
        ...
        if !(toid.length == (*mech).length &&
                 memcmp(toid.elements, (*mech).elements,
                        toid.length as usize) == 0 as i32) {
            return -(2045022965 as isize) as gss_int32
        }
    }
    if tok_type != -(1 as i32) {
        toksize -= 2 as i32;
        ...
        buf = buf.offset(1);
        if *fresh16 as i32 !=
               tok_type >> 8 as i32 & 0xff as i32 ||
               {
                   let fresh17 = buf;
                   buf = buf.offset(1);
                   (*fresh17 as i32) != tok_type & 0xff as i32
               } {
            return -(2045022960 as isize) as gss_int32
        }
    }
    *buf_in = buf;
