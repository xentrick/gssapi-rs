...
pub use crate::src::krb5::rel_oid::krb5_gss_release_oid;
...
pub unsafe extern "C" fn krb5_gss_delete_sec_context(
    ...
    if !(*ctx).seqstate.is_null() {
        crate::src::generic::util_seqstate::gssint_g_seqstate_free((*ctx).seqstate);
    }
    if !(*ctx).enc.is_null() {
    ...
    if !(*ctx).here.is_null() {
        crate::src::krb5::naming_exts::kg_release_name(context, &mut (*ctx).here);
    }
    if !(*ctx).there.is_null() {
        crate::src::krb5::naming_exts::kg_release_name(context, &mut (*ctx).there);
    }
    if !(*ctx).subkey.is_null() {
        crate::krb5_h::krb5_k_free_key(context, (*ctx).subkey);
    }
    if !(*ctx).acceptor_subkey.is_null() {
        crate::krb5_h::krb5_k_free_key(context, (*ctx).acceptor_subkey);
    }
    if !(*ctx).auth_context.is_null() {
        ...
        crate::krb5_h::krb5_auth_con_free(context, (*ctx).auth_context);
    }
    if !(*ctx).mech_used.is_null() {
        crate::src::krb5::rel_oid::krb5_gss_release_oid(minor_status, &mut (*ctx).mech_used);
    }
    if !(*ctx).authdata.is_null() {
        crate::krb5_h::krb5_free_authdata(context, (*ctx).authdata);
    }
    if !(*ctx).k5_context.is_null() {
        crate::krb5_h::krb5_free_context((*ctx).k5_context);
    }
    /* Zero out context */
