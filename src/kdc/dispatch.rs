...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/kdc/realm_data.h:28"]
pub mod realm_data_h {
    ...
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "85:1"]
        pub fn setup_server_realm(_: *mut server_handle, _: krb5_principal)
         -> *mut kdc_realm_t;
    }
    /* REALM_DATA_H */
    /*
 ...
 */
    ...
    use super::krb5_h::{krb5_error_code, krb5_data, krb5_address, krb5_ui_4};
    /* NET_SERVER_H */
}
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/include/krb5/kdcpreauth_plugin.h:28"]
...
#[c2rust::header_src = "/usr/include/arpa/inet.h:26"]
pub mod inet_h {
...
pub mod kdc_util_h {
    use super::krb5_h::{krb5_kdc_req, krb5_data, krb5_error_code,
                        krb5_context, krb5_boolean};
    use super::net_server_h::{krb5_fulladdr, loop_respond_fn};
    ...
    extern "C" {
        ...
        pub fn process_tgs_req(_: *mut krb5_kdc_req, _: *mut krb5_data,
                               _: *const krb5_fulladdr, _: *mut kdc_realm_t,
                               _: *mut *mut krb5_data) -> krb5_error_code;
        #[no_mangle]
        #[c2rust::src_loc = "140:1"]
        pub fn process_as_req(_: *mut krb5_kdc_req, _: *mut krb5_data,
                              _: *const krb5_fulladdr,
                              _: *const krb5_fulladdr, _: *mut kdc_realm_t,
                              _: *mut verto_ctx, _: loop_respond_fn,
                              _: *mut libc::c_void);
        #[no_mangle]
        #[c2rust::src_loc = "246:1"]
        pub fn kdc_insert_lookaside(_: krb5_context, _: *mut krb5_data,
                                    _: *mut krb5_data);
        #[no_mangle]
        #[c2rust::src_loc = "247:1"]
        pub fn kdc_remove_lookaside(kcontext: krb5_context,
                                    _: *mut krb5_data);
        #[no_mangle]
        #[c2rust::src_loc = "245:1"]
        pub fn kdc_check_lookaside(_: krb5_context, _: *mut krb5_data,
                                   _: *mut *mut krb5_data) -> krb5_boolean;
    }
    /* __KRB5_KDC_UTIL__ */
}
#[c2rust::header_src = "/home/nmavis/dev/gssapi-rs/code/src/kdc/extern.h:29"]
pub mod extern_h {
...
pub use self::types_h::{__uint8_t, __int32_t, __uint32_t, __socklen_t};
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_ui_4, krb5_boolean,
                       ...
                       _krb5_enc_data, krb5_enc_data, C2RustUnnamed,
                       ...
                       krb5_c_random_add_entropy, krb5_c_random_os_entropy,
                       krb5_mk_error, krb5_free_data, krb5_us_timeofday};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         krb5_crypto_us_timeofday, decode_krb5_as_req,
                         decode_krb5_tgs_req, krb5_free_kdc_req};
...
pub use self::realm_data_h::{kdc_realm_t, __kdc_realm_data, server_handle,
                             setup_server_realm};
pub use self::net_server_h::{loop_respond_fn, _krb5_fulladdr, krb5_fulladdr};
use self::kdcpreauth_plugin_h::verto_ctx;
use self::stdlib_h::{free, calloc, malloc};
use self::string_h::memset;
use self::inet_h::inet_ntop;
use self::kdc_util_h::{process_tgs_req, process_as_req, kdc_insert_lookaside,
                       kdc_remove_lookaside, kdc_check_lookaside};
use self::extern_h::max_dgram_reply_size;
use self::adm_proto_h::krb5_klog_syslog;
#[derive(Copy, Clone)]
#[repr(C)]
#[c2rust::src_loc = "41:8"]
pub struct dispatch_state {
    pub respond: loop_respond_fn,
    pub arg: *mut libc::c_void,
    pub request: *mut krb5_data,
    pub is_tcp: libc::c_int,
...
static mut last_usec: krb5_int32 = 0 as libc::c_int;
#[c2rust::src_loc = "36:34"]
static mut last_os_random: krb5_int32 = 0 as libc::c_int;
#[c2rust::src_loc = "50:1"]
unsafe extern "C" fn finish_dispatch(mut state: *mut dispatch_state,
                                     mut code: krb5_error_code,
                                     mut response: *mut krb5_data) {
    let mut oldrespond: loop_respond_fn = (*state).respond;
    let mut oldarg: *mut libc::c_void = (*state).arg;
    let mut kdc_active_realm: *mut kdc_realm_t = (*state).active_realm;
    if (*state).is_tcp == 0 as libc::c_int && !response.is_null() &&
           (*response).length > max_dgram_reply_size as libc::c_uint {
        krb5_free_data((*kdc_active_realm).realm_context, response);
        response = 0 as *mut krb5_data;
        code = make_too_big_error(kdc_active_realm, &mut response);
        if code != 0 {
            krb5_klog_syslog(3 as libc::c_int,
                             b"error constructing KRB_ERR_RESPONSE_TOO_BIG error: %s\x00"
...
#[c2rust::src_loc = "73:1"]
unsafe extern "C" fn finish_dispatch_cache(mut arg: *mut libc::c_void,
                                           mut code: krb5_error_code,
                                           mut response: *mut krb5_data) {
    let mut state: *mut dispatch_state = arg as *mut dispatch_state;
    let mut kdc_err_context: krb5_context = (*state).kdc_err_context;
    /* Remove the null cache entry unless we actually want to discard this
     * request. */
    if code as libc::c_long != -(1750600189 as libc::c_long) {
        kdc_remove_lookaside(kdc_err_context, (*state).request);
    }
    /* Put the response into the lookaside buffer (if we produced one). */
    if code == 0 as libc::c_int && !response.is_null() {
        kdc_insert_lookaside(kdc_err_context, (*state).request, response);
    }
    finish_dispatch(state, code, response);
}
#[c2rust::src_loc = "93:1"]
unsafe extern "C" fn reseed_random(mut kdc_err_context: krb5_context) {
    let mut retval: krb5_error_code = 0;
    let mut now: krb5_timestamp = 0;
    let mut now_usec: krb5_int32 = 0;
    let mut usec_difference: krb5_int32 = 0;
    let mut data: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    retval = krb5_crypto_us_timeofday(&mut now, &mut now_usec);
    if retval == 0 as libc::c_int {
        usec_difference = now_usec - last_usec;
        if last_os_random == 0 as libc::c_int { last_os_random = now }
        /* Grab random data from OS every hour*/
        if ts_delta(now, last_os_random) >=
               60 as libc::c_int * 60 as libc::c_int {
            krb5_c_random_os_entropy(kdc_err_context, 0 as libc::c_int,
                                     0 as *mut libc::c_int);
            last_os_random = now
        }
        data.length =
            ::std::mem::size_of::<krb5_int32>() as libc::c_ulong as
                libc::c_uint;
        data.data =
            &mut usec_difference as *mut krb5_int32 as *mut libc::c_void as
                *mut libc::c_char;
        krb5_c_random_add_entropy(kdc_err_context,
                                  KRB5_C_RANDSOURCE_TIMING as libc::c_int as
                                      libc::c_uint, &mut data);
        last_usec = now_usec
    };
}
#[no_mangle]
#[c2rust::src_loc = "121:1"]
pub unsafe extern "C" fn dispatch(mut cb: *mut libc::c_void,
    ...
    let mut retval: krb5_error_code = 0;
    let mut req: *mut krb5_kdc_req = 0 as *mut krb5_kdc_req;
    let mut response: *mut krb5_data = 0 as *mut krb5_data;
    let mut state: *mut dispatch_state = 0 as *mut dispatch_state;
    let mut handle: *mut server_handle = cb as *mut server_handle;
    let mut kdc_err_context: krb5_context = (*handle).kdc_err_context;
    state =
        k5alloc(::std::mem::size_of::<dispatch_state>() as libc::c_ulong,
                &mut retval) as *mut dispatch_state;
    if state.is_null() {
    ...
    if kdc_check_lookaside(kdc_err_context, pkt, &mut response) != 0 {
        /* a hit! */
        let mut name: *const libc::c_char = 0 as *const libc::c_char;
        let mut buf: [libc::c_char; 46] = [0; 46];
        name =
            inet_ntop(if (*(*remote_addr).address).addrtype ==
                             0x18 as libc::c_int {
                          10 as libc::c_int
                      ...
                      (*(*remote_addr).address).contents as
                          *const libc::c_void, buf.as_mut_ptr(),
                      ::std::mem::size_of::<[libc::c_char; 46]>() as
                          libc::c_ulong as socklen_t);
        if name.is_null() {
            name =
                b"[unknown address type]\x00" as *const u8 as
                    *const libc::c_char
        }
        if !response.is_null() {
            krb5_klog_syslog(6 as libc::c_int,
                             b"DISPATCH: repeated (retransmitted?) request from %s, resending previous response\x00"
                                 as *const u8 as *const libc::c_char, name);
        } else {
            krb5_klog_syslog(6 as libc::c_int,
                             b"DISPATCH: repeated (retransmitted?) request from %s during request processing, dropping repeated request\x00"
                                 as *const u8 as *const libc::c_char, name);
        }
        finish_dispatch(state,
                        if !response.is_null() {
                            0 as libc::c_int as libc::c_long
                        } else { -(1750600189 as libc::c_long) } as
                            krb5_error_code, response);
        return
    }
    /* Insert a NULL entry into the lookaside to indicate that this request
     * is currently being processed. */
    kdc_insert_lookaside(kdc_err_context, pkt, 0 as *mut krb5_data);
    reseed_random(kdc_err_context);
    /* try TGS_REQ first; they are more common! */
    if !pkt.is_null() && (*pkt).length != 0 &&
           *(*pkt).data.offset(0 as libc::c_int as isize) as libc::c_int &
               !(0x20 as libc::c_int) ==
               12 as libc::c_int | 0x40 as libc::c_int {
        retval = decode_krb5_tgs_req(pkt, &mut req)
    } else if !pkt.is_null() && (*pkt).length != 0 &&
                  *(*pkt).data.offset(0 as libc::c_int as isize) as
                      libc::c_int & !(0x20 as libc::c_int) ==
                      10 as libc::c_int | 0x40 as libc::c_int {
        retval = decode_krb5_as_req(pkt, &mut req)
    } else { retval = -(1765328344 as libc::c_long) as krb5_error_code }
    if !(retval != 0) {
        (*state).active_realm = setup_server_realm(handle, (*req).server);
        if (*state).active_realm.is_null() {
            retval = -(1765328316 as libc::c_long) as krb5_error_code
        } else if !pkt.is_null() && (*pkt).length != 0 &&
                      *(*pkt).data.offset(0 as libc::c_int as isize) as
                          libc::c_int & !(0x20 as libc::c_int) ==
                          12 as libc::c_int | 0x40 as libc::c_int {
            ...
            retval =
                process_tgs_req(req, pkt, remote_addr, (*state).active_realm,
                                &mut response);
            req = 0 as *mut krb5_kdc_req
        } else if !pkt.is_null() && (*pkt).length != 0 &&
                      *(*pkt).data.offset(0 as libc::c_int as isize) as
                          libc::c_int & !(0x20 as libc::c_int) ==
                          10 as libc::c_int | 0x40 as libc::c_int {
            /* process_as_req frees the request and calls finish_dispatch_cache. */
            process_as_req(req, pkt, local_addr, remote_addr,
                           (*state).active_realm, vctx,
                           Some(finish_dispatch_cache as
    ...
    finish_dispatch_cache(state as *mut libc::c_void, retval, response);
}
#[c2rust::src_loc = "211:1"]
unsafe extern "C" fn make_too_big_error(mut kdc_active_realm:
                                            *mut kdc_realm_t,
 ...
 -> krb5_error_code {
    ...
    *out = 0 as *mut krb5_data;
    memset(&mut errpkt as *mut krb5_error as *mut libc::c_void,
           0 as libc::c_int,
    ...
    errpkt.error = 52 as libc::c_int as krb5_ui_4;
    errpkt.server = (*kdc_active_realm).realm_tgsprinc;
...
pub unsafe extern "C" fn get_context(mut handle: *mut libc::c_void)
 -> krb5_context {
    let mut sh: *mut server_handle = handle as *mut server_handle;
