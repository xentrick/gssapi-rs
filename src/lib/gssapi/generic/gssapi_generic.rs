...
pub struct mech_attr_info_desc {
    ...
    pub short_desc: *const libc::c_char,
    pub long_desc: *const libc::c_char,
}
...
static mut const_oids: [gss_OID_desc; 40] =
    [{
     ...
     {
         let mut init =
             gss_OID_desc_struct{length: 7 as libc::c_int as OM_uint32,
                                 elements:
...
static mut mech_attr_info: [mech_attr_info_desc; 28] =
    [mech_attr_info_desc{mech_attr: 0 as *mut gss_OID_desc_struct,
                         ...
                         short_desc: 0 as *const libc::c_char,
                         long_desc: 0 as *const libc::c_char,}; 28];
...
pub unsafe extern "C" fn generic_gss_display_mech_attr(mut minor_status:
 ...
 -> OM_uint32 {
    ...
    if !short_desc.is_null() {
        (*short_desc).length = 0 as libc::c_int as size_t;
        (*short_desc).value = 0 as *mut libc::c_void
    }
    if !long_desc.is_null() {
        (*long_desc).length = 0 as libc::c_int as size_t;
    ...
    while i <
              (::std::mem::size_of::<[mech_attr_info_desc; 28]>() as
                   libc::c_ulong).wrapping_div(::std::mem::size_of::<mech_attr_info_desc>()
                                                   as libc::c_ulong) {
        ...
        if (*mech_attr).length == (*(*mai).mech_attr).length &&
               memcmp((*mech_attr).elements, (*(*mai).mech_attr).elements,
                      (*mech_attr).length as libc::c_ulong) ==
                   0 as libc::c_int {
            if !name.is_null() &&
    ...
    return (19 as libc::c_ulong as OM_uint32) << 16 as libc::c_int;
}
// Initialized in run_static_initializers
...
unsafe extern "C" fn run_static_initializers() {
    GSS_C_NT_USER_NAME =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(0 as libc::c_int as isize);
    gss_nt_user_name =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(0 as libc::c_int as isize);
    GSS_C_NT_MACHINE_UID_NAME =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(1 as libc::c_int as isize);
    gss_nt_machine_uid_name =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(1 as libc::c_int as isize);
    GSS_C_NT_STRING_UID_NAME =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(2 as libc::c_int as isize);
    gss_nt_string_uid_name =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(2 as libc::c_int as isize);
    GSS_C_NT_HOSTBASED_SERVICE_X =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(3 as libc::c_int as isize);
    gss_nt_service_name_v2 =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(3 as libc::c_int as isize);
    GSS_C_NT_HOSTBASED_SERVICE =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(4 as libc::c_int as isize);
    gss_nt_service_name =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(4 as libc::c_int as isize);
    GSS_C_NT_ANONYMOUS =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(5 as libc::c_int as isize);
    GSS_C_NT_EXPORT_NAME =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(6 as libc::c_int as isize);
    gss_nt_exported_name =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(6 as libc::c_int as isize);
    GSS_C_NT_COMPOSITE_EXPORT =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(7 as libc::c_int as isize);
    GSS_C_INQ_SSPI_SESSION_KEY =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(8 as libc::c_int as isize);
    GSS_C_INQ_NEGOEX_KEY =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(9 as libc::c_int as isize);
    GSS_C_INQ_NEGOEX_VERIFY_KEY =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(10 as libc::c_int as isize);
    GSS_C_MA_MECH_CONCRETE =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(11 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_MECH_PSEUDO =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(12 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_MECH_COMPOSITE =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(13 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_MECH_NEGO =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(14 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_MECH_GLUE =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(15 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_NOT_MECH =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(16 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_DEPRECATED =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(17 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_NOT_DFLT_MECH =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(18 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_ITOK_FRAMED =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(19 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_AUTH_INIT =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(20 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_AUTH_TARG =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(21 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_AUTH_INIT_INIT =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(22 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_AUTH_TARG_INIT =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(23 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_AUTH_INIT_ANON =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(24 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_AUTH_TARG_ANON =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(25 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_DELEG_CRED =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(26 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_INTEG_PROT =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(27 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_CONF_PROT =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(28 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_MIC =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(29 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_WRAP =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(30 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_PROT_READY =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(31 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_REPLAY_DET =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(32 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_OOS_DET =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(33 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_CBINDINGS =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(34 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_PFS =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(35 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_COMPRESS =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(36 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_CTX_TRANS =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(37 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_MA_NEGOEX_AND_SPNEGO =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(38 as libc::c_int as isize) as
            gss_const_OID;
    GSS_C_SEC_CONTEXT_SASL_SSF =
        (const_oids.as_ptr() as
             *mut gss_OID_desc).offset(39 as libc::c_int as isize);
    gss_ma_known_attrs_desc =
        {
            let mut init =
                gss_OID_set_desc_struct{count: 28 as libc::c_int as size_t,
                                        elements:
                                            (const_oids.as_ptr() as
    ...
    GSS_C_ATTR_LOCAL_LOGIN_USER =
        &mut *const_attrs.as_mut_ptr().offset(0 as libc::c_int as isize) as
            *mut gss_buffer_desc
}
#[used]
