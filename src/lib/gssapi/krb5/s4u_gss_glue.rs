 ...
 */
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        _krb5_ccache, krb5_ccache, krb5_creds, krb5_data,
                        krb5_timestamp, krb5_authdata};
    ...
    extern "C" {
        ...
        pub fn krb5_get_credentials_for_user(context: krb5_context,
                                             options: krb5_flags,
                                             ccache: krb5_ccache,
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_free_context, krb5_unparse_name,
                       krb5_copy_principal, krb5_cc_copy_creds,
                       krb5_cc_set_config, krb5_free_authdata,
                       krb5_free_creds, krb5_free_unparsed_name,
                       krb5_timeofday};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         ts_delta, make_data, string2data, plugin_mapping,
                         _kdb_log_context, k5_tls_vtable_st,
                         hostrealm_module_handle, localauth_module_handle,
                         ccselect_module_handle, krb5_preauth_context_st,
                         _kdb5_dal_handle, krb5_get_credentials_for_user,
...
pub use self::gssapiP_krb5_h::{_krb5_gss_name_rec, krb5_gss_name_t,
                               ...
                               kg_cred_resolve, kg_release_name, kg_init_name,
                               krb5_gss_init_context};
use self::stdio_h::{stderr, fprintf};
use self::stdlib_h::{free, malloc};
use self::string_h::{strerror, strlen, memset};
 ...
 */
#[c2rust::src_loc = "32:1"]
unsafe extern "C" fn kg_is_initiator_cred(mut cred: krb5_gss_cred_id_t)
 -> libc::c_int {
    return (((*cred).usage == 1 as libc::c_int ||
                 (*cred).usage == 0 as libc::c_int) &&
                !(*cred).ccache.is_null()) as libc::c_int;
}
#[c2rust::src_loc = "39:1"]
unsafe extern "C" fn kg_impersonate_name(mut minor_status: *mut OM_uint32,
                                         impersonator_cred:
                                             krb5_gss_cred_id_t,
                                         user: krb5_gss_name_t,
 ...
 -> OM_uint32 {
    ...
    *output_cred = 0 as krb5_gss_cred_id_t;
    ...
    if !(*impersonator_cred).req_enctypes.is_null() {
        in_creds.keyblock.enctype =
            *(*impersonator_cred).req_enctypes.offset(0 as libc::c_int as
                                                          isize)
    }
    k5_mutex_lock(&mut (*user).lock);
    if !(*user).ad_context.is_null() {
        code =
            krb5_authdata_export_authdata(context, (*user).ad_context,
                                          0x2 as libc::c_int,
    ...
    code =
        krb5_get_credentials_for_user(context,
                                      ...
                                      &mut in_creds, 0 as *mut krb5_data,
                                      &mut out_creds);
    if code != 0 as libc::c_int {
...
#[no_mangle]
#[c2rust::src_loc = "103:1"]
pub unsafe extern "C" fn krb5_gss_acquire_cred_impersonate_name(mut minor_status:
                                                                    *mut OM_uint32,
 ...
 -> OM_uint32 {
    let mut major_status: OM_uint32 = 0;
    let mut code: krb5_error_code = 0;
    let mut imp_cred: krb5_gss_cred_id_t =
        impersonator_cred_handle as krb5_gss_cred_id_t;
    ...
    if cred_usage != 1 as libc::c_int {
        *minor_status = -(2045022969 as libc::c_long) as OM_uint32;
        return (13 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    if (*imp_cred).usage != 1 as libc::c_int &&
           (*imp_cred).usage != 0 as libc::c_int {
    ...
    *output_cred_handle = 0 as gss_cred_id_t;
    ...
    major_status =
        kg_cred_resolve(minor_status, context, impersonator_cred_handle,
                        0 as gss_name_t);
    if major_status &
    ...
    major_status =
        kg_impersonate_name(minor_status, imp_cred,
                            desired_name as krb5_gss_name_t, time_req,
                            &mut cred, time_rec, context);
    if major_status &
           ((0o377 as libc::c_ulong as OM_uint32) << 24 as libc::c_int |
                (0o377 as libc::c_ulong as OM_uint32) << 16 as libc::c_int) ==
           0 {
        *output_cred_handle = cred as gss_cred_id_t
    }
 ...
 */
#[c2rust::src_loc = "179:1"]
unsafe extern "C" fn make_proxy_cred(mut context: krb5_context,
                                     mut cred: krb5_gss_cred_id_t,
                                     mut impersonator_cred:
                                         krb5_gss_cred_id_t)
 -> krb5_error_code {
    let mut code: krb5_error_code = 0;
    ...
    code =
        krb5_cc_copy_creds(context, (*impersonator_cred).ccache,
                           (*cred).ccache);
    if code != 0 { return code }
    code =
        krb5_unparse_name(context,
                          (*(*impersonator_cred).name).princ as
                              krb5_const_principal, &mut str);
    if code != 0 { return code }
    data = string2data(str);
    code =
        krb5_cc_set_config(context, (*cred).ccache, 0 as krb5_const_principal,
                           b"proxy_impersonator\x00" as *const u8 as
                               *const libc::c_char, &mut data);
...
pub unsafe extern "C" fn kg_compose_deleg_cred(mut minor_status:
                                                   *mut OM_uint32,
                                               mut impersonator_cred:
                                                   krb5_gss_cred_id_t,
                                               mut subject_creds:
 ...
 -> OM_uint32 {
    ...
    *output_cred = 0 as krb5_gss_cred_id_t;
    if kg_is_initiator_cred(impersonator_cred) == 0 ||
        ...
        code = -(2045022969 as libc::c_long) as krb5_error_code
    } else {
        if !(*(*impersonator_cred).name).princ.is_null() {
        } else {
            __assert_fail(b"impersonator_cred->name->princ != NULL\x00" as
                              *const u8 as *const libc::c_char,
                          b"s4u_gss_glue.c\x00" as *const u8 as
                              *const libc::c_char,
                          230 as libc::c_int as libc::c_uint,
                          (*::std::mem::transmute::<&[u8; 139],
                                                    &[libc::c_char; 139]>(b"OM_uint32 kg_compose_deleg_cred(OM_uint32 *, krb5_gss_cred_id_t, krb5_creds *, OM_uint32, krb5_gss_cred_id_t *, OM_uint32 *, krb5_context)\x00")).as_ptr());
        }
        if !subject_creds.is_null() {
        } else {
            __assert_fail(b"subject_creds != NULL\x00" as *const u8 as
                              *const libc::c_char,
                          b"s4u_gss_glue.c\x00" as *const u8 as
                              *const libc::c_char,
        ...
        if !(*subject_creds).client.is_null() {
            ...
            __assert_fail(b"subject_creds->client != NULL\x00" as *const u8 as
                              *const libc::c_char,
                          b"s4u_gss_glue.c\x00" as *const u8 as
                              *const libc::c_char,
        ...
        cred =
            malloc(::std::mem::size_of::<_krb5_gss_cred_id_rec>() as
        ...
        if cred.is_null() {
            ...
            if !(code != 0 as libc::c_int) {
                ...
                code =
                    kg_init_name(context, (*subject_creds).client,
                                 0 as *mut libc::c_char,
                                 0 as *mut libc::c_char,
                                 0 as krb5_authdata_context, 0 as libc::c_int,
                                 &mut (*cred).name);
                if !(code != 0 as libc::c_int) {
                    code =
                        krb5_cc_new_unique(context,
                                           ...
                                           &mut (*cred).ccache);
                    if !(code != 0 as libc::c_int) {
                        (*cred).set_destroy_ccache(1 as libc::c_int as
                                                       libc::c_uint);
                        code =
                            krb5_cc_initialize(context, (*cred).ccache,
                                               (*subject_creds).client);
                        if !(code != 0 as libc::c_int) {
                            code =
                                make_proxy_cred(context, cred,
                                                impersonator_cred);
                            if !(code != 0 as libc::c_int) {
                                code =
                                ...
                                if !(code != 0 as libc::c_int) {
                                    if !time_rec.is_null() {
                                        ...
                                        code =
                                            krb5_timeofday(context, &mut now);
                                        if code != 0 as libc::c_int {
                                            current_block =
