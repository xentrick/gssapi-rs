...
pub mod gssapiP_krb5_h {
    ...
    use super::krb5_h::{krb5_context, krb5_key, krb5_pointer,
                        krb5_const_pointer, krb5_error_code};
    extern "C" {
        #[no_mangle]
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_enctype,
                       ...
                       krb5_keyblock, krb5_key, _profile_t, krb5_k_create_key,
...
use self::gssapiP_krb5_h::kg_encrypt;
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
/*
 ...
 */
#[c2rust::src_loc = "29:28"]
static mut zeros: [libc::c_uchar; 16] =
...
#[no_mangle]
#[c2rust::src_loc = "31:1"]
pub unsafe extern "C" fn kg_make_seed(mut context: krb5_context,
                                      mut key: krb5_key,
                                      mut seed: *mut libc::c_uchar)
 -> krb5_error_code {
    let mut code: krb5_error_code = 0;
    let mut rkey: krb5_key = 0 as krb5_key;
    let mut tmpkey: *mut krb5_keyblock = 0 as *mut krb5_keyblock;
    let mut kb: *mut krb5_keyblock = 0 as *mut krb5_keyblock;
    let mut i: libc::c_uint = 0;
    code = krb5_k_key_keyblock(context, key, &mut tmpkey);
    if code != 0 { return code }
    /* reverse the key bytes, as per spec */
    kb = &mut (*key).keyblock;
    i = 0 as libc::c_int as libc::c_uint;
    while i < (*tmpkey).length {
        *(*tmpkey).contents.offset(i as isize) =
            *(*kb).contents.offset((*kb).length.wrapping_sub(1 as libc::c_int
                                                                 as
                                                                 libc::c_uint).wrapping_sub(i)
                                       as isize);
        i = i.wrapping_add(1)
    }
    code = krb5_k_create_key(context, tmpkey, &mut rkey);
    if !(code != 0) {
        code =
            kg_encrypt(context, rkey, 22 as libc::c_int,
                       0 as *mut libc::c_void,
                       zeros.as_ptr() as krb5_const_pointer,
                       seed as krb5_pointer,
