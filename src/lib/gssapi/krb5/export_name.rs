...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_gss_export_name(mut minor_status:
                                                  *mut OM_uint32,
                                              input_name: gss_name_t,
                                              mut exported_name: gss_buffer_t)
 -> OM_uint32 {
    ...
    if code != 0 {
        if !minor_status.is_null() { *minor_status = code as OM_uint32 }
        return (13 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    (*exported_name).length = 0 as libc::c_int as size_t;
    ...
    (*exported_name).length =
        (10 as libc::c_int as
             libc::c_ulong).wrapping_add(length).wrapping_add((*gss_mech_krb5).length
                                                                  as
                                                                  libc::c_ulong);
    (*exported_name).value = gssalloc_malloc((*exported_name).length);
    if (*exported_name).value.is_null() {
    ...
    cp = (*exported_name).value as *mut libc::c_uchar;
    ...
    *fresh0 = 0x4 as libc::c_int as libc::c_uchar;
    let fresh1 = cp;
    cp = cp.offset(1);
    *fresh1 = 0x1 as libc::c_int as libc::c_uchar;
    store_16_be((*gss_mech_krb5).length.wrapping_add(2 as libc::c_int as
                                                         libc::c_uint),
    ...
    *fresh2 = 0x6 as libc::c_int as libc::c_uchar;
    ...
    cp = cp.offset(1);
    *fresh3 =
        ((*gss_mech_krb5).length & 0xff as libc::c_int as libc::c_uint) as
            libc::c_uchar;
    ...
    store_32_be(length as libc::c_uint, cp as *mut libc::c_void);
    cp = cp.offset(4 as libc::c_int as isize);
    memcpy(cp as *mut libc::c_void, str as *const libc::c_void, length);
    free(str as *mut libc::c_void);
