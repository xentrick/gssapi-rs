...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_cc_destroy, krb5_cc_close, krb5_kt_close,
...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         zapfree, plugin_mapping, _kdb_log_context,
...
use self::string_h::{explicit_bzero, strlen};
/* -*- mode: c; indent-tabs-mode: nil -*- */
/*
...
#[no_mangle]
#[c2rust::src_loc = "26:1"]
pub unsafe extern "C" fn krb5_gss_release_cred(mut minor_status:
                                                   *mut OM_uint32,
                                               mut cred_handle:
                                                   *mut gss_cred_id_t)
 -> OM_uint32 {
    ...
    let mut code1: krb5_error_code = 0;
    let mut code2: krb5_error_code = 0;
    code1 = krb5_gss_init_context(&mut context);
    if code1 != 0 {
        *minor_status = code1 as OM_uint32;
    ...
    if !(*cred).ccache.is_null() {
        ...
        } else { code1 = krb5_cc_close(context, (*cred).ccache) }
    } else { code1 = 0 as libc::c_int }
    ...
    if !(*cred).password.is_null() {
        zapfree((*cred).password as *mut libc::c_void,
                strlen((*cred).password));
    }
    free(cred as *mut libc::c_void);
    *cred_handle = 0 as gss_cred_id_t;
    *minor_status = 0 as libc::c_int as OM_uint32;
    if code1 != 0 { *minor_status = code1 as OM_uint32 }
    if code2 != 0 { *minor_status = code2 as OM_uint32 }
    if *minor_status != 0 {
