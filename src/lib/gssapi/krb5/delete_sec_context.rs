...
#[c2rust::header_src = "/usr/include/bits/thread-shared-types.h:24"]
...
#[c2rust::header_src = "/usr/include/bits/pthreadtypes.h:24"]
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/include/krb5/authdata_plugin.h:24"]
...
pub mod gssapiP_krb5_h {
    ...
    extern "C" {
        ...
        pub fn krb5_gss_release_oid(_: *mut OM_uint32, _: *mut gss_OID)
         -> OM_uint32;
        #[no_mangle]
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_ticket, _krb5_ap_req, krb5_ap_req, krb5_rcache,
                       _profile_t, _krb5_auth_context, krb5_rc_st,
                       krb5_k_free_key, krb5_free_context, krb5_free_authdata,
                       krb5_auth_con_free, krb5_auth_con_setrcache};
...
pub use self::gssapiP_krb5_h::{_krb5_gss_name_rec, krb5_gss_name_t,
                               ...
                               krb5_gss_ctx_id_t, krb5_gss_release_oid,
                               kg_release_name};
...
#[no_mangle]
#[c2rust::src_loc = "30:1"]
pub unsafe extern "C" fn krb5_gss_delete_sec_context(mut minor_status:
                                                         *mut OM_uint32,
                                                     mut context_handle:
                                                         *mut gss_ctx_id_t,
                                                     mut output_token:
 ...
 -> OM_uint32 {
    ...
    if !(*ctx).seqstate.is_null() { gssint_g_seqstate_free((*ctx).seqstate); }
    if !(*ctx).enc.is_null() { krb5_k_free_key(context, (*ctx).enc); }
    if !(*ctx).seq.is_null() { krb5_k_free_key(context, (*ctx).seq); }
    if !(*ctx).here.is_null() { kg_release_name(context, &mut (*ctx).here); }
    if !(*ctx).there.is_null() {
        kg_release_name(context, &mut (*ctx).there);
    }
    if !(*ctx).subkey.is_null() { krb5_k_free_key(context, (*ctx).subkey); }
    if !(*ctx).acceptor_subkey.is_null() {
        krb5_k_free_key(context, (*ctx).acceptor_subkey);
    }
    if !(*ctx).auth_context.is_null() {
        if (*ctx).cred_rcache != 0 {
            krb5_auth_con_setrcache(context, (*ctx).auth_context,
                                    0 as krb5_rcache);
        }
        krb5_auth_con_free(context, (*ctx).auth_context);
    }
    if !(*ctx).mech_used.is_null() {
        krb5_gss_release_oid(minor_status, &mut (*ctx).mech_used);
    }
    if !(*ctx).authdata.is_null() {
        krb5_free_authdata(context, (*ctx).authdata);
    }
    if !(*ctx).k5_context.is_null() { krb5_free_context((*ctx).k5_context); }
    /* Zero out context */
    explicit_bzero(ctx as *mut libc::c_void,
