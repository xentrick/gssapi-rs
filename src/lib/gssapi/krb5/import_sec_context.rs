...
pub mod gssapiP_krb5_h {
    ...
    extern "C" {
        ...
        pub fn kg_ctx_internalize(kcontext: krb5_context,
                                  argp: *mut krb5_gss_ctx_id_t,
...
pub mod gssapi_krb5_h {
    ...
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "92:38"]
        pub static krb5_gss_oid_array: [gss_OID_desc; 0];
    }
    /* _GSSAPI_KRB5_H_ */
    /* __cplusplus */
}
...
pub use self::gssapiP_krb5_h::{_krb5_gss_name_rec, krb5_gss_name_t,
                               ...
                               kg_ctx_internalize, krb5_gss_save_error_info,
                               krb5_gss_init_context};
use self::string_h::memcmp;
use self::gssapi_krb5_h::krb5_gss_oid_array;
/* LEAN_CLIENT */
/* minor_status */
/* oid */
/* minor_status */
...
#[no_mangle]
#[c2rust::src_loc = "35:1"]
pub unsafe extern "C" fn krb5_gss_convert_static_mech_oid(mut oid: gss_OID)
 -> gss_OID {
    let mut p: *const gss_OID_desc = 0 as *const gss_OID_desc;
    let mut minor_status: OM_uint32 = 0;
    p = krb5_gss_oid_array.as_ptr();
    while (*p).length != 0 {
        if (*oid).length == (*p).length &&
               memcmp((*oid).elements, (*p).elements,
                      (*p).length as libc::c_ulong) == 0 as libc::c_int {
            generic_gss_release_oid(&mut minor_status, &mut oid);
            return p as gss_OID
        }
        p = p.offset(1)
    }
...
#[no_mangle]
#[c2rust::src_loc = "51:1"]
pub unsafe extern "C" fn krb5_gss_import_sec_context(mut minor_status:
                                                         *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    let mut ibp: *mut krb5_octet = 0 as *mut krb5_octet;
    ...
    kret = krb5_gss_init_context(&mut context);
    if kret != 0 {
        *minor_status = kret as OM_uint32;
        return (13 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    /* Assume a tragic failure */
    ctx = 0 as *mut libc::c_void as krb5_gss_ctx_id_t;
    *minor_status = 0 as libc::c_int as OM_uint32;
    /* Internalize the context */
    ibp = (*interprocess_token).value as *mut krb5_octet;
    blen = (*interprocess_token).length;
    kret = kg_ctx_internalize(context, &mut ctx, &mut ibp, &mut blen);
    if kret != 0 {
        *minor_status = kret as OM_uint32;
