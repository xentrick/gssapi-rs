...
pub mod gssapiP_krb5_h {
    ...
    use super::gssapi_h::{gss_cred_usage_t, OM_uint32, gss_cred_id_t,
                          gss_cred_id_struct, gss_name_t, gss_OID_set,
                          gss_name_struct, gss_OID_set_desc_struct};
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_ticket, _krb5_ap_req, krb5_ap_req, krb5_ccache,
                       ...
                       krb5_cc_initialize, krb5_cc_close, krb5_free_context,
                       krb5_cc_resolve, krb5_cc_copy_creds};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _kdb5_dal_handle, krb5int_cc_default};
...
pub use self::gssapi_h::{OM_uint32, gss_uint32, gss_name_t, gss_cred_id_t,
                         ...
                         gss_OID_set, gss_cred_usage_t, gss_name_struct,
                         gss_cred_id_struct};
...
pub use self::gssapiP_krb5_h::{_krb5_gss_name_rec, krb5_gss_name_t,
                               ...
                               krb5_gss_release_cred, krb5_gss_inquire_cred,
 ...
 */
#[c2rust::src_loc = "30:1"]
unsafe extern "C" fn has_unexpired_creds(mut kcred: krb5_gss_cred_id_t,
                                         desired_mech: gss_OID,
                                         mut default_cred: libc::c_int,
                                         mut cred_store:
                                             gss_const_key_value_set_t)
 -> libc::c_int {
    let mut major_status: OM_uint32 = 0;
    let mut minor: OM_uint32 = 0;
    let mut cred_name: gss_name_t = 0 as *mut gss_name_struct;
    let mut desired_mechs: gss_OID_set_desc =
        gss_OID_set_desc{count: 0, elements: 0 as *mut gss_OID_desc_struct,};
    let mut tmp_cred: gss_cred_id_t = 0 as gss_cred_id_t;
    let mut time_rec: OM_uint32 = 0;
    desired_mechs.count = 1 as libc::c_int as size_t;
    desired_mechs.elements = desired_mech;
    if default_cred != 0 {
        cred_name = 0 as gss_name_t
    } else { cred_name = (*kcred).name as gss_name_t }
    major_status =
        krb5_gss_acquire_cred_from(&mut minor, cred_name,
                                   0 as libc::c_int as OM_uint32,
                                   &mut desired_mechs, 1 as libc::c_int,
                                   cred_store, &mut tmp_cred,
                                   0 as *mut gss_OID_set, &mut time_rec);
    krb5_gss_release_cred(&mut minor, &mut tmp_cred);
    return (major_status &
                ((0o377 as libc::c_ulong as OM_uint32) << 24 as libc::c_int |
                     (0o377 as libc::c_ulong as OM_uint32) <<
                         16 as libc::c_int) != 0 || time_rec != 0) as
               libc::c_int;
}
#[c2rust::src_loc = "60:1"]
unsafe extern "C" fn copy_initiator_creds(mut minor_status: *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    let mut kcred: krb5_gss_cred_id_t = 0 as krb5_gss_cred_id_t;
    let mut context: krb5_context = 0 as krb5_context;
    let mut ccache: krb5_ccache = 0 as krb5_ccache;
    let mut ccache_name: *const libc::c_char = 0 as *const libc::c_char;
    *minor_status = 0 as libc::c_int as OM_uint32;
    if default_cred == 0 && cred_store.is_null() {
        *minor_status = -(2045022957 as libc::c_long) as OM_uint32;
        ...
        if code != 0 as libc::c_int {
            *minor_status = code as OM_uint32;
            ...
            major_status =
                krb5_gss_validate_cred_1(minor_status, input_cred_handle,
                                         context);
            if !(major_status &
                     ((0o377 as libc::c_ulong as OM_uint32) <<
                          24 as libc::c_int |
                          (0o377 as libc::c_ulong as OM_uint32) <<
                              16 as libc::c_int) != 0) {
                kcred = input_cred_handle as krb5_gss_cred_id_t;
                if (*kcred).ccache.is_null() {
                    *minor_status = 39756032 as libc::c_long as OM_uint32;
                    major_status =
                        (10 as libc::c_ulong as OM_uint32) <<
                            16 as libc::c_int
                } else if overwrite_cred == 0 &&
                              has_unexpired_creds(kcred, desired_mech,
                                                  default_cred as libc::c_int,
                                                  cred_store) != 0 {
                    major_status =
                        (17 as libc::c_ulong as OM_uint32) <<
                            16 as libc::c_int
                } else {
                    major_status =
                    ...
                    if !(major_status &
                        ...
                        if !ccache_name.is_null() {
                            code =
                                krb5_cc_resolve(context, ccache_name,
                                                &mut ccache);
                            if code != 0 as libc::c_int {
                                *minor_status = code as OM_uint32;
                                major_status =
    ...
    if !kcred.is_null() { k5_mutex_unlock(&mut (*kcred).lock); }
...
pub unsafe extern "C" fn krb5_gss_store_cred(mut minor_status: *mut OM_uint32,
                                             mut input_cred_handle:
                                                 gss_cred_id_t,
...
pub unsafe extern "C" fn krb5_gss_store_cred_into(mut minor_status:
                                                      *mut OM_uint32,
                                                  mut input_cred_handle:
                                                      gss_cred_id_t,
                                                  mut cred_usage:
 ...
 -> OM_uint32 {
    let mut major_status: OM_uint32 = 0;
    let mut actual_usage: gss_cred_usage_t = 0;
    ...
    if cred_usage == 2 as libc::c_int {
        *minor_status = -(2045022958 as libc::c_long) as OM_uint32;
        return (13 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    } else {
        if cred_usage != 1 as libc::c_int && cred_usage != 0 as libc::c_int {
            *minor_status = -(2045022969 as libc::c_long) as OM_uint32;
            return (13 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
        }
    }
    major_status =
        krb5_gss_inquire_cred(minor_status, input_cred_handle,
                              0 as *mut gss_name_t, &mut lifetime,
                              &mut actual_usage, elements_stored);
    if major_status &
           ((0o377 as libc::c_ulong as OM_uint32) << 24 as libc::c_int |
    ...
    if lifetime == 0 as libc::c_int as libc::c_uint {
        return (11 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    if actual_usage != 1 as libc::c_int && actual_usage != 0 as libc::c_int {
        *minor_status = -(2045022958 as libc::c_long) as OM_uint32;
        return (13 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    major_status =
    ...
    if !cred_usage_stored.is_null() { *cred_usage_stored = 1 as libc::c_int }
