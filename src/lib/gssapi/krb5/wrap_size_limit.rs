...
pub use self::gssapi_h::{OM_uint32, gss_uint32, gss_ctx_id_t,
                         gss_OID_desc_struct, gss_OID_desc, gss_qop_t,
                         gss_ctx_id_struct};
...
#[no_mangle]
#[c2rust::src_loc = "76:1"]
pub unsafe extern "C" fn krb5_gss_wrap_size_limit(mut minor_status:
                                                      *mut OM_uint32,
                                                  mut context_handle:
                                                      gss_ctx_id_t,
                                                  mut conf_req_flag:
 ...
 -> OM_uint32 {
    ...
    let mut data_size: OM_uint32 = 0;
    let mut conflen: OM_uint32 = 0;
    let mut ohlen: OM_uint32 = 0;
    let mut overhead: libc::c_int = 0;
    ...
    if (*ctx).proto == 1 as libc::c_int {
        /* No pseudo-ASN.1 wrapper overhead, so no sequence length and
           OID.  */
        let mut sz: OM_uint32 = req_output_size;
        ...
        if conf_req_flag != 0 {
            ...
            while sz > 0 as libc::c_int as libc::c_uint &&
                      krb5_encrypt_size(sz as size_t,
                                        enctype).wrapping_add(16 as
                                                                  libc::c_int
                                                                  as
                                                                  libc::c_ulong)
                          > req_output_size as libc::c_ulong {
                sz = sz.wrapping_sub(1)
            }
            /* Allow for encrypted copy of header.  */
            if sz > 16 as libc::c_int as libc::c_uint {
                sz =
                    (sz as
                         libc::c_uint).wrapping_sub(16 as libc::c_int as
                                                        libc::c_uint) as
                        OM_uint32 as OM_uint32
            } else { sz = 0 as libc::c_int as OM_uint32 }
        } else {
            ...
            err =
                krb5_c_checksum_length((*ctx).k5_context, cksumtype,
                                       &mut cksumsize);
            if err != 0 {
            ...
            if (sz as libc::c_ulong) <
                   (16 as libc::c_int as
                        libc::c_ulong).wrapping_add(cksumsize) {
                sz = 0 as libc::c_int as OM_uint32
            } else {
                sz =
                    (sz as
    ...
    overhead =
        (7 as libc::c_int as
             libc::c_uint).wrapping_add((*(*ctx).mech_used).length) as
            libc::c_int;
    data_size = req_output_size;
    conflen =
        kg_confounder_size((*ctx).k5_context, (*(*ctx).enc).keyblock.enctype)
            as OM_uint32;
    data_size =
        conflen.wrapping_add(data_size).wrapping_add(8 as libc::c_int as
                                                         libc::c_uint) &
            !(7 as libc::c_int as OM_uint32);
    ohlen =
        gssint_g_token_size((*ctx).mech_used,
                            (data_size as
                                 libc::c_ulong).wrapping_add((*ctx).cksum_size).wrapping_add(14
                                                                                                 as
                                                                                                 libc::c_int
                                                                                                 as
                                                                                                 libc::c_ulong)
                                as
                                libc::c_uint).wrapping_sub(req_output_size);
    if ohlen.wrapping_add(overhead as libc::c_uint) < req_output_size {
        /*
         * Cannot have trailer length that will cause us to pad over our
         * length.
         */
        *max_input_size =
            req_output_size.wrapping_sub(ohlen).wrapping_sub(overhead as
                                                                 libc::c_uint)
                & !(7 as libc::c_int as OM_uint32)
