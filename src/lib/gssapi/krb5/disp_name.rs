...
pub mod gssapiP_krb5_h {
    ...
    use super::krb5_h::{krb5_principal, krb5_context, krb5_error_code};
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_principal_compare, krb5_free_unparsed_name};
...
#[no_mangle]
#[c2rust::src_loc = "26:1"]
pub unsafe extern "C" fn krb5_gss_display_name(mut minor_status:
                                                   *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    let mut k5name: krb5_gss_name_t = input_name as krb5_gss_name_t;
    let mut nametype: gss_OID = gss_nt_krb5_name;
    (*output_name_buffer).length = 0 as libc::c_int as size_t;
    (*output_name_buffer).value = 0 as *mut libc::c_void;
    if !output_name_type.is_null() { *output_name_type = 0 as gss_OID }
    ...
    if (*(*k5name).princ).type_0 == 11 as libc::c_int {
        if krb5_principal_compare(context,
                                  (*k5name).princ as krb5_const_principal,
    ...
    code =
        krb5_unparse_name(context,
                          (*(input_name as krb5_gss_name_t)).princ as
    ...
    if gssint_g_make_string_buffer(str, output_name_buffer) == 0 {
        krb5_free_unparsed_name(context, str);
        krb5_free_context(context);
        *minor_status = -(2045022972 as libc::c_long) as OM_uint32;
        return (13 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    krb5_free_unparsed_name(context, str);
    krb5_free_context(context);
    *minor_status = 0 as libc::c_int as OM_uint32;
    if !output_name_type.is_null() { *output_name_type = nametype }
