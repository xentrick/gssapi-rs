 ...
 */
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_authdatatype, krb5_context, krb5_error_code,
                        krb5_data};
    ...
    extern "C" {
        ...
        pub fn krb5_authdata_import_attributes(kcontext: krb5_context,
                                               context: krb5_authdata_context,
                                               usage: krb5_flags,
                                               attributes: *const krb5_data)
         -> krb5_error_code;
    }
    /* _KRB5_INT_H */
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/include/krb5/authdata_plugin.h:28"]
...
pub mod gssapiP_krb5_h {
    ...
    use super::krb5_h::{krb5_principal, krb5_context, krb5_principal_data,
                        krb5_flags, krb5_error_code};
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/gssapi/generic/gssapi_generic.h:28"]
...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _kdb5_dal_handle, krb5_authdata_context_init,
...
pub use self::gssapi_h::{OM_uint32, gss_uint32, gss_name_t,
                         ...
                         gss_buffer_t, gss_name_struct, GSS_C_NT_ANONYMOUS};
...
use self::string_h::{strncpy, memchr, memcmp, memcpy};
...
use self::gssapi_krb5_h::{GSS_KRB5_NT_ENTERPRISE_NAME, gss_mech_krb5,
 ...
 */
#[c2rust::src_loc = "51:1"]
unsafe extern "C" fn import_name_composite(mut context: krb5_context,
                                           mut enc_data: *mut libc::c_uchar,
                                           mut enc_length: size_t,
                                           mut pad_context:
                                               *mut krb5_authdata_context)
 -> krb5_error_code {
    let mut ad_context: krb5_authdata_context =
        0 as *mut _krb5_authdata_context;
    let mut code: krb5_error_code = 0;
    let mut data: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    if enc_length == 0 as libc::c_int as libc::c_ulong {
        return 0 as libc::c_int
    }
    code = krb5_authdata_context_init(context, &mut ad_context);
    if code != 0 as libc::c_int { return code }
    data.data = enc_data as *mut libc::c_char;
    data.length = enc_length as libc::c_uint;
    code =
        krb5_authdata_import_attributes(context, ad_context,
                                        0x2f as libc::c_int, &mut data);
...
#[c2rust::src_loc = "86:1"]
unsafe extern "C" fn parse_hostbased(mut str: *const libc::c_char,
                                     mut len: size_t,
                                     mut service_out: *mut *mut libc::c_char,
                                     mut host_out: *mut *mut libc::c_char)
 -> krb5_error_code {
    let mut at: *const libc::c_char = 0 as *const libc::c_char;
    let mut servicelen: size_t = 0;
    ...
    *service_out = *host_out;
    ...
    servicelen =
        if at.is_null() {
            len
        } else { at.wrapping_offset_from(str) as libc::c_long as size_t };
    service =
        malloc(servicelen.wrapping_add(1 as libc::c_int as libc::c_ulong)) as
            *mut libc::c_char;
    if service.is_null() { return 12 as libc::c_int }
    memcpy(service as *mut libc::c_void, str as *const libc::c_void,
           servicelen);
    *service.offset(servicelen as isize) = '\u{0}' as i32 as libc::c_char;
    /* If present, copy the hostname. */
    if !at.is_null() {
        hostlen =
            len.wrapping_sub(servicelen).wrapping_sub(1 as libc::c_int as
                                                          libc::c_ulong);
        host =
            malloc(hostlen.wrapping_add(1 as libc::c_int as libc::c_ulong)) as
                *mut libc::c_char;
        if host.is_null() {
            free(service as *mut libc::c_void);
            return 12 as libc::c_int
        }
        memcpy(host as *mut libc::c_void,
               at.offset(1 as libc::c_int as isize) as *const libc::c_void,
               hostlen);
        *host.offset(hostlen as isize) = '\u{0}' as i32 as libc::c_char
    }
    *service_out = service;
    *host_out = host;
    return 0 as libc::c_int;
...
#[no_mangle]
#[c2rust::src_loc = "122:1"]
pub unsafe extern "C" fn krb5_gss_import_name(mut minor_status:
                                                  *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    let mut service: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut host: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut stringrep: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut length: ssize_t = 0;
    let mut pw: *mut passwd = 0 as *mut passwd;
    let mut is_composite: libc::c_int = 0 as libc::c_int;
    ...
    *output_name = 0 as gss_name_t;
    *minor_status = 0 as libc::c_int as OM_uint32;
    code = krb5_gss_init_context(&mut context);
    if !(code != 0) {
        if !input_name_type.is_null() &&
               ((*input_name_type).length == (*gss_nt_service_name).length &&
                    memcmp((*input_name_type).elements,
                           (*gss_nt_service_name).elements,
                           (*input_name_type).length as libc::c_ulong) ==
                        0 as libc::c_int ||
                    (*input_name_type).length ==
                        (*gss_nt_service_name_v2).length &&
                        memcmp((*input_name_type).elements,
                               (*gss_nt_service_name_v2).elements,
                               (*input_name_type).length as libc::c_ulong) ==
                            0 as libc::c_int) {
            /* Split the name into service and host (or NULL). */
            code =
                parse_hostbased((*input_name_buffer).value as
                                    *const libc::c_char,
                                (*input_name_buffer).length, &mut service,
                                &mut host);
            if code != 0 {
         ...
         */
                code =
                    krb5_sname_to_principal(context, host, service,
                                            3 as libc::c_int, &mut princ);
                if code != 0 {
                    current_block = 5854819196614252297;
                } else { current_block = 12079920068676227593; }
            }
        } else if !input_name_type.is_null() &&
                      ((*input_name_type).length ==
                           (*gss_nt_krb5_principal).length &&
                           memcmp((*input_name_type).elements,
                                  (*gss_nt_krb5_principal).elements,
                                  (*input_name_type).length as libc::c_ulong)
                               == 0 as libc::c_int) {
            let mut input: krb5_principal = 0 as *mut krb5_principal_data;
            if (*input_name_buffer).length !=
                   ::std::mem::size_of::<krb5_principal>() as libc::c_ulong {
                code = -(2045022970 as libc::c_long) as krb5_error_code;
                status =
                    (2 as libc::c_ulong as OM_uint32) << 16 as libc::c_int;
                current_block = 5854819196614252297;
            } else {
                input = *((*input_name_buffer).value as *mut krb5_principal);
                code =
                    krb5_copy_principal(context,
                                        input as krb5_const_principal,
                                        &mut princ);
                if code != 0 {
                    current_block = 5854819196614252297;
                } else { current_block = 12079920068676227593; }
                      ...
                      ((*input_name_type).length ==
                           (*GSS_C_NT_ANONYMOUS).length &&
                           memcmp((*input_name_type).elements,
                                  (*GSS_C_NT_ANONYMOUS).elements,
                                  (*input_name_type).length as libc::c_ulong)
                               == 0 as libc::c_int) {
            code =
                krb5_copy_principal(context, krb5_anonymous_principal(),
                                    &mut princ);
            if code != 0 {
                current_block = 5854819196614252297;
            } else { current_block = 12079920068676227593; }
            ...
            stringrep = 0 as *mut libc::c_char;
            tmp =
                k5memdup0((*input_name_buffer).value,
                          (*input_name_buffer).length, &mut code) as
                    *mut libc::c_char;
            if tmp.is_null() {
                current_block = 5854819196614252297;
            } else {
                tmp2 = 0 as *mut libc::c_char;
                /* Find the appropriate string rep to pass into parse_name. */
                if input_name_type.is_null() ||
                       (*input_name_type).length == (*gss_nt_krb5_name).length
                           &&
                           memcmp((*input_name_type).elements,
                                  (*gss_nt_krb5_name).elements,
                                  (*input_name_type).length as libc::c_ulong)
                               == 0 as libc::c_int ||
                       (*input_name_type).length == (*gss_nt_user_name).length
                           &&
                           memcmp((*input_name_type).elements,
                                  (*gss_nt_user_name).elements,
                                  (*input_name_type).length as libc::c_ulong)
                               == 0 as libc::c_int {
                    stringrep = tmp;
                    current_block = 2652804691515851435;
                } else if (*input_name_type).length ==
                              (*GSS_KRB5_NT_ENTERPRISE_NAME).length &&
                              memcmp((*input_name_type).elements,
                                     (*GSS_KRB5_NT_ENTERPRISE_NAME).elements,
                                     (*input_name_type).length as
                    ...
                    if (*input_name_type).length ==
                           (*gss_nt_machine_uid_name).length &&
                           memcmp((*input_name_type).elements,
                                  (*gss_nt_machine_uid_name).elements,
                                  (*input_name_type).length as libc::c_ulong)
                               == 0 as libc::c_int {
                        uid = *((*input_name_buffer).value as *mut uid_t);
                        current_block = 5042286318545367694;
                    } else if (*input_name_type).length ==
                                  (*gss_nt_string_uid_name).length &&
                                  memcmp((*input_name_type).elements,
                                         (*gss_nt_string_uid_name).elements,
                                         (*input_name_type).length as
                                             libc::c_ulong) ==
                                      0 as libc::c_int {
                        uid = atoi(tmp) as uid_t;
                        current_block = 5042286318545367694;
                    } else if (*input_name_type).length ==
                                  (*gss_nt_exported_name).length &&
                                  memcmp((*input_name_type).elements,
                                         (*gss_nt_exported_name).elements,
                                         (*input_name_type).length as
                                             libc::c_ulong) ==
                                      0 as libc::c_int ||
                                  (*input_name_type).length ==
                                      ...
                                      memcmp((*input_name_type).elements,
                                             ...
                                             (*input_name_type).length as
                                                 libc::c_ulong) ==
                                          0 as libc::c_int {
                        cp = tmp as *mut libc::c_uchar;
                        end = cp.offset((*input_name_buffer).length as isize);
                        if (end.wrapping_offset_from(cp) as libc::c_long) <
                               2 as libc::c_int as libc::c_long {
                            current_block = 16859350023670137067;
                        } else {
                            let fresh0 = cp;
                            cp = cp.offset(1);
                            if *fresh0 as libc::c_int != 0x4 as libc::c_int {
                                current_block = 16859350023670137067;
                            } else {
                                let fresh1 = cp;
                                cp = cp.offset(1);
                                match *fresh1 as libc::c_int {
                                    1 => {
                                        ...
                                        if (end.wrapping_offset_from(cp) as
                                                libc::c_long) <
                                               2 as libc::c_int as
                                                   libc::c_long {
                                            current_block =
                                                                                       ...
                                                                                       end
                                                                                   {
                                                                                    ...
                                                                                    __assert_fail(b"cp == end\x00"
                                                                                                  ...
                                                                                                  (*::std::mem::transmute::<&[u8; 33],
                                                                                                                            &[libc::c_char; 33]>(b"OM_uint32 krb5_gss_import_name()\x00")).as_ptr());
                                                                                }
                                                                                current_block
                                                                                    =
                                                                                    2652804691515851435;
                        ...
                        status =
                            (3 as libc::c_ulong as OM_uint32) <<
                                16 as libc::c_int;
                        current_block = 5854819196614252297;
                    }
                    match current_block {
                        ...
                        16859350023670137067 => { }
                        _ => {
                            if (if getpwuid_r(uid, &mut pwx,
                                              pwbuf.as_mut_ptr(),
                                              ::std::mem::size_of::<[libc::c_char; 8192]>()
                                                  as libc::c_ulong, &mut pw)
                                       == 0 as libc::c_int {
                                    (if pw.is_null() {
                                         -(1 as libc::c_int)
                ...
                match current_block {
                    ...
                    _ => {
                        ...
                        match current_block {
                            ...
                            _ => {
                                status =
                                    (2 as libc::c_ulong as OM_uint32) <<
                                        16 as libc::c_int;
                                current_block = 5854819196614252297;
                            }
                        }
        ...
        match current_block {
            ...
            _ => {
                ...
                code =
                    kg_init_name(context, princ, service, host, ad_context,
                                 0x1 as libc::c_int, &mut name);
                if !(code != 0) {
                    princ = 0 as krb5_principal;
                    ad_context = 0 as krb5_authdata_context;
                    host = 0 as *mut libc::c_char;
                    service = host;
                    *output_name = name as gss_name_t;
