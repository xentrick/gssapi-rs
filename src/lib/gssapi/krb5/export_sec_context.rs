...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/gssapi/krb5/gssapiP_krb5.h:27"]
pub mod gssapiP_krb5_h {
    ...
    use super::gssapi_h::{OM_uint32, gss_OID_desc, gss_ctx_id_t,
    ...
    extern "C" {
        ...
        pub fn kg_ctx_size(kcontext: krb5_context, ctx: krb5_gss_ctx_id_t,
                           sizep: *mut size_t) -> krb5_error_code;
        #[no_mangle]
        #[c2rust::src_loc = "366:1"]
        pub fn kg_ctx_externalize(kcontext: krb5_context,
                                  ctx: krb5_gss_ctx_id_t,
                                  buffer: *mut *mut krb5_octet,
                                  lenremain: *mut size_t) -> krb5_error_code;
        #[no_mangle]
...
pub use self::gssapiP_krb5_h::{_krb5_gss_name_rec, krb5_gss_name_t,
                               ...
                               kg_ctx_size, kg_ctx_externalize,
...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_gss_export_sec_context(mut minor_status:
                                                         *mut OM_uint32,
                                                     mut context_handle:
                                                         *mut gss_ctx_id_t,
 ...
 -> OM_uint32 {
    ...
    let mut obuffer: *mut krb5_octet = 0 as *mut krb5_octet;
    let mut obp: *mut krb5_octet = 0 as *mut krb5_octet;
    /* Assume a tragic failure */
    obuffer = 0 as *mut libc::c_void as *mut krb5_octet;
    ...
    *minor_status = 0 as libc::c_int as OM_uint32;
    ...
    kret = kg_ctx_size(context, ctx, &mut bufsize);
    if !(kret != 0) {
        /* Allocate the buffer */
        obuffer = gssalloc_malloc(bufsize) as *mut krb5_octet;
        if obuffer.is_null() {
            ...
            kret = kg_ctx_externalize(context, ctx, &mut obp, &mut blen);
            if !(kret != 0) {
                /* Success!  Return the buffer */
                (*interprocess_token).length = bufsize.wrapping_sub(blen);
                (*interprocess_token).value = obuffer as *mut libc::c_void;
                *minor_status = 0 as libc::c_int as OM_uint32;
    ...
    if retval != 0 as libc::c_int as libc::c_uint {
        if kret != 0 as libc::c_int && !context.is_null() {
            krb5_gss_save_error_info(kret as OM_uint32, context);
        }
    }
    if !obuffer.is_null() && bufsize != 0 {
        explicit_bzero(obuffer as *mut libc::c_void, bufsize);
        free(obuffer as *mut libc::c_void);
    }
