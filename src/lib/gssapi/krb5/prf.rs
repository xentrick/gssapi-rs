...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       _krb5_auth_context, krb5_c_prf_length,
                       krb5_k_key_enctype, krb5_k_prf,
...
use self::stdlib_h::{calloc, malloc};
...
#[no_mangle]
#[c2rust::src_loc = "34:1"]
pub unsafe extern "C" fn krb5_gss_pseudo_random(mut minor_status:
                                                    *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    let mut key: krb5_key = 0 as krb5_key;
    let mut ctx: krb5_gss_ctx_id_t = 0 as *mut _krb5_gss_ctx_id_rec;
    let mut i: libc::c_int = 0;
    let mut minor: OM_uint32 = 0;
    let mut prflen: size_t = 0;
    let mut t: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut ns: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut p: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    (*prf_out).length = 0 as libc::c_int as size_t;
    (*prf_out).value = 0 as *mut libc::c_void;
    t.length = 0 as libc::c_int as libc::c_uint;
    t.data = 0 as *mut libc::c_char;
    ns.length = 0 as libc::c_int as libc::c_uint;
    ns.data = 0 as *mut libc::c_char;
    ctx = context as krb5_gss_ctx_id_t;
    ...
    match prf_key {
        0 => {
            ...
            } else { current_block = 9367819545240679859; }
        }
        1 => { current_block = 9367819545240679859; }
        _ => {
            code = 22 as libc::c_int;
            current_block = 17368220239626828796;
        }
    }
    match current_block {
        9367819545240679859 =>
    ...
    match current_block {
        224731115979188411 => {
            if key.is_null() {
                ...
                if desired_output_len == 0 as libc::c_int as libc::c_long {
                    return 0 as libc::c_int as OM_uint32
                }
                (*prf_out).value =
                    gssalloc_malloc(desired_output_len as size_t);
                if (*prf_out).value.is_null() {
                    code = 39756047 as libc::c_long as krb5_error_code
                } else {
                    (*prf_out).length = desired_output_len as size_t;
                    code =
                        krb5_c_prf_length((*ctx).k5_context,
                                          krb5_k_key_enctype((*ctx).k5_context,
                                                             key),
                                          &mut prflen);
                    if !(code != 0 as libc::c_int) {
                        ns.length =
                            (4 as libc::c_int as
                                 libc::c_ulong).wrapping_add((*prf_in).length)
                                as libc::c_uint;
                        ns.data =
                            k5alloc(ns.length as size_t, &mut code) as
                                *mut libc::c_char;
                        if ns.data.is_null() {
                            code = 39756047 as libc::c_long as krb5_error_code
                        } else {
                            t.length = prflen as libc::c_uint;
                            t.data =
                                k5alloc(t.length as size_t, &mut code) as
                                    *mut libc::c_char;
                            if !t.data.is_null() {
                                memcpy(ns.data.offset(4 as libc::c_int as
                                                          isize) as
                                           *mut libc::c_void, (*prf_in).value,
                                       (*prf_in).length);
                                i = 0 as libc::c_int;
                                p = (*prf_out).value as *mut libc::c_uchar;
                                while desired_output_len >
                                          0 as libc::c_int as libc::c_long {
                                    store_32_be(i as libc::c_uint,
                                                ns.data as *mut libc::c_void);
                                    code =
                                        krb5_k_prf((*ctx).k5_context, key,
                                                   &mut ns, &mut t);
                                    if code != 0 as libc::c_int { break ; }
                                    memcpy(p as *mut libc::c_void,
                                           t.data as *const libc::c_void,
                                           if (t.length as libc::c_long) <
                                                  desired_output_len {
                                               t.length as libc::c_long
                                           } else { desired_output_len } as
                                               libc::c_ulong);
                                    p = p.offset(t.length as isize);
                                    desired_output_len -=
                                        t.length as libc::c_long;
                                    i += 1
                                }
    ...
    if code != 0 as libc::c_int { gss_release_buffer(&mut minor, prf_out); }
    krb5_free_data_contents((*ctx).k5_context, &mut ns);
    krb5_free_data_contents((*ctx).k5_context, &mut t);
