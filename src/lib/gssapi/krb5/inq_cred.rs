...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/include/gssapi/gssapi.h:73"]
pub mod gssapi_h {
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/gssapi/generic/gssapi_ext.h:73"]
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/gssapi/krb5/gssapiP_krb5.h:73"]
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/gssapi/generic/gssapiP_generic.h:73"]
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/gssapi/krb5/gssapi_krb5.h:73"]
...
#[no_mangle]
#[c2rust::src_loc = "75:1"]
pub unsafe extern "C" fn krb5_gss_inquire_cred(mut minor_status:
                                                   *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    let mut ret_name: krb5_gss_name_t = 0 as *mut _krb5_gss_name_rec;
    ...
    if cred_handle.is_null() {
        major = kg_get_defcred(minor_status, &mut defcred);
        if major &
    ...
    major =
        kg_cred_resolve(minor_status, context, cred_handle, 0 as gss_name_t);
    if major &
           ((0o377 as libc::c_ulong as OM_uint32) << 24 as libc::c_int |
                (0o377 as libc::c_ulong as OM_uint32) << 16 as libc::c_int) !=
           0 {
        krb5_gss_release_cred(minor_status, &mut defcred);
        krb5_free_context(context);
        return major
    }
    ...
    if code != 0 {
        *minor_status = code as OM_uint32;
        ...
        if (*cred).expire != 0 as libc::c_int {
            ...
            lifetime = 0xffffffff as libc::c_ulong as OM_uint32 as krb5_deltat
        }
        if !name.is_null() {
            if !(*cred).name.is_null() {
                code = kg_duplicate_name(context, (*cred).name, &mut ret_name)
            } else if ((*cred).usage == 2 as libc::c_int ||
                           (*cred).usage == 0 as libc::c_int) &&
                          !(*cred).keytab.is_null() {
                /* This is a default acceptor cred; use a name from the keytab if
             * we can. */
                code =
                    k5_kt_get_principal(context, (*cred).keytab, &mut princ);
                if code == 0 as libc::c_int {
                    code =
                        kg_init_name(context, princ, 0 as *mut libc::c_char,
                                     0 as *mut libc::c_char,
                                     0 as krb5_authdata_context,
                                     0x1 as libc::c_int, &mut ret_name);
                    if code != 0 { krb5_free_principal(context, princ); }
            ...
            _ => {
                if !mechanisms.is_null() {
                    ...
                    if ret &
                        ...
                        if !ret_name.is_null() {
                            kg_release_name(context, &mut ret_name);
                        }
                        current_block = 7135116673376365024;
                    } else { current_block = 2116367355679836638; }
                } else { current_block = 2116367355679836638; }
                match current_block {
                    7135116673376365024 => { }
                    _ => {
                        if !name.is_null() {
                            if !ret_name.is_null() {
                                *name = ret_name as gss_name_t
                            } else { *name = 0 as gss_name_t }
                        }
                        if !lifetime_ret.is_null() {
                            *lifetime_ret = lifetime as OM_uint32
                        }
                        ...
                        *minor_status = 0 as libc::c_int as OM_uint32;
                        return if lifetime == 0 as libc::c_int {
                                   ((11 as libc::c_ulong as OM_uint32)) <<
                                       16 as libc::c_int
    ...
    k5_mutex_unlock(&mut (*cred).lock);
    krb5_gss_release_cred(&mut tmpmin, &mut defcred);
    krb5_free_context(context);
    generic_gss_release_oid_set(&mut tmpmin, &mut mechs);
    return ret;
}
/* V2 interface */
#[no_mangle]
#[c2rust::src_loc = "215:1"]
pub unsafe extern "C" fn krb5_gss_inquire_cred_by_mech(mut minor_status:
                                                           *mut OM_uint32,
                                                       mut cred_handle:
                                                           gss_cred_id_t,
 ...
 -> OM_uint32 {
    ...
    let mut mstat: OM_uint32 = 0;
    cred = cred_handle as krb5_gss_cred_id_t;
    mstat =
        krb5_gss_inquire_cred(minor_status, cred_handle, name, &mut lifetime,
                              cred_usage,
                              0 as *mut libc::c_void as *mut gss_OID_set);
    if mstat == 0 as libc::c_int as libc::c_uint {
        if !cred.is_null() &&
...
pub unsafe extern "C" fn gss_krb5int_get_cred_impersonator(mut minor_status:
                                                               *mut OM_uint32,
                                                           cred_handle:
                                                               gss_cred_id_t,
                                                           desired_object:
 ...
 -> OM_uint32 {
    ...
    let mut impersonator: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    *data_set = 0 as gss_buffer_set_t;
    ...
    ret =
        krb5_unparse_name(context,
                          (*cred).impersonator as krb5_const_principal,
                          &mut impersonator);
    if ret != 0 {
        krb5_free_context(context);
        *minor_status = ret as OM_uint32;
    ...
    major =
        generic_gss_add_buffer_set_member(minor_status, &mut rep, data_set);
