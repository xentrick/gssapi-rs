...
#[no_mangle]
#[c2rust::src_loc = "30:1"]
pub unsafe extern "C" fn gss_get_name_attribute(mut minor_status:
                                                    *mut OM_uint32,
                                                mut name: gss_name_t,
 ...
 -> OM_uint32 {
    ...
    if !authenticated.is_null() { *authenticated = 0 as libc::c_int }
    if !complete.is_null() { *complete = 0 as libc::c_int }
    if !value.is_null() {
        (*value).value = 0 as *mut libc::c_void;
        (*value).length = 0 as libc::c_int as size_t
    }
    if !display_value.is_null() {
        (*display_value).value = 0 as *mut libc::c_void;
        (*display_value).length = 0 as libc::c_int as size_t
    }
    if minor_status.is_null() {
    ...
    status =
        Some((*mech).gss_get_name_attribute.expect("non-null function pointer")).expect("non-null function pointer")(minor_status,
