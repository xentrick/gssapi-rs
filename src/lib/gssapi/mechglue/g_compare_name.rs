 ...
 */
#[c2rust::src_loc = "36:1"]
unsafe extern "C" fn val_comp_name_args(mut minor_status: *mut OM_uint32,
                                        mut name1: gss_name_t,
                                        mut name2: gss_name_t,
                                        mut name_equal: *mut libc::c_int)
 -> OM_uint32 {
    /* Initialize outputs. */
...
#[no_mangle]
#[c2rust::src_loc = "61:1"]
pub unsafe extern "C" fn gss_compare_name(mut minor_status: *mut OM_uint32,
                                          mut name1: gss_name_t,
                                          mut name2: gss_name_t,
                                          mut name_equal: *mut libc::c_int)
 -> OM_uint32 {
    ...
    let mut union_name1: gss_union_name_t = 0 as *mut gss_name_struct;
    let mut union_name2: gss_union_name_t = 0 as *mut gss_name_struct;
    ...
    union_name1 = name1 as gss_union_name_t;
    union_name2 = name2 as gss_union_name_t;
    /*
    ...
    if (*union_name1).mech_type.is_null() {
        union_name1 = name2 as gss_union_name_t;
        union_name2 = name1 as gss_union_name_t
    }
    /*
    ...
    if !(*union_name1).mech_type.is_null() {
        mech =
            gssint_get_mechanism((*union_name1).mech_type as
                                     gss_const_OID); /* Default to *not* equal.... */
        if mech.is_null() {
    ...
    *name_equal = 0 as libc::c_int;
    /*
     * First case... both names are mechanism-specific
     */
    if !(*union_name1).mech_type.is_null() &&
           !(*union_name2).mech_type.is_null() {
        if !((*(*union_name1).mech_type).length ==
                 (*(*union_name2).mech_type).length &&
                 memcmp((*(*union_name1).mech_type).elements,
                        (*(*union_name2).mech_type).elements,
                        (*(*union_name1).mech_type).length as libc::c_ulong)
                     == 0 as libc::c_int) {
            return 0 as libc::c_int as OM_uint32
        }
        if (*union_name1).mech_name.is_null() ||
               (*union_name2).mech_name.is_null() {
            /* should never happen */
        ...
        major_status =
            (*mech).gss_compare_name.expect("non-null function pointer")(minor_status,
    ...
    if !(*union_name2).mech_type.is_null() {
        ...
        union_name1 =
            name2 as gss_union_name_t; /* return complete, but not equal */
        union_name2 = name1 as gss_union_name_t
    }
    major_status =
        gssint_import_internal_name(minor_status, (*union_name1).mech_type,
                                    union_name2, &mut internal_name);
    if major_status != 0 as libc::c_int as libc::c_uint {
        return 0 as libc::c_int as OM_uint32
    }
    if mech.is_null() {
    ...
    gssint_release_internal_name(&mut temp_minor, (*union_name1).mech_type,
                                 &mut internal_name);
    return major_status;
}
