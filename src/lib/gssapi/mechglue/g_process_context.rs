...
#[no_mangle]
#[c2rust::src_loc = "31:1"]
pub unsafe extern "C" fn gss_process_context_token(mut minor_status:
                                                       *mut OM_uint32,
                                                   mut context_handle:
                                                       gss_ctx_id_t,
                                                   mut token_buffer:
 ...
 -> OM_uint32 {
    ...
    *minor_status = 0 as libc::c_int as OM_uint32;
    ...
    if token_buffer.is_null() || (*token_buffer).value.is_null() ||
    ...
    if !mech.is_null() {
        if (*mech).gss_process_context_token.is_some() {
            status =
                (*mech).gss_process_context_token.expect("non-null function pointer")(minor_status,
                                                                                      (*ctx).internal_ctx_id,
                                                                                      token_buffer);
