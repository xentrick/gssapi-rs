 ...
 */
#[c2rust::src_loc = "38:1"]
unsafe extern "C" fn val_exp_sec_ctx_args(mut minor_status: *mut OM_uint32,
                                          mut context_handle:
                                              *mut gss_ctx_id_t,
    ...
    if !interprocess_token.is_null() {
        (*interprocess_token).length = 0 as libc::c_int as size_t;
...
#[no_mangle]
#[c2rust::src_loc = "70:1"]
pub unsafe extern "C" fn gss_export_sec_context(mut minor_status:
                                                    *mut OM_uint32,
                                                mut context_handle:
                                                    *mut gss_ctx_id_t,
                                                mut interprocess_token:
 ...
 -> OM_uint32 {
    ...
    let mut length: OM_uint32 = 0;
    let mut ctx: gss_union_ctx_id_t = 0 as gss_union_ctx_id_t;
    ...
    status =
        val_exp_sec_ctx_args(minor_status, context_handle,
                             interprocess_token);
    ...
    status =
        (*mech).gss_export_sec_context.expect("non-null function pointer")(minor_status,
    ...
    if status != 0 as libc::c_int as libc::c_uint {
        *minor_status =
        ...
        length =
            token.length.wrapping_add(4 as libc::c_int as
                                          libc::c_ulong).wrapping_add((*(*ctx).mech_type).length
                                                                          as
                                                                          libc::c_ulong)
                as OM_uint32;
        (*interprocess_token).length = length as size_t;
        (*interprocess_token).value = gssalloc_malloc(length as size_t);
        if (*interprocess_token).value.is_null() {
            *minor_status = 12 as libc::c_int as OM_uint32;
            ...
            buf = (*interprocess_token).value as *mut libc::c_char;
            length = (*(*ctx).mech_type).length;
            *buf.offset(3 as libc::c_int as isize) =
                (length & 0xff as libc::c_int as libc::c_uint) as
                    libc::c_uchar as libc::c_char;
            ...
            *buf.offset(0 as libc::c_int as isize) =
                (length & 0xff as libc::c_int as libc::c_uint) as
                    libc::c_uchar as libc::c_char;
            memcpy(buf.offset(4 as libc::c_int as isize) as *mut libc::c_void,
                   (*(*ctx).mech_type).elements,
                   (*(*ctx).mech_type).length as size_t);
            memcpy(buf.offset(4 as libc::c_int as
                                  isize).offset((*(*ctx).mech_type).length as
                                                    isize) as
                       *mut libc::c_void, token.value, token.length);
            status = 0 as libc::c_int as OM_uint32
        }
    }
    gss_release_buffer(minor_status, &mut token);
