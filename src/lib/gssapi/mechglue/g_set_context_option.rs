...
#[no_mangle]
#[c2rust::src_loc = "34:1"]
pub unsafe extern "C" fn gss_set_sec_context_option(mut minor_status:
                                                        *mut OM_uint32,
                                                    mut context_handle:
                                                        *mut gss_ctx_id_t,
 ...
 -> OM_uint32 {
    ...
    let mut internal_ctx: gss_ctx_id_t = 0 as gss_ctx_id_t;
    ...
    *minor_status = 0 as libc::c_int as OM_uint32;
    ...
    } else { mech = gssint_get_mechanism((*ctx).mech_type as gss_const_OID) }
    if mech.is_null() {
    ...
    status =
        (*mech).gss_set_sec_context_option.expect("non-null function pointer")(minor_status,
                                                                               if !ctx.is_null()
                                                                                  {
    ...
    if ctx.is_null() && !internal_ctx.is_null() {
        status =
            gssint_create_union_context(minor_status,
                                        &mut (*mech).mech_type as
                                            *mut gss_OID_desc as
                                            gss_const_OID, &mut ctx);
        if status != 0 as libc::c_int as libc::c_uint {
            gssint_delete_internal_sec_context(&mut minor, (*ctx).mech_type,
                                               &mut internal_ctx,
                                               0 as gss_buffer_t);
            return status
        }
