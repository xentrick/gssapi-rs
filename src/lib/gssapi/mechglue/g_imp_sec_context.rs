...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/gssapi/mechglue/mglueP.h:31"]
pub mod mglueP_h {
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/gssapi/generic/gssapiP_generic.h:31"]
...
pub use self::mglueP_h::{gss_name_struct, gss_cred_id_struct,
                         ...
                         gss_mechanism, gss_config, gssint_get_public_oid,
                         gssint_create_union_context, gssint_get_mechanism,
 ...
 */
#[c2rust::src_loc = "39:1"]
unsafe extern "C" fn val_imp_sec_ctx_args(mut minor_status: *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    if !context_handle.is_null() { *context_handle = 0 as gss_ctx_id_t }
    /* Validate arguments. */
    if minor_status.is_null() {
    ...
    if interprocess_token.is_null() || (*interprocess_token).value.is_null()
           ||
           (*interprocess_token).length == 0 as libc::c_int as libc::c_ulong {
        return (1 as libc::c_ulong as OM_uint32) << 24 as libc::c_int |
                   (9 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
...
#[no_mangle]
#[c2rust::src_loc = "71:1"]
pub unsafe extern "C" fn gss_import_sec_context(mut minor_status:
                                                    *mut OM_uint32,
                                                mut interprocess_token:
                                                    gss_buffer_t,
                                                mut context_handle:
                                                    *mut gss_ctx_id_t)
 -> OM_uint32 {
    let mut length: OM_uint32 = 0 as libc::c_int as OM_uint32;
    ...
    let mut token_mech: gss_OID_desc =
    ...
    if (*interprocess_token).length >=
           ::std::mem::size_of::<OM_uint32>() as libc::c_ulong {
        p = (*interprocess_token).value as *mut libc::c_char;
        let fresh0 = p;
        p = p.offset(1);
        length = *fresh0 as OM_uint32;
        let fresh1 = p;
        p = p.offset(1);
        length =
            (length <<
                 8 as libc::c_int).wrapping_add(*fresh1 as libc::c_uint);
        let fresh2 = p;
        p = p.offset(1);
        length =
            (length <<
                 8 as libc::c_int).wrapping_add(*fresh2 as libc::c_uint);
        let fresh3 = p;
        p = p.offset(1);
        length =
            (length << 8 as libc::c_int).wrapping_add(*fresh3 as libc::c_uint)
    }
    if length == 0 as libc::c_int as libc::c_uint ||
           length as libc::c_ulong >
               (*interprocess_token).length.wrapping_sub(::std::mem::size_of::<OM_uint32>()
                                                             as libc::c_ulong)
       {
        return (3 as libc::c_ulong as OM_uint32) << 24 as libc::c_int |
                   (9 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    ...
    token.length =
        (*interprocess_token).length.wrapping_sub(::std::mem::size_of::<OM_uint32>()
                                                      as
                                                      libc::c_ulong).wrapping_sub(length
                                                                                      as
                                                                                      libc::c_ulong);
    ...
    status =
        gssint_select_mech_type(minor_status,
                                &mut token_mech as *mut gss_OID_desc as
                                    gss_const_OID, &mut selected_mech);
    if status != 0 as libc::c_int as libc::c_uint { return status }
    ...
    status =
        gssint_create_union_context(minor_status,
                                    selected_mech as gss_const_OID, &mut ctx);
    if status != 0 as libc::c_int as libc::c_uint { return status }
    if (*mech).gssspi_import_sec_context_by_mech.is_some() {
        public_mech = gssint_get_public_oid(selected_mech as gss_const_OID);
        status =
            (*mech).gssspi_import_sec_context_by_mech.expect("non-null function pointer")(minor_status,
                                                                                          public_mech,
                                                                                          &mut token,
                                                                                          &mut mctx)
    } else {
        status =
            (*mech).gss_import_sec_context.expect("non-null function pointer")(minor_status,
                                                                               &mut token,
                                                                               &mut mctx)
    }
