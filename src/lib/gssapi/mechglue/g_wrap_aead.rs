...
pub use self::mglueP_h::{gss_name_struct, gss_cred_id_struct,
                         ...
                         gss_mechanism, gss_config, gssint_get_mechanism};
 ...
 */
#[c2rust::src_loc = "31:1"]
unsafe extern "C" fn val_wrap_aead_args(mut minor_status: *mut OM_uint32,
                                        mut context_handle: gss_ctx_id_t,
                                        mut conf_req_flag: libc::c_int,
                                        mut qop_req: gss_qop_t,
                                        mut input_assoc_buffer: gss_buffer_t,
...
#[c2rust::src_loc = "65:1"]
unsafe extern "C" fn gssint_wrap_aead_iov_shim(mut mech: gss_mechanism,
                                               mut minor_status:
                                                   *mut OM_uint32,
                                               mut context_handle:
                                                   gss_ctx_id_t,
 ...
 -> OM_uint32 {
    ...
    let mut iov_count: libc::c_int = 0;
    /* HEADER | SIGN_ONLY_DATA | DATA | PADDING | TRAILER */
    iov[i as usize].type_0 = 2 as libc::c_int as OM_uint32;
    iov[i as usize].buffer.value = 0 as *mut libc::c_void;
    iov[i as usize].buffer.length = 0 as libc::c_int as size_t;
    i += 1;
    if !input_assoc_buffer.is_null() {
        iov[i as usize].type_0 = 11 as libc::c_int as OM_uint32;
        iov[i as usize].buffer = *input_assoc_buffer;
        i += 1
    }
    iov[i as usize].type_0 = 1 as libc::c_int as OM_uint32;
    iov[i as usize].buffer = *input_payload_buffer;
    i += 1;
    iov[i as usize].type_0 = 9 as libc::c_int as OM_uint32;
    iov[i as usize].buffer.value = 0 as *mut libc::c_void;
    iov[i as usize].buffer.length = 0 as libc::c_int as size_t;
    i += 1;
    iov[i as usize].type_0 = 7 as libc::c_int as OM_uint32;
    iov[i as usize].buffer.value = 0 as *mut libc::c_void;
    iov[i as usize].buffer.length = 0 as libc::c_int as size_t;
    i += 1;
    iov_count = i;
    if (*mech).gss_wrap_iov_length.is_some() {
        ...
        __assert_fail(b"mech->gss_wrap_iov_length\x00" as *const u8 as
                      ...
                      (*::std::mem::transmute::<&[u8; 143],
                                                &[libc::c_char; 143]>(b"OM_uint32 gssint_wrap_aead_iov_shim(gss_mechanism, OM_uint32 *, gss_ctx_id_t, int, gss_qop_t, gss_buffer_t, gss_buffer_t, int *, gss_buffer_t)\x00")).as_ptr());
    }
    status =
    ...
    while i < iov_count {
        if !(iov[i as usize].type_0 & !(0xffff0000 as libc::c_uint) ==
                 11 as libc::c_int as libc::c_uint) {
            (*output_message_buffer).length =
                ((*output_message_buffer).length as
                     libc::c_ulong).wrapping_add(iov[i as
                                                         usize].buffer.length)
                    as size_t as size_t
        }
        i += 1
    ...
    iov[i as usize].buffer.value =
        ((*output_message_buffer).value as
             *mut libc::c_uchar).offset(offset as isize) as *mut libc::c_void;
    offset =
        (offset as libc::c_ulong).wrapping_add(iov[i as usize].buffer.length)
            as size_t as size_t;
    i += 1;
    /* SIGN_ONLY_DATA */
    if !input_assoc_buffer.is_null() { i += 1 }
    /* DATA */
    iov[i as usize].buffer.value =
        ((*output_message_buffer).value as
             *mut libc::c_uchar).offset(offset as isize) as *mut libc::c_void;
    offset =
        (offset as libc::c_ulong).wrapping_add(iov[i as usize].buffer.length)
            as size_t as size_t;
    memcpy(iov[i as usize].buffer.value, (*input_payload_buffer).value,
    ...
    if offset == (*output_message_buffer).length {
        ...
        __assert_fail(b"offset == output_message_buffer->length\x00" as
    ...
    if (*mech).gss_wrap_iov.is_some() {
        ...
        __assert_fail(b"mech->gss_wrap_iov\x00" as *const u8 as
                          *const libc::c_char,
                      b"g_wrap_aead.c\x00" as *const u8 as
                          *const libc::c_char,
    ...
    status =
        (*mech).gss_wrap_iov.expect("non-null function pointer")(minor_status,
                                                                 context_handle,
                                                                 conf_req_flag,
                                                                 qop_req,
                                                                 conf_state,
    ...
    if status != 0 as libc::c_int as libc::c_uint {
        ...
        *minor_status =
            gssint_mecherrmap_map(*minor_status, &mut (*mech).mech_type);
        gss_release_buffer(&mut minor, output_message_buffer);
    }
    return status;
}
/*
...
pub unsafe extern "C" fn gssint_wrap_aead(mut mech: gss_mechanism,
                                          mut minor_status: *mut OM_uint32,
                                          mut ctx: gss_union_ctx_id_t,
                                          mut conf_req_flag: libc::c_int,
                                          mut qop_req: gss_qop_t,
                                          mut input_assoc_buffer:
                                              gss_buffer_t,
    ...
    if !ctx.is_null() {
        ...
        __assert_fail(b"ctx != NULL\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 140],
                                                &[libc::c_char; 140]>(b"OM_uint32 gssint_wrap_aead(gss_mechanism, OM_uint32 *, gss_union_ctx_id_t, int, gss_qop_t, gss_buffer_t, gss_buffer_t, int *, gss_buffer_t)\x00")).as_ptr());
    }
    if !mech.is_null() {
    } else {
    ...
    if (*mech).gss_wrap_aead.is_some() {
        status =
            (*mech).gss_wrap_aead.expect("non-null function pointer")(minor_status,
                                                                      (*ctx).internal_ctx_id,
                                                                      conf_req_flag,
                                                                      qop_req,
                                                                      input_assoc_buffer,
    ...
    } else if (*mech).gss_wrap_iov.is_some() &&
                  (*mech).gss_wrap_iov_length.is_some() {
        status =
            gssint_wrap_aead_iov_shim(mech, minor_status,
                                      (*ctx).internal_ctx_id, conf_req_flag,
...
pub unsafe extern "C" fn gss_wrap_aead(mut minor_status: *mut OM_uint32,
                                       mut context_handle: gss_ctx_id_t,
                                       mut conf_req_flag: libc::c_int,
                                       mut qop_req: gss_qop_t,
                                       mut input_assoc_buffer: gss_buffer_t,
                                       mut input_payload_buffer: gss_buffer_t,
                                       mut conf_state: *mut libc::c_int,
                                       mut output_message_buffer:
                                           gss_buffer_t) -> OM_uint32 {
    let mut status: OM_uint32 = 0;
    ...
    status =
        val_wrap_aead_args(minor_status, context_handle, conf_req_flag,
