...
pub use self::gssapi_ext_h::{gss_buffer_set_desc_struct, gss_buffer_set_t,
                             ...
                             gss_key_value_element_struct, gss_any};
...
#[no_mangle]
#[c2rust::src_loc = "30:1"]
pub unsafe extern "C" fn gss_map_name_to_any(mut minor_status: *mut OM_uint32,
                                             mut name: gss_name_t,
 ...
 -> OM_uint32 {
    ...
    if !output.is_null() { *output = 0 as gss_any_t }
    if minor_status.is_null() || output.is_null() {
    ...
    if (*mech).gss_map_name_to_any.is_none() {
        return (16 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    status =
        Some((*mech).gss_map_name_to_any.expect("non-null function pointer")).expect("non-null function pointer")(minor_status,
