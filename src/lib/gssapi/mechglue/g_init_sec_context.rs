...
pub use self::mglueP_h::{gss_name_struct, gss_cred_id_struct,
                         ...
                         gss_union_cred_t, gssint_release_internal_name,
                         gssint_get_public_oid, gssint_get_mechanism_cred,
 ...
 */
#[c2rust::src_loc = "36:1"]
unsafe extern "C" fn val_init_sec_ctx_args(mut minor_status: *mut OM_uint32,
...
pub unsafe extern "C" fn gss_init_sec_context(mut minor_status:
 ...
 -> OM_uint32 {
    ...
    let mut union_ctx_id: gss_union_ctx_id_t =
        0 as *mut gss_union_ctx_id_struct;
    let mut selected_mech: gss_OID = 0 as *mut gss_OID_desc_struct;
    let mut mech: gss_mechanism = 0 as *mut gss_config;
    let mut input_cred_handle: gss_cred_id_t = 0 as *mut gss_cred_id_struct;
    status =
        val_init_sec_ctx_args(minor_status, claimant_cred_handle,
                              context_handle, target_name, req_mech_type,
                              req_flags, time_req, input_chan_bindings,
                              input_token, actual_mech_type, output_token,
                              ret_flags, time_rec);
    if status != 0 as libc::c_int as libc::c_uint { return status }
    status =
        gssint_select_mech_type(minor_status, req_mech_type as gss_const_OID,
                                &mut selected_mech);
    if status != 0 as libc::c_int as libc::c_uint { return status }
    union_name = target_name as gss_union_name_t;
    /*
    ...
    if (*context_handle).is_null() {
        ...
        if (*union_ctx_id).internal_ctx_id.is_null() {
            status = (8 as libc::c_ulong as OM_uint32) << 16 as libc::c_int;
            current_block = 15764960595762745900;
        } else { current_block = 14763689060501151050; }
    }
    match current_block {
     ...
     */
            union_cred = claimant_cred_handle as gss_union_cred_t;
            input_cred_handle =
                gssint_get_mechanism_cred(union_cred, selected_mech);
            /*
     * now call the approprate underlying mechanism routine
     */
            status =
                (*mech).gss_init_sec_context.expect("non-null function pointer")(minor_status,
                                                                                 input_cred_handle,
