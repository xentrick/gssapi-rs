...
#[no_mangle]
#[c2rust::src_loc = "31:1"]
pub unsafe extern "C" fn gss_verify_mic(mut minor_status: *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    *minor_status = 0 as libc::c_int as OM_uint32;
    ...
    if message_buffer.is_null() ||
           (token_buffer.is_null() || (*token_buffer).value.is_null() ||
                (*token_buffer).length == 0 as libc::c_int as libc::c_ulong) {
        return (1 as libc::c_ulong as OM_uint32) << 24 as libc::c_int
    }
    /*
    ...
    if !mech.is_null() {
        if (*mech).gss_verify_mic.is_some() {
            status =
                (*mech).gss_verify_mic.expect("non-null function pointer")(minor_status,
                                                                           (*ctx).internal_ctx_id,
                                                                           message_buffer,
                                                                           token_buffer,
                                                                           qop_state);
...
#[no_mangle]
#[c2rust::src_loc = "91:1"]
pub unsafe extern "C" fn gss_verify(mut minor_status: *mut OM_uint32,
                                    mut context_handle: gss_ctx_id_t,
                                    mut message_buffer: gss_buffer_t,
                                    mut token_buffer: gss_buffer_t,
 ...
 -> OM_uint32 {
    return gss_verify_mic(minor_status, context_handle, message_buffer,
                          token_buffer, qop_state as *mut gss_qop_t);
}
