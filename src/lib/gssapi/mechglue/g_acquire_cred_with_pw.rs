 ...
 */
#[c2rust::src_loc = "38:1"]
unsafe extern "C" fn val_acq_cred_pw_args(mut minor_status: *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    if password.is_null() ||
           (*password).length == 0 as libc::c_int as libc::c_ulong ||
...
pub unsafe extern "C" fn gss_acquire_cred_with_password(mut minor_status:
 ...
 -> OM_uint32 {
    ...
    major =
        val_acq_cred_pw_args(minor_status, desired_name, password, time_req,
    ...
    while (i as libc::c_ulong) < (*mechs).count {
        major =
            gss_add_cred_with_password(minor_status, creds as gss_cred_id_t,
                                       desired_name,
                                       &mut *(*mechs).elements.offset(i as
                                                                          isize),
                                       password, cred_usage, time_req,
                                       time_req, 0 as *mut gss_cred_id_t,
                                       0 as *mut gss_OID_set,
                                       &mut initTimeOut, &mut acceptTimeOut);
    ...
    if !actual_mechs.is_null() {
        major =
            gssint_make_public_oid_set(minor_status, (*creds).mechs_array,
...
unsafe extern "C" fn val_add_cred_pw_args(mut minor_status: *mut OM_uint32,
...
pub unsafe extern "C" fn gss_add_cred_with_password(mut minor_status:
                                                        *mut OM_uint32,
                                                    input_cred_handle:
                                                        gss_cred_id_t,
                                                    desired_name: gss_name_t,
 ...
 -> OM_uint32 {
    ...
    status =
        val_add_cred_pw_args(minor_status, input_cred_handle, desired_name,
                             desired_mech, password, cred_usage,
    ...
    if !(status != 0 as libc::c_int as libc::c_uint) {
        ...
        if !(status != 0 as libc::c_int as libc::c_uint) {
            status =
                (*mech).gssspi_acquire_cred_with_password.expect("non-null function pointer")(minor_status,
                                                                                              internal_name,
                                                                                              password,
                                                                                              time_req,
                                                                                              target_mechs,
