 ...
 */
#[c2rust::src_loc = "31:1"]
unsafe extern "C" fn val_get_mic_args(mut minor_status: *mut OM_uint32,
                                      mut context_handle: gss_ctx_id_t,
                                      mut qop_req: gss_qop_t,
                                      mut message_buffer: gss_buffer_t,
                                      mut msg_token: gss_buffer_t)
 -> OM_uint32 {
    /* Initialize outputs. */
    if !minor_status.is_null() {
        *minor_status = 0 as libc::c_int as OM_uint32
    }
    if !msg_token.is_null() {
        (*msg_token).value = 0 as *mut libc::c_void;
        (*msg_token).length = 0 as libc::c_int as size_t
    }
    /* Validate arguments. */
    if minor_status.is_null() {
...
#[no_mangle]
#[c2rust::src_loc = "68:1"]
pub unsafe extern "C" fn gss_get_mic(mut minor_status: *mut OM_uint32,
                                     mut context_handle: gss_ctx_id_t,
                                     mut qop_req: gss_qop_t,
                                     mut message_buffer: gss_buffer_t,
                                     mut msg_token: gss_buffer_t)
 -> OM_uint32 {
    ...
    status =
        val_get_mic_args(minor_status, context_handle, qop_req,
    ...
    if !mech.is_null() {
        if (*mech).gss_get_mic.is_some() {
            status =
                (*mech).gss_get_mic.expect("non-null function pointer")(minor_status,
                                                                        (*ctx).internal_ctx_id,
                                                                        qop_req,
                                                                        message_buffer,
                                                                        msg_token);
...
pub unsafe extern "C" fn gss_sign(mut minor_status: *mut OM_uint32,
                                  mut context_handle: gss_ctx_id_t,
                                  mut qop_req: libc::c_int,
                                  mut message_buffer: gss_buffer_t,
                                  mut msg_token: gss_buffer_t) -> OM_uint32 {
    return gss_get_mic(minor_status, context_handle, qop_req as gss_qop_t,
                       message_buffer, msg_token);
}
