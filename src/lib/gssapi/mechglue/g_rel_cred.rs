...
pub unsafe extern "C" fn gss_release_cred(mut minor_status: *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    *minor_status = 0 as libc::c_int as OM_uint32;
    ...
    if union_cred == 0 as gss_cred_id_t as gss_union_cred_t {
    ...
    *cred_handle = 0 as gss_cred_id_t;
    ...
    while j < (*union_cred).count {
        mech =
            gssint_get_mechanism(&mut *(*union_cred).mechs_array.offset(j as
                                                                            isize)
                                     as *mut gss_OID_desc_struct as
                                     gss_const_OID);
        if !(*(*union_cred).mechs_array.offset(j as isize)).elements.is_null()
           {
            free((*(*union_cred).mechs_array.offset(j as isize)).elements);
        }
        if !mech.is_null() {
            if (*mech).gss_release_cred.is_some() {
                temp_status =
                    (*mech).gss_release_cred.expect("non-null function pointer")(minor_status,
                                                                                 &mut *(*union_cred).cred_array.offset(j
                                                                                                                           as
                                                                                                                           isize));
    ...
    free((*union_cred).cred_array as *mut libc::c_void);
    free((*union_cred).mechs_array as *mut libc::c_void);
