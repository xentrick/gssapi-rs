...
pub use self::mglueP_h::{gss_name_struct, gss_cred_id_struct, gss_mechanism,
                         gss_config, gss_union_name_t, gssint_get_mechanism};
...
pub use self::gssapi_ext_h::{gss_buffer_set_desc_struct, gss_buffer_set_t,
                             ...
                             gss_any};
use self::gssapiP_generic_h::{gssint_mecherrmap_map, generic_gss_release_oid,
...
#[no_mangle]
#[c2rust::src_loc = "30:1"]
pub unsafe extern "C" fn gss_inquire_name(mut minor_status: *mut OM_uint32,
                                          mut name: gss_name_t,
                                          mut name_is_MN: *mut libc::c_int,
                                          mut MN_mech: *mut gss_OID,
                                          mut attrs: *mut gss_buffer_set_t)
 -> OM_uint32 {
    ...
    if !MN_mech.is_null() { *MN_mech = 0 as gss_OID }
    ...
    union_name = name as gss_union_name_t;
    if (*union_name).mech_type.is_null() {
        /* We don't yet support non-mechanism attributes */
        if !name_is_MN.is_null() { *name_is_MN = 0 as libc::c_int }
        *minor_status = 0 as libc::c_int as OM_uint32;
        return 0 as libc::c_int as OM_uint32
    }
    if !name_is_MN.is_null() { *name_is_MN = 1 as libc::c_int }
    if !MN_mech.is_null() {
        status =
            generic_gss_copy_oid(minor_status,
                                 (*union_name).mech_type as
                                     *const gss_OID_desc, MN_mech);
        if status &
    ...
    mech = gssint_get_mechanism((*name).mech_type as gss_const_OID);
    if mech.is_null() {
        gss_release_oid(&mut tmp, MN_mech);
    ...
    status =
        Some((*mech).gss_inquire_name.expect("non-null function pointer")).expect("non-null function pointer")(minor_status,
    ...
    if status != 0 as libc::c_int as libc::c_uint {
        generic_gss_release_oid(&mut tmp, MN_mech);
        *minor_status =
