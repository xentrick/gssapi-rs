...
pub use self::gssapi_ext_h::{gss_buffer_set_desc_struct, gss_buffer_set_t,
                             ...
                             gss_any};
use self::gssapiP_generic_h::{gssint_mecherrmap_map, generic_gss_copy_oid};
...
unsafe extern "C" fn alloc_union_cred(mut minor_status: *mut OM_uint32,
 ...
 -> OM_uint32 {
    ...
    *pcred = 0 as gss_union_cred_t;
        ...
        *minor_status = 12 as libc::c_int as OM_uint32
        ...
        if (*cred).cred_array.is_null() {
            *minor_status = 12 as libc::c_int as OM_uint32
            ...
            let ref mut fresh0 =
                *(*cred).cred_array.offset(0 as libc::c_int as isize);
            *fresh0 = mech_cred;
            status =
    ...
    if status != 0 as libc::c_int as libc::c_uint {
        gss_release_cred(&mut temp_minor_status,
                         &mut cred as *mut gss_union_cred_t as
                             *mut gss_cred_id_t);
    }
    return status;
}
...
#[no_mangle]
#[c2rust::src_loc = "91:1"]
pub unsafe extern "C" fn gss_set_cred_option(mut minor_status: *mut OM_uint32,
                                             mut cred_handle:
                                                 *mut gss_cred_id_t,
 ...
 -> OM_uint32 {
    ...
    let mut status: OM_uint32 = 0;
    let mut mech_status: OM_uint32 = 0;
    let mut mech_minor_status: OM_uint32 = 0;
    ...
    *minor_status = 0 as libc::c_int as OM_uint32;
    ...
    if (*cred_handle).is_null() {
        let mut mech_cred: gss_cred_id_t = 0 as gss_cred_id_t;
        /*
	 ...
	 */
        ...
        status =
            (*mech).gssspi_set_cred_option.expect("non-null function pointer")(minor_status,
                                                                               &mut mech_cred,
                                                                               desired_object,
                                                                               value);
        if status != 0 as libc::c_int as libc::c_uint {
        ...
        if !mech_cred.is_null() {
            status =
                alloc_union_cred(minor_status, mech, mech_cred,
                                 &mut union_cred);
            if status != 0 as libc::c_int as libc::c_uint { return status }
            *cred_handle = union_cred as gss_cred_id_t
        }
    } else {
        ...
        while i < (*union_cred).count {
            mech =
                gssint_get_mechanism(&mut *(*union_cred).mechs_array.offset(i
                                                                                as
                                                                                isize)
            ...
            if mech.is_null() {
                ...
                if !(*mech).gssspi_set_cred_option.is_none() {
                    mech_status =
                        (*mech).gssspi_set_cred_option.expect("non-null function pointer")(&mut mech_minor_status,
                                                                                           &mut *(*union_cred).cred_array.offset(i
                                                                                                                                     as
                                                                                                                                     isize),
                                                                                           desired_object,
                                                                                           value);
...
pub unsafe extern "C" fn gssspi_set_cred_option(mut minor_status:
                                                    *mut OM_uint32,
                                                mut cred: gss_cred_id_t,
                                                desired_object: gss_OID,
                                                value: gss_buffer_t)
 -> OM_uint32 {
    return gss_set_cred_option(minor_status, &mut cred, desired_object,
                               value);
}
