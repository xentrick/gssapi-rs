 ...
 */
#[c2rust::src_loc = "36:1"]
unsafe extern "C" fn val_dsp_name_ext_args(mut minor_status: *mut OM_uint32,
                                           mut input_name: gss_name_t,
                                           mut display_as_name_type: gss_OID,
    ...
    if display_as_name_type.is_null() {
        return (1 as libc::c_ulong as OM_uint32) << 24 as libc::c_int |
                   (3 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    return 0 as libc::c_int as OM_uint32;
}
#[no_mangle]
#[c2rust::src_loc = "72:1"]
pub unsafe extern "C" fn gss_display_name_ext(mut minor_status:
                                                  *mut OM_uint32,
                                              mut input_name: gss_name_t,
    ...
    if !(*union_name).mech_type.is_null() {
        mech = gssint_get_mechanism((*union_name).mech_type as gss_const_OID);
        if mech.is_null() {
            ...
            if (*mech).gss_display_name.is_some() &&
                   ...
                   ((*display_as_name_type).length ==
                        (*(*union_name).name_type).length &&
                        memcmp((*display_as_name_type).elements,
                               (*(*union_name).name_type).elements,
                               (*display_as_name_type).length as
                                   libc::c_ulong) == 0 as libc::c_int) {
                status =
                    Some((*mech).gss_display_name.expect("non-null function pointer")).expect("non-null function pointer")(minor_status,
            ...
            status =
                Some((*mech).gss_display_name_ext.expect("non-null function pointer")).expect("non-null function pointer")(minor_status,
                                                                                                                           (*union_name).mech_name,
                                                                                                                           display_as_name_type,
                                                                                                                           output_name_buffer);
            if status != 0 as libc::c_int as libc::c_uint {
    ...
    if (*union_name).name_type.is_null() ||
           !((*display_as_name_type).length ==
                 ...
                 memcmp((*display_as_name_type).elements,
                        (*(*union_name).name_type).elements,
                        (*display_as_name_type).length as libc::c_ulong) ==
                     0 as libc::c_int) {
        return (16 as libc::c_ulong as OM_uint32) << 16 as libc::c_int
    }
    (*output_name_buffer).value =
        malloc((*(*union_name).external_name).length.wrapping_add(1 as
                                                                      libc::c_int
                                                                      as
                                                                      libc::c_ulong));
    ...
    *((*output_name_buffer).value as
          *mut libc::c_char).offset((*output_name_buffer).length as isize) =
