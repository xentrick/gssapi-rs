...
#[no_mangle]
#[c2rust::src_loc = "37:1"]
pub unsafe extern "C" fn gss_complete_auth_token(mut minor_status:
                                                     *mut OM_uint32,
    ...
    *minor_status = 0 as libc::c_int as OM_uint32;
    ...
    if !mech.is_null() {
        if (*mech).gss_complete_auth_token.is_some() {
            status =
                (*mech).gss_complete_auth_token.expect("non-null function pointer")(minor_status,
                                                                                    (*ctx).internal_ctx_id,
                                                                                    input_message_buffer);
        ...
        } else { status = 0 as libc::c_int as OM_uint32 }
    } else { status = (1 as libc::c_ulong as OM_uint32) << 16 as libc::c_int }
    return status;
}
