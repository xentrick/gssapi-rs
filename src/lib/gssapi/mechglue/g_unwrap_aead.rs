 ...
 */
#[c2rust::src_loc = "31:1"]
unsafe extern "C" fn val_unwrap_aead_args(mut minor_status: *mut OM_uint32,
...
#[c2rust::src_loc = "64:1"]
unsafe extern "C" fn gssint_unwrap_aead_iov_shim(mut mech: gss_mechanism,
                                                 mut minor_status:
                                                     *mut OM_uint32,
                                                 mut context_handle:
                                                     gss_ctx_id_t,
 ...
 -> OM_uint32 {
    ...
    let mut iov: [gss_iov_buffer_desc; 3] =
    ...
    iov[i as usize].type_0 = 10 as libc::c_int as OM_uint32;
    ...
    if (*mech).gss_unwrap_iov.is_some() {
        ...
        __assert_fail(b"mech->gss_unwrap_iov\x00" as *const u8 as
                          *const libc::c_char,
                      b"g_unwrap_aead.c\x00" as *const u8 as
                          *const libc::c_char,
                      93 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 142],
                                                &[libc::c_char; 142]>(b"OM_uint32 gssint_unwrap_aead_iov_shim(gss_mechanism, OM_uint32 *, gss_ctx_id_t, gss_buffer_t, gss_buffer_t, gss_buffer_t, int *, gss_qop_t *)\x00")).as_ptr());
    }
    status =
        (*mech).gss_unwrap_iov.expect("non-null function pointer")(minor_status,
                                                                   context_handle,
                                                                   conf_state,
                                                                   qop_state,
                                                                   iov.as_mut_ptr(),
    ...
    if status == 0 as libc::c_int as libc::c_uint {
        *output_payload_buffer = iov[(i - 1 as libc::c_int) as usize].buffer
    } else {
        let mut minor: OM_uint32 = 0;
        *minor_status =
            gssint_mecherrmap_map(*minor_status, &mut (*mech).mech_type);
        if iov[(i - 1 as libc::c_int) as usize].type_0 &
               0x20000 as libc::c_int as libc::c_uint != 0 {
            gss_release_buffer(&mut minor,
                               &mut (*iov.as_mut_ptr().offset((i -
                                                                   1 as
                                                                       libc::c_int)
                                                                  as
                                                                  isize)).buffer);
            iov[(i - 1 as libc::c_int) as usize].type_0 &=
...
pub unsafe extern "C" fn gssint_unwrap_aead(mut mech: gss_mechanism,
 ...
 -> OM_uint32 {
    ...
    if !mech.is_null() {
        ...
        __assert_fail(b"mech != NULL\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 139],
                                                &[libc::c_char; 139]>(b"OM_uint32 gssint_unwrap_aead(gss_mechanism, OM_uint32 *, gss_union_ctx_id_t, gss_buffer_t, gss_buffer_t, gss_buffer_t, int *, gss_qop_t *)\x00")).as_ptr());
    }
    if !ctx.is_null() {
    } else {
    ...
    if (*mech).gss_unwrap_aead.is_some() {
        status =
            (*mech).gss_unwrap_aead.expect("non-null function pointer")(minor_status,
                                                                        (*ctx).internal_ctx_id,
                                                                        input_message_buffer,
                                                                        input_assoc_buffer,
                                                                        output_payload_buffer,
        ...
        status =
            gssint_unwrap_aead_iov_shim(mech, minor_status,
                                        (*ctx).internal_ctx_id,
                                        input_message_buffer,
                                        input_assoc_buffer,
...
pub unsafe extern "C" fn gss_unwrap_aead(mut minor_status: *mut OM_uint32,
                                         mut context_handle: gss_ctx_id_t,
                                         mut input_message_buffer:
                                             gss_buffer_t,
 ...
 -> OM_uint32 {
    ...
    status =
        val_unwrap_aead_args(minor_status, context_handle,
