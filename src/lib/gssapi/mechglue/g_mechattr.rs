...
pub mod gssapiP_generic_h {
    use super::gssapi_h::{OM_uint32, gss_OID_desc, gss_OID_set_desc_struct,
                          gss_OID_set, gss_OID_set_desc, gss_const_OID,
 ...
 */
        ...
        pub fn generic_gss_display_mech_attr(minor_status: *mut OM_uint32,
...
pub use self::gssapi_ext_h::{gss_iov_buffer_desc, gss_iov_buffer_desc_struct,
                             ...
                             gss_any};
use self::gssapiP_generic_h::{generic_gss_test_oid_set_member,
                              gss_ma_known_attrs, generic_gss_copy_oid_set,
 ...
 */
#[c2rust::src_loc = "29:1"]
unsafe extern "C" fn testMechAttr(mut attr: gss_const_OID,
                                  mut against: gss_const_OID_set)
 -> libc::c_int {
    let mut present: libc::c_int = 0 as libc::c_int;
    let mut minor: OM_uint32 = 0;
    if generic_gss_test_oid_set_member(&mut minor, attr,
                                       against as gss_OID_set, &mut present) &
 ...
 */
#[c2rust::src_loc = "48:1"]
unsafe extern "C" fn testMechAttrsOffered(mut desired: gss_const_OID_set,
                                          mut except: gss_const_OID_set,
                                          mut available: gss_const_OID_set)
 -> libc::c_int {
    let mut i: size_t = 0;
    if !desired.is_null() {
        i = 0 as libc::c_int as size_t;
        while i < (*desired).count {
            if testMechAttr(&mut *(*desired).elements.offset(i as isize) as
                                *mut gss_OID_desc_struct as gss_const_OID,
                            available) == 0 {
                return 0 as libc::c_int
            }
            i = i.wrapping_add(1)
    ...
    if !except.is_null() {
        ...
        while i < (*except).count {
            if testMechAttr(&mut *(*except).elements.offset(i as isize) as
                                *mut gss_OID_desc_struct as gss_const_OID,
                            available) != 0 {
                return 0 as libc::c_int
            }
 ...
 */
#[c2rust::src_loc = "75:1"]
unsafe extern "C" fn testMechAttrsKnown(mut critical: gss_const_OID_set,
                                        mut known: gss_const_OID_set)
 -> libc::c_int {
    let mut i: size_t = 0;
    if !critical.is_null() {
        i = 0 as libc::c_int as size_t;
        while i < (*critical).count {
            if testMechAttr(&mut *(*critical).elements.offset(i as isize) as
                                *mut gss_OID_desc_struct as gss_const_OID,
                            known) == 0 {
                return 0 as libc::c_int
            }
...
#[no_mangle]
#[c2rust::src_loc = "91:1"]
pub unsafe extern "C" fn gss_indicate_mechs_by_attrs(mut minor:
                                                         *mut OM_uint32,
                                                     mut desired_mech_attrs:
                                                         gss_const_OID_set,
                                                     mut except_mech_attrs:
                                                         gss_const_OID_set,
                                                     mut critical_mech_attrs:
                                                         gss_const_OID_set,
                                                     mut mechs:
                                                         *mut gss_OID_set)
 -> OM_uint32 {
    ...
    let mut allMechs: gss_OID_set = 0 as gss_OID_set;
    let mut i: size_t = 0;
    if !minor.is_null() { *minor = 0 as libc::c_int as OM_uint32 }
    if !mechs.is_null() { *mechs = 0 as gss_OID_set }
    if minor.is_null() || mechs.is_null() {
        return (2 as libc::c_ulong as OM_uint32) << 24 as libc::c_int
    }
    status = gss_indicate_mechs(minor, &mut allMechs);
    if !(status &
             ((0o377 as libc::c_ulong as OM_uint32) << 24 as libc::c_int |
                  (0o377 as libc::c_ulong as OM_uint32) << 16 as libc::c_int)
             != 0) {
        ...
        if !(status &
            ...
            loop  {
                ...
                let mut supportedAttrs: gss_OID_set = 0 as gss_OID_set;
                let mut knownAttrs: gss_OID_set = 0 as gss_OID_set;
                status =
                    gss_inquire_attrs_for_mech(minor,
                                               &mut *(*allMechs).elements.offset(i
                                                                                     as
                                                                                     isize)
                ...
                if !(status &
                    ...
                    if testMechAttrsOffered(desired_mech_attrs,
                                            ...
                                            supportedAttrs as
                                                gss_const_OID_set) != 0 &&
                           testMechAttrsKnown(critical_mech_attrs,
                                              knownAttrs as gss_const_OID_set)
                               != 0 {
                        status =
                        ...
                        if status &
                               ((0o377 as libc::c_ulong as OM_uint32) <<
                                    ...
                                    (0o377 as libc::c_ulong as OM_uint32) <<
                                        16 as libc::c_int) != 0 {
                            gss_release_oid_set(&mut tmpMinor,
                                                &mut supportedAttrs);
                            gss_release_oid_set(&mut tmpMinor,
                                                &mut knownAttrs);
                    ...
                    gss_release_oid_set(&mut tmpMinor, &mut supportedAttrs);
                    gss_release_oid_set(&mut tmpMinor, &mut knownAttrs);
                }
                i = i.wrapping_add(1)
            }
            match current_block {
    ...
    gss_release_oid_set(&mut tmpMinor, &mut allMechs);
    return status;
}
#[no_mangle]
#[c2rust::src_loc = "154:1"]
pub unsafe extern "C" fn gss_inquire_attrs_for_mech(mut minor: *mut OM_uint32,
                                                    mut mech_oid:
                                                        gss_const_OID,
                                                    mut mech_attrs:
                                                        *mut gss_OID_set,
 ...
 -> OM_uint32 {
    ...
    if !minor.is_null() { *minor = 0 as libc::c_int as OM_uint32 }
    if !mech_attrs.is_null() { *mech_attrs = 0 as gss_OID_set }
    if !known_mech_attrs.is_null() { *known_mech_attrs = 0 as gss_OID_set }
    if minor.is_null() {
        return (2 as libc::c_ulong as OM_uint32) << 24 as libc::c_int
    }
    status = gssint_select_mech_type(minor, mech_oid, &mut selected_mech);
    ...
    status =
        (*mech).gss_inquire_attrs_for_mech.expect("non-null function pointer")(minor,
                                                                               public_mech
                                                                                   as
                                                                                   gss_const_OID,
                                                                               mech_attrs,
    ...
    if !known_mech_attrs.is_null() && (*known_mech_attrs).is_null() {
        status =
            generic_gss_copy_oid_set(minor,
                                     gss_ma_known_attrs as
...
#[no_mangle]
#[c2rust::src_loc = "212:1"]
pub unsafe extern "C" fn gss_display_mech_attr(mut minor: *mut OM_uint32,
