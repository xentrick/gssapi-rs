...
#[no_mangle]
#[c2rust::src_loc = "31:1"]
pub unsafe extern "C" fn krb5_kt_remove_entry(mut context: krb5_context,
                                              mut id: krb5_keytab,
                                              mut entry:
                                                  *mut krb5_keytab_entry)
 -> krb5_error_code {
    if (*(*id).ops).remove.is_some() {
        return Some((*(*id).ops).remove.expect("non-null function pointer")).expect("non-null function pointer")(context,
                                                                                                                 id,
                                                                                                                 entry)
    } else { return -(1765328201 as libc::c_long) as krb5_error_code };
}
/* LEAN_CLIENT  */
