...
#[no_mangle]
#[c2rust::src_loc = "31:1"]
pub unsafe extern "C" fn krb5_kt_add_entry(mut context: krb5_context,
                                           mut id: krb5_keytab,
                                           mut entry: *mut krb5_keytab_entry)
 -> krb5_error_code {
    if (*(*id).ops).add.is_some() {
        return Some((*(*id).ops).add.expect("non-null function pointer")).expect("non-null function pointer")(context,
                                                                                                              id,
                                                                                                              entry)
