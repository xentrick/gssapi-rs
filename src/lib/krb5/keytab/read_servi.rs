...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_kt_close, krb5_kt_get_entry, krb5_copy_keyblock,
                       krb5_kt_resolve, krb5_kt_default_name,
                       krb5_kt_free_entry};
...
use self::string_h::{strncpy, memset};
/* *
...
#[no_mangle]
#[c2rust::src_loc = "48:1"]
pub unsafe extern "C" fn krb5_kt_read_service_key(mut context: krb5_context,
                                                  mut keyprocarg:
                                                      krb5_pointer,
                                                  mut principal:
                                                      krb5_principal,
 ...
 -> krb5_error_code {
    let mut kerror: krb5_error_code =
        0 as libc::c_int; /* + 1 for NULL termination */
    let mut keytabname: [libc::c_char; 1101] = [0; 1101];
     ...
     */
    if keyprocarg.is_null() {
        kerror =
            krb5_kt_default_name(context, keytabname.as_mut_ptr(),
                                 (::std::mem::size_of::<[libc::c_char; 1101]>()
                                      as
                                      libc::c_ulong).wrapping_sub(1 as
                                                                      ...
                                                                      libc::c_ulong)
                                     as libc::c_int);
        if kerror != 0 as libc::c_int { return kerror }
    } else {
        memset(keytabname.as_mut_ptr() as *mut libc::c_void, 0 as libc::c_int,
               ::std::mem::size_of::<[libc::c_char; 1101]>() as
                   libc::c_ulong);
        strncpy(keytabname.as_mut_ptr(), keyprocarg as *mut libc::c_char,
                (::std::mem::size_of::<[libc::c_char; 1101]>() as
                     libc::c_ulong).wrapping_sub(1 as libc::c_int as
                                                     libc::c_ulong));
    }
    kerror = krb5_kt_resolve(context, keytabname.as_mut_ptr(), &mut id);
    if kerror != 0 { return kerror }
    kerror =
        krb5_kt_get_entry(context, id, principal as krb5_const_principal, vno,
                          enctype, &mut entry);
    krb5_kt_close(context, id);
    if kerror != 0 { return kerror }
    krb5_copy_keyblock(context, &mut entry.key, key);
