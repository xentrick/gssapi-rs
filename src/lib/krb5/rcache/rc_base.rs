...
#[c2rust::header_src = "/usr/lib/clang/9.0.1/include/stddef.h:13"]
pub mod stddef_h {
    #[c2rust::src_loc = "46:1"]
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/krb5/rcache/rc-int.h:14"]
pub mod rc_int_h {
    #[derive(Copy, Clone)]
    #[repr(C)]
    #[c2rust::src_loc = "41:8"]
    ...
    use super::krb5_h::{krb5_magic, krb5_error_code, krb5_context, krb5_data};
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "48:26"]
        pub static k5_rc_dfl_ops: krb5_rc_ops;
        #[no_mangle]
        #[c2rust::src_loc = "49:26"]
        pub static k5_rc_file2_ops: krb5_rc_ops;
        #[no_mangle]
        #[c2rust::src_loc = "50:26"]
        pub static k5_rc_none_ops: krb5_rc_ops;
    }
    /* RC_INT_H */
}
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/krb5/os/os-proto.h:16"]
pub mod os_proto_h {
    use super::k5_int_h::_krb5_context;
...
pub use self::krb5_h::{krb5_int32, krb5_boolean, krb5_kvno, krb5_enctype,
                       ...
                       krb5_rcache, _profile_t, krb5_c_crypto_length};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         CANONHOST_FALLBACK, CANONHOST_TRUE, CANONHOST_FALSE,
                         krb5_preauth_context, kdb5_dal_handle,
                         _krb5_os_context, k5alloc, k5calloc, make_data,
...
pub use self::rc_int_h::{krb5_rc_st, krb5_rc_ops, k5_rc_dfl_ops,
                         k5_rc_file2_ops, k5_rc_none_ops};
use self::stdlib_h::{calloc, free, secure_getenv};
use self::stdio_h::asprintf;
use self::string_h::{strchr, strdup, strncmp};
...
pub struct typelist {
    pub ops: *const krb5_rc_ops,
    pub next: *mut typelist,
}
#[c2rust::src_loc = "22:24"]
static mut none: typelist =
    unsafe {
        {
            let mut init =
                typelist{ops: &k5_rc_none_ops as *const krb5_rc_ops,
                         next: 0 as *const typelist as *mut typelist,};
            init
        }
    };
#[c2rust::src_loc = "23:24"]
static mut file2: typelist =
    unsafe {
        {
            let mut init =
                typelist{ops: &k5_rc_file2_ops as *const krb5_rc_ops,
                         next: &none as *const typelist as *mut typelist,};
            init
        }
    };
#[c2rust::src_loc = "24:24"]
static mut dfl: typelist =
    unsafe {
        {
            let mut init =
                typelist{ops: &k5_rc_dfl_ops as *const krb5_rc_ops,
                         next: &file2 as *const typelist as *mut typelist,};
            init
        }
    };
#[c2rust::src_loc = "25:25"]
static mut typehead: *mut typelist =
    unsafe { &dfl as *const typelist as *mut typelist };
#[no_mangle]
#[c2rust::src_loc = "27:1"]
pub unsafe extern "C" fn k5_rc_default(mut context: krb5_context,
                                       mut rc_out: *mut krb5_rcache)
 -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut val: *const libc::c_char = 0 as *const libc::c_char;
    let mut profstr: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut rcname: *mut libc::c_char = 0 as *mut libc::c_char;
    *rc_out = 0 as krb5_rcache;
    /* If KRB5RCACHENAME is set in the environment, resolve it. */
    val =
        secure_getenv(b"KRB5RCACHENAME\x00" as *const u8 as
                          *const libc::c_char);
    if !val.is_null() { return k5_rc_resolve(context, val, rc_out) }
    /* If KRB5RCACHETYPE is set in the environment, resolve it with an empty
     * residual (primarily to support KRB5RCACHETYPE=none). */
    val =
        secure_getenv(b"KRB5RCACHETYPE\x00" as *const u8 as
                          *const libc::c_char);
    if !val.is_null() {
        if asprintf(&mut rcname as *mut *mut libc::c_char,
                    b"%s:\x00" as *const u8 as *const libc::c_char, val) <
               0 as libc::c_int {
            return 12 as libc::c_int
        }
        ret = k5_rc_resolve(context, rcname, rc_out);
        free(rcname as *mut libc::c_void);
        return ret
    }
    ...
    return k5_rc_resolve(context,
                         b"dfl:\x00" as *const u8 as *const libc::c_char,
                         rc_out);
}
#[no_mangle]
#[c2rust::src_loc = "69:1"]
pub unsafe extern "C" fn k5_rc_resolve(mut context: krb5_context,
                                       mut name: *const libc::c_char,
                                       mut rc_out: *mut krb5_rcache)
 -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut t: *mut typelist = 0 as *mut typelist;
    ...
    *rc_out = 0 as krb5_rcache;
    ...
    if sep.is_null() {
        return -(1765328218 as libc::c_long) as krb5_error_code
    }
    len = sep.wrapping_offset_from(name) as libc::c_long as size_t;
    t = typehead;
    while !t.is_null() {
        if strncmp((*(*t).ops).type_0, name, len) == 0 as libc::c_int &&
               *(*(*t).ops).type_0.offset(len as isize) as libc::c_int ==
                   '\u{0}' as i32 {
            break ;
        }
        t = (*t).next
    }
    if t.is_null() { return -(1765328223 as libc::c_long) as krb5_error_code }
    rc =
        k5alloc(::std::mem::size_of::<krb5_rc_st>() as libc::c_ulong,
                &mut ret) as krb5_rcache;
    if !rc.is_null() {
        (*rc).name = strdup(name);
        if (*rc).name.is_null() {
            ...
            ret =
                (*(*t).ops).resolve.expect("non-null function pointer")(context,
                                                                        sep.offset(1
                                                                                       as
                                                                                       libc::c_int
                                                                                       as
            ...
            if !(ret != 0) {
                (*rc).ops = (*t).ops;
                (*rc).magic = -(1760647381 as libc::c_long) as krb5_magic;
                *rc_out = rc;
    ...
    if !rc.is_null() {
        free((*rc).name as *mut libc::c_void);
...
pub unsafe extern "C" fn k5_rc_close(mut context: krb5_context,
                                     mut rc: krb5_rcache) {
    (*(*rc).ops).close.expect("non-null function pointer")(context,
                                                           (*rc).data);
    free((*rc).name as *mut libc::c_void);
    free(rc as *mut libc::c_void);
}
...
pub unsafe extern "C" fn k5_rc_store(mut context: krb5_context,
                                     mut rc: krb5_rcache,
                                     mut authenticator: *const krb5_enc_data)
 -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut tag: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    ret = k5_rc_tag_from_ciphertext(context, authenticator, &mut tag);
    if ret != 0 { return ret }
    return (*(*rc).ops).store.expect("non-null function pointer")(context,
                                                                  (*rc).data,
                                                                  &mut tag);
}
#[no_mangle]
#[c2rust::src_loc = "138:1"]
pub unsafe extern "C" fn k5_rc_get_name(mut context: krb5_context,
                                        mut rc: krb5_rcache)
 -> *const libc::c_char {
    return (*rc).name;
}
...
pub unsafe extern "C" fn k5_rc_tag_from_ciphertext(mut context: krb5_context,
                                                   mut enc:
                                                       *const krb5_enc_data,
                                                   mut tag_out:
                                                       *mut krb5_data)
 -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut cdata: *const krb5_data = &(*enc).ciphertext;
    let mut len: libc::c_uint = 0;
    *tag_out = empty_data();
    ret =
        krb5_c_crypto_length(context, (*enc).enctype, 6 as libc::c_int,
                             &mut len);
    if ret != 0 { return ret }
    if (*cdata).length < len { return 22 as libc::c_int }
    *tag_out =
        make_data((*cdata).data.offset((*cdata).length as
