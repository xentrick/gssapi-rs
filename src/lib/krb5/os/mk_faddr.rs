...
#[no_mangle]
#[c2rust::src_loc = "37:1"]
pub unsafe extern "C" fn krb5_make_fulladdr(mut context: krb5_context,
                                            mut kaddr: *mut krb5_address,
                                            mut kport: *mut krb5_address,
                                            mut raddr: *mut krb5_address)
 -> krb5_error_code {
    let mut marshal: *mut krb5_octet = 0 as *mut krb5_octet;
    let mut tmp32: krb5_int32 = 0;
    let mut tmp16: krb5_int16 = 0;
    if kaddr.is_null() || kport.is_null() { return 22 as libc::c_int }
    (*raddr).length =
        ((*kaddr).length.wrapping_add((*kport).length) as
            ...
            as libc::c_uint;
    (*raddr).contents =
        malloc((*raddr).length as libc::c_ulong) as *mut krb5_octet;
    if (*raddr).contents.is_null() { return 12 as libc::c_int }
    (*raddr).addrtype = 0x100 as libc::c_int;
    marshal = (*raddr).contents;
    tmp16 = (*kaddr).addrtype as krb5_int16;
    let fresh0 = marshal;
    marshal = marshal.offset(1);
    *fresh0 = 0 as libc::c_int as krb5_octet;
    let fresh1 = marshal;
    marshal = marshal.offset(1);
    *fresh1 = 0 as libc::c_int as krb5_octet;
    store_16_le(tmp16 as libc::c_uint, marshal as *mut libc::c_void);
    marshal = marshal.offset(2 as libc::c_int as isize);
    tmp32 = (*kaddr).length as krb5_int32;
    store_32_le(tmp32 as libc::c_uint, marshal as *mut libc::c_void);
    marshal = marshal.offset(4 as libc::c_int as isize);
    memcpy(marshal as *mut libc::c_void,
           (*kaddr).contents as *const libc::c_void,
           (*kaddr).length as libc::c_ulong);
    marshal = marshal.offset((*kaddr).length as isize);
    tmp16 = (*kport).addrtype as krb5_int16;
    let fresh2 = marshal;
    marshal = marshal.offset(1);
    *fresh2 = 0 as libc::c_int as krb5_octet;
    let fresh3 = marshal;
    marshal = marshal.offset(1);
    *fresh3 = 0 as libc::c_int as krb5_octet;
    store_16_le(tmp16 as libc::c_uint, marshal as *mut libc::c_void);
    marshal = marshal.offset(2 as libc::c_int as isize);
    tmp32 = (*kport).length as krb5_int32;
    store_32_le(tmp32 as libc::c_uint, marshal as *mut libc::c_void);
    marshal = marshal.offset(4 as libc::c_int as isize);
    memcpy(marshal as *mut libc::c_void,
           (*kport).contents as *const libc::c_void,
           (*kport).length as libc::c_ulong);
    marshal = marshal.offset((*kport).length as isize);
