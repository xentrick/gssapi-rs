...
pub use self::krb5_h::{krb5_int32, krb5_boolean, krb5_enctype, krb5_flags,
                       ...
                       _krb5_trace_info, krb5_prompt_type, krb5_prompt,
                       _krb5_prompt, _profile_t, krb5_prompter_posix};
...
use self::string_h::{explicit_bzero, strncmp};
...
#[no_mangle]
#[c2rust::src_loc = "40:1"]
pub unsafe extern "C" fn krb5_read_password(mut context: krb5_context,
                                            mut prompt: *const libc::c_char,
                                            mut prompt2: *const libc::c_char,
                                            mut return_pwd: *mut libc::c_char,
                                            mut size_return:
                                                *mut libc::c_uint)
 -> krb5_error_code {
    let mut reply_data: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut verify_data: krb5_data = empty_data();
    let mut k5prompt: krb5_prompt =
        krb5_prompt{prompt: 0 as *mut libc::c_char,
                    hidden: 0,
                    reply: 0 as *mut krb5_data,};
    let mut vprompt: krb5_prompt =
        krb5_prompt{prompt: 0 as *mut libc::c_char,
                    hidden: 0,
                    reply: 0 as *mut krb5_data,};
    let mut retval: krb5_error_code = 0;
    /* *size_return is the space available in the return buffer on input. */
    reply_data = make_data(return_pwd as *mut libc::c_void, *size_return);
    k5prompt.prompt = prompt as *mut libc::c_char;
    k5prompt.hidden = 1 as libc::c_int;
    k5prompt.reply = &mut reply_data;
    retval =
        krb5_prompter_posix(0 as krb5_context, 0 as *mut libc::c_void,
                            0 as *const libc::c_char,
                            0 as *const libc::c_char, 1 as libc::c_int,
                            &mut k5prompt);
    if !(retval != 0 || prompt2.is_null()) {
        retval = alloc_data(&mut verify_data, *size_return);
        if !(retval != 0) {
            vprompt.prompt = prompt2 as *mut libc::c_char;
            vprompt.hidden = 1 as libc::c_int;
            vprompt.reply = &mut verify_data;
            retval =
                krb5_prompter_posix(0 as krb5_context, 0 as *mut libc::c_void,
                                    0 as *const libc::c_char,
                                    0 as *const libc::c_char,
                                    1 as libc::c_int, &mut vprompt);
            if !(retval != 0) {
                if strncmp(return_pwd, verify_data.data,
                           *size_return as libc::c_ulong) != 0 as libc::c_int
                   {
                    retval = -(1765328253 as libc::c_long) as krb5_error_code
                }
            }
        }
    }
    zapfree(verify_data.data as *mut libc::c_void,
            verify_data.length as size_t);
    if retval == 0 {
        *size_return = (*k5prompt.reply).length
    } else {
        explicit_bzero(return_pwd as *mut libc::c_void,
                       *size_return as size_t);
    }
    return retval;
}
