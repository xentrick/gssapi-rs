...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_addrtype,
                       ...
                       _profile_t, _krb5_auth_context, krb5_auth_con_setaddrs,
...
#[c2rust::src_loc = "34:8"]
pub struct addrpair {
    pub addr: krb5_address,
    pub port: krb5_address,
}
#[c2rust::src_loc = "43:1"]
unsafe extern "C" fn cvtaddr(mut a: *mut sockaddr_storage,
                             mut ap: *mut addrpair) -> *mut libc::c_void {
    match (*ss2sa(a)).sa_family as libc::c_int {
        2 => {
            (*ap).port.contents =
                &mut (*(ss2sin as
                            unsafe extern "C" fn(_: *mut sockaddr_storage)
                                -> *mut sockaddr_in)(a)).sin_port as
                    *mut in_port_t as *mut krb5_octet;
            (*ap).port.length =
                ::std::mem::size_of::<in_port_t>() as libc::c_ulong as
                    libc::c_uint;
            (*ap).port.addrtype = 0x101 as libc::c_int;
            (*ap).addr.contents =
                &mut (*(ss2sin as
                            unsafe extern "C" fn(_: *mut sockaddr_storage)
            ...
            (*ap).addr.length =
                ::std::mem::size_of::<in_addr>() as libc::c_ulong as
                    libc::c_uint;
            (*ap).addr.addrtype = 0x2 as libc::c_int;
            return a as *mut libc::c_void
        }
        10 => {
            (*ap).port.contents =
                &mut (*(ss2sin6 as
                            unsafe extern "C" fn(_: *mut sockaddr_storage)
                                -> *mut sockaddr_in6)(a)).sin6_port as
                    *mut in_port_t as *mut krb5_octet;
            (*ap).port.length =
                ::std::mem::size_of::<in_port_t>() as libc::c_ulong as
                    libc::c_uint;
            ...
            if ({
                    let mut __a: *const in6_addr =
                        &mut (*(ss2sin6 as
                                    unsafe extern "C" fn(_:
                                        ...
                                        -> *mut sockaddr_in6)(a)).sin6_addr as
                ...
                (*ap).addr.length = 4 as libc::c_int as libc::c_uint
            } else {
                (*ap).addr.contents =
                    &mut (*(ss2sin6 as
                                unsafe extern "C" fn(_: *mut sockaddr_storage)
                ...
                (*ap).addr.length =
                    ::std::mem::size_of::<in6_addr>() as libc::c_ulong as
                        libc::c_uint;
                (*ap).addr.addrtype = 0x18 as libc::c_int
            }
            return a as *mut libc::c_void
        }
...
#[no_mangle]
#[c2rust::src_loc = "64:1"]
pub unsafe extern "C" fn krb5_auth_con_genaddrs(mut context: krb5_context,
                                                mut auth_context:
                                                    krb5_auth_context,
                                                mut infd: libc::c_int,
 ...
 -> krb5_error_code {
    ...
    let mut laddr: *mut krb5_address = 0 as *mut krb5_address;
    let mut lport: *mut krb5_address = 0 as *mut krb5_address;
    let mut raddr: *mut krb5_address = 0 as *mut krb5_address;
    let mut rport: *mut krb5_address = 0 as *mut krb5_address;
    let mut fd: libc::c_int = infd;
    let mut laddrs: addrpair =
        addrpair{addr:
        ...
        addrpair{addr:
                 ...
                 port:
                     krb5_address{magic: 0,
                                  ...
                                  length: 0,
                                  contents: 0 as *mut krb5_octet,},};
    let mut lsaddr: sockaddr_storage =
        sockaddr_storage{ss_family: 0,
                         __ss_padding: [0; 118],
                         __ss_align: 0,};
    let mut rsaddr: sockaddr_storage =
        sockaddr_storage{ss_family: 0,
                         __ss_padding: [0; 118],
                         __ss_align: 0,};
    let mut ssize: socklen_t = 0;
    ssize =
        ::std::mem::size_of::<sockaddr_storage>() as libc::c_ulong as
            socklen_t;
    if flags & 0x4 as libc::c_int != 0 || flags & 0x1 as libc::c_int != 0 {
        retval =
            getsockname(fd, __SOCKADDR_ARG{__sockaddr__: ss2sa(&mut lsaddr),},
                        &mut ssize);
        if retval != 0 { return retval }
        if !cvtaddr(&mut lsaddr, &mut laddrs).is_null() {
            laddr = &mut laddrs.addr;
            if flags & 0x4 as libc::c_int != 0 {
                lport = &mut laddrs.port
            } else { lport = 0 as *mut krb5_address }
        } else { return -(1765328170 as libc::c_long) as krb5_error_code }
    } else { laddr = 0 as *mut krb5_address; lport = 0 as *mut krb5_address }
    ssize =
        ::std::mem::size_of::<sockaddr_storage>() as libc::c_ulong as
            socklen_t;
    if flags & 0x8 as libc::c_int != 0 || flags & 0x2 as libc::c_int != 0 {
        retval =
            getpeername(fd, __SOCKADDR_ARG{__sockaddr__: ss2sa(&mut rsaddr),},
                        &mut ssize);
        if retval != 0 { return *__errno_location() }
        if !cvtaddr(&mut rsaddr, &mut raddrs).is_null() {
            raddr = &mut raddrs.addr;
            if flags & 0x8 as libc::c_int != 0 {
                rport = &mut raddrs.port
            } else { rport = 0 as *mut krb5_address }
        } else { return -(1765328170 as libc::c_long) as krb5_error_code }
    } else { raddr = 0 as *mut krb5_address; rport = 0 as *mut krb5_address }
    retval = krb5_auth_con_setaddrs(context, auth_context, laddr, raddr);
    if retval == 0 {
