...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _krb5_os_context, krb5_os_context, ts_within,
...
#[no_mangle]
#[c2rust::src_loc = "31:1"]
pub unsafe extern "C" fn krb5_timeofday(mut context: krb5_context,
                                        mut timeret: *mut krb5_timestamp)
 -> krb5_error_code {
    ...
    if (*os_ctx).os_flags & 2 as libc::c_int != 0 {
        *timeret = (*os_ctx).time_offset;
        return 0 as libc::c_int
    }
    tval = time(0 as *mut time_t);
    if tval == -(1 as libc::c_int) as time_t { return *__errno_location() }
    if (*os_ctx).os_flags & 1 as libc::c_int != 0 {
        tval += (*os_ctx).time_offset as libc::c_long
    }
    *timeret = tval as krb5_timestamp;
    return 0 as libc::c_int;
}
#[no_mangle]
#[c2rust::src_loc = "54:1"]
pub unsafe extern "C" fn krb5_check_clockskew(mut context: krb5_context,
                                              mut date: krb5_timestamp)
 -> krb5_error_code {
    let mut currenttime: krb5_timestamp = 0;
    let mut retval: krb5_error_code = 0;
    retval = krb5_timeofday(context, &mut currenttime);
    if retval != 0 { return retval }
    if ts_within(date, currenttime, (*context).clockskew) == 0 {
        return -(1765328347 as libc::c_long) as krb5_error_code
    }
    return 0 as libc::c_int;
}
