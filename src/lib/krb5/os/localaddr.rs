...
#[c2rust::header_src = "/usr/include/ifaddrs.h:197"]
pub mod ifaddrs_h {
    #[derive(Copy, Clone)]
    #[repr(C)]
    #[c2rust::src_loc = "29:8"]
...
#[c2rust::header_src = "/usr/include/net/if.h:36"]
pub mod if_h {
    #[c2rust::src_loc = "44:5"]
...
#[c2rust::header_src = "/usr/include/ctype.h:47"]
pub mod ctype_h {
    #[c2rust::src_loc = "53:3"]
...
#[c2rust::header_src = "/usr/include/errno.h:36"]
pub mod errno_h {
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "37:1"]
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/include/socket-utils.h:36"]
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/krb5/os/os-proto.h:37"]
pub mod os_proto_h {
    use super::k5_int_h::_krb5_context;
    use super::krb5_h::{krb5_context, krb5_address, krb5_error_code};
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "174:1"]
        pub fn k5_os_hostaddr(_: krb5_context, _: *const libc::c_char,
                              _: *mut *mut *mut krb5_address)
         -> krb5_error_code;
    }
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_addrtype,
                       krb5_enctype, krb5_flags, krb5_deltat, krb5_error_code,
...
pub use self::profile_h::{profile_t, profile_get_values};
pub use self::socket_h::sockaddr;
pub use self::sockaddr_h::sa_family_t;
pub use self::in_h::{in6_addr, C2RustUnnamed, sockaddr_in6, in_port_t,
                     in_addr, in_addr_t, sockaddr_in, htonl};
pub use self::ifaddrs_h::{ifaddrs, C2RustUnnamed_0, getifaddrs, freeifaddrs};
pub use self::if_h::{IFF_UP, C2RustUnnamed_1, IFF_DYNAMIC, IFF_AUTOMEDIA,
                     IFF_PORTSEL, IFF_MULTICAST, IFF_SLAVE, IFF_MASTER,
                     IFF_ALLMULTI, IFF_PROMISC, IFF_NOARP, IFF_RUNNING,
                     IFF_NOTRAILERS, IFF_POINTOPOINT, IFF_LOOPBACK, IFF_DEBUG,
                     IFF_BROADCAST};
pub use self::ctype_h::{_ISspace, C2RustUnnamed_2, _ISalnum, _ISpunct,
...
pub use self::socket_utils_h::{sa2sin6, sa2sin};
use self::os_proto_h::k5_os_hostaddr;
/* not TESTing */
#[derive(Copy, Clone)]
#[repr(C)]
#[c2rust::src_loc = "1097:8"]
pub struct localaddr_data {
    pub count: libc::c_int,
    pub mem_err: libc::c_int,
    pub cur_idx: libc::c_int,
    pub cur_size: libc::c_int,
    pub addr_temp: *mut *mut krb5_address,
}
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
...
/* HAVE_SA_LEN*/
#[c2rust::src_loc = "179:1"]
unsafe extern "C" fn is_loopback_address(mut sa: *mut sockaddr)
 -> libc::c_int {
    match (*sa).sa_family as libc::c_int {
        2 => {
            let mut s4: *mut sockaddr_in = sa2sin(sa);
            return ((*s4).sin_addr.s_addr ==
                        htonl(0x7f000001 as libc::c_int as in_addr_t)) as
                       libc::c_int
        }
        10 => {
            let mut s6: *mut sockaddr_in6 = sa2sin6(sa);
            return ({
                        let mut __a: *const in6_addr =
                            &mut (*s6).sin6_addr as *mut in6_addr as
                                *const in6_addr;
                        ((*__a).__in6_u.__u6_addr32[0 as libc::c_int as usize]
                             ...
                             (*__a).__in6_u.__u6_addr32[3 as libc::c_int as
                                                            usize] ==
                                 htonl(1 as libc::c_int as uint32_t)) as
                            libc::c_int
                    })
        }
...
unsafe extern "C" fn addr_eq(mut s1: *mut sockaddr, mut s2: *mut sockaddr)
 -> libc::c_int {
    if (*s1).sa_family as libc::c_int != (*s2).sa_family as libc::c_int {
        return 0 as libc::c_int
    }
    match (*s1).sa_family as libc::c_int {
        2 => {
            return (memcmp(&mut (*(sa2sin as
                                       unsafe extern "C" fn(_: *mut sockaddr)
                                           -> *mut sockaddr_in)(s1)).sin_addr
                               as *mut in_addr as *const libc::c_void,
                           &mut (*(sa2sin as
                                       unsafe extern "C" fn(_: *mut sockaddr)
                                           -> *mut sockaddr_in)(s2)).sin_addr
                               as *mut in_addr as *const libc::c_void,
                           ::std::mem::size_of::<in_addr>() as libc::c_ulong)
                        == 0) as libc::c_int
        }
        10 => {
            return (memcmp(&mut (*(sa2sin6 as
                                       unsafe extern "C" fn(_: *mut sockaddr)
                                           ->
                                               *mut sockaddr_in6)(s1)).sin6_addr
                               as *mut in6_addr as *const libc::c_void,
                           &mut (*(sa2sin6 as
                                       unsafe extern "C" fn(_: *mut sockaddr)
                                           ->
                                               *mut sockaddr_in6)(s2)).sin6_addr
                               as *mut in6_addr as *const libc::c_void,
...
pub unsafe extern "C" fn krb5int_foreach_localaddr(mut data:
                                                       *mut libc::c_void,
                                                   mut pass1fn:
 ...
 /*@*/
 {
    ...
    let mut ifp_head: *mut ifaddrs = 0 as *mut ifaddrs;
    let mut ifp: *mut ifaddrs = 0 as *mut ifaddrs;
    let mut ifp2: *mut ifaddrs = 0 as *mut ifaddrs;
    let mut match_0: libc::c_int = 0;
    if getifaddrs(&mut ifp_head) < 0 as libc::c_int {
        return *__errno_location()
    }
    ifp = ifp_head;
    loop  {
        if ifp.is_null() { current_block = 14576567515993809846; break ; }
        if !((*ifp).ifa_flags & IFF_UP as libc::c_int as libc::c_uint ==
                 0 as libc::c_int as libc::c_uint) {
            if (*ifp).ifa_addr.is_null() {
               ...
               it.  */
                (*ifp).ifa_flags &= !(IFF_UP as libc::c_int) as libc::c_uint
            } else if is_loopback_address((*ifp).ifa_addr) != 0 {
                /* Pretend it's not up, so the second pass will skip
               it.  */
                (*ifp).ifa_flags &= !(IFF_UP as libc::c_int) as libc::c_uint
            } else {
                ...
                ifp2 = ifp_head;
                while !ifp2.is_null() && ifp2 != ifp {
                    if !((*ifp2).ifa_flags &
                             IFF_UP as libc::c_int as libc::c_uint ==
                             0 as libc::c_int as libc::c_uint) {
                        if addr_eq((*ifp).ifa_addr, (*ifp2).ifa_addr) != 0 {
                ...
                if !(match_0 != 0) {
                    if Some(pass1fn.expect("non-null function pointer")).expect("non-null function pointer")(data,
                                                                                                             (*ifp).ifa_addr)
                           != 0 {
                        current_block = 9699622025891577383;
                        break ;
                    }
                }
            }
    ...
    match current_block {
        14576567515993809846 => {
            if !(betweenfn.is_some() &&
                     Some(betweenfn.expect("non-null function pointer")).expect("non-null function pointer")(data)
                         != 0) {
                if pass2fn.is_some() {
                    ifp = ifp_head;
                    while !ifp.is_null() {
                        if (*ifp).ifa_flags &
                               IFF_UP as libc::c_int as libc::c_uint != 0 {
                            if Some(pass2fn.expect("non-null function pointer")).expect("non-null function pointer")(data,
                                                                                                                     (*ifp).ifa_addr)
                                   != 0 {
                                break ;
                            }
...
unsafe extern "C" fn count_addrs(mut P_data: *mut libc::c_void,
                                 mut a: *mut sockaddr) -> libc::c_int 
 /*@*/
 {
    let mut data: *mut localaddr_data = P_data as *mut localaddr_data;
...
unsafe extern "C" fn allocate(mut P_data: *mut libc::c_void) -> libc::c_int 
 /*@*/
 {
    let mut data: *mut localaddr_data = P_data as *mut localaddr_data;
    ...
    n =
        realloc((*data).addr_temp as *mut libc::c_void,
                ((1 as libc::c_int + (*data).count + (*data).cur_idx) as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<*mut krb5_address>()
                                                     as libc::c_ulong));
    if n.is_null() { (*data).mem_err += 1; return 1 as libc::c_int }
    (*data).addr_temp = n as *mut *mut krb5_address;
    (*data).cur_size = 1 as libc::c_int + (*data).count + (*data).cur_idx;
    i = (*data).cur_idx;
    while i <= (*data).count + (*data).cur_idx {
        let ref mut fresh0 = *(*data).addr_temp.offset(i as isize);
...
unsafe extern "C" fn make_addr(mut type_0: libc::c_int, mut length: size_t,
                               mut contents: *const libc::c_void)
 -> *mut krb5_address 
 /*@*/
 {
    let mut a: *mut krb5_address = 0 as *mut krb5_address;
    let mut data: *mut libc::c_void = 0 as *mut libc::c_void;
    data = malloc(length);
    if data.is_null() { return 0 as *mut krb5_address }
    a =
        malloc(::std::mem::size_of::<krb5_address>() as libc::c_ulong) as
            *mut krb5_address;
    if a.is_null() { free(data); return 0 as *mut krb5_address }
    memcpy(data, contents, length);
    (*a).magic = -(1760647390 as libc::c_long) as krb5_magic;
    (*a).addrtype = type_0;
    (*a).length = length as libc::c_uint;
    (*a).contents = data as *mut krb5_octet;
    return a;
}
#[c2rust::src_loc = "1165:1"]
unsafe extern "C" fn add_addr(mut P_data: *mut libc::c_void,
                              mut a: *mut sockaddr) -> libc::c_int 
 /*@modifies *P_data@*/
 {
    let mut data: *mut localaddr_data = P_data as *mut localaddr_data;
    ...
    match (*a).sa_family as libc::c_int {
        2 => {
            address =
                make_addr(0x2 as libc::c_int,
                          ::std::mem::size_of::<in_addr>() as libc::c_ulong,
                          &mut (*(sa2sin as
                                      unsafe extern "C" fn(_: *mut sockaddr)
                                          -> *mut sockaddr_in)(a)).sin_addr as
                              *mut in_addr as *const libc::c_void);
            if address.is_null() { (*data).mem_err += 1 }
        }
        10 => {
            let mut in_0: *const sockaddr_in6 = sa2sin6(a);
            if !(({
                      let mut __a: *const in6_addr =
                          &(*in_0).sin6_addr as *const in6_addr;
                      ((*__a).__in6_u.__u6_addr32[0 as libc::c_int as usize] &
                           htonl(0xffc00000 as libc::c_uint) ==
                           htonl(0xfe800000 as libc::c_uint)) as libc::c_int
                  }) != 0) {
                address =
                    make_addr(0x18 as libc::c_int,
                              ::std::mem::size_of::<in6_addr>() as
                                  libc::c_ulong,
                              &(*in_0).sin6_addr as *const in6_addr as
                                  *const libc::c_void);
                if address.is_null() { (*data).mem_err += 1 }
            }
        }
    ...
    if !address.is_null() {
        let fresh1 = (*data).cur_idx;
        (*data).cur_idx = (*data).cur_idx + 1;
        let ref mut fresh2 = *(*data).addr_temp.offset(fresh1 as isize);
        *fresh2 = address
    }
    return (*data).mem_err;
}
#[c2rust::src_loc = "1229:1"]
unsafe extern "C" fn krb5_os_localaddr_profile(mut context: krb5_context,
                                               mut datap: *mut localaddr_data)
 -> krb5_error_code {
    let mut err: krb5_error_code = 0;
    static mut profile_name: [*const libc::c_char; 3] =
        [b"libdefaults\x00" as *const u8 as *const libc::c_char,
         b"extra_addresses\x00" as *const u8 as *const libc::c_char,
         0 as *const libc::c_char];
    let mut values: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    let mut iter: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    let mut newaddrs: *mut *mut krb5_address = 0 as *mut *mut krb5_address;
    err =
        profile_get_values((*context).profile, profile_name.as_ptr(),
                           &mut values) as krb5_error_code;
    /* Ignore all errors for now?  */
    if err != 0 { return 0 as libc::c_int }
    iter = values;
    while !(*iter).is_null() {
        let mut cp: *mut libc::c_char = *iter;
        ...
        while *cp != 0 {
            while *(*__ctype_b_loc()).offset(*cp as libc::c_int as isize) as
                      libc::c_int &
                      _ISspace as libc::c_int as libc::c_ushort as libc::c_int
                      != 0 || *cp as libc::c_int == ',' as i32 {
                cp = cp.offset(1)
            }
            if *cp as libc::c_int == 0 as libc::c_int { break ; }
            /* Start of an address.  */
            current = cp;
            while *cp as libc::c_int != 0 as libc::c_int &&
                      *(*__ctype_b_loc()).offset(*cp as libc::c_int as isize)
                          as libc::c_int &
            ...
            if *cp as libc::c_int != 0 as libc::c_int {
                next = cp.offset(1 as libc::c_int as isize);
                *cp = 0 as libc::c_int as libc::c_char
            } else { next = cp }
            /* Got a single address, process it.  */
            newaddrs = 0 as *mut *mut krb5_address;
            err = k5_os_hostaddr(context, current, &mut newaddrs);
            if !(err != 0) {
                i = 0 as libc::c_int;
                while !(*newaddrs.offset(i as isize)).is_null() { i += 1 }
                count = i;
                if (*datap).cur_idx + count >= (*datap).cur_size {
                    let mut bigger: *mut *mut krb5_address =
                        0 as *mut *mut krb5_address;
                    bigger =
                        realloc((*datap).addr_temp as *mut libc::c_void,
                                (::std::mem::size_of::<*mut krb5_address>() as
                                     libc::c_ulong).wrapping_mul(((*datap).cur_idx
                                                                      + count)
                                                                     as
                                                                     libc::c_ulong))
                            as *mut *mut krb5_address;
                    if !bigger.is_null() {
                ...
                while i < count {
                    if (*datap).cur_idx < (*datap).cur_size {
                        let fresh3 = (*datap).cur_idx;
                        (*datap).cur_idx = (*datap).cur_idx + 1;
                        let ref mut fresh4 =
                            *(*datap).addr_temp.offset(fresh3 as isize);
                        *fresh4 = *newaddrs.offset(i as isize)
                    } else {
                        free((**newaddrs.offset(i as isize)).contents as
                                 *mut libc::c_void);
                        free(*newaddrs.offset(i as isize) as
                                 *mut libc::c_void);
                    }
                    i += 1
                }
                free(newaddrs as *mut libc::c_void);
            }
            cp = next
        }
        iter = iter.offset(1)
...
pub unsafe extern "C" fn krb5_os_localaddr(mut context: krb5_context,
                                           mut addr:
                                               *mut *mut *mut krb5_address)
 -> krb5_error_code {
    return get_localaddrs(context, addr, 1 as libc::c_int);
}
/* Solaris 8 and later; Sol 7? */
/* not HAVE_IFADDRS_H and not SIOCGLIFNUM */
#[c2rust::src_loc = "1320:1"]
unsafe extern "C" fn get_localaddrs(mut context: krb5_context,
                                    mut addr: *mut *mut *mut krb5_address,
                                    mut use_profile: libc::c_int)
 -> krb5_error_code {
    let mut data: localaddr_data =
        {
            let mut init =
                localaddr_data{count: 0 as libc::c_int,
                               mem_err: 0,
                               cur_idx: 0,
                               cur_size: 0,
                               addr_temp: 0 as *mut *mut krb5_address,};
            init
        };
    let mut r: libc::c_int = 0;
    /* Ignore errors for now. */
    if use_profile != 0 {
        krb5_os_localaddr_profile(context, &mut data); /* null termination */
    }
    r =
        krb5int_foreach_localaddr(&mut data as *mut localaddr_data as
                                      *mut libc::c_void,
                                  Some(count_addrs as
    ...
    if r != 0 as libc::c_int {
        let mut i: libc::c_int = 0;
        if !data.addr_temp.is_null() {
            i = 0 as libc::c_int;
            while i < data.count {
                free(*data.addr_temp.offset(i as isize) as *mut libc::c_void);
                i += 1
            }
            free(data.addr_temp as *mut libc::c_void);
        }
        if data.mem_err != 0 { return 12 as libc::c_int } else { return r }
    }
    data.cur_idx += 1;
    if data.mem_err != 0 {
        return 12 as libc::c_int
    } else {
        if data.cur_idx == data.count {
            *addr = data.addr_temp
        } else {
            ...
            *addr =
                realloc(data.addr_temp as *mut libc::c_void,
                        (::std::mem::size_of::<*mut krb5_address>() as
                             libc::c_ulong).wrapping_mul(data.cur_idx as
                                                             libc::c_ulong))
                    as *mut *mut krb5_address;
            if (*addr).is_null() {
