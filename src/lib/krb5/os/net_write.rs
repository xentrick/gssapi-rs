...
pub use self::types_h::{__int32_t, __ssize_t, __socklen_t};
...
pub use self::socket_h::{C2RustUnnamed, MSG_CMSG_CLOEXEC, MSG_FASTOPEN,
                         MSG_ZEROCOPY, MSG_BATCH, MSG_WAITFORONE, MSG_MORE,
                         MSG_NOSIGNAL, MSG_ERRQUEUE, MSG_RST, MSG_CONFIRM,
                         MSG_SYN, MSG_FIN, MSG_WAITALL, MSG_EOR, MSG_DONTWAIT,
                         MSG_TRUNC, MSG_PROXY, MSG_CTRUNC, MSG_TRYHARD,
                         MSG_DONTROUTE, MSG_PEEK, MSG_OOB, msghdr};
pub use self::port_sockets_h::{sg_buf, socket_sendmsg};
use self::stdlib_h::abort;
use self::errno_h::__errno_location;
use self::string_h::memset;
use self::assert_h::__assert_fail;
use self::sys_socket_h::sendmsg;
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
/*
...
#[no_mangle]
#[c2rust::src_loc = "39:1"]
pub unsafe extern "C" fn krb5_net_write(mut context: krb5_context,
                                        mut fd: libc::c_int,
                                        mut buf: *const libc::c_char,
                                        mut len: libc::c_int) -> libc::c_int {
    let mut sg: sg_buf =
        sg_buf{iov_base: 0 as *mut libc::c_void, iov_len: 0,};
    sg.iov_base = buf as *mut libc::c_char as *mut libc::c_void;
    sg.iov_len = len as size_t;
    return krb5int_net_writev(context, fd, &mut sg, 1 as libc::c_int);
}
/* The io vector is *not* const here, unlike writev()!  */
#[no_mangle]
#[c2rust::src_loc = "47:1"]
pub unsafe extern "C" fn krb5int_net_writev(mut context: krb5_context,
                                            mut fd: libc::c_int,
                                            mut sgp: *mut sg_buf,
                                            mut nsg: libc::c_int)
 -> libc::c_int {
    ...
    while nsg > 0 as libc::c_int {
        /* Skip any empty data blocks.  */
        if (*sgp).iov_len.wrapping_add(0 as libc::c_int as libc::c_ulong) ==
               0 as libc::c_int as libc::c_ulong {
            sgp = sgp.offset(1);
            nsg -= 1
        } else {
            tmp = socket_sendmsg(fd, sgp, nsg) as libc::c_int;
            cc = tmp as ssize_t;
            if cc < 0 as libc::c_int as libc::c_long {
                ...
                *__errno_location() = *__errno_location();
                ...
                len += cc;
                while cc > 0 as libc::c_int as libc::c_long {
                    if (cc as libc::c_uint as libc::c_ulong) <
                           (*sgp).iov_len.wrapping_add(0 as libc::c_int as
                                                           libc::c_ulong) {
                        if (*sgp).iov_len <
                               cc as libc::c_uint as libc::c_ulong {
                            abort();
                        } else {
                            (*sgp).iov_base =
                                ((*sgp).iov_base as
                                     *mut libc::c_char).offset(cc as
                                                                   libc::c_uint
                                                                   as isize)
                                    as *mut libc::c_void;
                            (*sgp).iov_len =
                                ((*sgp).iov_len as
                                     libc::c_ulong).wrapping_sub(cc as
                                                                     libc::c_uint
                                                                     as
                                                                     libc::c_ulong)
                                    as size_t as size_t
                        };
                        cc = 0 as libc::c_int as ssize_t
                    } else {
                        cc =
                            (cc as
                        ...
                        sgp = sgp.offset(1);
                        nsg -= 1;
                        if nsg > 0 as libc::c_int ||
                               cc == 0 as libc::c_int as libc::c_long {
                        } else {
                            __assert_fail(b"nsg > 0 || cc == 0\x00" as
                                              *const u8 as
                                              *const libc::c_char,
                                          b"net_write.c\x00" as *const u8 as
                                              *const libc::c_char,
                                          76 as libc::c_int as libc::c_uint,
                                          (*::std::mem::transmute::<&[u8; 57],
                                                                    &[libc::c_char; 57]>(b"int krb5int_net_writev(krb5_context, int, sg_buf *, int)\x00")).as_ptr());
                        }
                    }
                }
            }
