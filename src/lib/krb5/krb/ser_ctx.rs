...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _kdb5_dal_handle, krb5_ser_pack_int32,
                         krb5_ser_pack_bytes, krb5_ser_unpack_int32,
...
use self::int_proto_h::k5_count_etypes;
extern "C" {
    /* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
 ...
 */
    #[no_mangle]
    #[c2rust::src_loc = "30:1"]
    pub fn profile_ser_size(_: krb5_context, _: profile_t, _: *mut size_t)
     -> krb5_error_code;
    #[no_mangle]
    #[c2rust::src_loc = "31:1"]
    pub fn profile_ser_externalize(_: krb5_context, _: profile_t,
                                   _: *mut *mut krb5_octet, _: *mut size_t)
     -> krb5_error_code;
    #[no_mangle]
    #[c2rust::src_loc = "33:1"]
    pub fn profile_ser_internalize(_: krb5_context, _: *mut profile_t,
                                   _: *mut *mut krb5_octet, _: *mut size_t)
     -> krb5_error_code;
}
#[inline]
#[c2rust::src_loc = "44:1"]
unsafe extern "C" fn etypes_len(mut list: *mut krb5_enctype) -> libc::c_uint {
    return if list.is_null() {
               0 as libc::c_int as libc::c_ulong
           } else { k5_count_etypes(list) } as libc::c_uint;
}
#[no_mangle]
#[c2rust::src_loc = "50:1"]
pub unsafe extern "C" fn k5_size_context(mut context: krb5_context,
 ...
 -> krb5_error_code {
    ...
    if !context.is_null() {
        ...
        required =
            (10 as libc::c_int as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<krb5_int32>()
                                                 as
                                                 libc::c_ulong).wrapping_add((etypes_len((*context).in_tkt_etypes)
                                                                                  as
                                                                                  libc::c_ulong).wrapping_mul(::std::mem::size_of::<krb5_int32>()
                                                                                                                  as
                                                                                                                  libc::c_ulong)).wrapping_add((etypes_len((*context).tgs_etypes)
                                                                                                                                                    as
                                                                                                                                                    libc::c_ulong).wrapping_mul(::std::mem::size_of::<krb5_int32>()
                                                                                                                                                                                    as
                                                                                                                                                                                    libc::c_ulong));
        if !(*context).default_realm.is_null() {
            required =
                (required as
                     libc::c_ulong).wrapping_add(strlen((*context).default_realm))
                    as size_t as size_t
        }
        /* Calculate size required by os_context, if appropriate */
        kret = size_oscontext(&mut (*context).os_context, &mut required);
        /* Calculate size required by profile, if appropriate */
        if kret == 0 && !(*context).profile.is_null() {
            kret =
                profile_ser_size(0 as krb5_context, (*context).profile,
                                 &mut required)
        }
    }
    if kret == 0 {
...
#[no_mangle]
#[c2rust::src_loc = "96:1"]
pub unsafe extern "C" fn k5_externalize_context(mut context: krb5_context,
                                                mut buffer:
                                                    *mut *mut krb5_octet,
 ...
 -> krb5_error_code {
    ...
    bp = *buffer;
    remain = *lenremain;
    if context.is_null() { return 22 as libc::c_int }
    if (*context).magic as libc::c_long != -(1760647388 as libc::c_long) {
        return -(1760647388 as libc::c_long) as krb5_error_code
    }
    kret = k5_size_context(context, &mut required);
    if kret != 0 { return kret }
    if required > remain { return 12 as libc::c_int }
    /* First write our magic number */
    kret =
        krb5_ser_pack_int32(-(1760647388 as libc::c_long) as krb5_int32,
                            &mut bp, &mut remain);
    if kret != 0 { return kret }
    /* Now sizeof default realm */
    kret =
        krb5_ser_pack_int32(if !(*context).default_realm.is_null() {
                                strlen((*context).default_realm) as krb5_int32
                            } else { 0 as libc::c_int }, &mut bp,
                            &mut remain);
    if kret != 0 { return kret }
    /* Now default_realm bytes */
    if !(*context).default_realm.is_null() {
        kret =
            krb5_ser_pack_bytes((*context).default_realm as *mut krb5_octet,
                                strlen((*context).default_realm), &mut bp,
                                &mut remain);
        if kret != 0 { return kret }
    }
    /* Now number of initial ticket ktypes */
    kret =
        krb5_ser_pack_int32(etypes_len((*context).in_tkt_etypes) as
                                krb5_int32, &mut bp, &mut remain);
    if kret != 0 { return kret }
    /* Now serialize ktypes */
    if !(*context).in_tkt_etypes.is_null() {
        i = 0 as libc::c_int as libc::c_uint;
        while *(*context).in_tkt_etypes.offset(i as isize) != 0 {
            kret =
                krb5_ser_pack_int32(*(*context).in_tkt_etypes.offset(i as
                                                                         isize),
                                    &mut bp, &mut remain);
            if kret != 0 { return kret }
            i = i.wrapping_add(1)
    ...
    kret =
        krb5_ser_pack_int32(etypes_len((*context).tgs_etypes) as krb5_int32,
                            &mut bp, &mut remain);
    if kret != 0 { return kret }
    /* Now serialize ktypes */
    if !(*context).tgs_etypes.is_null() {
        i = 0 as libc::c_int as libc::c_uint;
        while *(*context).tgs_etypes.offset(i as isize) != 0 {
            kret =
                krb5_ser_pack_int32(*(*context).tgs_etypes.offset(i as isize),
                                    &mut bp, &mut remain);
            if kret != 0 { return kret }
            i = i.wrapping_add(1)
        }
    ...
    kret = krb5_ser_pack_int32((*context).clockskew, &mut bp, &mut remain);
    if kret != 0 { return kret }
    /* Now kdc_default_options */
    kret =
        krb5_ser_pack_int32((*context).kdc_default_options, &mut bp,
                            &mut remain);
    if kret != 0 { return kret }
    /* Now library_options */
    kret =
        krb5_ser_pack_int32((*context).library_options, &mut bp, &mut remain);
    if kret != 0 { return kret }
    /* Now profile_secure */
    kret =
        krb5_ser_pack_int32((*context).profile_secure as krb5_int32, &mut bp,
                            &mut remain);
    if kret != 0 { return kret }
    /* Now fcc_default_format */
    kret =
        krb5_ser_pack_int32((*context).fcc_default_format, &mut bp,
                            &mut remain);
    if kret != 0 { return kret }
    /* Now handle os_context, if appropriate */
    kret =
        externalize_oscontext(&mut (*context).os_context, &mut bp,
                              &mut remain);
    if kret != 0 { return kret }
    /* Finally, handle profile, if appropriate */
    if !(*context).profile.is_null() {
        kret =
            profile_ser_externalize(0 as krb5_context, (*context).profile,
                                    &mut bp, &mut remain);
        if kret != 0 { return kret }
    }
    /*
...
pub unsafe extern "C" fn k5_internalize_context(mut argp: *mut krb5_context,
                                                mut buffer:
                                                    *mut *mut krb5_octet,
                                                mut lenremain: *mut size_t)
 -> krb5_error_code {
    ...
    if kret != 0 { return 22 as libc::c_int }
    if ibuf as libc::c_long != -(1760647388 as libc::c_long) {
    ...
    context =
        calloc(1 as libc::c_int as libc::c_ulong,
            ...
            krb5_context;
    if context.is_null() { return 12 as libc::c_int }
    ...
    if !(kret != 0) {
        if ibuf != 0 {
            ...
            if (*context).default_realm.is_null() {
                ...
                current_block = 13984646848123448087;
            } else {
                kret =
                    krb5_ser_unpack_bytes((*context).default_realm as
                                              *mut krb5_octet, ibuf as size_t,
                                          &mut bp, &mut remain);
                if kret != 0 {
                    current_block = 13984646848123448087;
                } else {
                    *(*context).default_realm.offset(ibuf as isize) =
        ...
        match current_block {
            13984646848123448087 => { }
            _ =>
            /* Get the in_tkt_etypes */
            {
                kret = krb5_ser_unpack_int32(&mut ibuf, &mut bp, &mut remain);
                if !(kret != 0) {
                    count = ibuf as libc::c_uint;
                    if count > 0 as libc::c_int as libc::c_uint {
                        (*context).in_tkt_etypes =
                            calloc(count.wrapping_add(1 as libc::c_int as
                                                          libc::c_uint) as
                                       libc::c_ulong,
                                   ::std::mem::size_of::<krb5_enctype>() as
                                       libc::c_ulong) as *mut krb5_enctype;
                        if (*context).in_tkt_etypes.is_null() {
                            kret = 12 as libc::c_int;
                            current_block = 13984646848123448087;
                        } else {
                            ...
                            loop  {
                                ...
                                *(*context).in_tkt_etypes.offset(i as isize) =
                                    ibuf;
                                i = i.wrapping_add(1)
                            }
                            match current_block {
                                13984646848123448087 => { }
                                _ => {
                                    *(*context).in_tkt_etypes.offset(count as
                                                                         isize)
                                        = 0 as libc::c_int;
                        ...
                        (*context).in_tkt_etypes = 0 as *mut krb5_enctype;
                        current_block = 11913429853522160501;
                    }
                    match current_block {
                        13984646848123448087 => { }
                        ...
                        /* Get the tgs_etypes */
                        {
                            ...
                            if !(kret != 0) {
                                ...
                                if count > 0 as libc::c_int as libc::c_uint {
                                    (*context).tgs_etypes =
                                        calloc(count.wrapping_add(1 as
                                                                      libc::c_int
                                                                      as
                                                                      libc::c_uint)
                                                   as libc::c_ulong,
                                    ...
                                    if (*context).tgs_etypes.is_null() {
                                        ...
                                        loop  {
                                            ...
                                            kret =
                                                krb5_ser_unpack_int32(&mut ibuf,
                                                                      &mut bp,
                                                                      &mut remain);
                                            if kret != 0 {
                                                current_block =
                                            ...
                                            *(*context).tgs_etypes.offset(i as
                                                                              isize)
                                                = ibuf;
                                            i = i.wrapping_add(1)
                                        }
                                        match current_block {
                                            13984646848123448087 => { }
                                            _ => {
                                                *(*context).tgs_etypes.offset(count
                                                                                  as
                                                                                  isize)
                                                    = 0 as libc::c_int;
                                                current_block =
...
unsafe extern "C" fn size_oscontext(mut os_ctx: krb5_os_context,
                                    mut sizep: *mut size_t)
 -> krb5_error_code {
    /*
...
unsafe extern "C" fn externalize_oscontext(mut os_ctx: krb5_os_context,
 ...
 -> krb5_error_code {
    ...
    if !os_ctx.is_null() {
        kret = 12 as libc::c_int;
        if size_oscontext(os_ctx, &mut required) == 0 && required <= remain {
            krb5_ser_pack_int32(-(1760647387 as libc::c_long) as krb5_int32,
                                &mut bp, &mut remain);
            krb5_ser_pack_int32((*os_ctx).time_offset, &mut bp, &mut remain);
            krb5_ser_pack_int32((*os_ctx).usec_offset, &mut bp, &mut remain);
            krb5_ser_pack_int32((*os_ctx).os_flags, &mut bp, &mut remain);
            krb5_ser_pack_int32(-(1760647387 as libc::c_long) as krb5_int32,
                                &mut bp, &mut remain);
...
unsafe extern "C" fn internalize_oscontext(mut argp: *mut krb5_os_context,
                                           mut buffer: *mut *mut krb5_octet,
                                           mut lenremain: *mut size_t)
 -> krb5_error_code {
    let mut kret: krb5_error_code = 0;
    ...
    remain = *lenremain;
    kret = 22 as libc::c_int;
    os_ctx = 0 as *mut libc::c_void as krb5_os_context;
    /* Read our magic number */
    if krb5_ser_unpack_int32(&mut ibuf, &mut bp, &mut remain) != 0 {
        ibuf = 0 as libc::c_int
    }
    if ibuf as libc::c_long == -(1760647387 as libc::c_long) {
        kret = 12 as libc::c_int;
        /* Get memory for the context */
        os_ctx =
            calloc(1 as libc::c_int as libc::c_ulong,
                   ::std::mem::size_of::<_krb5_os_context>() as libc::c_ulong)
                as krb5_os_context;
        if !os_ctx.is_null() &&
               remain >=
                   (4 as libc::c_int as
            ...
            krb5_ser_unpack_int32(&mut (*os_ctx).time_offset, &mut bp,
                                  &mut remain);
            krb5_ser_unpack_int32(&mut (*os_ctx).usec_offset, &mut bp,
                                  &mut remain);
            krb5_ser_unpack_int32(&mut (*os_ctx).os_flags, &mut bp,
                                  &mut remain);
    ...
    if kret == 0 {
        *argp = os_ctx
    } else if !os_ctx.is_null() { free(os_ctx as *mut libc::c_void); }
    return kret;
}
