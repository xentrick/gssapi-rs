...
pub mod k5_int_h {
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_enc_kdc_rep_part, krb5_data, krb5_error_code,
                        krb5_kdc_rep};
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_msgtype,
                       ...
                       _krb5_pa_data, krb5_pa_data, _krb5_enc_kdc_rep_part,
                       krb5_enc_kdc_rep_part, _krb5_kdc_rep, krb5_kdc_rep,
                       _profile_t, krb5_c_valid_enctype, krb5_free_data};
...
#[no_mangle]
#[c2rust::src_loc = "43:1"]
pub unsafe extern "C" fn krb5_encode_kdc_rep(mut context: krb5_context,
                                             mut type_0: krb5_msgtype,
                                             mut encpart:
                                                 *const krb5_enc_kdc_rep_part,
                                             mut using_subkey: libc::c_int,
                                             mut client_key:
                                                 *const krb5_keyblock,
                                             mut dec_rep: *mut krb5_kdc_rep,
                                             mut enc_rep: *mut *mut krb5_data)
 -> krb5_error_code {
    let mut scratch: *mut krb5_data = 0 as *mut krb5_data;
    let mut retval: krb5_error_code = 0;
    let mut tmp_encpart: krb5_enc_kdc_rep_part =
    ...
    if krb5_c_valid_enctype((*dec_rep).enc_part.enctype) == 0 {
        return -(1765328234 as libc::c_long) as krb5_error_code
    }
    match type_0 {
        11 => { usage = 3 as libc::c_int }
        13 => {
            if using_subkey != 0 {
                usage = 9 as libc::c_int
            } else { usage = 8 as libc::c_int }
        }
        _ => { return -(1765328246 as libc::c_long) as krb5_error_code }
    }
    /*
     ...
     */
    tmp_encpart = *encpart;
    tmp_encpart.msg_type = type_0;
    retval = encode_krb5_enc_kdc_rep_part(&mut tmp_encpart, &mut scratch);
    if retval != 0 { return retval }
    memset(&mut tmp_encpart as *mut krb5_enc_kdc_rep_part as
               *mut libc::c_void, 0 as libc::c_int,
           ::std::mem::size_of::<krb5_enc_kdc_rep_part>() as libc::c_ulong);
    retval =
        krb5_encrypt_helper(context, client_key, usage, scratch,
                            &mut (*dec_rep).enc_part);
    ...
    match type_0 {
        11 => { retval = encode_krb5_as_rep(dec_rep, enc_rep) }
        13 => { retval = encode_krb5_tgs_rep(dec_rep, enc_rep) }
        _ => { }
    }
    if retval != 0 {
        memset((*dec_rep).enc_part.ciphertext.data as *mut libc::c_void,
               0 as libc::c_int,
               (*dec_rep).enc_part.ciphertext.length as libc::c_ulong);
        free((*dec_rep).enc_part.ciphertext.data as *mut libc::c_void);
        (*dec_rep).enc_part.ciphertext.length =
            0 as libc::c_int as libc::c_uint;
        (*dec_rep).enc_part.ciphertext.data = 0 as *mut libc::c_char
    }
    return retval;
}
