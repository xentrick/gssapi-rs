...
pub use self::krb5_h::{krb5_int32, krb5_boolean, krb5_enctype, krb5_flags,
                       ...
                       krb5_prompt_type, _profile_t, krb5_principal_compare};
...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_sname_match(mut context: krb5_context,
                                          mut matching: krb5_const_principal,
                                          mut princ: krb5_const_principal)
 -> krb5_boolean {
    if matching.is_null() { return 1 as libc::c_int as krb5_boolean }
    if (*matching).type_0 != 3 as libc::c_int ||
           (*matching).length != 2 as libc::c_int {
    ...
    if (*matching).realm.length != 0 as libc::c_int as libc::c_uint &&
           data_eq((*matching).realm, (*princ).realm) == 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    /* Check the service name (must be present in matching). */
    if data_eq(*(*matching).data.offset(0 as libc::c_int as isize),
               *(*princ).data.offset(0 as libc::c_int as isize)) == 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    /* Check the hostname if present in matching and not ignored. */
    if (*(*matching).data.offset(1 as libc::c_int as isize)).length !=
           0 as libc::c_int as libc::c_uint &&
           (*context).ignore_acceptor_hostname == 0 &&
           data_eq(*(*matching).data.offset(1 as libc::c_int as isize),
                   *(*princ).data.offset(1 as libc::c_int as isize)) == 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    /* All elements match. */
