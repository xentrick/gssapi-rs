...
#[c2rust::header_src = "/usr/include/strings.h:7"]
pub mod strings_h {
    extern "C" {
        #[no_mangle]
...
#[c2rust::src_loc = "22:1"]
unsafe extern "C" fn conf_boolean(mut s: *mut libc::c_char) -> libc::c_int {
...
#[c2rust::src_loc = "38:1"]
unsafe extern "C" fn appdefault_get(mut context: krb5_context,
 ...
 -> krb5_error_code {
    ...
    let mut realmstr: *const libc::c_char =
        if !realm.is_null() { (*realm).data } else { 0 as *mut libc::c_char };
    *ret_value = 0 as *mut libc::c_char;
    ...
    names[0 as libc::c_int as usize] =
        b"appdefaults\x00" as *const u8 as *const libc::c_char;
    names[1 as libc::c_int as usize] = appname;
    if !realmstr.is_null() {
        names[2 as libc::c_int as usize] = realmstr;
        names[3 as libc::c_int as usize] = option;
        names[4 as libc::c_int as usize] = 0 as *const libc::c_char;
        retval =
        ...
        if retval == 0 as libc::c_int && !nameval.is_null() &&
            ...
            *ret_value = strdup(*nameval.offset(0 as libc::c_int as isize));
            current_block = 15077432600037132814;
     ...
     */
            ...
            retval =
                profile_get_values(profile, names.as_mut_ptr(), &mut nameval)
                    as krb5_error_code;
            if retval == 0 as libc::c_int && !nameval.is_null() &&
                   !(*nameval.offset(0 as libc::c_int as isize)).is_null() {
                *ret_value =
                    strdup(*nameval.offset(0 as libc::c_int as isize))
            } else {
                /*
     * Try number three:
     *
     ...
     */
                if !realmstr.is_null() {
                    ...
                    names[2 as libc::c_int as usize] = option;
                    names[3 as libc::c_int as usize] =
                        0 as *const libc::c_char;
                    retval =
                        profile_get_values(profile, names.as_mut_ptr(),
                                           &mut nameval) as krb5_error_code;
                    if retval == 0 as libc::c_int && !nameval.is_null() &&
                           !(*nameval.offset(0 as libc::c_int as
                                                 isize)).is_null() {
                        *ret_value =
                            strdup(*nameval.offset(0 as libc::c_int as
                                                       isize));
                        current_block = 15077432600037132814;
    ...
    if !nameval.is_null() {
        ...
        cpp = nameval;
        while !(*cpp).is_null() {
            free(*cpp as *mut libc::c_void);
            cpp = cpp.offset(1)
        }
        free(nameval as *mut libc::c_void);
    }
    return 0 as libc::c_int;
}
/* *
...
pub unsafe extern "C" fn krb5_appdefault_boolean(mut context: krb5_context,
                                                 mut appname:
                                                     *const libc::c_char,
                                                 mut realm: *const krb5_data,
                                                 mut option:
    ...
    let mut retval: krb5_error_code = 0;
    retval = appdefault_get(context, appname, realm, option, &mut string);
    if retval == 0 && !string.is_null() {
...
pub unsafe extern "C" fn krb5_appdefault_string(mut context: krb5_context,
                                                mut appname:
                                                    *const libc::c_char,
                                                mut realm: *const krb5_data,
                                                mut option:
    ...
    if retval == 0 && !string.is_null() {
        *ret_value = string
    } else { *ret_value = strdup(default_value) };
}
