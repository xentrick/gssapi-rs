...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_addrtype,
                       ...
                       _profile_t, krb5_copy_keyblock_contents,
                       krb5_free_principal, krb5_free_addresses,
                       krb5_copy_authdata, krb5_copy_data,
                       krb5_copy_addresses, krb5_copy_principal};
...
#[no_mangle]
#[c2rust::src_loc = "34:1"]
pub unsafe extern "C" fn krb5_copy_creds(mut context: krb5_context,
                                         mut incred: *const krb5_creds,
                                         mut outcred: *mut *mut krb5_creds)
 -> krb5_error_code {
    let mut tempcred: *mut krb5_creds = 0 as *mut krb5_creds;
    let mut retval: krb5_error_code = 0;
    tempcred =
        malloc(::std::mem::size_of::<krb5_creds>() as libc::c_ulong) as
            *mut krb5_creds;
    if tempcred.is_null() { return 12 as libc::c_int }
    retval = k5_copy_creds_contents(context, incred, tempcred);
    if retval != 0 {
        free(tempcred as *mut libc::c_void);
    } else { *outcred = tempcred }
    return retval;
}
/*
...
#[no_mangle]
#[c2rust::src_loc = "57:1"]
pub unsafe extern "C" fn k5_copy_creds_contents(mut context: krb5_context,
                                                mut incred: *const krb5_creds,
                                                mut tempcred: *mut krb5_creds)
 -> krb5_error_code {
    let mut retval: krb5_error_code = 0;
    let mut scratch: *mut krb5_data = 0 as *mut krb5_data;
    *tempcred = *incred;
    retval =
        krb5_copy_principal(context, (*incred).client as krb5_const_principal,
                            &mut (*tempcred).client);
    if !(retval != 0) {
        retval =
            krb5_copy_principal(context,
                                (*incred).server as krb5_const_principal,
                                &mut (*tempcred).server);
        if !(retval != 0) {
            retval =
                krb5_copy_keyblock_contents(context, &(*incred).keyblock,
                                            &mut (*tempcred).keyblock);
            if !(retval != 0) {
                retval =
                    krb5_copy_addresses(context, (*incred).addresses,
                                        &mut (*tempcred).addresses);
                if !(retval != 0) {
                    retval =
                        krb5_copy_data(context, &(*incred).ticket,
                                       &mut scratch);
                    if !(retval != 0) {
                        (*tempcred).ticket = *scratch;
                        free(scratch as *mut libc::c_void);
                        retval =
                            krb5_copy_data(context, &(*incred).second_ticket,
                                           &mut scratch);
                        if !(retval != 0) {
                            (*tempcred).second_ticket = *scratch;
                            free(scratch as *mut libc::c_void);
                            retval =
                                krb5_copy_authdata(context,
                                                   (*incred).authdata,
                                                   &mut (*tempcred).authdata);
                            if retval != 0 {
                                memset((*tempcred).second_ticket.data as
                                           *mut libc::c_void,
                                       0 as libc::c_int,
                                       (*tempcred).second_ticket.length as
                                           libc::c_ulong);
                                free((*tempcred).second_ticket.data as
                                         *mut libc::c_void);
                            } else { return 0 as libc::c_int }
                        }
                        memset((*tempcred).ticket.data as *mut libc::c_void,
                               0 as libc::c_int,
                               (*tempcred).ticket.length as libc::c_ulong);
                        free((*tempcred).ticket.data as *mut libc::c_void);
                    }
                    krb5_free_addresses(context, (*tempcred).addresses);
                }
                free((*tempcred).keyblock.contents as *mut libc::c_void);
            }
