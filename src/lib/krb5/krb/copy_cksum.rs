...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_copy_checksum(mut context: krb5_context,
                                            mut ckfrom: *const krb5_checksum,
                                            mut ckto: *mut *mut krb5_checksum)
 -> krb5_error_code {
    let mut tempto: *mut krb5_checksum = 0 as *mut krb5_checksum;
    tempto =
        malloc(::std::mem::size_of::<krb5_checksum>() as libc::c_ulong) as
            *mut krb5_checksum;
    if tempto.is_null() { return 12 as libc::c_int }
    *tempto = *ckfrom;
    (*tempto).contents =
        malloc((*tempto).length as libc::c_ulong) as *mut krb5_octet;
    if (*tempto).contents.is_null() {
        free(tempto as *mut libc::c_void);
        return 12 as libc::c_int
    }
    memcpy((*tempto).contents as *mut libc::c_void,
           (*ckfrom).contents as *const libc::c_void,
           (*ckfrom).length as libc::c_ulong);
    *ckto = tempto;
