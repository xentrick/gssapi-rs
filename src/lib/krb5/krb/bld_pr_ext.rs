...
#[no_mangle]
#[c2rust::src_loc = "36:1"]
pub unsafe extern "C" fn krb5_build_principal_ext(mut context: krb5_context,
                                                  mut princ:
                                                      *mut krb5_principal,
 ...
 -> krb5_error_code {
    ...
    let mut princ_data: *mut krb5_data = 0 as *mut krb5_data;
    let mut princ_ret: krb5_principal = 0 as *mut krb5_principal_data;
    let mut tmpdata: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    ap = args.clone();
    /* count up */
    while ap.as_va_list().arg::<libc::c_int>() != 0 as libc::c_int {
        ap.as_va_list().arg::<*mut libc::c_char>(); /* pass one up */
        count += 1
    }
    ...
    /* get space for array */
    princ_data =
        malloc((::std::mem::size_of::<krb5_data>() as
                    libc::c_ulong).wrapping_mul(count as libc::c_ulong)) as
            *mut krb5_data;
    if princ_data.is_null() { return 12 as libc::c_int }
    princ_ret =
        malloc(::std::mem::size_of::<krb5_principal_data>() as libc::c_ulong)
            as krb5_principal;
    if princ_ret.is_null() {
        free(princ_data as *mut libc::c_void);
        return 12 as libc::c_int
    }
    (*princ_ret).data = princ_data;
    (*princ_ret).length = count;
    tmpdata.length = rlen;
    tmpdata.data = realm as *mut libc::c_char;
    if krb5int_copy_data_contents_add0(context, &mut tmpdata,
                                       &mut (*princ_ret).realm) !=
           0 as libc::c_int {
        free(princ_data as *mut libc::c_void);
        free(princ_ret as *mut libc::c_void);
        return 12 as libc::c_int
    }
    ...
    i = 0 as libc::c_int;
    loop  {
        if !(i < count) { current_block = 16924917904204750491; break ; }
        tmpdata.length = ap.as_va_list().arg::<libc::c_uint>();
        tmpdata.data = ap.as_va_list().arg::<*mut libc::c_char>();
        if krb5int_copy_data_contents_add0(context, &mut tmpdata,
                                           &mut *princ_data.offset(i as
                                                                       isize))
               != 0 as libc::c_int {
    ...
    match current_block {
        12087451728421612484 => {
            loop  {
                ...
                free((*princ_data.offset(i as isize)).data as
                         *mut libc::c_void);
            }
            free(princ_data as *mut libc::c_void);
            free((*princ_ret).realm.data as *mut libc::c_void);
            free(princ_ret as *mut libc::c_void);
            return 12 as libc::c_int
        }
        _ => {
            *princ = princ_ret;
            (*princ_ret).type_0 = 0 as libc::c_int;
