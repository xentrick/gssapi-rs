...
pub mod k5_int_h {
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_authdatatype, krb5_keyblock, krb5_data,
                        krb5_key};
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_ui_4, krb5_boolean,
                       ...
                       krb5_copy_keyblock, krb5_copy_principal,
                       krb5_copy_authdata, krb5_free_principal,
                       krb5_free_checksum, krb5_free_keyblock,
                       krb5_copy_checksum};
...
#[no_mangle]
#[c2rust::src_loc = "31:1"]
pub unsafe extern "C" fn krb5_copy_authenticator(mut context: krb5_context,
                                                 mut authfrom:
 ...
 -> krb5_error_code {
    ...
    let mut tempto: *mut krb5_authenticator = 0 as *mut krb5_authenticator;
    tempto =
        malloc(::std::mem::size_of::<krb5_authenticator>() as libc::c_ulong)
            as *mut krb5_authenticator;
    if tempto.is_null() { return 12 as libc::c_int }
    *tempto = *authfrom;
    retval =
        krb5_copy_principal(context,
                            (*authfrom).client as krb5_const_principal,
                            &mut (*tempto).client);
    if retval != 0 { free(tempto as *mut libc::c_void); return retval }
    if !(*authfrom).checksum.is_null() &&
           {
               retval =
                   krb5_copy_checksum(context, (*authfrom).checksum,
                                      &mut (*tempto).checksum);
               (retval) != 0
           } {
    ...
    if !(*authfrom).subkey.is_null() {
        retval =
            krb5_copy_keyblock(context, (*authfrom).subkey,
                               &mut (*tempto).subkey);
        if retval != 0 {
    ...
    if !(*authfrom).authorization_data.is_null() {
        retval =
            krb5_copy_authdata(context, (*authfrom).authorization_data,
...
#[no_mangle]
#[c2rust::src_loc = "81:1"]
pub unsafe extern "C" fn krb5_auth_con_getauthenticator(mut context:
                                                            krb5_context,
                                                        mut auth_context:
                                                            krb5_auth_context,
                                                        mut authenticator:
