...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_copy_principal, krb5_copy_addresses,
                       krb5_free_principal, krb5_free_keyblock,
                       krb5_free_addresses, krb5_copy_authdata};
 ...
 */
#[c2rust::src_loc = "29:1"]
unsafe extern "C" fn copy_enc_tkt_part(mut context: krb5_context,
                                       mut partfrom: *const krb5_enc_tkt_part,
                                       mut partto:
                                           *mut *mut krb5_enc_tkt_part)
 -> krb5_error_code {
    let mut retval: krb5_error_code = 0;
    let mut tempto: *mut krb5_enc_tkt_part = 0 as *mut krb5_enc_tkt_part;
    tempto =
        malloc(::std::mem::size_of::<krb5_enc_tkt_part>() as libc::c_ulong) as
            *mut krb5_enc_tkt_part;
    if tempto.is_null() { return 12 as libc::c_int }
    *tempto = *partfrom;
    retval =
        krb5_copy_keyblock(context, (*partfrom).session,
                           &mut (*tempto).session);
    if retval != 0 { free(tempto as *mut libc::c_void); return retval }
    retval =
        krb5_copy_principal(context,
                            (*partfrom).client as krb5_const_principal,
                            &mut (*tempto).client);
    if retval != 0 {
        krb5_free_keyblock(context, (*tempto).session);
        free(tempto as *mut libc::c_void);
        return retval
    }
    (*tempto).transited = (*partfrom).transited;
    if (*tempto).transited.tr_contents.length ==
           0 as libc::c_int as libc::c_uint {
        (*tempto).transited.tr_contents.data = 0 as *mut libc::c_char
    } else {
        (*tempto).transited.tr_contents.data =
            k5memdup((*partfrom).transited.tr_contents.data as
                         *const libc::c_void,
                     (*partfrom).transited.tr_contents.length as size_t,
                     &mut retval) as *mut libc::c_char;
        if (*tempto).transited.tr_contents.data.is_null() {
    ...
    retval =
        krb5_copy_addresses(context, (*partfrom).caddrs,
                            &mut (*tempto).caddrs);
    if retval != 0 {
        free((*tempto).transited.tr_contents.data as *mut libc::c_void);
        krb5_free_principal(context, (*tempto).client);
        krb5_free_keyblock(context, (*tempto).session);
        free(tempto as *mut libc::c_void);
        return retval
    }
    if !(*partfrom).authorization_data.is_null() {
        retval =
            krb5_copy_authdata(context, (*partfrom).authorization_data,
                               &mut (*tempto).authorization_data);
        if retval != 0 {
...
#[no_mangle]
#[c2rust::src_loc = "90:1"]
pub unsafe extern "C" fn krb5_copy_ticket(mut context: krb5_context,
                                          mut from: *const krb5_ticket,
                                          mut pto: *mut *mut krb5_ticket)
 -> krb5_error_code {
    let mut retval: krb5_error_code = 0;
    let mut tempto: *mut krb5_ticket = 0 as *mut krb5_ticket;
    let mut scratch: *mut krb5_data = 0 as *mut krb5_data;
    tempto =
        malloc(::std::mem::size_of::<krb5_ticket>() as libc::c_ulong) as
            *mut krb5_ticket;
    if tempto.is_null() { return 12 as libc::c_int }
    *tempto = *from;
    retval =
        krb5_copy_principal(context, (*from).server as krb5_const_principal,
                            &mut (*tempto).server);
    if retval != 0 { free(tempto as *mut libc::c_void); return retval }
    retval =
        krb5_copy_data(context, &(*from).enc_part.ciphertext, &mut scratch);
    if retval != 0 {
    ...
    retval =
        copy_enc_tkt_part(context, (*from).enc_part2,
                          &mut (*tempto).enc_part2);
    if retval != 0 {
        free((*tempto).enc_part.ciphertext.data as *mut libc::c_void);
        krb5_free_principal(context, (*tempto).server);
        free(tempto as *mut libc::c_void);
        return retval
    }
