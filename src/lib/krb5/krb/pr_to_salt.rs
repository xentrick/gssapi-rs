...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _krb5_os_context, alloc_data, empty_data, make_data,
 ...
 */
#[c2rust::src_loc = "36:1"]
unsafe extern "C" fn principal2salt_internal(mut context: krb5_context,
                                             mut pr: krb5_const_principal,
                                             mut ret: *mut krb5_data,
                                             mut use_realm: libc::c_int)
 -> krb5_error_code {
    ...
    *ret = empty_data();
    if pr.is_null() { return 0 as libc::c_int }
    if use_realm != 0 { size = size.wrapping_add((*pr).realm.length) }
    i = 0 as libc::c_int;
    while i < (*pr).length {
        size = size.wrapping_add((*(*pr).data.offset(i as isize)).length);
        i += 1
    }
    if alloc_data(ret, size) != 0 { return 12 as libc::c_int }
    if use_realm != 0 {
        offset = (*pr).realm.length;
        if offset > 0 as libc::c_int as libc::c_uint {
            memcpy((*ret).data as *mut libc::c_void,
                   (*pr).realm.data as *const libc::c_void,
                   offset as libc::c_ulong);
        }
    }
    i = 0 as libc::c_int;
    while i < (*pr).length {
        if (*(*pr).data.offset(i as isize)).length >
               0 as libc::c_int as libc::c_uint {
            memcpy(&mut *(*ret).data.offset(offset as isize) as
                       *mut libc::c_char as *mut libc::c_void,
                   (*(*pr).data.offset(i as isize)).data as
                       *const libc::c_void,
                   (*(*pr).data.offset(i as isize)).length as libc::c_ulong);
        }
        offset = offset.wrapping_add((*(*pr).data.offset(i as isize)).length);
        i += 1
    }
    return 0 as libc::c_int;
}
#[no_mangle]
#[c2rust::src_loc = "70:1"]
pub unsafe extern "C" fn krb5_principal2salt(mut context: krb5_context,
                                             mut pr: krb5_const_principal,
                                             mut ret: *mut krb5_data)
 -> krb5_error_code {
    return principal2salt_internal(context, pr, ret, 1 as libc::c_int);
}
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
/*
...
#[no_mangle]
#[c2rust::src_loc = "77:1"]
pub unsafe extern "C" fn krb5_principal2salt_norealm(mut context:
                                                         krb5_context,
                                                     mut pr:
                                                         krb5_const_principal,
                                                     mut ret: *mut krb5_data)
 -> krb5_error_code {
    return principal2salt_internal(context, pr, ret, 0 as libc::c_int);
}
