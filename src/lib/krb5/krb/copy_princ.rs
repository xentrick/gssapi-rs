...
#[no_mangle]
#[c2rust::src_loc = "32:1"]
pub unsafe extern "C" fn krb5_copy_principal(mut context: krb5_context,
                                             mut inprinc:
                                                 krb5_const_principal,
                                             mut outprinc:
                                                 *mut krb5_principal)
 -> krb5_error_code {
    let mut tempprinc: krb5_principal = 0 as *mut krb5_principal_data;
    let mut i: krb5_int32 = 0;
    tempprinc =
        malloc(::std::mem::size_of::<krb5_principal_data>() as libc::c_ulong)
            as krb5_principal;
    if tempprinc.is_null() { return 12 as libc::c_int }
    *tempprinc = *inprinc;
    (*tempprinc).data =
        malloc(((*inprinc).length as
                    libc::c_ulong).wrapping_mul(::std::mem::size_of::<krb5_data>()
                                                    as libc::c_ulong)) as
            *mut krb5_data;
    if (*tempprinc).data.is_null() {
        free(tempprinc as *mut libc::c_void);
        return 12 as libc::c_int
    }
    i = 0 as libc::c_int;
    while i < (*inprinc).length {
        if krb5int_copy_data_contents(context,
                                      &mut *(*inprinc).data.offset(i as
                                                                       isize),
                                      &mut *(*tempprinc).data.offset(i as
                                                                         isize))
               != 0 as libc::c_int {
            loop  {
                i -= 1;
                if !(i >= 0 as libc::c_int) { break ; }
                free((*(*tempprinc).data.offset(i as isize)).data as
                         *mut libc::c_void);
            }
            free((*tempprinc).data as *mut libc::c_void);
            free(tempprinc as *mut libc::c_void);
            return 12 as libc::c_int
        }
        i += 1
