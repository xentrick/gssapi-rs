...
#[no_mangle]
#[c2rust::src_loc = "37:1"]
pub unsafe extern "C" fn krb5_address_order(mut context: krb5_context,
                                            mut addr1: *const krb5_address,
                                            mut addr2: *const krb5_address)
 -> libc::c_int {
    let mut dir: libc::c_int = 0;
    let mut i: libc::c_int = 0;
    let minlen: libc::c_int =
        if (*addr1).length < (*addr2).length {
            (*addr1).length
        } else { (*addr2).length } as libc::c_int;
    if (*addr1).addrtype != (*addr2).addrtype { return 0 as libc::c_int }
    dir = (*addr1).length.wrapping_sub((*addr2).length) as libc::c_int;
    i = 0 as libc::c_int;
    while i < minlen {
        if (*(*addr1).contents.offset(i as isize) as libc::c_int) <
               *(*addr2).contents.offset(i as isize) as libc::c_int {
