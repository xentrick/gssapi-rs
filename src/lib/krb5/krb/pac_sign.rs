...
pub mod k5_int_h {
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_error_code, krb5_context, krb5_cksumtype};
...
pub mod authdata_h {
    ...
    use super::krb5_h::{krb5_data, krb5_boolean, krb5_ui_4, krb5_context,
                        krb5_pac, krb5_timestamp, krb5_principal_data,
 ...
 */
        ...
        #[no_mangle]
        #[c2rust::src_loc = "89:1"]
        pub fn k5_pac_validate_client(context: krb5_context, pac: krb5_pac,
        ...
        #[no_mangle]
        #[c2rust::src_loc = "83:1"]
        pub fn k5_pac_locate_buffer(context: krb5_context, pac: krb5_pac,
                                    type_0: krb5_ui_4, data: *mut krb5_data)
         -> krb5_error_code;
        #[no_mangle]
        #[c2rust::src_loc = "96:1"]
        pub fn k5_pac_add_buffer(context: krb5_context, pac: krb5_pac,
                                 type_0: krb5_ui_4, data: *const krb5_data,
                                 zerofill: krb5_boolean,
                                 out_data: *mut krb5_data) -> krb5_error_code;
        #[no_mangle]
        #[c2rust::src_loc = "104:1"]
        pub fn k5_seconds_since_1970_to_time(elapsedSeconds: krb5_timestamp,
                                             ntTime: *mut uint64_t)
         -> krb5_error_code;
    }
    /* !KRB_AUTHDATA_H */
 ...
 */
        #[no_mangle]
        #[c2rust::src_loc = "96:1"]
        pub fn k5_utf8_to_utf16le(utf8: *const libc::c_char,
                                  utf16_out: *mut *mut uint8_t,
                                  nbytes_out: *mut size_t) -> libc::c_int;
    }
    /* K5_UTF8_H */
    /*
...
pub use self::k5_platform_h::{C2RustUnnamed, C2RustUnnamed_0, C2RustUnnamed_1,
                              store_16_le, store_32_le, store_64_le};
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_ui_4, krb5_boolean,
                       krb5_enctype, krb5_cksumtype, krb5_keyusage,
                       krb5_cryptotype, krb5_flags, krb5_timestamp,
                       ...
                       _krb5_crypto_iov, krb5_crypto_iov, krb5_pac,
                       _profile_t, krb5_c_checksum_length,
                       krb5_c_make_checksum_iov, krb5_unparse_name_flags,
...
#[c2rust::src_loc = "32:1"]
unsafe extern "C" fn k5_insert_client_info(mut context: krb5_context,
                                           mut pac: krb5_pac,
 ...
 -> krb5_error_code {
    ...
    let mut princ_name_utf8: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut princ_name_utf16: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut p: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut princ_name_utf16_len: size_t = 0 as libc::c_int as size_t;
    let mut nt_authtime: uint64_t = 0;
    let mut flags: libc::c_int = 0 as libc::c_int;
    /* If we already have a CLIENT_INFO buffer, then just validate it */
    if k5_pac_locate_buffer(context, pac, 10 as libc::c_int as krb5_ui_4,
                            &mut client_info) == 0 as libc::c_int {
        return k5_pac_validate_client(context, pac, authtime, principal,
                                      with_realm)
    }
    if with_realm == 0 {
        flags |= 0x2 as libc::c_int
    } else if (*principal).type_0 == 10 as libc::c_int {
        /* Avoid quoting the first @ sign for enterprise name with realm. */
        flags |= 0x4 as libc::c_int
    }
    ret =
        krb5_unparse_name_flags(context, principal, flags,
                                &mut princ_name_utf8);
    if !(ret != 0 as libc::c_int) {
        ret =
            k5_utf8_to_utf16le(princ_name_utf8, &mut princ_name_utf16,
                               &mut princ_name_utf16_len);
        if !(ret != 0 as libc::c_int) {
            client_info.length =
                (10 as libc::c_uint as
                     libc::c_ulong).wrapping_add(princ_name_utf16_len) as
                    libc::c_uint;
            client_info.data = 0 as *mut libc::c_char;
            ret =
                k5_pac_add_buffer(context, pac,
                                  10 as libc::c_int as krb5_ui_4,
                                  &mut client_info,
            ...
            if !(ret != 0 as libc::c_int) {
                ...
                k5_seconds_since_1970_to_time(authtime, &mut nt_authtime);
                store_64_le(nt_authtime, p as *mut libc::c_void);
                p = p.offset(8 as libc::c_int as isize);
                /* copy in number of UTF-16 bytes in principal name */
                store_16_le(princ_name_utf16_len as libc::c_uint,
                            p as *mut libc::c_void);
                p = p.offset(2 as libc::c_int as isize);
                /* copy in principal name */
                memcpy(p as *mut libc::c_void,
                       princ_name_utf16 as *const libc::c_void,
                       princ_name_utf16_len);
            }
        }
    }
    if !princ_name_utf16.is_null() {
        free(princ_name_utf16 as *mut libc::c_void);
    }
    krb5_free_unparsed_name(context, princ_name_utf8);
    return ret;
}
#[c2rust::src_loc = "100:1"]
unsafe extern "C" fn k5_insert_checksum(mut context: krb5_context,
                                        mut pac: krb5_pac,
 ...
 -> krb5_error_code {
    ...
    let mut cksumdata: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    ret = krb5int_c_mandatory_cksumtype(context, (*key).enctype, cksumtype);
    if ret != 0 as libc::c_int { return ret }
    ret = krb5_c_checksum_length(context, *cksumtype, &mut len);
    if ret != 0 as libc::c_int { return ret }
    ret = k5_pac_locate_buffer(context, pac, type_0, &mut cksumdata);
    if ret == 0 as libc::c_int {
        /*
         * If we're resigning PAC, make sure we can fit checksum
         * into existing buffer
         */
        if cksumdata.length as libc::c_ulong !=
               (4 as libc::c_uint as libc::c_ulong).wrapping_add(len) {
            return 34 as libc::c_int
        }
        memset(cksumdata.data as *mut libc::c_void, 0 as libc::c_int,
               cksumdata.length as libc::c_ulong);
    } else {
        /* Add a zero filled buffer */
        cksumdata.length =
            (4 as libc::c_uint as libc::c_ulong).wrapping_add(len) as
                libc::c_uint;
        cksumdata.data = 0 as *mut libc::c_char;
        ret =
            k5_pac_add_buffer(context, pac, type_0, &mut cksumdata,
                              1 as libc::c_int as krb5_boolean,
                              &mut cksumdata);
        if ret != 0 as libc::c_int { return ret }
    }
    /* Encode checksum type into buffer */
    store_32_le(*cksumtype as krb5_ui_4, cksumdata.data as *mut libc::c_void);
    return 0 as libc::c_int;
}
/* in-place encoding of PAC header */
#[c2rust::src_loc = "148:1"]
unsafe extern "C" fn k5_pac_encode_header(mut context: krb5_context,
                                          mut pac: krb5_pac)
 -> krb5_error_code {
    let mut i: size_t = 0;
    let mut p: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    ...
    header_len =
        (8 as
             libc::c_uint).wrapping_add((*(*pac).pac).cBuffers.wrapping_mul(16
                                                                                ...
                                                                                libc::c_uint))
            as size_t;
    if (*pac).data.length as libc::c_ulong >= header_len {
    } else {
        __assert_fail(b"pac->data.length >= header_len\x00" as *const u8 as
                          *const libc::c_char,
                      b"pac_sign.c\x00" as *const u8 as *const libc::c_char,
                      157 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 61],
                                                &[libc::c_char; 61]>(b"krb5_error_code k5_pac_encode_header(krb5_context, krb5_pac)\x00")).as_ptr());
    }
    p = (*pac).data.data as *mut libc::c_uchar;
    store_32_le((*(*pac).pac).cBuffers, p as *mut libc::c_void);
    p = p.offset(4 as libc::c_int as isize);
    store_32_le((*(*pac).pac).Version, p as *mut libc::c_void);
    p = p.offset(4 as libc::c_int as isize);
    ...
    while i < (*(*pac).pac).cBuffers as libc::c_ulong {
        let mut buffer: *mut PAC_INFO_BUFFER =
            &mut *(*(*pac).pac).Buffers.as_mut_ptr().offset(i as isize) as
                *mut PAC_INFO_BUFFER;
        store_32_le((*buffer).ulType, p as *mut libc::c_void);
        p = p.offset(4 as libc::c_int as isize);
        store_32_le((*buffer).cbBufferSize, p as *mut libc::c_void);
        p = p.offset(4 as libc::c_int as isize);
        store_64_le((*buffer).Offset, p as *mut libc::c_void);
        p = p.offset(8 as libc::c_int as isize);
        if (*buffer).Offset.wrapping_rem(8 as libc::c_int as libc::c_ulong) ==
               0 as libc::c_int as libc::c_ulong {
        } else {
            __assert_fail(b"(buffer->Offset % PAC_ALIGNMENT) == 0\x00" as
                              *const u8 as *const libc::c_char,
                          b"pac_sign.c\x00" as *const u8 as
                              *const libc::c_char,
                          176 as libc::c_int as libc::c_uint,
                          (*::std::mem::transmute::<&[u8; 61],
                                                    &[libc::c_char; 61]>(b"krb5_error_code k5_pac_encode_header(krb5_context, krb5_pac)\x00")).as_ptr());
        }
        if (*buffer).Offset.wrapping_add((*buffer).cbBufferSize as
                                             libc::c_ulong) <=
        ...
        if (*buffer).Offset >= header_len {
            ...
            __assert_fail(b"buffer->Offset >= header_len\x00" as *const u8 as
                              *const libc::c_char,
                          b"pac_sign.c\x00" as *const u8 as
                              *const libc::c_char,
...
pub unsafe extern "C" fn krb5_pac_sign(mut context: krb5_context,
                                       mut pac: krb5_pac,
                                       mut authtime: krb5_timestamp,
                                       mut principal: krb5_const_principal,
                                       mut server_key: *const krb5_keyblock,
                                       mut privsvr_key: *const krb5_keyblock,
                                       mut data: *mut krb5_data)
 -> krb5_error_code {
    return krb5_pac_sign_ext(context, pac, authtime, principal, server_key,
                             privsvr_key, 0 as libc::c_int as krb5_boolean,
                             data);
}
/* *
...
pub unsafe extern "C" fn krb5_pac_sign_ext(mut context: krb5_context,
                                           mut pac: krb5_pac,
                                           mut authtime: krb5_timestamp,
                                           mut principal:
                                               krb5_const_principal,
 ...
 -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut server_cksum: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut privsvr_cksum: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut server_cksumtype: krb5_cksumtype = 0;
    let mut privsvr_cksumtype: krb5_cksumtype = 0;
    let mut iov: [krb5_crypto_iov; 2] =
        [krb5_crypto_iov{flags: 0,
                         data:
                             krb5_data{magic: 0,
                                       length: 0,
                                       data: 0 as *mut libc::c_char,},}; 2];
    (*data).length = 0 as libc::c_int as libc::c_uint;
    (*data).data = 0 as *mut libc::c_char;
    if !principal.is_null() {
        ret =
            k5_insert_client_info(context, pac, authtime, principal,
                                  with_realm);
        if ret != 0 as libc::c_int { return ret }
    }
    /* Create zeroed buffers for both checksums */
    ret =
        k5_insert_checksum(context, pac, 6 as libc::c_int as krb5_ui_4,
                           server_key, &mut server_cksumtype);
    if ret != 0 as libc::c_int { return ret }
    ret =
        k5_insert_checksum(context, pac, 7 as libc::c_int as krb5_ui_4,
                           privsvr_key, &mut privsvr_cksumtype);
    if ret != 0 as libc::c_int { return ret }
    ...
    ret =
        k5_pac_locate_buffer(context, pac, 6 as libc::c_int as krb5_ui_4,
                             &mut server_cksum);
    if ret != 0 as libc::c_int { return ret }
    if server_cksum.length > 4 as libc::c_uint {
    } else {
        __assert_fail(b"server_cksum.length > PAC_SIGNATURE_DATA_LENGTH\x00"
                          as *const u8 as *const libc::c_char,
                      b"pac_sign.c\x00" as *const u8 as *const libc::c_char,
                      242 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 169],
                                                &[libc::c_char; 169]>(b"krb5_error_code krb5_pac_sign_ext(krb5_context, krb5_pac, krb5_timestamp, krb5_const_principal, const krb5_keyblock *, const krb5_keyblock *, krb5_boolean, krb5_data *)\x00")).as_ptr());
    }
    iov[0 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[0 as libc::c_int as usize].data = (*pac).data;
    iov[1 as libc::c_int as usize].flags = 6 as libc::c_int;
    iov[1 as libc::c_int as usize].data.data =
        server_cksum.data.offset(4 as libc::c_uint as isize);
    iov[1 as libc::c_int as usize].data.length =
        server_cksum.length.wrapping_sub(4 as libc::c_uint);
    ret =
        krb5_c_make_checksum_iov(context, server_cksumtype, server_key,
                                 17 as libc::c_int, iov.as_mut_ptr(),
                                 (::std::mem::size_of::<[krb5_crypto_iov; 2]>()
                                      as
                                      libc::c_ulong).wrapping_div(::std::mem::size_of::<krb5_crypto_iov>()
                                                                      as
                                                                      libc::c_ulong));
    if ret != 0 as libc::c_int { return ret }
    ...
    ret =
        k5_pac_locate_buffer(context, pac, 7 as libc::c_int as krb5_ui_4,
                             &mut privsvr_cksum);
    if ret != 0 as libc::c_int { return ret }
    if privsvr_cksum.length > 4 as libc::c_uint {
    } else {
        __assert_fail(b"privsvr_cksum.length > PAC_SIGNATURE_DATA_LENGTH\x00"
                          as *const u8 as *const libc::c_char,
                      b"pac_sign.c\x00" as *const u8 as *const libc::c_char,
    ...
    iov[0 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[0 as libc::c_int as usize].data.data =
        server_cksum.data.offset(4 as libc::c_uint as isize);
    iov[0 as libc::c_int as usize].data.length =
        server_cksum.length.wrapping_sub(4 as libc::c_uint);
    iov[1 as libc::c_int as usize].flags = 6 as libc::c_int;
    iov[1 as libc::c_int as usize].data.data =
        privsvr_cksum.data.offset(4 as libc::c_uint as isize);
    iov[1 as libc::c_int as usize].data.length =
        privsvr_cksum.length.wrapping_sub(4 as libc::c_uint);
    ret =
        krb5_c_make_checksum_iov(context, privsvr_cksumtype, privsvr_key,
                                 17 as libc::c_int, iov.as_mut_ptr(),
                                 (::std::mem::size_of::<[krb5_crypto_iov; 2]>()
                                      as
                                      libc::c_ulong).wrapping_div(::std::mem::size_of::<krb5_crypto_iov>()
    ...
    memset((*pac).data.data as *mut libc::c_void, 0 as libc::c_int,
           (8 as
                libc::c_uint).wrapping_add((*(*pac).pac).cBuffers.wrapping_mul(16
                                                                                   as
                                                                                   libc::c_int
