...
#[no_mangle]
#[c2rust::src_loc = "37:1"]
pub unsafe extern "C" fn krb5_copy_data(mut context: krb5_context,
                                        mut indata: *const krb5_data,
                                        mut outdata: *mut *mut krb5_data)
 -> krb5_error_code {
    let mut tempdata: *mut krb5_data = 0 as *mut krb5_data;
    let mut retval: krb5_error_code = 0;
    if indata.is_null() {
        *outdata = 0 as *mut krb5_data;
        return 0 as libc::c_int
    }
    tempdata =
        malloc(::std::mem::size_of::<krb5_data>() as libc::c_ulong) as
            *mut krb5_data;
    if tempdata.is_null() { return 12 as libc::c_int }
    retval = krb5int_copy_data_contents(context, indata, tempdata);
    if retval != 0 { free(tempdata as *mut libc::c_void); return retval }
    *outdata = tempdata;
    return 0 as libc::c_int;
}
#[no_mangle]
#[c2rust::src_loc = "61:1"]
pub unsafe extern "C" fn krb5int_copy_data_contents(mut context: krb5_context,
                                                    mut indata:
                                                        *const krb5_data,
                                                    mut outdata:
                                                        *mut krb5_data)
 -> krb5_error_code {
    if indata.is_null() { return 22 as libc::c_int }
    (*outdata).length = (*indata).length;
    if (*outdata).length != 0 {
        (*outdata).data =
            malloc((*outdata).length as libc::c_ulong) as *mut libc::c_char;
        if (*outdata).data.is_null() { return 12 as libc::c_int }
        memcpy((*outdata).data as *mut libc::c_void,
               (*indata).data as *const libc::c_void,
               (*outdata).length as libc::c_ulong);
    } else { (*outdata).data = 0 as *mut libc::c_char }
    (*outdata).magic = -(1760647422 as libc::c_long) as krb5_magic;
    return 0 as libc::c_int;
}
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
/*
...
#[no_mangle]
#[c2rust::src_loc = "84:1"]
pub unsafe extern "C" fn krb5int_copy_data_contents_add0(mut context:
                                                             krb5_context,
                                                         mut indata:
                                                             *const krb5_data,
                                                         mut outdata:
 ...
 -> krb5_error_code {
    ...
    (*outdata).length = (*indata).length;
    (*outdata).data =
        malloc((*outdata).length.wrapping_add(1 as libc::c_int as
                                                  libc::c_uint) as
    ...
    *(*outdata).data.offset((*outdata).length as isize) =
