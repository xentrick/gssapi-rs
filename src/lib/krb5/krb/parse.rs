...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _krb5_os_context, empty_data, make_data, string2data,
...
use self::stdlib_h::{calloc, realloc};
 ...
 */
#[c2rust::src_loc = "35:1"]
unsafe extern "C" fn allocate_princ(mut context: krb5_context,
                                    mut name: *const libc::c_char,
                                    mut enterprise: krb5_boolean,
                                    mut princ_out: *mut krb5_principal,
                                    mut has_realm_out: *mut krb5_boolean)
 -> krb5_error_code {
    let mut current_block: u64;
    let mut ret: krb5_error_code = 0;
    let mut p: *const libc::c_char = 0 as *const libc::c_char;
    let mut first_at: krb5_boolean = 1 as libc::c_int as krb5_boolean;
    let mut princ: krb5_principal = 0 as krb5_principal;
    let mut cur_data: *mut krb5_data = 0 as *mut krb5_data;
    let mut new_comps: *mut krb5_data = 0 as *mut krb5_data;
    let mut i: krb5_int32 = 0;
    *princ_out = 0 as krb5_principal;
    *has_realm_out = 0 as libc::c_int as krb5_boolean;
    ...
    if !princ.is_null() {
        ...
        if !(*princ).data.is_null() {
            ...
            *(*princ).data.offset(0 as libc::c_int as isize) = empty_data();
            (*princ).length = 1 as libc::c_int;
            cur_data =
            ...
            loop  {
                ...
                if *p as libc::c_int == '/' as i32 && enterprise == 0 {
                    /* Component separator (for non-enterprise principals).  We
             * shouldn't see this in the realm name. */
                    if cur_data == &mut (*princ).realm as *mut krb5_data {
                        ret =
                            -(1765328250 as libc::c_long) as krb5_error_code;
                        current_block = 992943401638000353;
                        break ;
                    } else {
                        new_comps =
                            realloc((*princ).data as *mut libc::c_void,
                                    (((*princ).length + 1 as libc::c_int) as
                        ...
                        if new_comps.is_null() {
                            ...
                            (*princ).length += 1;
                            cur_data =
                                &mut *(*princ).data.offset(((*princ).length -
                                                                1 as
                                                                    libc::c_int)
                                                               as isize) as
                                    *mut krb5_data;
             ...
             * these we see is part of the component. */
                    if cur_data == &mut (*princ).realm as *mut krb5_data {
                        ...
                        current_block = 992943401638000353;
                        break ;
                    } else { cur_data = &mut (*princ).realm }
                } else {
                    /* Component or realm character, possibly quoted.  Make note if
             * we're seeing the first '@' in an enterprise principal. */
                    (*cur_data).length = (*cur_data).length.wrapping_add(1);
                    if *p as libc::c_int == '@' as i32 && enterprise != 0 {
                        first_at = 0 as libc::c_int as krb5_boolean
                    }
     ...
     * terminators on each field. */
                {
                    ...
                    loop  {
                        ...
                        *fresh0 =
                            k5alloc((*(*princ).data.offset(i as
                                                               isize)).length.wrapping_add(1
                                                                                               as
                                                                                               libc::c_int
                                                                                               as
                                                                                               libc::c_uint)
                                        as size_t, &mut ret) as
                                *mut libc::c_char;
                        if (*(*princ).data.offset(i as isize)).data.is_null()
                           {
                            current_block = 992943401638000353;
                            break ;
                        }
                    ...
                    match current_block {
                        ...
                        _ => {
                            (*princ).realm.data =
                                k5alloc((*princ).realm.length.wrapping_add(1
                                                                               as
                                                                               libc::c_int
                                                                               as
                                                                               libc::c_uint)
                            ...
                            if !(*princ).realm.data.is_null() {
                                ...
                                *has_realm_out =
                                    (cur_data ==
                                         &mut (*princ).realm as
                                             *mut krb5_data) as libc::c_int as
                                        krb5_boolean;
...
unsafe extern "C" fn parse_name_into_princ(mut name: *const libc::c_char,
                                           mut enterprise: krb5_boolean,
                                           mut princ: krb5_principal) {
    let mut p: *const libc::c_char = 0 as *const libc::c_char;
    let mut c: libc::c_char = 0;
    let mut first_at: krb5_boolean = 1 as libc::c_int as krb5_boolean;
    let mut cur_data: *mut krb5_data = (*princ).data;
    let mut pos: libc::c_uint = 0 as libc::c_int as libc::c_uint;
    p = name;
    while *p as libc::c_int != '\u{0}' as i32 {
        if *p as libc::c_int == '/' as i32 && enterprise == 0 {
            /* Advance to the next component. */
            if pos == (*cur_data).length {
            } else {
                __assert_fail(b"pos == cur_data->length\x00" as *const u8 as
                                  *const libc::c_char,
                              b"parse.c\x00" as *const u8 as
                                  *const libc::c_char,
                              141 as libc::c_int as libc::c_uint,
                              (*::std::mem::transmute::<&[u8; 71],
                                                        &[libc::c_char; 71]>(b"void parse_name_into_princ(const char *, krb5_boolean, krb5_principal)\x00")).as_ptr());
            }
            if cur_data != &mut (*princ).realm as *mut krb5_data {
            } else {
                __assert_fail(b"cur_data != &princ->realm\x00" as *const u8 as
                                  *const libc::c_char,
                              b"parse.c\x00" as *const u8 as
                                  *const libc::c_char,
            ...
            if (cur_data.wrapping_offset_from((*princ).data) as libc::c_long +
                    1 as libc::c_int as libc::c_long) <
                   (*princ).length as libc::c_long {
            } else {
                __assert_fail(b"cur_data - princ->data + 1 < princ->length\x00"
            ...
            *(*cur_data).data.offset(fresh1 as isize) = c
        }
        p = p.offset(1)
    }
    if pos == (*cur_data).length {
        ...
        __assert_fail(b"pos == cur_data->length\x00" as *const u8 as
                          *const libc::c_char,
                      b"parse.c\x00" as *const u8 as *const libc::c_char,
...
#[no_mangle]
#[c2rust::src_loc = "174:1"]
pub unsafe extern "C" fn krb5_parse_name_flags(mut context: krb5_context,
                                               mut name: *const libc::c_char,
 ...
 -> krb5_error_code {
    ...
    let mut has_realm: krb5_boolean = 0;
    let mut enterprise: krb5_boolean =
        (flags & 0x4 as libc::c_int) as krb5_boolean;
    let mut require_realm: krb5_boolean =
        (flags & 0x2 as libc::c_int) as krb5_boolean;
    let mut no_realm: krb5_boolean =
        (flags & 0x1 as libc::c_int) as krb5_boolean;
    let mut ignore_realm: krb5_boolean =
        (flags & 0x8 as libc::c_int) as krb5_boolean;
    *principal_out = 0 as krb5_principal;
    ret =
        allocate_princ(context, name, enterprise, &mut princ, &mut has_realm);
    if !(ret != 0) {
     ...
     */
        if has_realm == 0 {
            if require_realm != 0 {
                ret = -(1765328250 as libc::c_long) as krb5_error_code;
                krb5_set_error_message(context, ret,
                                       dgettext(b"mit-krb5\x00" as *const u8
                                                    as *const libc::c_char,
                                                b"Principal %s is missing required realm\x00"
                                                    as *const u8 as
                                                    *const libc::c_char),
                                       name);
                current_block = 9268738303487145279;
            } else if no_realm == 0 && ignore_realm == 0 {
                ret = krb5_get_default_realm(context, &mut default_realm);
                if ret != 0 {
                    current_block = 9268738303487145279;
                } else {
                    krb5_free_data_contents(context, &mut (*princ).realm);
                    (*princ).realm = string2data(default_realm);
                    current_block = 16203760046146113240;
                }
            } else { current_block = 16203760046146113240; }
        } else if no_realm != 0 {
            ret = -(1765328250 as libc::c_long) as krb5_error_code;
            krb5_set_error_message(context, ret,
                                   dgettext(b"mit-krb5\x00" as *const u8 as
                                                *const libc::c_char,
                                            b"Principal %s has realm present\x00"
                                                as *const u8 as
...
pub unsafe extern "C" fn krb5_parse_name(mut context: krb5_context,
                                         mut name: *const libc::c_char,
                                         mut principal_out:
                                             *mut krb5_principal)
 -> krb5_error_code {
    return krb5_parse_name_flags(context, name, 0 as libc::c_int,
                                 principal_out);
}
