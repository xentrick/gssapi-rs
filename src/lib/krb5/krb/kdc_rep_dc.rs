...
pub mod k5_int_h {
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_data, krb5_enc_kdc_rep_part, krb5_error_code};
...
#[no_mangle]
#[c2rust::src_loc = "36:1"]
pub unsafe extern "C" fn krb5_kdc_rep_decrypt_proc(mut context: krb5_context,
                                                   mut key:
                                                       *const krb5_keyblock,
                                                   mut decryptarg:
                                                       krb5_const_pointer,
 ...
 -> krb5_error_code {
    ...
    let mut local_encpart: *mut krb5_enc_kdc_rep_part =
        0 as *mut krb5_enc_kdc_rep_part;
    let mut usage: krb5_keyusage = 0;
    if !decryptarg.is_null() {
        usage = *(decryptarg as *const krb5_keyusage)
    } else { usage = 3 as libc::c_int }
    /* set up scratch decrypt/decode area */
    scratch.length = (*dec_rep).enc_part.ciphertext.length;
    scratch.data =
        malloc((*dec_rep).enc_part.ciphertext.length as libc::c_ulong) as
            *mut libc::c_char;
    if scratch.data.is_null() { return 12 as libc::c_int }
    /*dec_rep->enc_part.enctype;*/
    retval =
        krb5_c_decrypt(context, key, usage, 0 as *const krb5_data,
                       &mut (*dec_rep).enc_part, &mut scratch);
    if retval != 0 { free(scratch.data as *mut libc::c_void); return retval }
    /* and do the decode */
    retval = decode_krb5_enc_kdc_rep_part(&mut scratch, &mut local_encpart);
