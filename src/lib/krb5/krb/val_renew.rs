...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/krb5/krb/int-proto.h:39"]
pub mod int_proto_h {
    use super::k5_int_h::_krb5_context;
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_addrtype,
                       ...
                       _profile_t, _krb5_ccache, krb5_cc_initialize,
                       krb5_cc_store_cred, krb5_cc_retrieve_cred,
                       krb5_cc_get_principal, krb5_free_creds,
                       krb5_free_principal, krb5_free_cred_contents,
                       krb5_parse_name, krb5_free_data_contents};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         krb5_get_cred_via_tkt, krb5int_copy_data_contents};
 ...
 */
#[c2rust::src_loc = "47:1"]
unsafe extern "C" fn get_new_creds(mut context: krb5_context,
 ...
 -> krb5_error_code {
    ...
    *out_creds = 0 as *mut krb5_creds;
    ...
    code =
        krb5_cc_retrieve_cred(context, ccache, 0x200 as libc::c_int, in_creds,
                              &mut old_creds);
    if code != 0 as libc::c_int { return code }
    /* Use KDC options from old credential as well as requested options. */
    kdcopt |= old_creds.ticket_flags & 0x54800000 as libc::c_int;
    /* Use the old credential to get a new credential from the KDC. */
    code =
        krb5_get_cred_via_tkt(context, &mut old_creds, kdcopt,
                              old_creds.addresses, in_creds, &mut new_creds);
    krb5_free_cred_contents(context, &mut old_creds);
    if code != 0 as libc::c_int { return code }
    *out_creds = new_creds;
    return code;
}
 ...
 */
#[c2rust::src_loc = "81:1"]
unsafe extern "C" fn gc_valrenew(mut context: krb5_context,
                                 mut ccache: krb5_ccache,
                                 mut in_creds: *mut krb5_creds,
                                 mut kdcopt: krb5_flags,
                                 mut out_creds: *mut *mut krb5_creds)
 -> krb5_error_code {
    let mut code: krb5_error_code = 0;
    let mut new_creds: *mut krb5_creds = 0 as *mut krb5_creds;
    let mut default_princ: krb5_principal = 0 as krb5_principal;
    /* Get the validated or renewed credential. */
    code = get_new_creds(context, ccache, in_creds, kdcopt, &mut new_creds);
    if !(code != 0 as libc::c_int) {
        /* Reinitialize the cache without changing its default principal. */
        code = krb5_cc_get_principal(context, ccache, &mut default_princ);
        if !(code != 0 as libc::c_int) {
...
pub unsafe extern "C" fn krb5_get_credentials_validate(mut context:
                                                           krb5_context,
                                                       mut options:
                                                           krb5_flags,
                                                       mut ccache:
                                                       ...
                                                       mut out_creds:
                                                           *mut *mut krb5_creds)
 -> krb5_error_code {
    return gc_valrenew(context, ccache, in_creds, 0x1 as libc::c_int,
                       out_creds);
}
/* * @deprecated Replaced by krb5_get_renewed_creds. */
#[no_mangle]
#[c2rust::src_loc = "124:1"]
pub unsafe extern "C" fn krb5_get_credentials_renew(mut context: krb5_context,
                                                    ...
                                                    mut out_creds:
                                                        *mut *mut krb5_creds)
 -> krb5_error_code {
    return gc_valrenew(context, ccache, in_creds, 0x2 as libc::c_int,
                       out_creds);
}
/*
 ...
 */
#[c2rust::src_loc = "136:1"]
unsafe extern "C" fn get_valrenewed_creds(mut context: krb5_context,
                                          mut out_creds: *mut krb5_creds,
                                          mut client: krb5_principal,
                                          mut ccache: krb5_ccache,
                                          mut in_tkt_service:
                                              *const libc::c_char,
                                          mut kdcopt: libc::c_int)
 -> krb5_error_code {
    ...
    if !in_tkt_service.is_null() {
        ...
        code = krb5_parse_name(context, in_tkt_service, &mut server);
        if code != 0 as libc::c_int {
            current_block = 2267890792681793740;
        } else {
            krb5_free_data_contents(context, &mut (*server).realm);
            code =
                krb5int_copy_data_contents(context, &mut (*client).realm,
                                           &mut (*server).realm);
            if code != 0 as libc::c_int {
                current_block = 2267890792681793740;
    ...
    match current_block {
        11650488183268122163 => {
            ...
            code =
                get_new_creds(context, ccache, &mut in_creds, kdcopt,
                              &mut new_creds);
            if !(code != 0 as libc::c_int) {
                /* Fill in *out_creds and free the unwanted new_creds container. */
                *out_creds = *new_creds;
                free(new_creds as *mut libc::c_void);
            }
        }
        _ => { }
    }
...
#[no_mangle]
#[c2rust::src_loc = "181:1"]
pub unsafe extern "C" fn krb5_get_validated_creds(mut context: krb5_context,
 ...
 -> krb5_error_code {
    return get_valrenewed_creds(context, creds, client, ccache,
                                in_tkt_service, 0x1 as libc::c_int);
}
/* *
...
pub unsafe extern "C" fn krb5_get_renewed_creds(mut context: krb5_context,
                                                mut creds: *mut krb5_creds,
                                                mut client: krb5_principal,
                                                mut ccache: krb5_ccache,
                                                mut in_tkt_service:
 ...
 -> krb5_error_code {
    return get_valrenewed_creds(context, creds, client, ccache,
                                in_tkt_service, 0x2 as libc::c_int);
}
