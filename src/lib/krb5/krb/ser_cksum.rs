...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_cksumtype,
                       krb5_error_code, krb5_magic, _krb5_data, krb5_data,
                       _krb5_checksum, krb5_checksum};
...
#[no_mangle]
#[c2rust::src_loc = "30:1"]
pub unsafe extern "C" fn k5_size_checksum(mut checksum: *mut krb5_checksum,
                 ...
                 libc::c_ulong).wrapping_add((::std::mem::size_of::<krb5_int32>()
                                                  ...
                                                  libc::c_ulong).wrapping_add(::std::mem::size_of::<krb5_int32>()
                                                                                  ...
                                                                                  libc::c_ulong).wrapping_add(::std::mem::size_of::<krb5_int32>()
                                                                                                                  ...
                                                                                                                  libc::c_ulong).wrapping_add(::std::mem::size_of::<krb5_int32>()
                                                                                                                                                  as
                                                                                                                                                  libc::c_ulong).wrapping_add((*checksum).length
                                                                                                                                                                                  as
                                                                                                                                                                                  size_t))
                as size_t as size_t;
...
#[no_mangle]
#[c2rust::src_loc = "55:1"]
pub unsafe extern "C" fn k5_externalize_checksum(mut checksum:
                                                     *mut krb5_checksum,
                                                 mut buffer:
                                                     *mut *mut krb5_octet,
 ...
 -> krb5_error_code {
    ...
    if !checksum.is_null() {
        ...
        if k5_size_checksum(checksum, &mut required) == 0 &&
               required <= remain {
            /* Our identifier */
            krb5_ser_pack_int32(-(1760647420 as libc::c_long) as krb5_int32,
                                &mut bp, &mut remain);
            /* Our checksum_type */
            krb5_ser_pack_int32((*checksum).checksum_type, &mut bp,
                                &mut remain);
            /* Our length */
            krb5_ser_pack_int32((*checksum).length as krb5_int32, &mut bp,
                                &mut remain);
            /* Our contents */
            krb5_ser_pack_bytes((*checksum).contents,
                                (*checksum).length as size_t, &mut bp,
                                &mut remain);
            /* Finally, our trailer */
            krb5_ser_pack_int32(-(1760647420 as libc::c_long) as krb5_int32,
                                &mut bp, &mut remain);
...
#[no_mangle]
#[c2rust::src_loc = "98:1"]
pub unsafe extern "C" fn k5_internalize_checksum(mut argp:
 ...
 -> krb5_error_code {
    ...
    if ibuf as libc::c_long == -(1760647420 as libc::c_long) {
        kret = 12 as libc::c_int;
        /* Get a checksum */
        if remain >=
               (2 as libc::c_int as
                   ...
                   checksum =
                       calloc(1 as libc::c_int as libc::c_ulong,
                              ::std::mem::size_of::<krb5_checksum>() as
                                  libc::c_ulong) as *mut krb5_checksum;
                   !checksum.is_null()
               } {
            /* Get the checksum_type */
            krb5_ser_unpack_int32(&mut ibuf, &mut bp, &mut remain);
            (*checksum).checksum_type = ibuf;
            /* Get the length */
            krb5_ser_unpack_int32(&mut ibuf, &mut bp, &mut remain);
            (*checksum).length = ibuf as libc::c_uint;
            /* Get the string */
            if ibuf == 0 ||
                   {
                       (*checksum).contents =
                           malloc(ibuf as size_t) as *mut krb5_octet;
                       (!(*checksum).contents.is_null()) &&
                           {
                               kret =
                                   krb5_ser_unpack_bytes((*checksum).contents,
                                                         ibuf as size_t,
                                                         &mut bp,
                                                         &mut remain);
                               (kret) == 0
                ...
                if kret == 0 &&
                       ibuf as libc::c_long == -(1760647420 as libc::c_long) {
                    (*checksum).magic =
            ...
            if kret != 0 {
                if !(*checksum).contents.is_null() {
                    free((*checksum).contents as *mut libc::c_void);
                }
                free(checksum as *mut libc::c_void);
            }
        }
    }
    return kret;
