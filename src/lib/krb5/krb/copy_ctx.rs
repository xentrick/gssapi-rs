...
pub use self::krb5_h::{krb5_int32, krb5_boolean, krb5_enctype, krb5_flags,
                       ...
                       krb5_free_context, krb5_get_profile};
...
#[no_mangle]
#[c2rust::src_loc = "56:1"]
pub unsafe extern "C" fn krb5_copy_context(mut ctx: krb5_context,
                                           mut nctx_out: *mut krb5_context)
 -> krb5_error_code {
    let mut current_block: u64; /* XXX */
    let mut ret: krb5_error_code = 0; /* It's OK if this fails */
    let mut nctx: krb5_context = 0 as *mut _krb5_context;
    *nctx_out = 0 as krb5_context;
    if ctx.is_null() { return 22 as libc::c_int }
    nctx =
        malloc(::std::mem::size_of::<_krb5_context>() as libc::c_ulong) as
            krb5_context;
    if nctx.is_null() { return 12 as libc::c_int }
    *nctx = *ctx;
    (*nctx).in_tkt_etypes = 0 as *mut krb5_enctype;
    (*nctx).tgs_etypes = 0 as *mut krb5_enctype;
    (*nctx).default_realm = 0 as *mut libc::c_char;
    (*nctx).profile = 0 as profile_t;
    (*nctx).dal_handle = 0 as *mut kdb5_dal_handle;
    (*nctx).prompt_types = 0 as *mut krb5_prompt_type;
    (*nctx).preauth_context = 0 as krb5_preauth_context;
    (*nctx).ccselect_handles = 0 as *mut *mut ccselect_module_handle;
    (*nctx).localauth_handles = 0 as *mut *mut localauth_module_handle;
    (*nctx).hostrealm_handles = 0 as *mut *mut hostrealm_module_handle;
    (*nctx).tls = 0 as *mut k5_tls_vtable_st;
    (*nctx).kdblog_context = 0 as *mut _kdb_log_context;
    (*nctx).trace_callback = None;
    (*nctx).trace_callback_data = 0 as *mut libc::c_void;
    (*nctx).err_fmt = 0 as *mut libc::c_char;
    if !(*ctx).err_fmt.is_null() { (*nctx).err_fmt = strdup((*ctx).err_fmt) }
    (*nctx).plugin_base_dir = 0 as *mut libc::c_char;
    (*nctx).os_context.default_ccname = 0 as *mut libc::c_char;
    memset(&mut (*nctx).libkrb5_plugins as *mut plugin_dir_handle as
               *mut libc::c_void, 0 as libc::c_int,
           ::std::mem::size_of::<plugin_dir_handle>() as libc::c_ulong);
    memset(&mut (*nctx).err as *mut errinfo as *mut libc::c_void,
           0 as libc::c_int,
           ::std::mem::size_of::<errinfo>() as libc::c_ulong);
    memset(&mut (*nctx).plugins as *mut [plugin_interface; 13] as
               *mut libc::c_void, 0 as libc::c_int,
           ::std::mem::size_of::<[plugin_interface; 13]>() as libc::c_ulong);
    ret = k5_copy_etypes((*ctx).in_tkt_etypes, &mut (*nctx).in_tkt_etypes);
    if !(ret != 0) {
        ret = k5_copy_etypes((*ctx).tgs_etypes, &mut (*nctx).tgs_etypes);
        if !(ret != 0) {
            if !(*ctx).os_context.default_ccname.is_null() {
                (*nctx).os_context.default_ccname =
                    strdup((*ctx).os_context.default_ccname);
                if (*nctx).os_context.default_ccname.is_null() {
                    ret = 12 as libc::c_int;
                    current_block = 2983046246157475477;
                } else { current_block = 8704759739624374314; }
            } else { current_block = 8704759739624374314; }
            match current_block {
                2983046246157475477 => { }
                _ => {
                    ret = krb5_get_profile(ctx, &mut (*nctx).profile);
                    if !(ret != 0) {
                        (*nctx).plugin_base_dir =
                            strdup((*ctx).plugin_base_dir);
                        if (*nctx).plugin_base_dir.is_null() {
                            ret = 12 as libc::c_int
                        }
                    }
                }
    ...
    if ret != 0 { krb5_free_context(nctx); } else { *nctx_out = nctx }
    return ret;
}
