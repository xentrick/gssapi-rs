...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       krb5_enctype, krb5_keyusage, krb5_flags, krb5_deltat,
                       ...
                       krb5_key, _krb5_enc_data, krb5_enc_data, _profile_t,
...
#[no_mangle]
#[c2rust::src_loc = "32:1"]
pub unsafe extern "C" fn k5_encrypt_keyhelper(mut context: krb5_context,
                                              mut key: krb5_key,
                                              mut usage: krb5_keyusage,
                                              mut plain: *const krb5_data,
                                              mut cipher: *mut krb5_enc_data)
 -> krb5_error_code {
    ...
    ret =
        krb5_c_encrypt_length(context, enctype, (*plain).length as size_t,
                              &mut enclen);
    if ret != 0 as libc::c_int { return ret }
    (*cipher).ciphertext.length = enclen as libc::c_uint;
    (*cipher).ciphertext.data = malloc(enclen) as *mut libc::c_char;
    if (*cipher).ciphertext.data.is_null() { return 12 as libc::c_int }
    ret =
        krb5_k_encrypt(context, key, usage, 0 as *const krb5_data, plain,
                       cipher);
    if ret != 0 {
        free((*cipher).ciphertext.data as *mut libc::c_void);
        (*cipher).ciphertext.data = 0 as *mut libc::c_char
    }
    return ret;
}
