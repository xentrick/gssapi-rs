...
pub mod k5_int_h {
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_context, krb5_principal, krb5_data,
                        krb5_error_code};
...
pub use self::krb5_h::{krb5_int32, krb5_boolean, krb5_enctype, krb5_flags,
                       ...
                       krb5_prompt_type, _profile_t, krb5_anonymous_realm};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _kdb5_dal_handle, krb5_free_realm_tree,
                         krb5_walk_realm_tree};
pub use self::k5_err_h::errinfo;
pub use self::k5_plugin_h::{plugin_dir_handle, plugin_file_handle};
pub use self::profile_h::profile_t;
use self::string_h::{memcmp, memset, memmove, memcpy};
#[derive(Copy, Clone)]
#[repr(C)]
#[c2rust::src_loc = "288:8"]
pub struct check_data {
    pub ctx: krb5_context,
    pub tgs: *mut krb5_principal,
}
#[c2rust::src_loc = "45:1"]
unsafe extern "C" fn process_intermediates(mut fn_0:
                                           ...
                                           mut n1: *const krb5_data,
                                           mut n2: *const krb5_data)
 -> krb5_error_code {
    ...
    /* Simplify...  */
    if len1 > len2 {
        let mut p: *const krb5_data = 0 as *const krb5_data;
        let mut tmp: libc::c_int = len1 as libc::c_int;
        len1 = len2;
        len2 = tmp as libc::c_uint;
        p = n1;
        n1 = n2;
        n2 = p
    }
    ...
    if len1 == len2 {
        if memcmp((*n1).data as *const libc::c_void,
                  (*n2).data as *const libc::c_void, len1 as libc::c_ulong) !=
               0 {
            return -(1765328341 as libc::c_long) as krb5_error_code
        }
        return 0 as libc::c_int
    }
    /* Now len1 is always shorter.  */
    if len1 == 0 as libc::c_int as libc::c_uint {
        /* Shouldn't be possible.  Internal error?  */
        return -(1765328341 as libc::c_long) as krb5_error_code
    }
    p1 = (*n1).data;
    p2 = (*n2).data;
    if *p1.offset(0 as libc::c_int as isize) as libc::c_int == '/' as i32 {
        /* X.500 style names, with common prefix.  */
        if *p2.offset(0 as libc::c_int as isize) as libc::c_int != '/' as i32
           {
            return -(1765328341 as libc::c_long) as krb5_error_code
        }
        if memcmp(p1 as *const libc::c_void, p2 as *const libc::c_void,
                  len1 as libc::c_ulong) != 0 {
            return -(1765328341 as libc::c_long) as krb5_error_code
        }
        i = len1.wrapping_add(1 as libc::c_int as libc::c_uint);
        while i < len2 {
            if *p2.offset(i as isize) as libc::c_int == '/' as i32 {
                let mut d: krb5_data =
                    krb5_data{magic: 0,
                              length: 0,
                ...
                d.length = i;
                r =
                    Some(fn_0.expect("non-null function pointer")).expect("non-null function pointer")(&mut d,
                                                                                                       data);
                if r != 0 { return r }
            }
            i = i.wrapping_add(1)
        ...
        if memcmp(p1 as *const libc::c_void,
                  p2.offset(len2.wrapping_sub(len1) as isize) as
                      *const libc::c_void, len1 as libc::c_ulong) != 0 {
            return -(1765328341 as libc::c_long) as krb5_error_code
        }
        i =
            len2.wrapping_sub(len1).wrapping_sub(1 as libc::c_int as
                                                     libc::c_uint);
        while i > 0 as libc::c_int as libc::c_uint {
            if *p2.offset(i.wrapping_sub(1 as libc::c_int as libc::c_uint) as
                              isize) as libc::c_int == '.' as i32 {
                let mut d_0: krb5_data =
                    krb5_data{magic: 0,
                              length: 0,
                              data: 0 as *mut libc::c_char,};
                let mut r_0: krb5_error_code = 0;
                d_0.data = p2.offset(i as isize);
                d_0.length = len2.wrapping_sub(i);
                r_0 =
                    Some(fn_0.expect("non-null function pointer")).expect("non-null function pointer")(&mut d_0,
                                                                                                       data);
                if r_0 != 0 { return r_0 }
            }
            i = i.wrapping_sub(1)
...
unsafe extern "C" fn maybe_join(mut last: *mut krb5_data,
                                mut buf: *mut krb5_data,
                                mut bufsiz: libc::c_uint) -> krb5_error_code {
    if (*buf).length == 0 as libc::c_int as libc::c_uint {
        return 0 as libc::c_int
    }
    if *(*buf).data.offset(0 as libc::c_int as isize) as libc::c_int ==
           '/' as i32 {
        if (*last).length.wrapping_add((*buf).length) > bufsiz {
            return -(1765328341 as libc::c_long) as krb5_error_code
        }
        memmove((*buf).data.offset((*last).length as isize) as
                    *mut libc::c_void, (*buf).data as *const libc::c_void,
                (*buf).length as libc::c_ulong);
        memcpy((*buf).data as *mut libc::c_void,
               (*last).data as *const libc::c_void,
               (*last).length as libc::c_ulong);
        (*buf).length = (*buf).length.wrapping_add((*last).length)
    } else if *(*buf).data.offset((*buf).length.wrapping_sub(1 as libc::c_int
                                                                 as
                                                                 libc::c_uint)
        ...
        memcpy((*buf).data.offset((*buf).length as isize) as
                   *mut libc::c_void, (*last).data as *const libc::c_void,
               (*last).length as libc::c_ulong);
        (*buf).length = (*buf).length.wrapping_add((*last).length)
    }
...
unsafe extern "C" fn foreach_realm(mut fn_0:
                                       Option<unsafe extern "C" fn(_:
                                                                       *mut krb5_data,
                                                                   _:
                                                                       *mut libc::c_void)
                                                  -> krb5_error_code>,
                                   mut data: *mut libc::c_void,
                                   mut crealm: *const krb5_data,
                                   mut srealm: *const krb5_data,
                                   mut transit: *const krb5_data)
 -> krb5_error_code {
    let mut buf: [libc::c_char; 512] = [0; 512];
    let mut last: [libc::c_char; 512] = [0; 512];
    let mut p: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut bufp: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut next_lit: libc::c_int = 0;
    let mut intermediates: libc::c_int = 0;
    let mut l: libc::c_int = 0;
    let mut this_component: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut r: krb5_error_code = 0;
    let mut last_component: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    /* Invariants:
    ...
    next_lit = 0 as libc::c_int;
    intermediates = 0 as libc::c_int;
    memset(buf.as_mut_ptr() as *mut libc::c_void, 0 as libc::c_int,
           ::std::mem::size_of::<[libc::c_char; 512]>() as libc::c_ulong);
    this_component.data = buf.as_mut_ptr();
    last_component.data = last.as_mut_ptr();
    last_component.length = 0 as libc::c_int as libc::c_uint;
    if (*transit).length == 0 as libc::c_int as libc::c_uint {
        return 0 as libc::c_int
    }
    bufp = buf.as_mut_ptr();
    p = (*transit).data;
    l = (*transit).length as libc::c_int;
    while l != 0 {
        if next_lit != 0 {
            let fresh0 = bufp;
            bufp = bufp.offset(1);
            *fresh0 = *p;
            if bufp ==
                   buf.as_mut_ptr().offset(::std::mem::size_of::<[libc::c_char; 512]>()
                                               as libc::c_ulong as isize) {
                return -(1765328341 as libc::c_long) as krb5_error_code
            }
            next_lit = 0 as libc::c_int
        } else if *p as libc::c_int == '\\' as i32 {
            next_lit = 1 as libc::c_int
        } else if *p as libc::c_int == ',' as i32 {
            if bufp != buf.as_mut_ptr() {
                this_component.length =
                    bufp.wrapping_offset_from(buf.as_mut_ptr()) as
                        libc::c_long as libc::c_uint;
                r =
                    maybe_join(&mut last_component, &mut this_component,
                               ::std::mem::size_of::<[libc::c_char; 512]>() as
                                   libc::c_ulong as libc::c_uint);
                if r != 0 { return r }
                r =
                    Some(fn_0.expect("non-null function pointer")).expect("non-null function pointer")(&mut this_component,
                                                                                                       data);
                if r != 0 { return r }
                if intermediates != 0 {
                    if p == (*transit).data {
                        r =
                            process_intermediates(fn_0, data,
                                                  &mut this_component, crealm)
                    } else {
                        r =
                            process_intermediates(fn_0, data,
                                                  &mut this_component,
                ...
                intermediates = 0 as libc::c_int;
                memcpy(last.as_mut_ptr() as *mut libc::c_void,
                ...
                intermediates = 1 as libc::c_int;
                if p == (*transit).data {
                    if (*crealm).length >= 512 as libc::c_int as libc::c_uint
                       {
                        return -(1765328341 as libc::c_long) as
                                   krb5_error_code
                    }
                    if (*crealm).length > 0 as libc::c_int as libc::c_uint {
                        memcpy(last.as_mut_ptr() as *mut libc::c_void,
                               (*crealm).data as *const libc::c_void,
                               (*crealm).length as libc::c_ulong);
                    }
                    last[(*crealm).length as usize] =
                        '\u{0}' as i32 as libc::c_char;
                    last_component.length = (*crealm).length
                }
            }
        } else if *p as libc::c_int == ' ' as i32 && bufp == buf.as_mut_ptr()
         {
            /* This next component stands alone, even if it has a
               trailing dot or leading slash.  */
            memset(last.as_mut_ptr() as *mut libc::c_void, 0 as libc::c_int,
                   ::std::mem::size_of::<[libc::c_char; 512]>() as
                       libc::c_ulong);
            last_component.length = 0 as libc::c_int as libc::c_uint
        } else {
    ...
    if bufp == buf.as_mut_ptr() {
        /* Trailing comma.  */
        r = process_intermediates(fn_0, data, &mut last_component, srealm)
    } else {
        /* Trailing component.  */
        this_component.length =
            bufp.wrapping_offset_from(buf.as_mut_ptr()) as libc::c_long as
                libc::c_uint;
        r =
            maybe_join(&mut last_component, &mut this_component,
                       ::std::mem::size_of::<[libc::c_char; 512]>() as
        ...
        if intermediates != 0 {
            r =
                process_intermediates(fn_0, data, &mut this_component,
                                      &mut last_component)
        }
    }
...
unsafe extern "C" fn check_realm_in_list(mut realm: *mut krb5_data,
                                         mut data: *mut libc::c_void)
 -> krb5_error_code {
    let mut cdata: *mut check_data = data as *mut check_data;
    let mut i: libc::c_int = 0;
    i = 0 as libc::c_int;
    while !(*(*cdata).tgs.offset(i as isize)).is_null() {
        if data_eq((**(*cdata).tgs.offset(i as isize)).realm, *realm) != 0 {
            return 0 as libc::c_int
        }
        i += 1
    }
...
pub unsafe extern "C" fn krb5_check_transited_list(mut ctx: krb5_context,
                                                   mut trans_in:
                                                       *const krb5_data,
                                                   mut crealm:
                                                       *const krb5_data,
                                                   mut srealm:
                                                       *const krb5_data)
 -> krb5_error_code {
    let mut trans: krb5_data =
        krb5_data{magic: 0,
                  length: 0,
                  data:
                      0 as
                          *mut libc::c_char,}; /* Nothing to check for anonymous */
    let mut cdata: check_data =
        check_data{ctx: 0 as *mut _krb5_context,
                   tgs: 0 as *mut krb5_principal,};
    let mut r: krb5_error_code = 0;
    let mut anonymous: *const krb5_data = 0 as *const krb5_data;
    trans.length = (*trans_in).length;
    trans.data = (*trans_in).data;
    if trans.length != 0 &&
           *trans.data.offset(trans.length.wrapping_sub(1 as libc::c_int as
                                                            libc::c_uint) as
                                  isize) as libc::c_int == '\u{0}' as i32 {
        trans.length = trans.length.wrapping_sub(1)
    }
    if trans.length == 0 as libc::c_int as libc::c_uint {
        return 0 as libc::c_int
    }
    anonymous = krb5_anonymous_realm();
    if (*crealm).length == (*anonymous).length &&
           memcmp((*crealm).data as *const libc::c_void,
                  (*anonymous).data as *const libc::c_void,
                  (*anonymous).length as libc::c_ulong) == 0 as libc::c_int {
        return 0 as libc::c_int
    }
    r = krb5_walk_realm_tree(ctx, crealm, srealm, &mut cdata.tgs, '.' as i32);
    if r != 0 { return r }
    /* avoid compiler warning about 'd' unused */
    cdata.ctx = ctx;
    r =
        foreach_realm(Some(check_realm_in_list as
                               unsafe extern "C" fn(_: *mut krb5_data,
                                                    _: *mut libc::c_void)
                                   -> krb5_error_code),
                      &mut cdata as *mut check_data as *mut libc::c_void,
                      crealm, srealm, &mut trans);
