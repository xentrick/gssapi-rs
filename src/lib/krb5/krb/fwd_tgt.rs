...
pub mod k5_int_h {
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_context, krb5_creds, krb5_address,
                        krb5_error_code, krb5_ccache};
...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/krb5/os/os-proto.h:32"]
pub mod os_proto_h {
    use super::k5_int_h::_krb5_context;
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_ui_4, krb5_boolean,
                       ...
                       krb5_free_data, krb5_mk_1cred, krb5_auth_con_getkey};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _kdb5_dal_handle, krb5_get_cred_via_tkt,
                         krb5int_cc_default};
...
#[no_mangle]
#[c2rust::src_loc = "38:1"]
pub unsafe extern "C" fn krb5_fwd_tgt_creds(mut context: krb5_context,
                                            mut auth_context:
                                                krb5_auth_context,
 ...
 /* Should forwarded TGT also be forwardable? */
 {
        ...
        krb5_creds{magic: 0,
                   ...
                   second_ticket:
                       krb5_data{magic: 0,
                                 ...
                                 data: 0 as *mut libc::c_char,},
                   authdata: 0 as *mut *mut krb5_authdata,};
    let mut pcreds: *mut krb5_creds = 0 as *mut krb5_creds;
    let mut kdcoptions: krb5_flags = 0;
    let mut defcc: krb5_ccache = 0 as krb5_ccache;
    let mut def_rhost: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut enctype: krb5_enctype = 0 as libc::c_int;
    let mut session_key: *mut krb5_keyblock = 0 as *mut krb5_keyblock;
    let mut old_use_conf_ktypes: krb5_boolean = (*context).use_conf_ktypes;
    ...
    memset(&mut tgt as *mut krb5_creds as *mut libc::c_void, 0 as libc::c_int,
           ::std::mem::size_of::<krb5_creds>() as libc::c_ulong);
    if cc.is_null() {
        retval = krb5int_cc_default(context, &mut defcc);
        if retval != 0 {
            current_block = 13714203521016113416;
        } else { cc = defcc; current_block = 5399440093318478209; }
    } else { current_block = 5399440093318478209; }
    match current_block {
        5399440093318478209 => {
            retval =
                krb5_auth_con_getkey(context, auth_context, &mut session_key);
            if !(retval != 0) {
                if !session_key.is_null() {
                    enctype = (*session_key).enctype;
                    krb5_free_keyblock(context, session_key);
                    session_key = 0 as *mut krb5_keyblock
           ...
           Not bulletproof, just a heuristic.  */
                    ...
                    memset(&mut in_0 as *mut krb5_creds as *mut libc::c_void,
                           0 as libc::c_int,
           ...
           throw them away.  */
                    ...
                    krb5_free_cred_contents(context, &mut in_0);
                }
                retval =
                    krb5_copy_principal(context,
                ...
                if !(retval != 0) {
                    retval =
                        krb5int_tgtname(context, &mut (*client).realm,
                                        &mut (*client).realm,
                                        &mut creds.server);
                    if !(retval != 0) {
                        ...
                        if !(retval != 0) {
                            ...
                            if krb5_principal_compare(context,
                                   ...
                                   == 0 {
                                retval =
                                    -(1765328238 as libc::c_long) as
                                        krb5_error_code
                            } else if tgt.ticket.length == 0 {
                                retval =
                                    -(1765328241 as libc::c_long) as
                                        krb5_error_code
                            } else {
                                if !tgt.addresses.is_null() &&
                                       !(*tgt.addresses).is_null() {
                                    if rhost.is_null() {
                                        if (*server).type_0 !=
                                               3 as libc::c_int {
                                            retval =
                                                -(1765328162 as libc::c_long)
                                                    as krb5_error_code;
                                            current_block =
                                                13714203521016113416;
                                        } else if (*server).length <
                                                      2 as libc::c_int {
                                            retval =
                                                -(1765328245 as libc::c_long)
                                                    as krb5_error_code;
                                            current_block =
                                                13714203521016113416;
                                        } else {
                                            def_rhost =
                                                k5memdup0((*(*server).data.offset(1
                                                                                      as
                                                                                      libc::c_int
                                                                                      as
                                                                                      isize)).data
                                ...
                                match current_block {
                                    ...
                                    _ => {
                                        ...
                                        kdcoptions =
                                            tgt.ticket_flags &
                                                0x54800000 as libc::c_int |
                                                0x20000000 as libc::c_int;
                                        if forwardable == 0 {
                                            /* Reset KDC_OPT_FORWARDABLE */
                                            kdcoptions &=
                                                !(0x40000000 as libc::c_int)
                                        }
                                        retval =
                                            krb5_get_cred_via_tkt(context,
                                                                  &mut tgt,
                                                                  kdcoptions,
                                                                  addrs,
                                                                  &mut creds,
    ...
    if !addrs.is_null() { krb5_free_addresses(context, addrs); }
    if !defcc.is_null() { krb5_cc_close(context, defcc); }
    free(def_rhost as *mut libc::c_void);
    krb5_free_cred_contents(context, &mut creds);
    krb5_free_cred_contents(context, &mut tgt);
    return retval;
}
