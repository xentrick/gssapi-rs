...
pub mod k5_int_h {
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_authdata, krb5_error_code, krb5_octet,
                        krb5_keyblock, krb5_checksum, krb5_principal_data,
                        krb5_principal};
    ...
    extern "C" {
        ...
        pub fn k5_size_keyblock(keyblock: *mut krb5_keyblock,
                                sizep: *mut size_t) -> krb5_error_code;
        #[no_mangle]
        #[c2rust::src_loc = "1839:1"]
        pub fn k5_size_checksum(checksum: *mut krb5_checksum,
                                sizep: *mut size_t) -> krb5_error_code;
        #[no_mangle]
        ...
        pub fn k5_externalize_checksum(checksum: *mut krb5_checksum,
                                       buffer: *mut *mut krb5_octet,
                                       lenremain: *mut size_t)
         -> krb5_error_code;
        #[no_mangle]
        ...
        pub fn k5_internalize_keyblock(argp: *mut *mut krb5_keyblock,
         ...
         -> krb5_error_code;
        #[no_mangle]
        #[c2rust::src_loc = "1846:1"]
        pub fn k5_internalize_checksum(argp: *mut *mut krb5_checksum,
                                       buffer: *mut *mut krb5_octet,
                                       lenremain: *mut size_t)
         -> krb5_error_code;
        #[no_mangle]
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_ui_4, krb5_boolean,
                       ...
                       krb5_checksum, _krb5_authdata, krb5_authdata,
                       _krb5_authenticator, krb5_authenticator, _profile_t,
...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         k5_size_authdata, k5_externalize_authdata,
                         k5_internalize_authdata, k5_size_keyblock,
                         k5_size_checksum, k5_size_principal,
                         krb5_ser_pack_int32, k5_externalize_keyblock,
                         k5_externalize_checksum, k5_externalize_principal,
                         krb5_ser_unpack_int32, k5_internalize_keyblock,
                         k5_internalize_checksum, k5_internalize_principal};
...
#[no_mangle]
#[c2rust::src_loc = "32:1"]
pub unsafe extern "C" fn k5_size_authenticator(mut authenticator:
 ...
 -> krb5_error_code {
    ...
    if !authenticator.is_null() {
        ...
        if !(*authenticator).client.is_null() {
            kret = k5_size_principal((*authenticator).client, &mut required)
        } else { kret = 0 as libc::c_int }
        /* Calculate size required by checksum, if appropriate */
        if kret == 0 && !(*authenticator).checksum.is_null() {
            kret = k5_size_checksum((*authenticator).checksum, &mut required)
        }
        /* Calculate size required by subkey, if appropriate */
        if kret == 0 && !(*authenticator).subkey.is_null() {
            kret = k5_size_keyblock((*authenticator).subkey, &mut required)
        }
        /* Calculate size required by authorization_data, if appropriate */
        if kret == 0 && !(*authenticator).authorization_data.is_null() {
            let mut i: libc::c_int = 0;
            i = 0 as libc::c_int;
            while kret == 0 &&
                      !(*(*authenticator).authorization_data.offset(i as
                                                                        isize)).is_null()
                  {
                kret =
                    k5_size_authdata(*(*authenticator).authorization_data.offset(i
                                                                                     as
                                                                                     isize),
                                     &mut required);
                i += 1
...
#[no_mangle]
#[c2rust::src_loc = "80:1"]
pub unsafe extern "C" fn k5_externalize_authenticator(mut authenticator:
                                                          *mut krb5_authenticator,
                                                      mut buffer:
                                                          *mut *mut krb5_octet,
                                                      mut lenremain:
 ...
 -> krb5_error_code {
    ...
    if !authenticator.is_null() {
        ...
        if k5_size_authenticator(authenticator, &mut required) == 0 &&
               required <= remain {
            /* First write our magic number */
            krb5_ser_pack_int32(-(1760647410 as libc::c_long) as krb5_int32,
                                &mut bp, &mut remain);
            /* Now ctime */
            krb5_ser_pack_int32((*authenticator).ctime, &mut bp, &mut remain);
            /* Now cusec */
            krb5_ser_pack_int32((*authenticator).cusec, &mut bp, &mut remain);
            /* Now seq_number */
            krb5_ser_pack_int32((*authenticator).seq_number as krb5_int32,
                                &mut bp, &mut remain);
            /* Now handle client, if appropriate */
            if !(*authenticator).client.is_null() {
                kret =
                    k5_externalize_principal((*authenticator).client, &mut bp,
                                             &mut remain)
            } else { kret = 0 as libc::c_int }
            /* Now handle checksum, if appropriate */
            if kret == 0 && !(*authenticator).checksum.is_null() {
                kret =
                    k5_externalize_checksum((*authenticator).checksum,
                                            &mut bp, &mut remain)
            }
            /* Now handle subkey, if appropriate */
            if kret == 0 && !(*authenticator).subkey.is_null() {
                kret =
                    k5_externalize_keyblock((*authenticator).subkey, &mut bp,
                                            &mut remain)
            }
            /* Now handle authorization_data, if appropriate */
            if kret == 0 {
                if !(*authenticator).authorization_data.is_null() {
                    i = 0 as libc::c_int;
                    while !(*(*authenticator).authorization_data.offset(i as
                                                                            isize)).is_null()
                          {
                        i += 1
                    }
                } else { i = 0 as libc::c_int }
                krb5_ser_pack_int32(i, &mut bp, &mut remain);
                /* Now pound out the authorization_data */
                if !(*authenticator).authorization_data.is_null() {
                    i = 0 as libc::c_int;
                    while kret == 0 &&
                              !(*(*authenticator).authorization_data.offset(i
                                                                                as
                                                                                isize)).is_null()
                          {
                        kret =
                            k5_externalize_authdata(*(*authenticator).authorization_data.offset(i
                                                                                                    as
                                                                                                    isize),
                                                    &mut bp, &mut remain);
                        i += 1
            ...
            if kret == 0 {
                /* Write our trailer */
                krb5_ser_pack_int32(-(1760647410 as libc::c_long) as
...
pub unsafe extern "C" fn k5_internalize_authenticator(mut argp:
 ...
 -> krb5_error_code {
    ...
    if ibuf as libc::c_long == -(1760647410 as libc::c_long) {
        kret = 12 as libc::c_int;
        ...
        if remain >=
                   ...
                   authenticator =
                       calloc(1 as libc::c_int as libc::c_ulong,
                              ::std::mem::size_of::<krb5_authenticator>() as
                                  libc::c_ulong) as *mut krb5_authenticator;
                   !authenticator.is_null()
               } {
            /* Get ctime */
            krb5_ser_unpack_int32(&mut ibuf, &mut bp, &mut remain);
            (*authenticator).ctime = ibuf;
            /* Get cusec */
            krb5_ser_unpack_int32(&mut ibuf, &mut bp, &mut remain);
            (*authenticator).cusec = ibuf;
            /* Get seq_number */
            krb5_ser_unpack_int32(&mut ibuf, &mut bp, &mut remain);
            (*authenticator).seq_number = ibuf as krb5_ui_4;
            kret = 0 as libc::c_int;
            /* Attempt to read in the client */
            kret =
                k5_internalize_principal(&mut (*authenticator).client,
                                         &mut bp, &mut remain);
            if kret == 22 as libc::c_int { kret = 0 as libc::c_int }
            /* Attempt to read in the checksum */
            if kret == 0 {
                kret =
                    k5_internalize_checksum(&mut (*authenticator).checksum,
                                            &mut bp, &mut remain);
                if kret == 22 as libc::c_int { kret = 0 as libc::c_int }
            }
            ...
            if kret == 0 {
                kret =
                    k5_internalize_keyblock(&mut (*authenticator).subkey,
                                            &mut bp, &mut remain);
                if kret == 22 as libc::c_int { kret = 0 as libc::c_int }
            }
            ...
            if kret == 0 {
                nadata = ibuf;
                len = (nadata + 1 as libc::c_int) as size_t;
                ...
                if !(*authenticator).authorization_data.is_null() {
                    ...
                    while kret == 0 && i < nadata {
                        kret =
                            k5_internalize_authdata(&mut *(*authenticator).authorization_data.offset(i
                                                                                                         as
                                                                                                         isize),
                                                    &mut bp, &mut remain);
                        i += 1
                    ...
                    if kret == 0 {
                        ...
                        if kret == 0 &&
                               ibuf as libc::c_long ==
                                   -(1760647410 as libc::c_long) {
                            (*authenticator).magic =
                                -(1760647410 as libc::c_long) as krb5_magic
            ...
            if kret == 0 {
                ...
                *argp = authenticator
                ...
                krb5_free_authenticator(0 as krb5_context, authenticator);
            }
        }
    }
    return kret;
