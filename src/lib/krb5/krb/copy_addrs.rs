...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_copy_addr(mut context: krb5_context,
                                        mut inad: *const krb5_address,
                                        mut outad: *mut *mut krb5_address)
 -> krb5_error_code {
    let mut tmpad: *mut krb5_address = 0 as *mut krb5_address;
    tmpad =
...
#[no_mangle]
#[c2rust::src_loc = "49:1"]
pub unsafe extern "C" fn krb5_copy_addresses(mut context: krb5_context,
                                             mut inaddr:
 ...
 -> krb5_error_code {
    ...
    let mut tempaddr: *mut *mut krb5_address = 0 as *mut *mut krb5_address;
    let mut nelems: libc::c_uint = 0 as libc::c_int as libc::c_uint;
    if inaddr.is_null() {
        *outaddr = 0 as *mut *mut krb5_address;
        return 0 as libc::c_int
    }
    while !(*inaddr.offset(nelems as isize)).is_null() {
        nelems = nelems.wrapping_add(1)
    }
    /* one more for a null terminated list */
    tempaddr =
        calloc(nelems.wrapping_add(1 as libc::c_int as libc::c_uint) as
                   libc::c_ulong,
               ::std::mem::size_of::<*mut krb5_address>() as libc::c_ulong) as
            *mut *mut krb5_address;
    if tempaddr.is_null() { return 12 as libc::c_int }
    nelems = 0 as libc::c_int as libc::c_uint;
    while !(*inaddr.offset(nelems as isize)).is_null() {
        retval =
            krb5_copy_addr(context, *inaddr.offset(nelems as isize),
                           &mut *tempaddr.offset(nelems as isize));
        if retval != 0 {
