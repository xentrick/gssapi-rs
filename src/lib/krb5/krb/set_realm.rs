...
use self::string_h::{strlen, strdup};
...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_set_principal_realm(mut context: krb5_context,
                                                  mut principal:
                                                      krb5_principal,
                                                  mut realm:
                                                      *const libc::c_char)
 -> krb5_error_code {
    ...
    let mut newrealm: *mut libc::c_char = 0 as *mut libc::c_char;
    if realm.is_null() { return 22 as libc::c_int }
    length = strlen(realm);
    newrealm = strdup(realm);
    if newrealm.is_null() { return 12 as libc::c_int }
    free((*principal).realm.data as *mut libc::c_void);
    (*principal).realm =
        make_data(newrealm as *mut libc::c_void, length as libc::c_uint);
