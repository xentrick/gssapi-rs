...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_addrtype,
                       ...
                       krb5_cc_cursor, krb5_ccache, _profile_t, _krb5_ccache,
                       krb5_cc_store_cred, krb5_cc_start_seq_get,
...
#[no_mangle]
#[c2rust::src_loc = "4:1"]
pub unsafe extern "C" fn krb5_cc_copy_creds(mut context: krb5_context,
                                            mut incc: krb5_ccache,
                                            mut outcc: krb5_ccache)
 -> krb5_error_code {
    let mut current_block: u64;
    let mut code: krb5_error_code = 0;
    let mut cur: krb5_cc_cursor = 0 as krb5_cc_cursor;
    ...
    code = krb5_cc_start_seq_get(context, incc, &mut cur);
    if code != 0 {
        current_block = 3116423069846023602;
    ...
    loop  {
        match current_block {
            3116423069846023602 => {
                ...
                if !cur.is_null() {
                    krb5_cc_end_seq_get(context, incc, &mut cur);
                }
                break ;
            }
            _ => {
                code = krb5_cc_next_cred(context, incc, &mut cur, &mut creds);
                if code == 0 {
                    code = krb5_cc_store_cred(context, outcc, &mut creds);
                    krb5_free_cred_contents(context, &mut creds);
                    if code != 0 {
                        current_block = 3116423069846023602;
                    } else { current_block = 12675440807659640239; }
                } else {
                    if code as libc::c_long != -(1765328242 as libc::c_long) {
                        current_block = 3116423069846023602;
                        continue ;
                    }
                    code = krb5_cc_end_seq_get(context, incc, &mut cur);
                    cur = 0 as krb5_cc_cursor;
                    if code != 0 {
                        current_block = 3116423069846023602;
                        continue ;
                    }
