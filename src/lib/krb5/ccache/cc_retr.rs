...
pub mod k5_int_h {
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_const_principal, krb5_error_code, krb5_data,
                        krb5_timestamp};
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_addrtype,
                       ...
                       krb5_ccache, _profile_t, krb5_cc_start_seq_get,
                       krb5_cc_next_cred, krb5_cc_end_seq_get,
                       krb5_principal_compare, krb5_free_cred_contents};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         CANONHOST_FALLBACK, CANONHOST_TRUE, CANONHOST_FALSE,
                         krb5_preauth_context, kdb5_dal_handle,
                         _krb5_os_context, data_eq, ts_after, plugin_mapping,
                         _kdb_log_context, k5_tls_vtable_st,
                         hostrealm_module_handle, localauth_module_handle,
                         ccselect_module_handle, krb5_preauth_context_st,
                         _kdb5_dal_handle, krb5_get_tgs_ktypes};
...
pub struct C2RustUnnamed {
    pub creds: krb5_creds,
    pub pref: libc::c_int,
}
#[c2rust::src_loc = "33:1"]
unsafe extern "C" fn times_match_exact(mut t1: *const krb5_ticket_times,
                                       mut t2: *const krb5_ticket_times)
 -> libc::c_int {
    return ((*t1).authtime == (*t2).authtime &&
                (*t1).starttime == (*t2).starttime &&
                (*t1).endtime == (*t2).endtime &&
                (*t1).renew_till == (*t2).renew_till) as libc::c_int;
}
#[c2rust::src_loc = "42:1"]
unsafe extern "C" fn times_match(mut t1: *const krb5_ticket_times,
                                 mut t2: *const krb5_ticket_times)
 -> krb5_boolean {
    if (*t1).renew_till != 0 {
        if ts_after((*t1).renew_till, (*t2).renew_till) != 0 {
            return 0 as libc::c_int as krb5_boolean
        }
        /* this one expires too late */
    }
    if (*t1).endtime != 0 {
        if ts_after((*t1).endtime, (*t2).endtime) != 0 {
            return 0 as libc::c_int as krb5_boolean
        }
        /* this one expires too late */
    }
...
#[c2rust::src_loc = "57:1"]
unsafe extern "C" fn princs_match(mut context: krb5_context,
                                  mut whichfields: krb5_flags,
                                  mut mcreds: *const krb5_creds,
                                  mut creds: *const krb5_creds)
 -> krb5_boolean {
    let mut princ: krb5_principal_data =
    ...
    if krb5_principal_compare(context,
                              (*mcreds).client as krb5_const_principal,
                              (*creds).client as krb5_const_principal) == 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    if whichfields & 0x40 as libc::c_int != 0 {
        /* Ignore the server realm. */
        princ = *(*mcreds).server;
        princ.realm = (*(*creds).server).realm;
        return krb5_principal_compare(context,
                                      &mut princ as *mut krb5_principal_data
...
#[c2rust::src_loc = "75:1"]
unsafe extern "C" fn authdata_match(mut mdata: *const *mut krb5_authdata,
                                    mut data: *const *mut krb5_authdata)
 -> krb5_boolean {
    let mut mdatap: *const krb5_authdata = 0 as *const krb5_authdata;
    let mut datap: *const krb5_authdata = 0 as *const krb5_authdata;
    if mdata == data { return 1 as libc::c_int as krb5_boolean }
    if mdata.is_null() {
        return (*data == 0 as *mut libc::c_void as *mut krb5_authdata) as
                   libc::c_int as krb5_boolean
    }
    if data.is_null() {
        return (*mdata == 0 as *mut libc::c_void as *mut krb5_authdata) as
                   libc::c_int as krb5_boolean
    }
    loop  {
        mdatap = *mdata;
        if !(!mdatap.is_null() && { datap = *data; !datap.is_null() }) {
            break ;
        }
        if (*mdatap).ad_type != (*datap).ad_type ||
               (*mdatap).length != (*datap).length ||
               memcmp((*mdatap).contents as *mut libc::c_char as
                          *const libc::c_void,
                      (*datap).contents as *mut libc::c_char as
                          *const libc::c_void,
                      (*mdatap).length as libc::c_ulong) != 0 as libc::c_int {
            return 0 as libc::c_int as krb5_boolean
        }
        mdata = mdata.offset(1);
        data = data.offset(1)
    }
    return ((*mdata).is_null() && (*data).is_null()) as libc::c_int as
               krb5_boolean;
}
#[c2rust::src_loc = "101:1"]
unsafe extern "C" fn data_match(mut data1: *const krb5_data,
                                mut data2: *const krb5_data) -> krb5_boolean {
    if data1.is_null() {
        if data2.is_null() {
            return 1 as libc::c_int as krb5_boolean
        } else { return 0 as libc::c_int as krb5_boolean }
    }
    if data2.is_null() { return 0 as libc::c_int as krb5_boolean }
    return if data_eq(*data1, *data2) != 0 {
               1 as libc::c_int
           } else { 0 as libc::c_int } as krb5_boolean;
}
#[c2rust::src_loc = "115:1"]
unsafe extern "C" fn pref(mut my_ktype: krb5_enctype,
                          mut nktypes: libc::c_int,
                          mut ktypes: *mut krb5_enctype) -> libc::c_int {
    let mut i: libc::c_int = 0;
    i = 0 as libc::c_int;
    while i < nktypes {
        if my_ktype == *ktypes.offset(i as isize) { return i }
        i += 1
    }
...
pub unsafe extern "C" fn krb5int_cc_creds_match_request(mut context:
                                                            krb5_context,
                                                        mut whichfields:
                                                            krb5_flags,
                                                        mut mcreds:
 ...
 -> krb5_boolean {
    ...
    if princs_match(context, whichfields, mcreds, creds) == 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    /* Only match a user-to-user credential if explicitly asked for, since the
     * ticket won't work as a regular service ticket. */
    is_skey =
        if whichfields & 0x2 as libc::c_int != 0 {
            (*mcreds).is_skey
        } else { 0 as libc::c_int as libc::c_uint };
    if (*creds).is_skey != is_skey { return 0 as libc::c_int as krb5_boolean }
    if whichfields & 0x10 as libc::c_int != 0 &&
           (*mcreds).ticket_flags != (*creds).ticket_flags {
        return 0 as libc::c_int as krb5_boolean
    }
    if whichfields & 0x4 as libc::c_int != 0 &&
           (*creds).ticket_flags & (*mcreds).ticket_flags !=
               (*mcreds).ticket_flags {
        return 0 as libc::c_int as krb5_boolean
    }
    if whichfields & 0x8 as libc::c_int != 0 &&
           times_match_exact(&mut (*mcreds).times, &mut (*creds).times) == 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    if whichfields & 0x1 as libc::c_int != 0 &&
           times_match(&mut (*mcreds).times, &mut (*creds).times) == 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    if whichfields & 0x20 as libc::c_int != 0 &&
           authdata_match((*mcreds).authdata, (*creds).authdata) == 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    if whichfields & 0x80 as libc::c_int != 0 &&
           data_match(&mut (*mcreds).second_ticket,
                      &mut (*creds).second_ticket) == 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    if whichfields & 0x100 as libc::c_int != 0 &&
...
unsafe extern "C" fn krb5_cc_retrieve_cred_seq(mut context: krb5_context,
                                               mut id: krb5_ccache,
                                               mut whichfields: krb5_flags,
 ...
 -> krb5_error_code {
    ...
    let mut nomatch_err: krb5_error_code =
        -(1765328243 as libc::c_long) as krb5_error_code;
    let mut fetched: C2RustUnnamed =
        C2RustUnnamed{creds:
                          krb5_creds{magic: 0,
    ...
    let mut best: C2RustUnnamed =
        C2RustUnnamed{creds:
                          krb5_creds{magic: 0,
                                     ...
                                     authdata: 0 as *mut *mut krb5_authdata,},
                      pref: 0,};
    let mut have_creds: libc::c_int = 0 as libc::c_int;
    kret = krb5_cc_start_seq_get(context, id, &mut cursor);
    if kret != 0 as libc::c_int { return kret }
    while krb5_cc_next_cred(context, id, &mut cursor, &mut fetched.creds) ==
              0 as libc::c_int {
        if krb5int_cc_creds_match_request(context, whichfields, mcreds,
                                          &mut fetched.creds) != 0 {
            if !ktypes.is_null() {
                fetched.pref =
                    pref(fetched.creds.keyblock.enctype, nktypes, ktypes);
                if fetched.pref < 0 as libc::c_int {
                    nomatch_err =
                        -(1765328184 as libc::c_long) as krb5_error_code
                } else if have_creds == 0 || fetched.pref < best.pref {
                    if have_creds != 0 {
                        krb5_free_cred_contents(context, &mut best.creds);
                    } else { have_creds = 1 as libc::c_int }
                    best = fetched;
                    continue ;
                }
            } else {
                krb5_cc_end_seq_get(context, id, &mut cursor);
                *creds = fetched.creds;
                return 0 as libc::c_int
            }
        }
        /* This one doesn't match */
        krb5_free_cred_contents(context, &mut fetched.creds);
    }
    ...
    if have_creds != 0 {
        *creds = best.creds;
        return 0 as libc::c_int
    } else { return nomatch_err };
}
#[no_mangle]
#[c2rust::src_loc = "254:1"]
pub unsafe extern "C" fn k5_cc_retrieve_cred_default(mut context:
                                                         krb5_context,
                                                     mut id: krb5_ccache,
    ...
    if flags & 0x200 as libc::c_int != 0 {
        ret =
            krb5_get_tgs_ktypes(context,
                                ...
                                &mut ktypes);
        if ret != 0 { return ret }
        nktypes = k5_count_etypes(ktypes) as libc::c_int;
        ret =
            krb5_cc_retrieve_cred_seq(context, id, flags, mcreds, creds,
                                      nktypes, ktypes);
