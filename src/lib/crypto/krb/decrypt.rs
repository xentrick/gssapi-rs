...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_kvno,
                       ...
                       krb5_key, _krb5_enc_data, krb5_enc_data,
...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         make_data, empty_data, k5alloc, k5calloc,
 ...
 -> krb5_error_code {
    ...
    trailer_len =
        (*ktp).crypto_length.expect("non-null function pointer")(ktp,
                                                                 5 as
                                                                     libc::c_int);
    if (*input).ciphertext.length < header_len.wrapping_add(trailer_len) {
        return -(1765328194 as libc::c_long) as krb5_error_code
    }
    plain_len =
        (*input).ciphertext.length.wrapping_sub(header_len).wrapping_sub(trailer_len);
    if (*output).length < plain_len {
        return -(1765328194 as libc::c_long) as krb5_error_code
    }
    scratch =
        k5alloc(header_len.wrapping_add(trailer_len) as size_t, &mut ret) as
            *mut libc::c_char;
    if scratch.is_null() { return ret }
    iov[0 as libc::c_int as usize].flags = 1 as libc::c_int;
    iov[0 as libc::c_int as usize].data =
        make_data(scratch as *mut libc::c_void, header_len);
    memcpy(iov[0 as libc::c_int as usize].data.data as *mut libc::c_void,
           (*input).ciphertext.data as *const libc::c_void,
           header_len as libc::c_ulong);
    iov[1 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[1 as libc::c_int as usize].data =
        make_data((*output).data as *mut libc::c_void, plain_len);
    memcpy(iov[1 as libc::c_int as usize].data.data as *mut libc::c_void,
           (*input).ciphertext.data.offset(header_len as isize) as
               *const libc::c_void, plain_len as libc::c_ulong);
    /* Use empty padding since tokens don't indicate the padding length. */
    iov[2 as libc::c_int as usize].flags = 4 as libc::c_int;
    iov[2 as libc::c_int as usize].data = empty_data();
    iov[3 as libc::c_int as usize].flags = 5 as libc::c_int;
    iov[3 as libc::c_int as usize].data =
        make_data(scratch.offset(header_len as isize) as *mut libc::c_void,
                  trailer_len);
    memcpy(iov[3 as libc::c_int as usize].data.data as *mut libc::c_void,
           (*input).ciphertext.data.offset(header_len as
                                               isize).offset(plain_len as
                                                                 isize) as
               *const libc::c_void, trailer_len as libc::c_ulong);
    ret =
        (*ktp).decrypt.expect("non-null function pointer")(ktp, key, usage,
                                                           cipher_state,
                                                           iov.as_mut_ptr(),
                                                           4 as libc::c_int as
                                                               size_t);
    if ret != 0 as libc::c_int {
        explicit_bzero((*output).data as *mut libc::c_void,
                       plain_len as size_t);
    } else { (*output).length = plain_len }
    zapfree(scratch as *mut libc::c_void,
            header_len.wrapping_add(trailer_len) as size_t);
    return ret;
}
/* *
...
#[no_mangle]
#[c2rust::src_loc = "87:1"]
pub unsafe extern "C" fn krb5_c_decrypt(mut context: krb5_context,
 ...
 -> krb5_error_code {
    ...
    let mut ret: krb5_error_code = 0;
    ret = krb5_k_create_key(context, keyblock, &mut key);
