...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5int_dk_prf(mut ktp: *const krb5_keytypes,
                                        mut key: krb5_key,
 ...
 -> krb5_error_code {
    ...
    let mut prfconst: krb5_data =
        make_data(b"prf\x00" as *const u8 as *const libc::c_char as
                      *mut libc::c_void, 3 as libc::c_int as libc::c_uint);
    let mut kp: krb5_key = 0 as krb5_key;
    let mut ret: krb5_error_code = 0;
    /* Hash the input data into an allocated buffer. */
    ret = alloc_data(&mut cksum, (*hash).hashsize as libc::c_uint);
    if !(ret != 0 as libc::c_int) {
        ...
        if !(ret != 0 as libc::c_int) {
            ...
            ret =
                krb5int_derive_key((*ktp).enc, 0 as *const krb5_hash_provider,
                                   key, &mut kp, &mut prfconst,
                                   DERIVE_RFC3961);
            if !(ret != 0 as libc::c_int) {
                /* Truncate the hash to the closest multiple of the block size. */
                iov.data.data = cksum.data;
                iov.data.length =
                    (*hash).hashsize.wrapping_div((*enc).block_size).wrapping_mul((*enc).block_size)
                        as libc::c_uint;
                /* Encrypt the truncated hash in the derived key to get the output. */
                ret =
                    (*(*ktp).enc).encrypt.expect("non-null function pointer")(kp,
                                                                              0
                                                                                  as
                                                                                  *const krb5_data,
                                                                              &mut iov,
                                                                              1
                                                                                  ...
                                                                                  size_t);
                if !(ret != 0 as libc::c_int) {
                    memcpy((*out).data as *mut libc::c_void,
    ...
    zapfree(cksum.data as *mut libc::c_void, (*hash).hashsize);
    krb5_k_free_key(0 as krb5_context, kp);
    return ret;
}
