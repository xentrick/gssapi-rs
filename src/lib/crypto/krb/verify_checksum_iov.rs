 ...
 */
    ...
    pub unsafe extern "C" fn verify_key(mut ctp: *const krb5_cksumtypes,
                                        mut key: krb5_key)
     -> krb5_error_code {
        let mut ktp: *const krb5_keytypes = 0 as *const krb5_keytypes;
        ktp =
            if !key.is_null() {
                find_enctype((*key).keyblock.enctype)
            } else { 0 as *const krb5_keytypes };
        if !(*ctp).enc.is_null() &&
               (ktp.is_null() || (*ktp).enc != (*ctp).enc) {
            return -(1765328196 as libc::c_long) as krb5_error_code
        }
        if !key.is_null() &&
               (ktp.is_null() ||
                    (*key).keyblock.length as libc::c_ulong !=
                        (*(*ktp).enc).keylength) {
...
pub use self::crypto_int_h::{krb5_cksumtypes, verify_func, checksum_func,
                             ...
                             verify_key, find_enctype, find_cksumtype,
                             krb5int_c_locate_iov, krb5int_enctypes_list,
                             krb5int_enctypes_length, krb5int_cksumtypes_list,
...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_k_verify_checksum_iov(mut context: krb5_context,
                                                    mut checksum_type:
                                                        krb5_cksumtype,
                                                    mut key: krb5_key,
 ...
 -> krb5_error_code {
    ...
    let mut computed: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut checksum: *mut krb5_crypto_iov = 0 as *mut krb5_crypto_iov;
    if checksum_type == 0 as libc::c_int {
        ret =
            krb5int_c_mandatory_cksumtype(context, (*key).keyblock.enctype,
                                          &mut checksum_type);
    ...
    if ret != 0 as libc::c_int { return ret }
    checksum =
        krb5int_c_locate_iov(data as *mut krb5_crypto_iov, num_data,
                             6 as libc::c_int);
    if checksum.is_null() || (*checksum).data.length != (*ctp).output_size {
        return -(1765328194 as libc::c_long) as krb5_error_code
    }
    /* If there's actually a verify function, call it. */
    if (*ctp).verify.is_some() {
        return (*ctp).verify.expect("non-null function pointer")(ctp, key,
                                                                 usage, data,
                                                                 num_data,
                                                                 &mut (*checksum).data,
                                                                 valid)
    }
    ret = alloc_data(&mut computed, (*ctp).compute_size);
    if ret != 0 as libc::c_int { return ret }
    ret =
        (*ctp).checksum.expect("non-null function pointer")(ctp, key, usage,
                                                            data, num_data,
                                                            &mut computed);
    if ret == 0 as libc::c_int {
        *valid =
            (k5_bcmp(computed.data as *const libc::c_void,
                     (*checksum).data.data as *const libc::c_void,
                     (*ctp).output_size as size_t) == 0 as libc::c_int) as
                libc::c_int as krb5_boolean
    }
    zapfree(computed.data as *mut libc::c_void,
            (*ctp).compute_size as size_t);
    return ret;
}
/* *
...
#[no_mangle]
#[c2rust::src_loc = "82:1"]
pub unsafe extern "C" fn krb5_c_verify_checksum_iov(mut context: krb5_context,
                                                    mut checksum_type:
                                                        krb5_cksumtype,
                                                    mut keyblock:
                                                        *const krb5_keyblock,
 ...
 -> krb5_error_code {
    ...
    if ret != 0 as libc::c_int { return ret }
    ret =
        krb5_k_verify_checksum_iov(context, checksum_type, key, usage, data,
                                   num_data, valid);
