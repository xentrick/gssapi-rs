...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5int_c_init_keyblock(mut context: krb5_context,
 ...
 -> krb5_error_code {
    ...
    if !out.is_null() {
        ...
        __assert_fail(b"out\x00" as *const u8 as *const libc::c_char,
                      b"keyblocks.c\x00" as *const u8 as *const libc::c_char,
                      35 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 94],
                                                &[libc::c_char; 94]>(b"krb5_error_code krb5int_c_init_keyblock(krb5_context, krb5_enctype, size_t, krb5_keyblock **)\x00")).as_ptr());
    }
    *out = 0 as *mut krb5_keyblock;
    kb =
        malloc(::std::mem::size_of::<krb5_keyblock>() as libc::c_ulong) as
            *mut krb5_keyblock;
    if kb.is_null() { return 12 as libc::c_int }
    (*kb).magic = -(1760647421 as libc::c_long) as krb5_magic;
    (*kb).enctype = enctype;
    (*kb).length = length as libc::c_uint;
    if length != 0 {
        (*kb).contents = malloc(length) as *mut krb5_octet;
        if (*kb).contents.is_null() {
            free(kb as *mut libc::c_void);
            return 12 as libc::c_int
        }
    } else { (*kb).contents = 0 as *mut krb5_octet }
    *out = kb;
    return 0 as libc::c_int;
}
#[no_mangle]
#[c2rust::src_loc = "58:1"]
pub unsafe extern "C" fn krb5int_c_free_keyblock(mut context: krb5_context,
                                                 mut val:
                                                     *mut krb5_keyblock) {
...
#[no_mangle]
#[c2rust::src_loc = "65:1"]
pub unsafe extern "C" fn krb5int_c_free_keyblock_contents(mut context:
                                                              krb5_context,
                                                          mut key:
                                                              *mut krb5_keyblock) {
    if !key.is_null() && !(*key).contents.is_null() {
        zapfree((*key).contents as *mut libc::c_void,
                (*key).length as size_t);
        (*key).contents = 0 as *mut krb5_octet;
        (*key).length = 0 as libc::c_int as libc::c_uint
    };
}
#[no_mangle]
#[c2rust::src_loc = "75:1"]
pub unsafe extern "C" fn krb5int_c_copy_keyblock(mut context: krb5_context,
                                                 mut from:
                                                     *const krb5_keyblock,
                                                 mut to:
                                                     *mut *mut krb5_keyblock)
 -> krb5_error_code {
    let mut new_key: *mut krb5_keyblock = 0 as *mut krb5_keyblock;
    let mut code: krb5_error_code = 0;
    *to = 0 as *mut krb5_keyblock;
    new_key =
        malloc(::std::mem::size_of::<krb5_keyblock>() as libc::c_ulong) as
            *mut krb5_keyblock;
    if new_key.is_null() { return 12 as libc::c_int }
    code = krb5int_c_copy_keyblock_contents(context, from, new_key);
    if code != 0 { free(new_key as *mut libc::c_void); return code }
    *to = new_key;
    return 0 as libc::c_int;
}
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
...
#[no_mangle]
#[c2rust::src_loc = "95:1"]
pub unsafe extern "C" fn krb5int_c_copy_keyblock_contents(mut context:
                                                              krb5_context,
                                                          mut from:
                                                              *const krb5_keyblock,
                                                          mut to:
 ...
 -> krb5_error_code {
    *to = *from;
    if (*to).length != 0 {
        (*to).contents =
            malloc((*to).length as libc::c_ulong) as *mut krb5_octet;
        if (*to).contents.is_null() { return 12 as libc::c_int }
        memcpy((*to).contents as *mut libc::c_void,
               (*from).contents as *const libc::c_void,
               (*to).length as libc::c_ulong);
    } else { (*to).contents = 0 as *mut krb5_octet }
    return 0 as libc::c_int;
}
