 ...
 */
        ...
        pub fn krb5int_c_padding_length(ktp: *const krb5_keytypes,
                                        data_length: size_t) -> libc::c_uint;
        #[no_mangle]
...
pub use self::crypto_int_h::{krb5_keytypes, prf_func, rand2key_func,
                             ...
                             find_enctype, krb5int_c_padding_length,
...
#[no_mangle]
#[c2rust::src_loc = "30:1"]
pub unsafe extern "C" fn krb5_k_encrypt(mut context: krb5_context,
                                        mut key: krb5_key,
                                        mut usage: krb5_keyusage,
                                        mut cipher_state: *const krb5_data,
                                        mut input: *const krb5_data,
                                        mut output: *mut krb5_enc_data)
 -> krb5_error_code {
    ...
    let mut padding_len: libc::c_uint = 0;
    let mut trailer_len: libc::c_uint = 0;
    let mut total_len: libc::c_uint = 0;
    ktp = find_enctype((*key).keyblock.enctype);
    if ktp.is_null() {
        return -(1765328196 as libc::c_long) as krb5_error_code
    }
    (*output).magic = -(1760647418 as libc::c_long) as krb5_magic;
    (*output).kvno = 0 as libc::c_int as krb5_kvno;
    ...
    header_len =
        (*ktp).crypto_length.expect("non-null function pointer")(ktp,
                                                                 1 as
                                                                     libc::c_int);
    padding_len = krb5int_c_padding_length(ktp, (*input).length as size_t);
    trailer_len =
        (*ktp).crypto_length.expect("non-null function pointer")(ktp,
                                                                 5 as
                                                                     libc::c_int);
    total_len =
        header_len.wrapping_add((*input).length).wrapping_add(padding_len).wrapping_add(trailer_len);
    ...
    iov[0 as libc::c_int as usize].data =
        make_data((*output).ciphertext.data as *mut libc::c_void, header_len);
    iov[1 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[1 as libc::c_int as usize].data =
        make_data((*output).ciphertext.data.offset(header_len as isize) as
                      *mut libc::c_void, (*input).length);
    if (*input).length > 0 as libc::c_int as libc::c_uint {
    ...
    iov[2 as libc::c_int as usize].data =
        make_data(iov[1 as libc::c_int as
                          usize].data.data.offset((*input).length as isize) as
                      *mut libc::c_void, padding_len);
    iov[3 as libc::c_int as usize].flags = 5 as libc::c_int;
    iov[3 as libc::c_int as usize].data =
        make_data(iov[2 as libc::c_int as
                          usize].data.data.offset(padding_len as isize) as
                      *mut libc::c_void, trailer_len);
    ret =
        (*ktp).encrypt.expect("non-null function pointer")(ktp, key, usage,
                                                           cipher_state,
                                                           iov.as_mut_ptr(),
                                                           4 as libc::c_int as
                                                               size_t);
    if ret != 0 as libc::c_int {
        explicit_bzero(iov[1 as libc::c_int as usize].data.data as
                           *mut libc::c_void,
                       iov[1 as libc::c_int as usize].data.length as size_t);
    } else { (*output).ciphertext.length = total_len }
    return ret;
}
...
#[no_mangle]
#[c2rust::src_loc = "80:1"]
pub unsafe extern "C" fn krb5_c_encrypt(mut context: krb5_context,
