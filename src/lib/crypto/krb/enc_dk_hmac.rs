...
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         ...
                         _krb5_os_context, krb5_key_st, derived_key, k5alloc,
...
pub use self::crypto_int_h::{krb5_keytypes, prf_func, rand2key_func,
                             ...
                             krb5int_hmac, krb5int_derive_key,
                             krb5int_c_locate_iov};
use self::stdlib_h::{calloc, free};
use self::string_h::{memset, memcpy};
use self::assert_h::__assert_fail;
pub use self::byteswap_h::__bswap_32;
/* 32 bit net byte order integer + one byte seed */
/* AEAD */
#[no_mangle]
#[c2rust::src_loc = "34:1"]
pub unsafe extern "C" fn krb5int_dk_crypto_length(mut ktp:
                                                      *const krb5_keytypes,
                                                  mut type_0: krb5_cryptotype)
 -> libc::c_uint {
    match type_0 {
        1 | 4 => { return (*(*ktp).enc).block_size as libc::c_uint }
        5 | 6 => { return (*(*ktp).hash).hashsize as libc::c_uint }
        _ => {
            if 0 as libc::c_int != 0 &&
                   !(b"invalid cryptotype passed to krb5int_dk_crypto_length\x00"
                         as *const u8 as *const libc::c_char).is_null() {
            } else {
                __assert_fail(b"0 && \"invalid cryptotype passed to krb5int_dk_crypto_length\"\x00"
                                  as *const u8 as *const libc::c_char,
                              b"enc_dk_hmac.c\x00" as *const u8 as
                                  *const libc::c_char,
                              45 as libc::c_int as libc::c_uint,
                              (*::std::mem::transmute::<&[u8; 85],
                                                        &[libc::c_char; 85]>(b"unsigned int krb5int_dk_crypto_length(const struct krb5_keytypes *, krb5_cryptotype)\x00")).as_ptr());
            }
...
#[no_mangle]
#[c2rust::src_loc = "50:1"]
pub unsafe extern "C" fn krb5int_aes_crypto_length(mut ktp:
                                                       *const krb5_keytypes,
                                                   mut type_0:
                                                       krb5_cryptotype)
 -> libc::c_uint {
    match type_0 {
        ...
        5 | 6 => {
            return (96 as libc::c_int / 8 as libc::c_int) as libc::c_uint
        }
        _ => {
            if 0 as libc::c_int != 0 &&
                   !(b"invalid cryptotype passed to krb5int_aes_crypto_length\x00"
                         as *const u8 as *const libc::c_char).is_null() {
            } else {
                __assert_fail(b"0 && \"invalid cryptotype passed to krb5int_aes_crypto_length\"\x00"
                                  as *const u8 as *const libc::c_char,
                              b"enc_dk_hmac.c\x00" as *const u8 as
                                  *const libc::c_char,
                              63 as libc::c_int as libc::c_uint,
                              (*::std::mem::transmute::<&[u8; 86],
                                                        &[libc::c_char; 86]>(b"unsigned int krb5int_aes_crypto_length(const struct krb5_keytypes *, krb5_cryptotype)\x00")).as_ptr());
            }
            return 0 as libc::c_int as libc::c_uint
        }
    };
...
#[no_mangle]
#[c2rust::src_loc = "68:1"]
pub unsafe extern "C" fn krb5int_dk_encrypt(mut ktp: *const krb5_keytypes,
                                            mut key: krb5_key,
 ...
 -> krb5_error_code {
    ...
    let mut constantdata: [libc::c_uchar; 5] = [0; 5];
    let mut d1: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut d2: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    ...
    if trailer.is_null() || (*trailer).data.length < hmacsize {
        return -(1765328194 as libc::c_long) as krb5_error_code
    }
    ...
    cksum = k5alloc((*hash).hashsize, &mut ret) as *mut libc::c_uchar;
    if !(ret != 0 as libc::c_int) {
        /* Derive the keys. */
        d1.data = constantdata.as_mut_ptr() as *mut libc::c_char;
        d1.length = 5 as libc::c_int as libc::c_uint;
        store_32_be(usage as libc::c_uint,
                    constantdata.as_mut_ptr() as *mut libc::c_void);
        *d1.data.offset(4 as libc::c_int as isize) =
            0xaa as libc::c_int as libc::c_char;
        ret =
            krb5int_derive_key(enc, 0 as *const krb5_hash_provider, key,
                               &mut ke, &mut d1, DERIVE_RFC3961);
        if !(ret != 0 as libc::c_int) {
            *d1.data.offset(4 as libc::c_int as isize) =
                0x55 as libc::c_int as libc::c_char;
            ret =
                krb5int_derive_key(enc, 0 as *const krb5_hash_provider, key,
                                   &mut ki, &mut d1, DERIVE_RFC3961);
            if !(ret != 0 as libc::c_int) {
                /* Generate confounder. */
                ...
                if !(ret != 0 as libc::c_int) {
                    /* Hash the plaintext. */
                    d2.length = (*hash).hashsize as libc::c_uint;
                    d2.data = cksum as *mut libc::c_char;
                    ret = krb5int_hmac(hash, ki, data, num_data, &mut d2);
                    if !(ret != 0 as libc::c_int) {
                        ...
                        if !(ret != 0 as libc::c_int) {
                            ...
                            if hmacsize <= d2.length {
                            } else {
                                __assert_fail(b"hmacsize <= d2.length\x00" as
                                                  *const u8 as
                                                  *const libc::c_char,
                                              b"enc_dk_hmac.c\x00" as
                                                  *const u8 as
                                                  *const libc::c_char,
                                              166 as libc::c_int as
                                                  libc::c_uint,
                                              (*::std::mem::transmute::<&[u8; 136],
                                                                        &[libc::c_char; 136]>(b"krb5_error_code krb5int_dk_encrypt(const struct krb5_keytypes *, krb5_key, krb5_keyusage, const krb5_data *, krb5_crypto_iov *, size_t)\x00")).as_ptr());
                            }
                            memcpy((*trailer).data.data as *mut libc::c_void,
                                   cksum as *const libc::c_void,
                                   hmacsize as libc::c_ulong);
    ...
    krb5_k_free_key(0 as krb5_context, ki);
    free(cksum as *mut libc::c_void);
    return ret;
}
...
pub unsafe extern "C" fn krb5int_dk_decrypt(mut ktp: *const krb5_keytypes,
                                            mut key: krb5_key,
                                            mut usage: krb5_keyusage,
                                            mut ivec: *const krb5_data,
                                            mut data: *mut krb5_crypto_iov,
                                            mut num_data: size_t)
 -> krb5_error_code {
    let mut enc: *const krb5_enc_provider =
        (*ktp).enc; /* enc block size, not confounder len */
    let mut hash: *const krb5_hash_provider = (*ktp).hash;
    let mut ret: krb5_error_code = 0;
    let mut constantdata: [libc::c_uchar; 5] = [0; 5];
    let mut d1: krb5_data =
    ...
    if blocksize != 0 as libc::c_int as libc::c_uint {
        ...
        if cipherlen.wrapping_rem(blocksize) !=
            ...
            *d1.data.offset(4 as libc::c_int as isize) =
            ...
            if !(ret != 0 as libc::c_int) {
                ...
                if !(ret != 0 as libc::c_int) {
                    ...
                    d1.length =
                        (*hash).hashsize as
                            libc::c_uint; /* non-truncated length */
                    d1.data = cksum as *mut libc::c_char;
                    ret = krb5int_hmac(hash, ki, data, num_data, &mut d1);
                    if !(ret != 0 as libc::c_int) {
                        /* Compare only the possibly truncated length. */
                        if k5_bcmp(cksum as *const libc::c_void,
                                   (*trailer).data.data as
                                       *const libc::c_void,
                                   hmacsize as size_t) != 0 as libc::c_int {
                            ret =
                                -(1765328353 as libc::c_long) as
                                    krb5_error_code
                        }
                    }
                }
