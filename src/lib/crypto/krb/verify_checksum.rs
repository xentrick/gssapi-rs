...
#[no_mangle]
#[c2rust::src_loc = "30:1"]
pub unsafe extern "C" fn krb5_k_verify_checksum(mut context: krb5_context,
                                                mut key: krb5_key,
                                                mut usage: krb5_keyusage,
                                                mut data: *const krb5_data,
                                                mut cksum:
                                                    *const krb5_checksum,
                                                mut valid: *mut krb5_boolean)
 -> krb5_error_code {
    ...
    let mut cksum_data: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut computed: krb5_checksum =
    ...
    if cksumtype == 0 as libc::c_int && !key.is_null() {
        ret =
    ...
    ret =
        krb5_k_make_checksum(context, (*cksum).checksum_type, key, usage,
                             data, &mut computed);
    if ret != 0 { return ret }
    *valid =
        (k5_bcmp(computed.contents as *const libc::c_void,
                 (*cksum).contents as *const libc::c_void,
                 (*ctp).output_size as size_t) == 0 as libc::c_int) as
            libc::c_int as krb5_boolean;
    free(computed.contents as *mut libc::c_void);
    return 0 as libc::c_int;
}
/* *
...
#[no_mangle]
#[c2rust::src_loc = "82:1"]
pub unsafe extern "C" fn krb5_c_verify_checksum(mut context: krb5_context,
                                                mut keyblock:
                                                    *const krb5_keyblock,
                                                mut usage: krb5_keyusage,
                                                mut data: *const krb5_data,
