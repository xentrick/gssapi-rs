...
#[no_mangle]
#[c2rust::src_loc = "30:1"]
pub unsafe extern "C" fn krb5_c_string_to_key(mut context: krb5_context,
...
#[no_mangle]
#[c2rust::src_loc = "39:1"]
pub unsafe extern "C" fn krb5_c_string_to_key_with_params(mut context:
                                                              krb5_context,
                                                          mut enctype:
                                                              krb5_enctype,
                                                          mut string:
 ...
 -> krb5_error_code {
    ...
    if salt.is_null() { salt = &mut empty }
    ...
    (*key).contents = malloc(keylength) as *mut krb5_octet;
    if (*key).contents.is_null() { return 12 as libc::c_int }
    (*key).magic = -(1760647421 as libc::c_long) as krb5_magic;
    (*key).enctype = enctype;
    (*key).length = keylength as libc::c_uint;
    ret =
        Some((*ktp).str2key.expect("non-null function pointer")).expect("non-null function pointer")(ktp,
                                                                                                     string,
                                                                                                     salt,
                                                                                                     params,
                                                                                                     key);
    if ret != 0 {
        zapfree((*key).contents as *mut libc::c_void, keylength);
