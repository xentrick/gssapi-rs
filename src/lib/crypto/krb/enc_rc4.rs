...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/include/k5-platform.h:10"]
...
pub mod k5_int_h {
    ...
    use super::krb5_h::{krb5_magic, krb5_enctype, krb5_deltat, krb5_flags,
                        ...
                        krb5_keyblock, krb5_data, krb5_key, krb5_context,
                        krb5_error_code};
    ...
    extern "C" {
        ...
        pub fn krb5int_c_copy_keyblock(context: krb5_context,
                                       from: *const krb5_keyblock,
                                       to: *mut *mut krb5_keyblock)
         -> krb5_error_code;
        #[no_mangle]
        #[c2rust::src_loc = "687:1"]
        pub fn krb5int_c_init_keyblock(_: krb5_context, enctype: krb5_enctype,
                                       length: size_t,
...
#[c2rust::header_src = "/usr/include/assert.h:10"]
pub mod assert_h {
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "67:1"]
...
pub use self::k5_platform_h::{C2RustUnnamed, store_32_le, k5_bcmp};
...
pub use self::crypto_int_h::{krb5_keytypes, prf_func, rand2key_func,
                             ...
                             krb5int_hmac_keyblock, krb5int_c_locate_iov};
use self::stdlib_h::{calloc, free};
use self::string_h::{explicit_bzero, memset, memcpy};
use self::assert_h::__assert_fail;
#[no_mangle]
#[c2rust::src_loc = "14:12"]
pub static mut l40: [libc::c_char; 10] =
    [102, 111, 114, 116, 121, 98, 105, 116, 115, 0];
#[no_mangle]
#[c2rust::src_loc = "16:1"]
pub unsafe extern "C" fn krb5int_arcfour_translate_usage(mut usage:
                                                             krb5_keyusage)
 -> krb5_keyusage {
    match usage {
        1 => { return 1 as libc::c_int }
        2 => { /* AS-REQ PA-ENC-TIMESTAMP padata timestamp,  */
            return 2 as libc::c_int
        }
        3 => { /* ticket from kdc */
            return 8 as libc::c_int
        }
        4 => { /* as-rep encrypted part */
            return 4 as libc::c_int
        }
        5 => { /* tgs-req authz data */
            return 5 as libc::c_int
        }
        6 => { /* tgs-req authz data in subkey */
            return 6 as libc::c_int
        }
        7 => { /* tgs-req authenticator cksum */
            return 7 as libc::c_int
        }
        8 => { return 8 as libc::c_int }
        9 => { /* tgs-req authenticator */
            return 9 as libc::c_int
        }
        10 => { /* tgs-rep encrypted with subkey */
            return 10 as libc::c_int
        }
        11 => { /* ap-rep authentication cksum (never used by MS) */
            return 11 as libc::c_int
        }
        12 => { /* app-req authenticator */
            return 12 as libc::c_int
        }
        23 => { /* app-rep encrypted part */
...
#[c2rust::src_loc = "38:1"]
unsafe extern "C" fn usage_key(mut enc: *const krb5_enc_provider,
                               mut hash: *const krb5_hash_provider,
                               mut session_keyblock: *const krb5_keyblock,
                               mut usage: krb5_keyusage,
                               mut out: *mut krb5_keyblock)
 -> krb5_error_code {
    let mut salt_buf: [libc::c_char; 14] = [0; 14];
    let mut salt_len: libc::c_uint = 0;
    let mut out_data: krb5_data =
        make_data((*out).contents as *mut libc::c_void, (*out).length);
    ...
    let mut ms_usage: krb5_keyusage = 0;
    /* Generate the salt. */
    ms_usage = krb5int_arcfour_translate_usage(usage);
    if (*session_keyblock).enctype == 0x18 as libc::c_int {
        memcpy(salt_buf.as_mut_ptr() as *mut libc::c_void,
               l40.as_ptr() as *const libc::c_void,
               10 as libc::c_int as libc::c_ulong);
        store_32_le(ms_usage as libc::c_uint,
                    salt_buf.as_mut_ptr().offset(10 as libc::c_int as isize)
                        as *mut libc::c_void);
        salt_len = 14 as libc::c_int as libc::c_uint
    } else {
        store_32_le(ms_usage as libc::c_uint,
                    salt_buf.as_mut_ptr() as *mut libc::c_void);
        salt_len = 4 as libc::c_int as libc::c_uint
    }
    /* Compute HMAC(key, salt) to produce the usage key. */
    iov.flags = 2 as libc::c_int;
    iov.data =
        make_data(salt_buf.as_mut_ptr() as *mut libc::c_void, salt_len);
    return krb5int_hmac_keyblock(hash, session_keyblock, &mut iov,
                                 1 as libc::c_int as size_t, &mut out_data);
}
/* Derive an encryption key from a usage key and (typically) checksum. */
#[c2rust::src_loc = "68:1"]
unsafe extern "C" fn enc_key(mut enc: *const krb5_enc_provider,
                             mut hash: *const krb5_hash_provider,
                             mut usage_keyblock: *const krb5_keyblock,
                             mut checksum: *const krb5_data,
                             mut out: *mut krb5_keyblock) -> krb5_error_code {
    let mut trunc_keyblock: *mut krb5_keyblock = 0 as *mut krb5_keyblock;
    let mut out_data: krb5_data =
        make_data((*out).contents as *mut libc::c_void, (*out).length);
    let mut iov: krb5_crypto_iov =
        krb5_crypto_iov{flags: 0,
    ...
    ret =
        krb5int_c_copy_keyblock(0 as krb5_context, usage_keyblock,
                                &mut trunc_keyblock);
    if ret != 0 as libc::c_int { return ret }
    if (*trunc_keyblock).enctype == 0x18 as libc::c_int {
        memset((*trunc_keyblock).contents.offset(7 as libc::c_int as isize) as
                   *mut libc::c_void, 0xab as libc::c_int,
    ...
    iov.data = *checksum;
    ret =
        krb5int_hmac_keyblock(hash, trunc_keyblock, &mut iov,
                              1 as libc::c_int as size_t, &mut out_data);
    krb5int_c_free_keyblock(0 as krb5_context, trunc_keyblock);
    return ret;
}
#[no_mangle]
#[c2rust::src_loc = "94:1"]
pub unsafe extern "C" fn krb5int_arcfour_crypto_length(mut ktp:
                                                           *const krb5_keytypes,
                                                       mut type_0:
                                                           krb5_cryptotype)
 -> libc::c_uint {
    match type_0 {
        1 => {
            return (*(*ktp).hash).hashsize.wrapping_add(8 as libc::c_int as
                                                            libc::c_ulong) as
                       libc::c_uint
        }
        4 | 5 => { return 0 as libc::c_int as libc::c_uint }
        6 => { return (*(*ktp).hash).hashsize as libc::c_uint }
        _ => {
            if 0 as libc::c_int != 0 &&
                   !(b"invalid cryptotype passed to krb5int_arcfour_crypto_length\x00"
                         as *const u8 as *const libc::c_char).is_null() {
            } else {
                __assert_fail(b"0 && \"invalid cryptotype passed to krb5int_arcfour_crypto_length\"\x00"
                                  as *const u8 as *const libc::c_char,
                              b"enc_rc4.c\x00" as *const u8 as
                                  *const libc::c_char,
                              108 as libc::c_int as libc::c_uint,
                              (*::std::mem::transmute::<&[u8; 90],
                                                        &[libc::c_char; 90]>(b"unsigned int krb5int_arcfour_crypto_length(const struct krb5_keytypes *, krb5_cryptotype)\x00")).as_ptr());
            }
...
unsafe extern "C" fn keyblock_crypt(mut enc: *const krb5_enc_provider,
                                    mut keyblock: *mut krb5_keyblock,
                                    mut ivec: *const krb5_data,
                                    mut data: *mut krb5_crypto_iov,
                                    mut num_data: size_t) -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut key: krb5_key = 0 as *mut krb5_key_st;
    ret = krb5_k_create_key(0 as krb5_context, keyblock, &mut key);
    if ret != 0 as libc::c_int { return ret }
    /* Works for encryption or decryption since arcfour is a stream cipher. */
    ret =
        (*enc).encrypt.expect("non-null function pointer")(key, ivec, data,
                                                           num_data);
    krb5_k_free_key(0 as krb5_context, key);
    return ret;
}
#[no_mangle]
#[c2rust::src_loc = "130:1"]
pub unsafe extern "C" fn krb5int_arcfour_encrypt(mut ktp:
                                                     *const krb5_keytypes,
                                                 mut key: krb5_key,
                                                 mut usage: krb5_keyusage,
                                                 mut ivec: *const krb5_data,
 ...
 -> krb5_error_code {
    ...
    let mut hash: *const krb5_hash_provider = (*ktp).hash;
    let mut ret: krb5_error_code = 0;
    let mut header: *mut krb5_crypto_iov = 0 as *mut krb5_crypto_iov;
    let mut trailer: *mut krb5_crypto_iov = 0 as *mut krb5_crypto_iov;
    let mut usage_keyblock: *mut krb5_keyblock = 0 as *mut krb5_keyblock;
    let mut enc_keyblock: *mut krb5_keyblock = 0 as *mut krb5_keyblock;
    let mut checksum: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut confounder: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut header_data: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut i: size_t = 0;
    /*
    ...
    if header.is_null() ||
           ((*header).data.length as libc::c_ulong) <
               (*hash).hashsize.wrapping_add(8 as libc::c_int as
                                                 libc::c_ulong) {
    ...
    if !trailer.is_null() {
        (*trailer).data.length = 0 as libc::c_int as libc::c_uint
    }
    /* Ensure that there is no padding. */
    i = 0 as libc::c_int as size_t;
    while i < num_data {
        if (*data.offset(i as isize)).flags == 4 as libc::c_int {
            (*data.offset(i as isize)).data.length =
                0 as libc::c_int as libc::c_uint
        }
        i = i.wrapping_add(1)
    }
    ret =
        krb5int_c_init_keyblock(0 as krb5_context, (*key).keyblock.enctype,
                                (*enc).keybytes, &mut usage_keyblock);
    if !(ret != 0 as libc::c_int) {
        ret =
            krb5int_c_init_keyblock(0 as krb5_context,
                                    (*key).keyblock.enctype, (*enc).keybytes,
                                    &mut enc_keyblock);
        if !(ret != 0 as libc::c_int) {
            /* Derive a usage key from the session key and usage. */
            ret =
                usage_key(enc, hash, &mut (*key).keyblock, usage,
                          usage_keyblock);
            if !(ret != 0 as libc::c_int) {
                ...
                (*header).data.length =
                    (*hash).hashsize.wrapping_add(8 as libc::c_int as
                        ...
                        libc::c_uint;
                confounder =
                    make_data((*header).data.data.offset((*hash).hashsize as
                                                             isize) as
                                  *mut libc::c_void,
                ...
                if !(ret != 0 as libc::c_int) {
                    checksum =
                        make_data((*header).data.data as *mut libc::c_void,
                                  (*hash).hashsize as libc::c_uint);
                    /* Adjust pointers so confounder is at start of header. */
                    (*header).data.length =
                        ((*header).data.length as
                             libc::c_ulong).wrapping_sub((*hash).hashsize) as
                            libc::c_uint as libc::c_uint;
                    (*header).data.data =
                        (*header).data.data.offset((*hash).hashsize as isize);
                    /* Compute the checksum using the usage key. */
                    ret =
                        krb5int_hmac_keyblock(hash, usage_keyblock, data,
                                              num_data, &mut checksum);
                    if !(ret != 0 as libc::c_int) {
                        /* Derive the encryption key from the usage key and checksum. */
                        ret =
                            enc_key(enc, hash, usage_keyblock, &mut checksum,
                                    enc_keyblock); /* Restore header pointers. */
                        if !(ret != 0) {
                            ret =
                                keyblock_crypt(enc, enc_keyblock, ivec, data,
                                               num_data)
                        }
                    }
                }
    ...
    krb5int_c_free_keyblock(0 as krb5_context, usage_keyblock);
    krb5int_c_free_keyblock(0 as krb5_context, enc_keyblock);
    return ret;
}
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
...
pub unsafe extern "C" fn krb5int_arcfour_decrypt(mut ktp:
                                                     *const krb5_keytypes,
                                                 mut key: krb5_key,
                                                 mut usage: krb5_keyusage,
                                                 mut ivec: *const krb5_data,
 ...
 -> krb5_error_code {
    ...
    let mut header_data: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut comp_checksum: krb5_data = empty_data();
    header = krb5int_c_locate_iov(data, num_data, 1 as libc::c_int);
    if header.is_null() ||
           (*header).data.length as libc::c_ulong !=
               (*hash).hashsize.wrapping_add(8 as libc::c_int as
    ...
    /* Allocate buffers. */
    ret = alloc_data(&mut comp_checksum, (*hash).hashsize as libc::c_uint);
    if !(ret != 0 as libc::c_int) {
        ret =
            krb5int_c_init_keyblock(0 as krb5_context,
                                    (*key).keyblock.enctype, (*enc).keybytes,
        ...
        if !(ret != 0 as libc::c_int) {
            ...
            if !(ret != 0 as libc::c_int) {
                ...
                loop 
                    ...
                    ret =
                        krb5int_hmac_keyblock(hash, usage_keyblock, data,
                                              num_data, &mut comp_checksum);
                    if ret != 0 as libc::c_int { break ; }
                    if !(k5_bcmp(checksum.data as *const libc::c_void,
                                 comp_checksum.data as *const libc::c_void,
                                 (*hash).hashsize) != 0 as libc::c_int) {
                        break ;
                    }
                    if usage == 9 as libc::c_int {
                        /*
                 ...
                 */
                        ret =
                            keyblock_crypt(enc, enc_keyblock,
                                           0 as *const krb5_data, data,
                                           num_data); /* Restore header pointers. */
                        if ret != 0 as libc::c_int { break ; }
                        usage = 8 as libc::c_int
                    } else {
                        ret =
                            -(1765328353 as libc::c_long) as krb5_error_code;
                        break ;
                    }
                }
            }
    ...
    krb5int_c_free_keyblock(0 as krb5_context, usage_keyblock);
    krb5int_c_free_keyblock(0 as krb5_context, enc_keyblock);
    zapfree(comp_checksum.data as *mut libc::c_void,
            comp_checksum.length as size_t);
    return ret;
}
#[no_mangle]
#[c2rust::src_loc = "311:1"]
pub unsafe extern "C" fn krb5int_arcfour_gsscrypt(mut keyblock:
                                                      *const krb5_keyblock,
                                                  mut usage: krb5_keyusage,
                                                  mut kd_data:
                                                      *const krb5_data,
                                                  ...
                                                  mut num_data: size_t)
 -> krb5_error_code {
    let mut enc: *const krb5_enc_provider = &krb5int_enc_arcfour;
    let mut hash: *const krb5_hash_provider = &krb5int_hash_md5;
    let mut usage_keyblock: *mut krb5_keyblock = 0 as *mut krb5_keyblock;
    let mut enc_keyblock: *mut krb5_keyblock = 0 as *mut krb5_keyblock;
    let mut ret: krb5_error_code = 0;
    ret =
        krb5int_c_init_keyblock(0 as krb5_context, (*keyblock).enctype,
                                (*enc).keybytes, &mut usage_keyblock);
    if !(ret != 0 as libc::c_int) {
        ret =
            krb5int_c_init_keyblock(0 as krb5_context, (*keyblock).enctype,
        ...
        if !(ret != 0 as libc::c_int) {
            ...
            if !(ret != 0 as libc::c_int) {
                ...
                ret =
                    enc_key(enc, hash, usage_keyblock, kd_data, enc_keyblock);
                if !(ret != 0 as libc::c_int) {
                    /* Encrypt or decrypt (encrypt_iov works for both) the input. */
                    ret =
                        keyblock_crypt(enc, enc_keyblock,
                                       0 as *const krb5_data, data, num_data)
                }
            }
        }
    }
