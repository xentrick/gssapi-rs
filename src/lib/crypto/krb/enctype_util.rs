...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/crypto/krb/crypto_int.h:37"]
pub mod crypto_int_h {
    ...
    pub unsafe extern "C" fn find_enctype(mut enctype: krb5_enctype)
     -> *const krb5_keytypes {
        let mut i: libc::c_int = 0;
        i = 0 as libc::c_int;
        while i < krb5int_enctypes_length {
            if (*krb5int_enctypes_list.as_ptr().offset(i as isize)).etype ==
                   enctype {
                break ;
            }
            i += 1
        }
        if i == krb5int_enctypes_length { return 0 as *const krb5_keytypes }
        return &*krb5int_enctypes_list.as_ptr().offset(i as isize) as
                   *const krb5_keytypes;
    }
    use super::krb5_h::{krb5_error_code, krb5_data, krb5_keyblock,
...
pub use self::crypto_int_h::{str2key_func, krb5_keytypes, prf_func,
                             rand2key_func, crypt_func, crypto_length_func,
                             krb5_hash_provider, krb5_enc_provider,
                             krb5_cksumtypes, verify_func, checksum_func,
                             find_enctype, krb5int_enctypes_list,
...
#[no_mangle]
#[c2rust::src_loc = "42:3"]
pub static mut unsupported_etypes: [C2RustUnnamed; 6] =
    [{
         let mut init =
             C2RustUnnamed{etype: 0x1 as libc::c_int,
                           name:
                               b"des-cbc-crc\x00" as *const u8 as
                                   *const libc::c_char,};
         init
     },
     {
         let mut init =
             C2RustUnnamed{etype: 0x2 as libc::c_int,
                           name:
                               b"des-cbc-md4\x00" as *const u8 as
                                   *const libc::c_char,};
         init
     },
     {
         let mut init =
             C2RustUnnamed{etype: 0x3 as libc::c_int,
                           name:
                               b"des-cbc-md5\x00" as *const u8 as
                                   *const libc::c_char,};
         init
     },
     {
         let mut init =
             C2RustUnnamed{etype: 0x4 as libc::c_int,
                           name:
                               b"des-cbc-raw\x00" as *const u8 as
                                   *const libc::c_char,};
         init
     },
     {
         let mut init =
             C2RustUnnamed{etype: 0x8 as libc::c_int,
                           name:
                               b"des-hmac-sha1\x00" as *const u8 as
                                   *const libc::c_char,};
         init
     },
     {
         let mut init =
             C2RustUnnamed{etype: 0 as libc::c_int,
...
#[no_mangle]
#[c2rust::src_loc = "51:1"]
pub unsafe extern "C" fn krb5_c_valid_enctype(mut etype: krb5_enctype)
 -> krb5_boolean {
    return (find_enctype(etype) !=
                0 as *mut libc::c_void as *const krb5_keytypes) as libc::c_int
               as krb5_boolean;
}
#[no_mangle]
#[c2rust::src_loc = "57:1"]
pub unsafe extern "C" fn krb5int_c_weak_enctype(mut etype: krb5_enctype)
 -> krb5_boolean {
    let mut ktp: *const krb5_keytypes = 0 as *const krb5_keytypes;
    ktp = find_enctype(etype);
    return (!ktp.is_null() &&
                (*ktp).flags & (1 as libc::c_int) << 0 as libc::c_int !=
                    0 as libc::c_int) as libc::c_int as krb5_boolean;
}
#[no_mangle]
#[c2rust::src_loc = "66:1"]
pub unsafe extern "C" fn krb5int_c_deprecated_enctype(mut etype: krb5_enctype)
 -> krb5_boolean {
    let mut ktp: *const krb5_keytypes = find_enctype(etype);
    return (ktp.is_null() ||
                (*ktp).flags & (1 as libc::c_int) << 1 as libc::c_int !=
                    0 as libc::c_int) as libc::c_int as krb5_boolean;
}
/* *
...
#[no_mangle]
#[c2rust::src_loc = "73:1"]
pub unsafe extern "C" fn krb5_c_enctype_compare(mut context: krb5_context,
                                                mut e1: krb5_enctype,
                                                mut e2: krb5_enctype,
                                                mut similar:
                                                    *mut krb5_boolean)
 -> krb5_error_code {
    let mut ktp1: *const krb5_keytypes = 0 as *const krb5_keytypes;
    let mut ktp2: *const krb5_keytypes = 0 as *const krb5_keytypes;
    ktp1 = find_enctype(e1);
    ktp2 = find_enctype(e2);
    if ktp1.is_null() || ktp2.is_null() {
        return -(1765328196 as libc::c_long) as krb5_error_code
    }
    *similar =
        ((*ktp1).enc == (*ktp2).enc && (*ktp1).str2key == (*ktp2).str2key) as
            libc::c_int as krb5_boolean;
    return 0 as libc::c_int;
}
#[no_mangle]
#[c2rust::src_loc = "88:1"]
pub unsafe extern "C" fn krb5_string_to_enctype(mut string: *mut libc::c_char,
                                                mut enctypep:
                                                    *mut krb5_enctype)
 -> krb5_error_code {
    let mut i: libc::c_int = 0;
    ...
    while i < krb5int_enctypes_length {
        ktp =
            &*krb5int_enctypes_list.as_ptr().offset(i as isize) as
                *const krb5_keytypes;
        if strcasecmp((*ktp).name, string) == 0 as libc::c_int {
            *enctypep = (*ktp).etype;
            return 0 as libc::c_int
        }
        j = 0 as libc::c_int as libc::c_uint;
        while j < 2 as libc::c_int as libc::c_uint {
            alias = (*ktp).aliases[j as usize];
            if alias.is_null() { break ; }
            if strcasecmp(alias, string) == 0 as libc::c_int {
                *enctypep = (*ktp).etype;
                return 0 as libc::c_int
            }
            j = j.wrapping_add(1)
...
pub unsafe extern "C" fn krb5_enctype_to_string(mut enctype: krb5_enctype,
                                                mut buffer: *mut libc::c_char,
                                                mut buflen: size_t)
 -> krb5_error_code {
    let mut ktp: *const krb5_keytypes = 0 as *const krb5_keytypes;
    ktp = find_enctype(enctype);
    if ktp.is_null() { return 22 as libc::c_int }
    if krb5int_strlcpy(buffer, (*ktp).out_string, buflen) >= buflen {
...
pub unsafe extern "C" fn krb5_enctype_to_name(mut enctype: krb5_enctype,
                                              mut shortest: krb5_boolean,
                                              mut buffer: *mut libc::c_char,
                                              mut buflen: size_t)
 -> krb5_error_code {
    let mut ktp: *const krb5_keytypes = 0 as *const krb5_keytypes;
    ...
    let mut i: libc::c_int = 0;
    i = 0 as libc::c_int;
    while unsupported_etypes[i as usize].etype != 0 as libc::c_int {
        if enctype == unsupported_etypes[i as usize].etype {
            if krb5int_strlcpy(buffer, unsupported_etypes[i as usize].name,
                               buflen) >= buflen {
                return 12 as libc::c_int
            }
            return 0 as libc::c_int
    ...
    if shortest != 0 {
        ...
        while i < 2 as libc::c_int {
            if (*ktp).aliases[i as usize].is_null() { break ; }
            if strlen((*ktp).aliases[i as usize]) < strlen(name) {
                name = (*ktp).aliases[i as usize]
            }
            i += 1
        }
    }
    if krb5int_strlcpy(buffer, name, buflen) >= buflen {
        return 12 as libc::c_int
    }
    return 0 as libc::c_int;
}
...
pub unsafe extern "C" fn k5_enctype_to_ssf(mut enctype: krb5_enctype,
                                           mut ssf_out: *mut libc::c_uint)
 -> krb5_error_code {
    let mut ktp: *const krb5_keytypes = 0 as *const krb5_keytypes;
    *ssf_out = 0 as libc::c_int as libc::c_uint;
    ktp = find_enctype(enctype);
    if ktp.is_null() { return 22 as libc::c_int }
    *ssf_out = (*ktp).ssf;
    return 0 as libc::c_int;
