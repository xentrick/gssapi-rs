...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_c_crypto_length(mut context: krb5_context,
                                              mut enctype: krb5_enctype,
                                              mut type_0: krb5_cryptotype,
                                              mut size: *mut libc::c_uint)
 -> krb5_error_code {
    let mut ktp: *const krb5_keytypes =
        0 as *const krb5_keytypes; /* match Heimdal */
    ktp = find_enctype(enctype);
    if ktp.is_null() {
        return -(1765328196 as libc::c_long) as krb5_error_code
    }
    match type_0 {
        0 | 3 => { *size = 0 as libc::c_int as libc::c_uint }
        2 => { *size = !(0 as libc::c_int) as libc::c_uint }
        1 | 4 | 5 | 6 => {
            *size =
...
#[no_mangle]
#[c2rust::src_loc = "60:1"]
pub unsafe extern "C" fn krb5_c_padding_length(mut context: krb5_context,
                                               mut enctype: krb5_enctype,
...
#[no_mangle]
#[c2rust::src_loc = "74:1"]
pub unsafe extern "C" fn krb5_c_crypto_length_iov(mut context: krb5_context,
                                                  mut enctype: krb5_enctype,
                                                  mut data:
                                                      *mut krb5_crypto_iov,
                                                  mut num_data: size_t)
 -> krb5_error_code {
    let mut i: size_t = 0;
    let mut ktp: *const krb5_keytypes = 0 as *const krb5_keytypes;
    let mut data_length: libc::c_uint = 0 as libc::c_int as libc::c_uint;
    let mut pad_length: libc::c_uint = 0;
    ...
    while i < num_data {
        ...
        match (*iov).flags {
            2 => {
                data_length = data_length.wrapping_add((*iov).data.length)
            }
            4 => {
                if !padding.is_null() { return 22 as libc::c_int }
                padding = iov
            }
            1 | 5 | 6 => {
                (*iov).data.length =
    ...
    pad_length = krb5int_c_padding_length(ktp, data_length as size_t);
    if pad_length != 0 as libc::c_int as libc::c_uint && padding.is_null() {
        return 22 as libc::c_int
    }
    if !padding.is_null() { (*padding).data.length = pad_length }
    return 0 as libc::c_int;
}
