...
use self::unistd_h::{close, read, syscall};
...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_c_random_seed(mut context: krb5_context,
                                            mut data: *mut krb5_data)
 -> krb5_error_code {
    return krb5_c_random_add_entropy(context,
                                     KRB5_C_RANDSOURCE_OLDAPI as libc::c_int
                                         as libc::c_uint, data);
}
...
#[c2rust::src_loc = "67:1"]
unsafe extern "C" fn read_entropy_from_device(mut device: *const libc::c_char,
 ...
 -> krb5_boolean {
    ...
    fd = open(device, 0 as libc::c_int);
    if fd == -(1 as libc::c_int) { return 0 as libc::c_int as krb5_boolean }
    ...
    if !(fstat(fd, &mut sb) == -(1 as libc::c_int) ||
        ...
        loop  {
            ...
            count = read(fd, bp as *mut libc::c_void, left);
            if count <= 0 as libc::c_int as libc::c_long {
                current_block = 12054586826052513374;
                break ;
            }
            left =
                (left as libc::c_ulong).wrapping_sub(count as libc::c_ulong)
                    as size_t as size_t;
            bp = bp.offset(count as isize)
        }
        match current_block {
            12054586826052513374 => { }
            _ => { result = 1 as libc::c_int as krb5_boolean }
        }
    }
    close(fd);
    return result;
...
#[no_mangle]
#[c2rust::src_loc = "98:1"]
pub unsafe extern "C" fn k5_get_os_entropy(mut buf: *mut libc::c_uchar,
                                           mut len: size_t,
 ...
 -> krb5_boolean {
    ...
    while len > 0 as libc::c_int as libc::c_ulong {
        ...
        *__errno_location() = 0 as libc::c_int;
        r =
            syscall(318 as libc::c_int as libc::c_long, buf, len,
                    0 as libc::c_int) as libc::c_int;
        if r <= 0 as libc::c_int {
            if !(*__errno_location() == 4 as libc::c_int) { break ; }
        } else {
            len =
                (len as libc::c_ulong).wrapping_sub(r as libc::c_ulong) as
                    size_t as size_t;
    ...
    /* defined(__linux__) && defined(SYS_getrandom) */
    device =
        if strong != 0 {
            b"/dev/random\x00" as *const u8 as *const libc::c_char
        } else { b"/dev/urandom\x00" as *const u8 as *const libc::c_char };
