...
#[no_mangle]
#[c2rust::src_loc = "29:1"]
pub unsafe extern "C" fn krb5_k_make_checksum_iov(mut context: krb5_context,
                                                  mut cksumtype:
                                                      krb5_cksumtype,
                                                  mut key: krb5_key,
 ...
 -> krb5_error_code {
    ...
    if !(ret != 0 as libc::c_int) {
        memcpy((*checksum).data.data as *mut libc::c_void,
               cksum_data.data as *const libc::c_void,
               (*ctp).output_size as libc::c_ulong);
        (*checksum).data.length = (*ctp).output_size
    }
    zapfree(cksum_data.data as *mut libc::c_void,
...
#[no_mangle]
#[c2rust::src_loc = "76:1"]
pub unsafe extern "C" fn krb5_c_make_checksum_iov(mut context: krb5_context,
                                                  mut cksumtype:
                                                      krb5_cksumtype,
                                                  mut keyblock:
                                                      *const krb5_keyblock,
 ...
 -> krb5_error_code {
    ...
    if ret != 0 as libc::c_int { return ret }
    ret =
        krb5_k_make_checksum_iov(context, cksumtype, key, usage, data,
                                 num_data);
