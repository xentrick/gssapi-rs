...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/crypto/krb/crypto_int.h:32"]
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_enctype,
                       ...
                       krb5_crypto_iov, _profile_t, krb5_c_prf,
                       krb5_c_prf_length};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         CANONHOST_FALLBACK, CANONHOST_TRUE, CANONHOST_FALSE,
                         krb5_preauth_context, kdb5_dal_handle,
                         _krb5_os_context, krb5_key_st, derived_key,
                         empty_data, make_data, zapfree, alloc_data,
                         ...
                         krb5int_c_free_keyblock, krb5int_c_init_keyblock};
...
#[no_mangle]
#[c2rust::src_loc = "38:1"]
pub unsafe extern "C" fn krb5_c_prfplus(mut context: krb5_context,
                                        mut k: *const krb5_keyblock,
                                        mut input: *const krb5_data,
                                        mut output: *mut krb5_data)
 -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut prf_in: krb5_data = empty_data();
    let mut prf_out: krb5_data = empty_data();
    let mut prflen: size_t = 0;
    let mut nblocks: size_t = 0;
    let mut i: size_t = 0;
    /* Calculate the number of PRF invocations we will need. */
    ret = krb5_c_prf_length(context, (*k).enctype, &mut prflen);
    if ret != 0 { return ret }
    nblocks =
        ((*output).length as
             libc::c_ulong).wrapping_add(prflen).wrapping_sub(1 as libc::c_int
                                                                  as
                                                                  libc::c_ulong).wrapping_div(prflen);
    if nblocks > 255 as libc::c_int as libc::c_ulong {
        return 7 as libc::c_int
    }
    /* Allocate PRF input and output buffers. */
    ret =
        alloc_data(&mut prf_in,
                   (*input).length.wrapping_add(1 as libc::c_int as
                                                    libc::c_uint));
    if !(ret != 0) {
        ret = alloc_data(&mut prf_out, prflen as libc::c_uint);
        if !(ret != 0) {
            /* Concatenate PRF(k, 1||input) || PRF(k, 2||input) || ... to produce the
     * desired number of bytes. */
            memcpy(&mut *prf_in.data.offset(1 as libc::c_int as isize) as
                       *mut libc::c_char as *mut libc::c_void,
                   (*input).data as *const libc::c_void,
                   (*input).length as libc::c_ulong);
            i = 0 as libc::c_int as size_t;
            while i < nblocks {
                *prf_in.data.offset(0 as libc::c_int as isize) =
                    i.wrapping_add(1 as libc::c_int as libc::c_ulong) as
                        libc::c_char;
                ret = krb5_c_prf(context, k, &mut prf_in, &mut prf_out);
                if ret != 0 { break ; }
                memcpy(&mut *(*output).data.offset(i.wrapping_mul(prflen) as
                                                       isize) as
                           *mut libc::c_char as *mut libc::c_void,
                       prf_out.data as *const libc::c_void,
                       if prflen <
                              ((*output).length as
                                   libc::c_ulong).wrapping_sub(i.wrapping_mul(prflen))
                          {
                           prflen
                       } else {
                           ((*output).length as
    ...
    zapfree(prf_out.data as *mut libc::c_void, prf_out.length as size_t);
    zapfree(prf_in.data as *mut libc::c_void, prf_in.length as size_t);
    return ret;
}
/* *
...
#[no_mangle]
#[c2rust::src_loc = "81:1"]
pub unsafe extern "C" fn krb5_c_derive_prfplus(mut context: krb5_context,
                                               mut k: *const krb5_keyblock,
                                               mut input: *const krb5_data,
 ...
 -> krb5_error_code {
    ...
    *out = 0 as *mut krb5_keyblock;
    ...
    ret = alloc_data(&mut rnd, (*(*ktp).enc).keybytes as libc::c_uint);
    if !(ret != 0) {
        ret = krb5_c_prfplus(context, k, input, &mut rnd);
        if !(ret != 0) {
            /* Generate a key from the pseudo-random bytes. */
            ret =
                krb5int_c_init_keyblock(context, (*ktp).etype,
                                        (*(*ktp).enc).keylength, &mut kb);
            if !(ret != 0) {
                ret =
                    Some((*ktp).rand2key.expect("non-null function pointer")).expect("non-null function pointer")(&mut rnd,
                                                                                                                  kb);
                if !(ret != 0) { *out = kb; kb = 0 as *mut krb5_keyblock }
            }
        }
    }
    zapfree(rnd.data as *mut libc::c_void, rnd.length as size_t);
...
pub unsafe extern "C" fn krb5_c_fx_cf2_simple(mut context: krb5_context,
                                              mut k1: *const krb5_keyblock,
                                              mut pepper1:
                                                  *const libc::c_char,
 ...
 -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut ktp: *const krb5_keytypes = 0 as *const krb5_keytypes;
    let pepper1_data: krb5_data = string2data(pepper1 as *mut libc::c_char);
    let pepper2_data: krb5_data = string2data(pepper2 as *mut libc::c_char);
    let mut prf1: krb5_data = empty_data();
    let mut prf2: krb5_data = empty_data();
    ...
    *out = 0 as *mut krb5_keyblock;
    ...
    ret = alloc_data(&mut prf1, (*(*ktp).enc).keybytes as libc::c_uint);
    if !(ret != 0) {
        ret = krb5_c_prfplus(context, k1, &pepper1_data, &mut prf1);
        if !(ret != 0) {
            ret =
                alloc_data(&mut prf2, (*(*ktp).enc).keybytes as libc::c_uint);
            if !(ret != 0) {
                ret = krb5_c_prfplus(context, k2, &pepper2_data, &mut prf2);
                if !(ret != 0) {
                    /* Compute the XOR of the two PRF+ values and generate a key. */
                    i = 0 as libc::c_int as libc::c_uint;
                    while i < prf1.length {
                        let ref mut fresh0 = *prf1.data.offset(i as isize);
                        *fresh0 =
                            (*fresh0 as libc::c_int ^
                                 *prf2.data.offset(i as isize) as libc::c_int)
                                as libc::c_char;
                        i = i.wrapping_add(1)
                    }
                    ret =
                    ...
                    if !(ret != 0) {
                        ret =
                            Some((*ktp).rand2key.expect("non-null function pointer")).expect("non-null function pointer")(&mut prf1,
                                                                                                                          kb);
                        if !(ret != 0) {
                            *out = kb;
    ...
    zapfree(prf2.data as *mut libc::c_void, prf2.length as size_t);
    zapfree(prf1.data as *mut libc::c_void, prf1.length as size_t);
    krb5int_c_free_keyblock(context, kb);
    return ret;
}
