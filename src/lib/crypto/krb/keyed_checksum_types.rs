...
pub use self::crypto_int_h::{krb5_cksumtypes, verify_func, checksum_func,
                             ...
                             find_enctype, krb5int_cksumtypes_list,
                             krb5int_cksumtypes_length, krb5int_enctypes_list,
 ...
 */
#[c2rust::src_loc = "30:1"]
unsafe extern "C" fn is_keyed_for(mut ctp: *const krb5_cksumtypes,
                                  mut ktp: *const krb5_keytypes)
 -> krb5_boolean {
    if (*ctp).flags & 0x1 as libc::c_int != 0 {
        return 0 as libc::c_int as krb5_boolean
    }
    return ((*ctp).enc.is_null() || (*ktp).enc == (*ctp).enc) as libc::c_int
               as krb5_boolean;
}
/* *
...
#[no_mangle]
#[c2rust::src_loc = "39:1"]
pub unsafe extern "C" fn krb5_c_keyed_checksum_types(mut context:
                                                         krb5_context,
                                                     mut enctype:
                                                         krb5_enctype,
                                                     mut count:
 ...
 -> krb5_error_code {
    ...
    let mut nctypes: libc::c_uint = 0;
    let mut ctypes: *mut krb5_cksumtype = 0 as *mut krb5_cksumtype;
    ...
    *cksumtypes = 0 as *mut krb5_cksumtype;
    ktp = find_enctype(enctype);
    if ktp.is_null() {
        return -(1765328196 as libc::c_long) as krb5_error_code
    }
    nctypes = 0 as libc::c_int as libc::c_uint;
    i = 0 as libc::c_int as libc::c_uint;
    while (i as libc::c_ulong) < krb5int_cksumtypes_length {
        ctp =
            &*krb5int_cksumtypes_list.as_ptr().offset(i as isize) as
                *const krb5_cksumtypes;
        if is_keyed_for(ctp, ktp) != 0 { nctypes = nctypes.wrapping_add(1) }
        i = i.wrapping_add(1)
    }
    ctypes =
        malloc((nctypes as
                    libc::c_ulong).wrapping_mul(::std::mem::size_of::<krb5_cksumtype>()
                                                    as libc::c_ulong)) as
            *mut krb5_cksumtype;
    if ctypes.is_null() { return 12 as libc::c_int }
    ...
    while (i as libc::c_ulong) < krb5int_cksumtypes_length {
        ctp =
            &*krb5int_cksumtypes_list.as_ptr().offset(i as isize) as
                *const krb5_cksumtypes;
        if is_keyed_for(ctp, ktp) != 0 {
            let fresh0 = c;
            c = c.wrapping_add(1);
            *ctypes.offset(fresh0 as isize) = (*ctp).ctype
        }
        i = i.wrapping_add(1)
    }
    *count = nctypes;
...
#[no_mangle]
#[c2rust::src_loc = "78:1"]
pub unsafe extern "C" fn krb5_free_cksumtypes(mut context: krb5_context,
                                              mut val: *mut krb5_cksumtype) {
