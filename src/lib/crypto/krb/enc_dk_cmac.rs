...
pub use self::crypto_int_h::{krb5_hash_provider, krb5_keytypes, prf_func,
                             ...
                             krb5int_cmac_checksum, krb5int_derive_key,
...
#[no_mangle]
#[c2rust::src_loc = "34:1"]
pub unsafe extern "C" fn krb5int_camellia_crypto_length(mut ktp:
                                                            *const krb5_keytypes,
                                                        mut type_0:
                                                            krb5_cryptotype)
 -> libc::c_uint {
    match type_0 {
        ...
        5 | 6 => { return (*(*ktp).enc).block_size as libc::c_uint }
        _ => {
            if 0 as libc::c_int != 0 &&
                   !(b"bad type passed to krb5int_camellia_crypto_length\x00"
                         as *const u8 as *const libc::c_char).is_null() {
            } else {
                __assert_fail(b"0 && \"bad type passed to krb5int_camellia_crypto_length\"\x00"
                                  as *const u8 as *const libc::c_char,
                              b"enc_dk_cmac.c\x00" as *const u8 as
                                  *const libc::c_char,
                              47 as libc::c_int as libc::c_uint,
                              (*::std::mem::transmute::<&[u8; 91],
                                                        &[libc::c_char; 91]>(b"unsigned int krb5int_camellia_crypto_length(const struct krb5_keytypes *, krb5_cryptotype)\x00")).as_ptr());
            }
...
#[c2rust::src_loc = "53:1"]
unsafe extern "C" fn derive_keys(mut enc: *const krb5_enc_provider,
                                 mut key: krb5_key, mut usage: krb5_keyusage,
                                 mut ke_out: *mut krb5_key,
                                 mut ki_out: *mut krb5_key)
 -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut buf: [libc::c_uchar; 5] = [0; 5];
    let mut constant: krb5_data =
        make_data(buf.as_mut_ptr() as *mut libc::c_void,
                  5 as libc::c_int as libc::c_uint);
    let mut ke: krb5_key = 0 as *mut krb5_key_st;
    let mut ki: krb5_key = 0 as *mut krb5_key_st;
    *ki_out = 0 as krb5_key;
    *ke_out = *ki_out;
    /* Derive the encryption key. */
    store_32_be(usage as libc::c_uint, buf.as_mut_ptr() as *mut libc::c_void);
    buf[4 as libc::c_int as usize] = 0xaa as libc::c_int as libc::c_uchar;
    ret =
        krb5int_derive_key(enc, 0 as *const krb5_hash_provider, key, &mut ke,
                           &mut constant, DERIVE_SP800_108_CMAC);
    if ret != 0 as libc::c_int { return ret }
    /* Derive the integrity key. */
    buf[4 as libc::c_int as usize] = 0x55 as libc::c_int as libc::c_uchar;
    ret =
        krb5int_derive_key(enc, 0 as *const krb5_hash_provider, key, &mut ki,
                           &mut constant, DERIVE_SP800_108_CMAC);
...
#[no_mangle]
#[c2rust::src_loc = "86:1"]
pub unsafe extern "C" fn krb5int_dk_cmac_encrypt(mut ktp:
                                                     *const krb5_keytypes,
                                                 mut key: krb5_key,
 ...
 -> krb5_error_code {
    ...
    if trailer.is_null() ||
           ((*trailer).data.length as libc::c_ulong) < (*enc).block_size {
        return -(1765328194 as libc::c_long) as krb5_error_code
    }
    ...
    ret = derive_keys(enc, key, usage, &mut ke, &mut ki);
    if !(ret != 0 as libc::c_int) {
        /* Generate confounder. */
        (*header).data.length = (*enc).block_size as libc::c_uint;
        ret =
            krb5_c_random_make_octets(0 as krb5_context, &mut (*header).data);
        if !(ret != 0 as libc::c_int) {
            /* Checksum the plaintext. */
            ret =
                krb5int_cmac_checksum(enc, ki, data, num_data,
                                      &mut (*trailer).data);
            if !(ret != 0 as libc::c_int) {
...
#[no_mangle]
#[c2rust::src_loc = "136:1"]
pub unsafe extern "C" fn krb5int_dk_cmac_decrypt(mut ktp:
                                                     *const krb5_keytypes,
                                                 mut key: krb5_key,
                                                 mut usage: krb5_keyusage,
                                                 mut ivec: *const krb5_data,
 ...
 -> krb5_error_code {
    ...
    if !(ret != 0 as libc::c_int) {
        ...
        ret =
            (*enc).decrypt.expect("non-null function pointer")(ke, ivec, data,
                                                               num_data);
        if !(ret != 0 as libc::c_int) {
            /* Verify the hash. */
            ret = alloc_data(&mut cksum, (*enc).block_size as libc::c_uint);
            if !(ret != 0 as libc::c_int) {
                ret =
                    krb5int_cmac_checksum(enc, ki, data, num_data,
                                          &mut cksum);
                if !(ret != 0 as libc::c_int) {
                    if k5_bcmp(cksum.data as *const libc::c_void,
                               ...
                               (*enc).block_size) != 0 as libc::c_int {
