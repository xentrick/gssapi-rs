...
pub mod crypto_int_h {
    ...
    use super::krb5_h::{krb5_error_code, krb5_crypto_iov, krb5_data,
                        krb5_enctype, krb5_cksumtype, krb5_flags, krb5_key,
                        krb5_keyblock, krb5_keyusage, krb5_cryptotype,
                        krb5_boolean};
    use super::k5_int_h::krb5_key_st;
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "332:1"]
        pub fn krb5int_derive_keyblock(enc: *const krb5_enc_provider,
                                       hash: *const krb5_hash_provider,
                                       inkey: krb5_key,
                                       outkey: *mut krb5_keyblock,
        ...
        pub fn krb5int_pbkdf2_hmac(hash: *const krb5_hash_provider,
                                   out: *const krb5_data,
                                   count: libc::c_ulong,
                                   password: *const krb5_data,
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_enctype,
                       ...
                       krb5_crypto_iov, _profile_t, krb5_k_create_key,
                       krb5_k_free_key};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         CANONHOST_FALLBACK, CANONHOST_TRUE, CANONHOST_FALSE,
                         krb5_preauth_context, kdb5_dal_handle,
                         _krb5_os_context, krb5_key_st, derived_key, zapfree,
                         make_data, empty_data, string2data, alloc_data,
...
pub use self::crypto_int_h::{krb5_hash_provider, krb5_keytypes, prf_func,
                             rand2key_func, str2key_func, crypt_func,
                             crypto_length_func, krb5_enc_provider,
                             krb5_cksumtypes, verify_func, checksum_func,
                             deriv_alg, DERIVE_SP800_108_HMAC,
                             DERIVE_SP800_108_CMAC, DERIVE_RFC3961,
                             krb5int_derive_keyblock, krb5int_nfold,
                             krb5int_pbkdf2_hmac, krb5int_hash_sha1};
use self::stdlib_h::{calloc, free};
use self::string_h::{memset, explicit_bzero, strlen, memcpy};
 ...
 * WARRANTIES OF MERCHANTIBILITY AND FITNESS FOR A PARTICULAR PURPOSE.
 */
#[c2rust::src_loc = "30:28"]
static mut kerberos: [libc::c_uchar; 9] =
    [107, 101, 114, 98, 101, 114, 111, 115, 0];
#[no_mangle]
#[c2rust::src_loc = "33:1"]
pub unsafe extern "C" fn krb5int_dk_string_to_key(mut ktp:
                                                      *const krb5_keytypes,
                                                  mut string:
                                                      *const krb5_data,
                                                  mut salt: *const krb5_data,
                                                  mut parms: *const krb5_data,
                                                  mut keyblock:
                                                      *mut krb5_keyblock)
 -> krb5_error_code {
    ...
    let mut concatlen: size_t = 0;
    let mut concat: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut foldstring: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut foldkeydata: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut indata: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut foldkeyblock: krb5_keyblock =
        krb5_keyblock{magic: 0,
                      enctype: 0,
                      length: 0,
                      contents: 0 as *mut krb5_octet,};
    let mut foldkey: krb5_key = 0 as krb5_key;
    /* keyblock->length is checked by krb5int_derive_key. */
    keybytes = (*(*ktp).enc).keybytes;
    keylength = (*(*ktp).enc).keylength;
    concatlen = (*string).length.wrapping_add((*salt).length) as size_t;
    concat = k5alloc(concatlen, &mut ret) as *mut libc::c_uchar;
    if !(ret != 0 as libc::c_int) {
        foldstring = k5alloc(keybytes, &mut ret) as *mut libc::c_uchar;
        if !(ret != 0 as libc::c_int) {
            foldkeydata = k5alloc(keylength, &mut ret) as *mut libc::c_uchar;
            if !(ret != 0 as libc::c_int) {
                /* construct input string ( = string + salt), fold it, make_key it */
                if (*string).length > 0 as libc::c_int as libc::c_uint {
                    memcpy(concat as *mut libc::c_void,
                           (*string).data as *const libc::c_void,
                           (*string).length as libc::c_ulong);
                }
                if (*salt).length > 0 as libc::c_int as libc::c_uint {
                    memcpy(concat.offset((*string).length as isize) as
                               *mut libc::c_void,
                           (*salt).data as *const libc::c_void,
                           (*salt).length as libc::c_ulong);
                }
                krb5int_nfold(concatlen.wrapping_mul(8 as libc::c_int as
                                                         libc::c_ulong) as
                                  libc::c_uint, concat,
                              keybytes.wrapping_mul(8 as libc::c_int as
                                                        libc::c_ulong) as
                                  libc::c_uint, foldstring);
                indata.length = keybytes as libc::c_uint;
                indata.data = foldstring as *mut libc::c_char;
                foldkeyblock.length = keylength as libc::c_uint;
                foldkeyblock.contents = foldkeydata;
                foldkeyblock.enctype = (*ktp).etype;
                ret =
                    (*ktp).rand2key.expect("non-null function pointer")(&mut indata,
                                                                        &mut foldkeyblock);
                if !(ret != 0 as libc::c_int) {
                    ret =
                        krb5_k_create_key(0 as krb5_context,
                                          &mut foldkeyblock, &mut foldkey);
                    if !(ret != 0 as libc::c_int) {
                        /* now derive the key from this one */
                        indata.length =
                            (::std::mem::size_of::<[libc::c_uchar; 9]>() as
                                 libc::c_ulong).wrapping_sub(1 as libc::c_int
                                                                 as
                                                                 libc::c_ulong)
                                as libc::c_uint;
                        indata.data = kerberos.as_ptr() as *mut libc::c_char;
                        ret =
                            krb5int_derive_keyblock((*ktp).enc,
                                                    0 as
                                                    ...
                                                    DERIVE_RFC3961);
                        if ret != 0 as libc::c_int {
                            memset((*keyblock).contents as *mut libc::c_void,
                                   0 as libc::c_int,
                                   (*keyblock).length as libc::c_ulong);
                        }
                    }
                }
            }
    ...
    zapfree(concat as *mut libc::c_void, concatlen);
    zapfree(foldstring as *mut libc::c_void, keybytes);
    zapfree(foldkeydata as *mut libc::c_void, keylength);
    krb5_k_free_key(0 as krb5_context, foldkey);
    return ret;
}
#[no_mangle]
#[c2rust::src_loc = "106:14"]
pub static mut k5_allow_weak_pbkdf2iter: krb5_boolean =
    0 as libc::c_int as krb5_boolean;
#[c2rust::src_loc = "108:1"]
unsafe extern "C" fn pbkdf2_string_to_key(mut ktp: *const krb5_keytypes,
                                          mut string: *const krb5_data,
                                          mut salt: *const krb5_data,
                                          mut pepper: *const krb5_data,
                                          mut params: *const krb5_data,
                                          mut key: *mut krb5_keyblock,
                                          mut deriv_alg: deriv_alg,
                                          mut def_iter_count: libc::c_ulong)
 -> krb5_error_code {
    let mut hash: *const krb5_hash_provider = 0 as *const krb5_hash_provider;
    let mut iter_count: libc::c_ulong = 0;
    let mut out: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    static mut usage: krb5_data =
        {
            let mut init =
                _krb5_data{magic: -(1760647422 as libc::c_long) as krb5_magic,
                           length: 8 as libc::c_int as libc::c_uint,
                           data:
                               b"kerberos\x00" as *const u8 as
                                   *const libc::c_char as *mut libc::c_char,};
            init
        };
    let mut tempkey: krb5_key = 0 as krb5_key;
    let mut err: krb5_error_code = 0;
    let mut sandp: krb5_data = empty_data();
    if !params.is_null() {
        let mut p: *mut libc::c_uchar = (*params).data as *mut libc::c_uchar;
        if (*params).length != 4 as libc::c_int as libc::c_uint {
            return -(1765328140 as libc::c_long) as krb5_error_code
        }
        iter_count = load_32_be(p as *const libc::c_void) as libc::c_ulong;
        ...
        if iter_count == 0 as libc::c_int as libc::c_ulong ||
               k5_allow_weak_pbkdf2iter == 0 && iter_count < def_iter_count {
            return -(1765328140 as libc::c_long) as krb5_error_code
        }
    } else { iter_count = def_iter_count }
    /* This is not a protocol specification constraint; this is an
       implementation limit, which should eventually be controlled by
       a config file.  */
    if iter_count >= 0x1000000 as libc::c_long as libc::c_ulong {
        return -(1765328140 as libc::c_long) as krb5_error_code
    }
    /* Use the output keyblock contents for temporary space. */
    out.data = (*key).contents as *mut libc::c_char;
    out.length = (*key).length;
    if out.length != 16 as libc::c_int as libc::c_uint &&
           out.length != 32 as libc::c_int as libc::c_uint {
        return -(1765328206 as libc::c_long) as krb5_error_code
    }
    if !pepper.is_null() {
        err =
            alloc_data(&mut sandp,
                       (*pepper).length.wrapping_add(1 as libc::c_int as
                                                         libc::c_uint).wrapping_add((*salt).length));
        if err != 0 { return err }
        if (*pepper).length > 0 as libc::c_int as libc::c_uint {
            memcpy(sandp.data as *mut libc::c_void,
                   (*pepper).data as *const libc::c_void,
                   (*pepper).length as libc::c_ulong);
        }
        *sandp.data.offset((*pepper).length as isize) =
            '\u{0}' as i32 as libc::c_char;
        if (*salt).length > 0 as libc::c_int as libc::c_uint {
            memcpy(&mut *sandp.data.offset((*pepper).length.wrapping_add(1 as
                                                                             libc::c_int
                                                                             as
                                                                             libc::c_uint)
    ...
    hash =
        if !(*ktp).hash.is_null() { (*ktp).hash } else { &krb5int_hash_sha1 };
    err = krb5int_pbkdf2_hmac(hash, &mut out, iter_count, string, salt);
    if !(err != 0) {
        err = krb5_k_create_key(0 as krb5_context, key, &mut tempkey);
        if !(err != 0) {
            err =
                krb5int_derive_keyblock((*ktp).enc, (*ktp).hash, tempkey, key,
                                        &usage, deriv_alg)
        }
    }
    if !sandp.data.is_null() { free(sandp.data as *mut libc::c_void); }
    if err != 0 {
        memset(out.data as *mut libc::c_void, 0 as libc::c_int,
               out.length as libc::c_ulong);
    }
    krb5_k_free_key(0 as krb5_context, tempkey);
    return err;
}
#[no_mangle]
#[c2rust::src_loc = "183:1"]
pub unsafe extern "C" fn krb5int_aes_string_to_key(mut ktp:
                                                       *const krb5_keytypes,
                                                   mut string:
                                                       *const krb5_data,
                                                   mut salt: *const krb5_data,
                                                   ...
                                                   mut key:
                                                       *mut krb5_keyblock)
 -> krb5_error_code {
    return pbkdf2_string_to_key(ktp, string, salt, 0 as *const krb5_data,
                                params, key, DERIVE_RFC3961,
                                4096 as libc::c_int as libc::c_ulong);
}
#[no_mangle]
#[c2rust::src_loc = "194:1"]
pub unsafe extern "C" fn krb5int_camellia_string_to_key(mut ktp:
                                                            *const krb5_keytypes,
                                                        mut string:
                                                            *const krb5_data,
                                                        mut salt:
                                                        ...
                                                        mut key:
                                                            *mut krb5_keyblock)
 -> krb5_error_code {
    let mut pepper: krb5_data = string2data((*ktp).name);
    return pbkdf2_string_to_key(ktp, string, salt, &mut pepper, params, key,
...
pub unsafe extern "C" fn krb5int_aes2_string_to_key(mut ktp:
                                                        *const krb5_keytypes,
                                                    mut string:
                                                        *const krb5_data,
                                                    mut salt:
