...
#[no_mangle]
#[c2rust::src_loc = "32:1"]
pub unsafe extern "C" fn krb5_k_make_checksum(mut context: krb5_context,
 ...
 -> krb5_error_code {
    ...
    let mut trunc: *mut krb5_octet = 0 as *mut krb5_octet;
    ...
    if !(ret != 0 as libc::c_int) {
        ...
        (*cksum).contents = cksum_data.data as *mut krb5_octet;
        cksum_data.data = 0 as *mut libc::c_char;
        if (*ctp).output_size < (*ctp).compute_size {
            trunc =
                realloc((*cksum).contents as *mut libc::c_void,
                        (*ctp).output_size as libc::c_ulong) as
                    *mut krb5_octet;
            if !trunc.is_null() { (*cksum).contents = trunc }
        }
    }
...
#[no_mangle]
#[c2rust::src_loc = "83:1"]
pub unsafe extern "C" fn krb5_c_make_checksum(mut context: krb5_context,
                                              mut cksumtype: krb5_cksumtype,
                                              mut keyblock:
                                                  *const krb5_keyblock,
                                              mut usage: krb5_keyusage,
