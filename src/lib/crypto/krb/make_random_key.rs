...
#[no_mangle]
#[c2rust::src_loc = "30:1"]
pub unsafe extern "C" fn krb5_c_make_random_key(mut context: krb5_context,
 ...
 -> krb5_error_code {
    ...
    let mut enc: *const krb5_enc_provider = 0 as *const krb5_enc_provider;
    let mut keybytes: size_t = 0;
    let mut keylength: size_t = 0;
    let mut random_data: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut bytes: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    ...
    bytes = k5alloc(keybytes, &mut ret) as *mut libc::c_uchar;
    if ret != 0 { return ret }
    (*random_key).contents = k5alloc(keylength, &mut ret) as *mut krb5_octet;
    if !(ret != 0) {
        random_data.data = bytes as *mut libc::c_char;
        random_data.length = keybytes as libc::c_uint;
        ret = krb5_c_random_make_octets(context, &mut random_data);
        if !(ret != 0) {
            (*random_key).magic = -(1760647421 as libc::c_long) as krb5_magic;
            (*random_key).enctype = enctype;
            (*random_key).length = keylength as libc::c_uint;
            ret =
                Some((*ktp).rand2key.expect("non-null function pointer")).expect("non-null function pointer")(&mut random_data,
                                                                                                              random_key)
        }
    }
    if ret != 0 {
        zapfree((*random_key).contents as *mut libc::c_void, keylength);
        (*random_key).contents = 0 as *mut krb5_octet
    }
    zapfree(bytes as *mut libc::c_void, keybytes);
    return ret;
}
