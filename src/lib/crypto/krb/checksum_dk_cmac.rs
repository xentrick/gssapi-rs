...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_enctype,
                       ...
                       krb5_crypto_iov, _profile_t, krb5_k_free_key};
...
#[no_mangle]
#[c2rust::src_loc = "31:1"]
pub unsafe extern "C" fn krb5int_dk_cmac_checksum(mut ctp:
 ...
 -> krb5_error_code {
    let mut enc: *const krb5_enc_provider = (*ctp).enc;
    let mut ret: krb5_error_code = 0;
    let mut constantdata: [libc::c_uchar; 5] = [0; 5];
    let mut datain: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut kc: krb5_key = 0 as *mut krb5_key_st;
    /* Derive the key. */
    datain =
        make_data(constantdata.as_mut_ptr() as *mut libc::c_void,
                  5 as libc::c_int as libc::c_uint);
    store_32_be(usage as libc::c_uint,
                constantdata.as_mut_ptr() as *mut libc::c_void);
    constantdata[4 as libc::c_int as usize] =
        0x99 as libc::c_int as libc::c_char as libc::c_uchar;
    ret =
        krb5int_derive_key(enc, 0 as *const krb5_hash_provider, key, &mut kc,
                           &mut datain, DERIVE_SP800_108_CMAC);
    if ret != 0 as libc::c_int { return ret }
    /* Hash the data. */
    ret = krb5int_cmac_checksum(enc, kc, data, num_data, output);
    if ret != 0 as libc::c_int {
        memset((*output).data as *mut libc::c_void, 0 as libc::c_int,
               (*output).length as libc::c_ulong);
    }
    krb5_k_free_key(0 as krb5_context, kc);
    return ret;
}
