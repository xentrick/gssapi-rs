...
#[no_mangle]
#[c2rust::src_loc = "37:1"]
pub unsafe extern "C" fn krb5_k_create_key(mut context: krb5_context,
                                           mut key_data: *const krb5_keyblock,
                                           mut out: *mut krb5_key)
 -> krb5_error_code {
    let mut key: krb5_key = 0 as krb5_key;
    let mut code: krb5_error_code = 0;
    *out = 0 as krb5_key;
    key =
        malloc(::std::mem::size_of::<krb5_key_st>() as libc::c_ulong) as
            krb5_key;
    if key.is_null() { return 12 as libc::c_int }
    code =
        krb5int_c_copy_keyblock_contents(context, key_data,
                                         &mut (*key).keyblock);
    if code != 0 {
        free(key as *mut libc::c_void);
        return code
    } else {
        (*key).refcount = 1 as libc::c_int;
        (*key).derived = 0 as *mut derived_key;
        (*key).cache = 0 as *mut libc::c_void;
        *out = key;
...
#[no_mangle]
#[c2rust::src_loc = "64:1"]
pub unsafe extern "C" fn krb5_k_reference_key(mut context: krb5_context,
                                              mut key: krb5_key) {
    if !key.is_null() { (*key).refcount += 1 };
}
/* * Decrement the reference count on a key and free it if it hits zero. */
/* Free the memory used by a krb5_key. */
#[no_mangle]
#[c2rust::src_loc = "72:1"]
pub unsafe extern "C" fn krb5_k_free_key(mut context: krb5_context,
                                         mut key: krb5_key) {
    let mut dk: *mut derived_key = 0 as *mut derived_key;
    let mut ktp: *const krb5_keytypes = 0 as *const krb5_keytypes;
    if key.is_null() ||
           { (*key).refcount -= 1; ((*key).refcount) > 0 as libc::c_int } {
        return
    }
    loop 
        ...
        if dk.is_null() { break ; }
        (*key).derived = (*dk).next;
        free((*dk).constant.data as *mut libc::c_void);
        krb5_k_free_key(context, (*dk).dkey);
        free(dk as *mut libc::c_void);
    }
    krb5int_c_free_keyblock_contents(context, &mut (*key).keyblock);
    if !(*key).cache.is_null() {
        ktp = find_enctype((*key).keyblock.enctype);
        if !ktp.is_null() && (*(*ktp).enc).key_cleanup.is_some() {
            (*(*ktp).enc).key_cleanup.expect("non-null function pointer")(key);
        }
    }
    free(key as *mut libc::c_void);
}
...
#[no_mangle]
#[c2rust::src_loc = "98:1"]
pub unsafe extern "C" fn krb5_k_key_keyblock(mut context: krb5_context,
                                             mut key: krb5_key,
                                             mut key_data:
                                                 *mut *mut krb5_keyblock)
 -> krb5_error_code {
    return krb5int_c_copy_keyblock(context, &mut (*key).keyblock, key_data);
}
/* * Retrieve the enctype of a krb5_key structure. */
/* Retrieve the enctype of a krb5_key. */
#[no_mangle]
#[c2rust::src_loc = "106:1"]
pub unsafe extern "C" fn krb5_k_key_enctype(mut context: krb5_context,
                                            mut key: krb5_key)
 -> krb5_enctype {
