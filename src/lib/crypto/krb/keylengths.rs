...
#[no_mangle]
#[c2rust::src_loc = "38:1"]
pub unsafe extern "C" fn krb5_c_keylengths(mut context: krb5_context,
                                           mut enctype: krb5_enctype,
                                           mut keybytes: *mut size_t,
                                           mut keylength: *mut size_t)
 -> krb5_error_code {
    let mut ktp: *const krb5_keytypes = 0 as *const krb5_keytypes;
    if keybytes.is_null() && keylength.is_null() { return 22 as libc::c_int }
    ktp = find_enctype(enctype);
    if ktp.is_null() {
        return -(1765328196 as libc::c_long) as krb5_error_code
    }
    if !keybytes.is_null() { *keybytes = (*(*ktp).enc).keybytes }
    if !keylength.is_null() { *keylength = (*(*ktp).enc).keylength }
