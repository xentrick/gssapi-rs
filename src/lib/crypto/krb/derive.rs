 ...
 */
    ...
    use super::string_h::{explicit_bzero, memcmp, memcpy};
...
pub mod crypto_int_h {
    ...
    extern "C" {
        ...
        pub fn krb5int_cmac_checksum(enc: *const krb5_enc_provider,
                                     key: krb5_key,
        ...
        pub fn krb5int_nfold(inbits: libc::c_uint, in_0: *const libc::c_uchar,
                             outbits: libc::c_uint, out: *mut libc::c_uchar);
    }
    /* CRYPTO_INT_H */
}
...
pub use self::krb5_h::{krb5_octet, krb5_int32, krb5_boolean, krb5_enctype,
                       ...
                       krb5_crypto_iov, _profile_t, krb5_c_random_to_key,
                       krb5_k_create_key, krb5_k_reference_key};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         CANONHOST_FALLBACK, CANONHOST_TRUE, CANONHOST_FALSE,
                         krb5_preauth_context, kdb5_dal_handle,
                         _krb5_os_context, krb5_key_st, derived_key, zapfree,
                         data_eq, make_data, empty_data, alloc_data, k5calloc,
...
pub use self::crypto_int_h::{krb5_keytypes, prf_func, rand2key_func,
                             ...
                             encrypt_block, krb5int_hmac,
                             krb5int_cmac_checksum, krb5int_nfold};
use self::stdlib_h::{malloc, calloc, free};
use self::string_h::{memcmp, explicit_bzero, memcpy};
 ...
 */
#[c2rust::src_loc = "30:1"]
unsafe extern "C" fn find_cached_dkey(mut list: *mut derived_key,
                                      mut constant: *const krb5_data)
 -> krb5_key {
    while !list.is_null() {
        if data_eq((*list).constant, *constant) != 0 {
            krb5_k_reference_key(0 as krb5_context, (*list).dkey);
            return (*list).dkey
        }
        list = (*list).next
    }
    return 0 as krb5_key;
}
#[c2rust::src_loc = "42:1"]
unsafe extern "C" fn add_cached_dkey(mut key: krb5_key,
                                     mut constant: *const krb5_data,
                                     mut dkeyblock: *const krb5_keyblock,
                                     mut cached_dkey: *mut krb5_key)
 -> krb5_error_code {
    let mut dkey: krb5_key = 0 as *mut krb5_key_st;
    let mut ret: krb5_error_code = 0;
    let mut dkent: *mut derived_key = 0 as *mut derived_key;
    let mut data: *mut libc::c_char = 0 as *mut libc::c_char;
    /* Allocate fields for the new entry. */
    dkent =
        malloc(::std::mem::size_of::<derived_key>() as libc::c_ulong) as
            *mut derived_key;
    if !dkent.is_null() {
        data =
            k5memdup((*constant).data as *const libc::c_void,
                     (*constant).length as size_t, &mut ret) as
                *mut libc::c_char;
        if !data.is_null() {
            ret = krb5_k_create_key(0 as krb5_context, dkeyblock, &mut dkey);
            if !(ret != 0 as libc::c_int) {
                ...
                krb5_k_reference_key(0 as krb5_context, dkey);
                *cached_dkey = dkey;
                return 0 as libc::c_int
            }
        }
    }
    free(dkent as *mut libc::c_void);
    free(data as *mut libc::c_void);
    return 12 as libc::c_int;
}
#[c2rust::src_loc = "80:1"]
unsafe extern "C" fn derive_random_rfc3961(mut enc: *const krb5_enc_provider,
                                           mut inkey: krb5_key,
                                           mut outrnd: *mut krb5_data,
                                           mut in_constant: *const krb5_data)
 -> krb5_error_code {
    let mut blocksize: size_t = 0;
    let mut keybytes: size_t = 0;
    let mut n: size_t = 0;
    let mut ret: krb5_error_code = 0;
    let mut block: krb5_data = empty_data();
    blocksize = (*enc).block_size;
    keybytes = (*enc).keybytes;
    if blocksize == 1 as libc::c_int as libc::c_ulong {
        return -(1765328196 as libc::c_long) as krb5_error_code
    }
    if (*inkey).keyblock.length as libc::c_ulong != (*enc).keylength ||
           (*outrnd).length as libc::c_ulong != keybytes {
        return -(1765328206 as libc::c_long) as krb5_error_code
    }
    /* Allocate encryption data buffer. */
    ret = alloc_data(&mut block, blocksize as libc::c_uint);
    if ret != 0 { return ret }
    /* Initialize the input block. */
    if (*in_constant).length as libc::c_ulong == blocksize {
        memcpy(block.data as *mut libc::c_void,
               (*in_constant).data as *const libc::c_void, blocksize);
    } else {
        krb5int_nfold((*in_constant).length.wrapping_mul(8 as libc::c_int as
                                                             libc::c_uint),
                      (*in_constant).data as *mut libc::c_uchar,
                      blocksize.wrapping_mul(8 as libc::c_int as
                                                 libc::c_ulong) as
                          libc::c_uint, block.data as *mut libc::c_uchar);
    }
    /* Loop encrypting the blocks until enough key bytes are generated. */
    n = 0 as libc::c_int as size_t;
    while n < keybytes {
        ret = encrypt_block(enc, inkey, &mut block);
        if ret != 0 { break ; }
        if keybytes.wrapping_sub(n) <= blocksize {
            memcpy((*outrnd).data.offset(n as isize) as *mut libc::c_void,
                   block.data as *const libc::c_void,
                   keybytes.wrapping_sub(n));
            break ;
        } else {
            memcpy((*outrnd).data.offset(n as isize) as *mut libc::c_void,
                   block.data as *const libc::c_void, blocksize);
            n =
                (n as libc::c_ulong).wrapping_add(blocksize) as size_t as
                    size_t
        }
    }
    zapfree(block.data as *mut libc::c_void, blocksize);
    return ret;
}
/*
...
unsafe extern "C" fn derive_random_sp800_108_feedback_cmac(mut enc:
 ...
 -> krb5_error_code {
    ...
    let mut keybytes: size_t = 0;
    let mut n: size_t = 0;
    let mut iov: [krb5_crypto_iov; 6] =
        [krb5_crypto_iov{flags: 0,
                         data:
                             krb5_data{magic: 0,
                                       length: 0,
                                       data: 0 as *mut libc::c_char,},}; 6];
    let mut ret: krb5_error_code = 0;
    let mut prf: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut i: libc::c_uint = 0;
    let mut ibuf: [libc::c_uchar; 4] = [0; 4];
    let mut Lbuf: [libc::c_uchar; 4] = [0; 4];
    blocksize = (*enc).block_size;
    keybytes = (*enc).keybytes;
    if (*inkey).keyblock.length as libc::c_ulong != (*enc).keylength ||
           (*outrnd).length as libc::c_ulong != keybytes {
    ...
    iov[1 as libc::c_int as usize].data =
        make_data(ibuf.as_mut_ptr() as *mut libc::c_void,
                  ::std::mem::size_of::<[libc::c_uchar; 4]>() as libc::c_ulong
                      as libc::c_uint);
    /* Label: the fixed derived-key input */
    iov[2 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[2 as libc::c_int as usize].data = *in_constant;
    /* 0x00: separator byte */
    iov[3 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[3 as libc::c_int as usize].data =
        make_data(b"\x00" as *const u8 as *const libc::c_char as
                      *mut libc::c_void, 1 as libc::c_int as libc::c_uint);
    /* Context: (unused) */
    iov[4 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[4 as libc::c_int as usize].data = empty_data();
    /* [L]2: four-byte big-endian binary string giving the output length */
    iov[5 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[5 as libc::c_int as usize].data =
        make_data(Lbuf.as_mut_ptr() as *mut libc::c_void,
                  ::std::mem::size_of::<[libc::c_uchar; 4]>() as libc::c_ulong
                      as libc::c_uint);
    store_32_be((*outrnd).length.wrapping_mul(8 as libc::c_int as
                                                  libc::c_uint),
                Lbuf.as_mut_ptr() as *mut libc::c_void);
    i = 1 as libc::c_int as libc::c_uint;
    n = 0 as libc::c_int as size_t;
    while n < keybytes {
        /* Update the block counter. */
        store_32_be(i, ibuf.as_mut_ptr() as *mut libc::c_void);
        /* Compute a CMAC checksum, storing the result into K(i-1). */
        ret =
            krb5int_cmac_checksum(enc, inkey, iov.as_mut_ptr(),
                                  6 as libc::c_int as size_t, &mut prf);
        if ret != 0 { break ; }
        /* Copy the result into the appropriate part of the output buffer. */
        if keybytes.wrapping_sub(n) <= blocksize {
            memcpy((*outrnd).data.offset(n as isize) as *mut libc::c_void,
                   prf.data as *const libc::c_void, keybytes.wrapping_sub(n));
            break ;
        } else {
            memcpy((*outrnd).data.offset(n as isize) as *mut libc::c_void,
                   prf.data as *const libc::c_void, blocksize);
            n =
                (n as libc::c_ulong).wrapping_add(blocksize) as size_t as
                    size_t;
            i = i.wrapping_add(1)
    ...
    zapfree(prf.data as *mut libc::c_void, blocksize);
    return ret;
}
/*
...
pub unsafe extern "C" fn k5_sp800_108_counter_hmac(mut hash:
                                                   ...
                                                   mut context:
                                                       *const krb5_data)
 -> krb5_error_code {
    let mut iov: [krb5_crypto_iov; 5] =
        [krb5_crypto_iov{flags: 0,
                         data:
                             krb5_data{magic: 0,
                                       length: 0,
                                       data: 0 as *mut libc::c_char,},}; 5];
    let mut ret: krb5_error_code = 0;
    let mut prf: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut ibuf: [libc::c_uchar; 4] = [0; 4];
    let mut lbuf: [libc::c_uchar; 4] = [0; 4];
    if hash.is_null() || (*outrnd).length as libc::c_ulong > (*hash).hashsize
       {
        return -(1765328206 as libc::c_long) as krb5_error_code
    }
    /* Allocate encryption data buffer. */
    ret = alloc_data(&mut prf, (*hash).hashsize as libc::c_uint);
    if ret != 0 { return ret }
    ...
    /* Label */
    iov[1 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[1 as libc::c_int as usize].data = *label;
    /* 0x00: separator byte */
    iov[2 as libc::c_int as usize].flags = 2 as libc::c_int;
    ...
    /* Context */
    iov[3 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[3 as libc::c_int as usize].data = *context;
    /* [L]2: four-byte big-endian binary string giving the output length */
    iov[4 as libc::c_int as usize].flags = 2 as libc::c_int;
    iov[4 as libc::c_int as usize].data =
        make_data(lbuf.as_mut_ptr() as *mut libc::c_void,
                  ::std::mem::size_of::<[libc::c_uchar; 4]>() as libc::c_ulong
                      as libc::c_uint);
    store_32_be((*outrnd).length.wrapping_mul(8 as libc::c_int as
                ...
                lbuf.as_mut_ptr() as *mut libc::c_void);
    ret =
        krb5int_hmac(hash, inkey, iov.as_mut_ptr(),
                     5 as libc::c_int as size_t, &mut prf);
    if ret == 0 {
        memcpy((*outrnd).data as *mut libc::c_void,
               prf.data as *const libc::c_void,
               (*outrnd).length as libc::c_ulong);
    }
    zapfree(prf.data as *mut libc::c_void, prf.length as size_t);
    return ret;
}
#[no_mangle]
#[c2rust::src_loc = "259:1"]
pub unsafe extern "C" fn krb5int_derive_random(mut enc:
                                                   *const krb5_enc_provider,
                                               mut hash:
                                                   *const krb5_hash_provider,
                                               mut inkey: krb5_key,
 ...
 -> krb5_error_code {
    ...
    match alg as libc::c_uint {
        0 => { return derive_random_rfc3961(enc, inkey, outrnd, in_constant) }
        1 => {
...
pub unsafe extern "C" fn krb5int_derive_keyblock(mut enc:
                                                     *const krb5_enc_provider,
                                                 mut hash:
                                                     *const krb5_hash_provider,
                                                 mut inkey: krb5_key,
 ...
 -> krb5_error_code {
    let mut ret: krb5_error_code = 0;
    let mut rawkey: krb5_data = empty_data();
    /* Allocate a buffer for the raw key bytes. */
    ret = alloc_data(&mut rawkey, (*enc).keybytes as libc::c_uint);
    if !(ret != 0) {
        /* Derive pseudo-random data for the key bytes. */
        ret =
            krb5int_derive_random(enc, hash, inkey, &mut rawkey, in_constant,
                                  alg);
        if !(ret != 0) {
            /* Postprocess the key. */
            ret =
                krb5_c_random_to_key(0 as krb5_context,
                                     (*inkey).keyblock.enctype, &mut rawkey,
                                     outkey)
        }
    }
    zapfree(rawkey.data as *mut libc::c_void, (*enc).keybytes);
    return ret;
}
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
...
pub unsafe extern "C" fn krb5int_derive_key(mut enc: *const krb5_enc_provider,
                                            mut hash:
                                                *const krb5_hash_provider,
                                            mut inkey: krb5_key,
                                            mut outkey: *mut krb5_key,
                                            mut in_constant: *const krb5_data,
                                            mut alg: deriv_alg)
 -> krb5_error_code {
    ...
    let mut ret: krb5_error_code = 0;
    let mut dkey: krb5_key = 0 as *mut krb5_key_st;
    *outkey = 0 as krb5_key;
    /* Check for a cached result. */
    dkey = find_cached_dkey((*inkey).derived, in_constant);
    if !dkey.is_null() { *outkey = dkey; return 0 as libc::c_int }
    /* Derive into a temporary keyblock. */
    keyblock.length = (*enc).keylength as libc::c_uint;
    keyblock.contents =
        malloc(keyblock.length as libc::c_ulong) as *mut krb5_octet;
    keyblock.enctype = (*inkey).keyblock.enctype;
    if keyblock.contents.is_null() { return 12 as libc::c_int }
    ret =
        krb5int_derive_keyblock(enc, hash, inkey, &mut keyblock, in_constant,
                                alg);
    if !(ret != 0) {
        /* Cache the derived key. */
        ret = add_cached_dkey(inkey, in_constant, &mut keyblock, &mut dkey);
        if !(ret != 0 as libc::c_int) { *outkey = dkey }
    }
    zapfree(keyblock.contents as *mut libc::c_void,
            keyblock.length as size_t);
    return ret;
}
