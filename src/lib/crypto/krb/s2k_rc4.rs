...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/include/k5-utf8.h:3"]
pub mod k5_utf8_h {
    use super::stdint_uintn_h::uint8_t;
    use super::stddef_h::size_t;
    extern "C" {
...
#[no_mangle]
#[c2rust::src_loc = "5:1"]
pub unsafe extern "C" fn krb5int_arcfour_string_to_key(mut ktp:
                                                           *const krb5_keytypes,
                                                       mut string:
                                                           *const krb5_data,
                                                       mut salt:
 ...
 -> krb5_error_code {
    ...
    let mut hash_out: krb5_data =
        krb5_data{magic: 0, length: 0, data: 0 as *mut libc::c_char,};
    let mut utf8: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut copystr: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut copystrlen: size_t = 0;
    if !params.is_null() {
        return -(1765328140 as libc::c_long) as krb5_error_code
    }
    if (*key).length != 16 as libc::c_int as libc::c_uint {
        return -(1765328194 as libc::c_long) as krb5_error_code
    }
    /* We ignore salt per the Microsoft spec. */
    utf8 =
        k5memdup0((*string).data as *const libc::c_void,
                  (*string).length as size_t, &mut err) as *mut libc::c_char;
    if utf8.is_null() { return err }
    err = k5_utf8_to_utf16le(utf8, &mut copystr, &mut copystrlen);
    zapfree(utf8 as *mut libc::c_void, (*string).length as size_t);
    if err != 0 { return err }
    /* the actual MD4 hash of the data */
    iov.flags = 2 as libc::c_int;
    iov.data =
        make_data(copystr as *mut libc::c_void, copystrlen as libc::c_uint);
    hash_out = make_data((*key).contents as *mut libc::c_void, (*key).length);
    err =
        krb5int_hash_md4.hash.expect("non-null function pointer")(&mut iov,
                                                                  1 as
                                                                      libc::c_int
                                                                      as
                                                                      size_t,
    ...
    zapfree(copystr as *mut libc::c_void, copystrlen);
    return err;
}
