...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/crypto/builtin/camellia/camellia.h:28"]
pub mod camellia_h {
    #[derive(Copy, Clone)]
    #[repr(C)]
    #[c2rust::src_loc = "95:9"]
    ...
    extern "C" {
        ...
        pub fn krb5int_camellia_enc_key(in_key: *const libc::c_uchar,
                                        klen: libc::c_uint,
                                        cx: *mut camellia_ctx) -> cam_fret;
        #[no_mangle]
        #[c2rust::src_loc = "112:1"]
        pub fn krb5int_camellia_enc_blk(in_blk: *const libc::c_uchar,
                                        out_blk: *mut libc::c_uchar,
                                        cx: *const camellia_ctx) -> cam_fret;
        #[no_mangle]
        #[c2rust::src_loc = "115:1"]
        pub fn krb5int_camellia_dec_key(in_key: *const libc::c_uchar,
                                        klen: libc::c_uint,
                                        cx: *mut camellia_ctx) -> cam_fret;
        #[no_mangle]
        #[c2rust::src_loc = "117:1"]
        pub fn krb5int_camellia_dec_blk(in_blk: *const libc::c_uchar,
                                        out_blk: *mut libc::c_uchar,
                                        cx: *const camellia_ctx) -> cam_fret;
    }
    /* HEADER_CAMELLIA_H */
...
pub use self::crypto_int_h::{krb5_enc_provider, krb5_cksumtypes, verify_func,
                             ...
                             iov_total_length, iov_cursor_contig_blocks,
                             iov_cursor_ptr, iov_cursor_advance,
                             k5_iov_cursor_put, k5_iov_cursor_get,
                             k5_iov_cursor_init, krb5int_default_free_state};
pub use self::camellia_h::{camellia_ctx, cam_fret, krb5int_camellia_enc_key,
                           krb5int_camellia_enc_blk, krb5int_camellia_dec_key,
                           krb5int_camellia_dec_blk};
...
#[repr(C)]
#[c2rust::src_loc = "36:8"]
pub struct camellia_key_info_cache {
    pub enc_ctx: camellia_ctx,
    pub dec_ctx: camellia_ctx,
}
/* out = out ^ in */
#[inline]
#[c2rust::src_loc = "42:1"]
...
unsafe extern "C" fn init_key_cache(mut key: krb5_key) -> krb5_error_code {
    ...
    (*key).cache =
        malloc(::std::mem::size_of::<camellia_key_info_cache>() as
                   libc::c_ulong);
    if (*key).cache.is_null() { return 12 as libc::c_int }
    let ref mut fresh0 =
        (*((*key).cache as *mut camellia_key_info_cache)).dec_ctx.keybitlen;
    *fresh0 = 0 as libc::c_int;
    (*((*key).cache as *mut camellia_key_info_cache)).enc_ctx.keybitlen =
        *fresh0;
...
unsafe extern "C" fn expand_enc_key(mut key: krb5_key) {
    if (*((*key).cache as *mut camellia_key_info_cache)).enc_ctx.keybitlen !=
           0 {
        return
    }
    if krb5int_camellia_enc_key((*key).keyblock.contents as
                                    *const libc::c_uchar,
                                (*key).keyblock.length,
                                &mut (*((*key).cache as
                                            *mut camellia_key_info_cache)).enc_ctx)
...
unsafe extern "C" fn expand_dec_key(mut key: krb5_key) {
    if (*((*key).cache as *mut camellia_key_info_cache)).dec_ctx.keybitlen !=
           0 {
        return
    }
    if krb5int_camellia_dec_key((*key).keyblock.contents as
                                    *const libc::c_uchar,
                                (*key).keyblock.length,
                                &mut (*((*key).cache as
                                            *mut camellia_key_info_cache)).dec_ctx)
           as libc::c_int != 1 as libc::c_int {
        abort();
    };
}
...
unsafe extern "C" fn cbc_enc(mut key: krb5_key, mut data: *mut libc::c_uchar,
    ...
    while nblocks > 0 as libc::c_int as libc::c_ulong {
        xorblock(iv, data);
        if krb5int_camellia_enc_blk(data as *const libc::c_uchar, data,
                                    &mut (*((*key).cache as
                                                *mut camellia_key_info_cache)).enc_ctx
                                        as *mut camellia_ctx as
                                        *const camellia_ctx) as libc::c_int !=
               1 as libc::c_int {
            abort();
        }
...
unsafe extern "C" fn cbc_dec(mut key: krb5_key, mut data: *mut libc::c_uchar,
    ...
    if nblocks > 0 as libc::c_int as libc::c_ulong {
        ...
        __assert_fail(b"nblocks > 0\x00" as *const u8 as *const libc::c_char,
                      b"camellia.c\x00" as *const u8 as *const libc::c_char,
    ...
    while nblocks > 0 as libc::c_int as libc::c_ulong {
        if krb5int_camellia_dec_blk(data as *const libc::c_uchar, data,
                                    &mut (*((*key).cache as
                                                *mut camellia_key_info_cache)).dec_ctx
                                        as *mut camellia_ctx as
                                        *const camellia_ctx) as libc::c_int !=
...
unsafe extern "C" fn krb5int_camellia_encrypt(mut key: krb5_key,
...
unsafe extern "C" fn krb5int_camellia_decrypt(mut key: krb5_key,
                                              mut ivec: *const krb5_data,
                                              mut data: *mut krb5_crypto_iov,
                                              mut num_data: size_t)
 -> krb5_error_code {
...
pub unsafe extern "C" fn krb5int_camellia_cbc_mac(mut key: krb5_key,
                                                  mut data:
                                                      *const krb5_crypto_iov,
                                                  mut num_data: size_t,
                                                  mut ivec: *const krb5_data,
 ...
 -> krb5_error_code {
    ...
    (*output).length = 16 as libc::c_int as libc::c_uint;
...
unsafe extern "C" fn camellia_init_state(mut key: *const krb5_keyblock,
...
unsafe extern "C" fn camellia_key_cleanup(mut key: krb5_key) {
    zapfree((*key).cache,
            ::std::mem::size_of::<camellia_key_info_cache>() as
                libc::c_ulong);
}
#[no_mangle]
#[c2rust::src_loc = "295:32"]
pub static mut krb5int_enc_camellia128: krb5_enc_provider =
    unsafe {
        {
            let mut init =
                krb5_enc_provider{block_size: 16 as libc::c_int as size_t,
                                  ...
                                  encrypt:
                                      Some(krb5int_camellia_encrypt as
                                               unsafe extern "C" fn(_:
                                                   ...
                                                   -> krb5_error_code),
                                  decrypt:
                                      Some(krb5int_camellia_decrypt as
                                               unsafe extern "C" fn(_:
                                                                        krb5_key,
                                                                    _:
                                                                        *const krb5_data,
                                                   ...
                                                   -> krb5_error_code),
                                  cbc_mac:
                                      Some(krb5int_camellia_cbc_mac as
                                               unsafe extern "C" fn(_:
                                                                        krb5_key,
                                                                    _:
                                                                        *const krb5_crypto_iov,
...
#[no_mangle]
#[c2rust::src_loc = "306:32"]
pub static mut krb5int_enc_camellia256: krb5_enc_provider =
    unsafe {
        {
            let mut init =
                krb5_enc_provider{block_size: 16 as libc::c_int as size_t,
