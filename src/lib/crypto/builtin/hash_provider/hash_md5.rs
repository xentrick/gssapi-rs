...
#[c2rust::header_src =
  "/home/nmavis/dev/gssapi-rs/code/src/lib/crypto/builtin/md5/rsa-md5.h:29"]
pub mod rsa_md5_h {
    #[derive(Copy, Clone)]
    #[repr(C)]
    #[c2rust::src_loc = "43:9"]
    ...
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "50:1"]
        pub fn krb5int_MD5Init(_: *mut krb5_MD5_CTX);
        #[no_mangle]
        #[c2rust::src_loc = "51:1"]
        pub fn krb5int_MD5Update(_: *mut krb5_MD5_CTX,
                                 _: *const libc::c_uchar, _: libc::c_uint);
        #[no_mangle]
        #[c2rust::src_loc = "52:1"]
        pub fn krb5int_MD5Final(_: *mut krb5_MD5_CTX);
    }
    /* KRB5_RSA_MD5__ */
}
...
pub use self::rsa_md5_h::{krb5_MD5_CTX, krb5int_MD5Init, krb5int_MD5Update,
                          krb5int_MD5Final};
 ...
 */
#[c2rust::src_loc = "31:1"]
unsafe extern "C" fn k5_md5_hash(mut data: *const krb5_crypto_iov,
                                 mut num_data: size_t,
                                 mut output: *mut krb5_data)
 -> krb5_error_code {
    let mut ctx: krb5_MD5_CTX =
        krb5_MD5_CTX{i: [0; 2], buf: [0; 4], in_0: [0; 64], digest: [0; 16],};
    let mut i: libc::c_uint = 0;
    if (*output).length != 16 as libc::c_int as libc::c_uint {
        return -(1765328206 as libc::c_long) as krb5_error_code
    }
    krb5int_MD5Init(&mut ctx);
    ...
    while (i as libc::c_ulong) < num_data {
        ...
        if (*iov).flags == 1 as libc::c_int ||
            ...
            krb5int_MD5Update(&mut ctx,
                              (*iov).data.data as *mut libc::c_uchar,
                              (*iov).data.length);
        }
        i = i.wrapping_add(1)
    }
    krb5int_MD5Final(&mut ctx);
...
pub static mut krb5int_hash_md5: krb5_hash_provider =
    unsafe {
        {
            let mut init =
                krb5_hash_provider{hash_name: [77, 68, 53, 0, 0, 0, 0, 0],
