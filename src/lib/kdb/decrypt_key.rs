...
pub use self::k5_platform_h::{C2RustUnnamed, load_16_le};
...
#[no_mangle]
#[c2rust::src_loc = "61:1"]
pub unsafe extern "C" fn krb5_dbe_def_decrypt_key_data(mut context:
                                                           krb5_context,
                                                       mut mkey:
                                                           *const krb5_keyblock,
                                                       mut key_data:
 ...
 -> krb5_error_code {
    ...
    if mkey.is_null() {
        return -(1780008428 as libc::c_long) as krb5_error_code
    }
    ptr = (*key_data).key_data_contents[0 as libc::c_int as usize];
    if !ptr.is_null() {
        *(&mut tmplen as *mut krb5_int16) =
            load_16_le(ptr as *const libc::c_void) as krb5_int16;
        ptr = ptr.offset(2 as libc::c_int as isize);
        if (tmplen as libc::c_int) < 0 as libc::c_int {
            return 22 as libc::c_int
        }
        cipher.enctype = 0x1ff as libc::c_int;
        cipher.ciphertext.length =
            ((*key_data).key_data_length[0 as libc::c_int as usize] as
                 libc::c_int - 2 as libc::c_int) as libc::c_uint;
        cipher.ciphertext.data = ptr as *mut libc::c_char;
        plain.length =
            ((*key_data).key_data_length[0 as libc::c_int as usize] as
                 libc::c_int - 2 as libc::c_int) as libc::c_uint;
        plain.data =
            malloc(plain.length as libc::c_ulong) as *mut libc::c_char;
        if plain.data.is_null() { return 12 as libc::c_int }
        retval =
            krb5_c_decrypt(context, mkey, 0 as libc::c_int,
                           0 as *const krb5_data, &mut cipher, &mut plain);
        if retval != 0 {
        ...
        if tmplen as libc::c_uint > plain.length {
            free(plain.data as *mut libc::c_void);
            return -(1765328206 as libc::c_long) as krb5_error_code
        }
        (*dbkey).magic = -(1760647421 as libc::c_long) as krb5_magic;
        (*dbkey).enctype =
            (*key_data).key_data_type[0 as libc::c_int as usize] as
                krb5_enctype;
        (*dbkey).length = tmplen as libc::c_uint;
        (*dbkey).contents = plain.data as *mut krb5_octet
    }
    /* Decode salt data */
    if !keysalt.is_null() {
        if (*key_data).key_data_ver as libc::c_int == 2 as libc::c_int {
            (*keysalt).type_0 =
                (*key_data).key_data_type[1 as libc::c_int as usize];
            (*keysalt).data.length =
                (*key_data).key_data_length[1 as libc::c_int as usize] as
                    libc::c_uint;
            if (*keysalt).data.length != 0 {
                (*keysalt).data.data =
                    malloc((*keysalt).data.length as libc::c_ulong) as
                        *mut libc::c_char;
                if (*keysalt).data.data.is_null() {
                    if !(*key_data).key_data_contents[0 as libc::c_int as
                                                          usize].is_null() {
                        free((*dbkey).contents as *mut libc::c_void);
                        (*dbkey).contents = 0 as *mut krb5_octet;
                        (*dbkey).length = 0 as libc::c_int as libc::c_uint
                    }
                    return 12 as libc::c_int
                }
                memcpy((*keysalt).data.data as *mut libc::c_void,
                       (*key_data).key_data_contents[1 as libc::c_int as
                                                         usize] as
                           *const libc::c_void,
            ...
            (*keysalt).type_0 = 0 as libc::c_int as krb5_int16;
            (*keysalt).data.data =
                0 as *mut libc::c_void as *mut libc::c_char;
            (*keysalt).data.length = 0 as libc::c_int as libc::c_uint
        }
    }
    return retval;
}
