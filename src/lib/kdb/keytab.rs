 ...
 */
    ...
    extern "C" {
        ...
        #[no_mangle]
        #[c2rust::src_loc = "2123:1"]
        pub fn krb5_kt_register(_: krb5_context, _: *const _krb5_kt_ops)
         -> krb5_error_code;
    }
    /* _KRB5_INT_H */
...
#[c2rust::header_src = "/home/nmavis/dev/gssapi-rs/code/src/include/kdb.h:30"]
pub mod kdb_h {
    /* # of array elements */
 ...
 */
    ...
    use super::krb5_h::{krb5_int16, krb5_data, krb5_ui_2, krb5_octet,
                        ...
                        krb5_const_principal, krb5_int32, krb5_keyblock};
    use super::k5_int_h::_krb5_context;
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "361:1"]
        pub fn krb5_db_inited(kcontext: krb5_context) -> krb5_error_code;
        #[no_mangle]
...
pub use self::krb5_h::{krb5_octet, krb5_int16, krb5_ui_2, krb5_int32,
                       ...
                       krb5_c_enctype_compare, krb5_copy_principal};
pub use self::k5_int_h::{_krb5_context, plugin_interface, dns_canonhost,
                         CANONHOST_FALLBACK, CANONHOST_TRUE, CANONHOST_FALSE,
                         krb5_preauth_context, kdb5_dal_handle,
                         _krb5_os_context, _krb5_kt, _krb5_kt_ops,
                         krb5_kt_ops, plugin_mapping, _kdb_log_context,
...
pub use self::kdb_h::{krb5_keysalt, _krb5_keysalt, _krb5_key_data,
                      _krb5_tl_data, krb5_tl_data, krb5_key_data,
                      _krb5_db_entry_new, krb5_db_entry, krb5_db_inited,
...
#[c2rust::src_loc = "40:1"]
unsafe extern "C" fn krb5_ktkdb_get_name(mut context: krb5_context,
                                         mut keytab: krb5_keytab,
                                         mut name: *mut libc::c_char,
                                         mut namelen: libc::c_uint)
 -> krb5_error_code {
    if krb5int_strlcpy(name, b"KDB:\x00" as *const u8 as *const libc::c_char,
                       namelen as size_t) >= namelen as libc::c_ulong {
...
pub static mut krb5_kt_kdb_ops: krb5_kt_ops =
    unsafe {
        {
            let mut init =
                             ...
                             get_name:
                                 Some(krb5_ktkdb_get_name as
                                          unsafe extern "C" fn(_:
                                              ...
                                              -> krb5_error_code),
                             close:
                                 Some(krb5_ktkdb_close as
                                          unsafe extern "C" fn(_:
                                                                   krb5_context,
                                                               _: krb5_keytab)
                                              -> krb5_error_code),
                             get:
                                 Some(krb5_ktkdb_get_entry as
                                          unsafe extern "C" fn(_:
                                                                   krb5_context,
                                                               _: krb5_keytab,
                                                               _:
...
#[no_mangle]
#[c2rust::src_loc = "67:1"]
pub unsafe extern "C" fn krb5_db_register_keytab(mut context: krb5_context)
 -> krb5_error_code {
    return krb5_kt_register(context, &mut krb5_kt_kdb_ops);
}
/* -*- mode: c; c-basic-offset: 4; indent-tabs-mode: nil -*- */
...
#[no_mangle]
#[c2rust::src_loc = "73:1"]
pub unsafe extern "C" fn krb5_ktkdb_resolve(mut context: krb5_context,
                                            mut name: *const libc::c_char,
                                            mut id: *mut krb5_keytab)
 -> krb5_error_code {
    *id =
        malloc(::std::mem::size_of::<_krb5_kt>() as libc::c_ulong) as
            krb5_keytab;
    if (*id).is_null() { return 12 as libc::c_int }
    (**id).ops = &mut krb5_kt_kdb_ops;
    (**id).magic = -(1760647382 as libc::c_long) as krb5_magic;
    return 0 as libc::c_int;
}
#[no_mangle]
#[c2rust::src_loc = "86:1"]
pub unsafe extern "C" fn krb5_ktkdb_close(mut context: krb5_context,
                                          mut kt: krb5_keytab)
 -> krb5_error_code {
    /*
     ...
     */
    (*kt).ops = 0 as *const _krb5_kt_ops;
    free(kt as *mut libc::c_void);
    return 0 as libc::c_int;
}
#[c2rust::src_loc = "105:21"]
static mut ktkdb_ctx: krb5_context =
...
pub unsafe extern "C" fn krb5_ktkdb_set_context(mut ctx: krb5_context)
 -> krb5_error_code {
    ktkdb_ctx = ctx;
    return 0 as libc::c_int;
}
...
pub unsafe extern "C" fn krb5_ktkdb_get_entry(mut in_context: krb5_context,
                                              mut id: krb5_keytab,
                                              mut principal:
                                                  krb5_const_principal,
                                              mut kvno: krb5_kvno,
 ...
 -> krb5_error_code {
    ...
    let mut db_entry: *mut krb5_db_entry = 0 as *mut krb5_db_entry;
    let mut xrealm_tgt: libc::c_int = 0;
    let mut similar: krb5_boolean = 0;
    if !ktkdb_ctx.is_null() {
        context = ktkdb_ctx
    } else { context = in_context }
    xrealm_tgt = is_xrealm_tgt(context, principal);
    /* Check whether database is inited. Open is commented */
    kerror = krb5_db_inited(context);
    if kerror != 0 { return kerror }
    /* get_principal */
    kerror =
        krb5_db_get_principal(context, principal,
                              0 as libc::c_int as libc::c_uint,
                              &mut db_entry);
    if kerror as libc::c_long == -(1780008443 as libc::c_long) {
        return -(1765328203 as libc::c_long) as krb5_error_code
    }
    if kerror != 0 { return kerror }
    if (*db_entry).attributes & 0x1000 as libc::c_int != 0 ||
           (*db_entry).attributes & 0x40 as libc::c_int != 0 {
                ...
                if enctype > 0 as libc::c_int {
                    kerror =
                        krb5_c_enctype_compare(context, enctype,
                                               ...
                                               &mut similar);
                    if kerror != 0 {
                        current_block = 13581770620653895671;
                    } else if similar == 0 {
                        kerror =
                            -(1780008418 as libc::c_long) as krb5_error_code;
                        current_block = 13581770620653895671;
                    } else { current_block = 15125582407903384992; }
                } else { current_block = 15125582407903384992; }
                match current_block {
                    13581770620653895671 => { }
                    _ => {
                        /*
...
unsafe extern "C" fn is_xrealm_tgt(mut context: krb5_context,
                                   mut princ: krb5_const_principal)
 -> libc::c_int {
    let mut dat: *mut krb5_data = 0 as *mut krb5_data;
    if (*princ).length != 2 as libc::c_int { return 0 as libc::c_int }
    dat =
        if (0 as libc::c_int) < (*princ).length {
            (*princ).data.offset(0 as libc::c_int as isize)
        } else { 0 as *mut krb5_data };
    if strncmp(b"krbtgt\x00" as *const u8 as *const libc::c_char, (*dat).data,
               (*dat).length as libc::c_ulong) != 0 as libc::c_int {
        return 0 as libc::c_int
    }
    dat =
    ...
    if (*dat).length != (*princ).realm.length { return 1 as libc::c_int }
    if strncmp((*dat).data, (*princ).realm.data,
               (*dat).length as libc::c_ulong) == 0 as libc::c_int {
        return 0 as libc::c_int
    }
