...
pub use crate::src::mechglue::g_unwrap_aead::gssint_unwrap_aead;
...
pub unsafe extern "C" fn gss_unwrap(
    ...
    if input_message_buffer.is_null()
        || (input_message_buffer.is_null()
            ...
            || (*input_message_buffer).length == 0usize)
    {
        return (1u32) << 24i32;
    }
    if output_message_buffer.is_null() {
    ...
    if !mech.is_null() {
        if (*mech).gss_unwrap.is_some() {
            status = (*mech).gss_unwrap.expect("non-null function pointer")(
                minor_status,
                (*ctx).internal_ctx_id,
        ...
        } else if (*mech).gss_unwrap_aead.is_some() || (*mech).gss_unwrap_iov.is_some() {
            status = crate::src::mechglue::g_unwrap_aead::gssint_unwrap_aead(
                mech,
                minor_status,
                ctx,
                input_message_buffer,
...
pub unsafe extern "C" fn gss_unseal(
    mut minor_status: *mut crate::gssapi_h::OM_uint32,
    mut context_handle: crate::gssapi_h::gss_ctx_id_t,
    mut input_message_buffer: crate::gssapi_h::gss_buffer_t,
    mut output_message_buffer: crate::gssapi_h::gss_buffer_t,
